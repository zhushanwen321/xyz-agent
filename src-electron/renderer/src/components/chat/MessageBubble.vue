<template>
    ...
    <template v-if="message.contentBlocks?.length">
      <template v-for="block in message.contentBlocks" :key="block.refId">
        ...
        <div
          ...
          class="py-3 px-4 leading-[1.6] text-xs bg-surface border-t border-border rounded-sm"
        >
          <div
  ...
  >
    ...
    <div class="py-3 px-4 leading-[1.6] text-xs text-fg rounded-sm" style="background:var(--user-bubble-bg);border:1px solid var(--user-bubble-border);">
      <div
        v-if="message.content"
        class="msg__body select-text"
        :data-message-id="message.id"
