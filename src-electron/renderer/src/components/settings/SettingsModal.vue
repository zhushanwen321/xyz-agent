<template>
  <!--
    ...
    v1 范围（FG6 骨架）：modal 形态 + 头部 + 左导航（5 菜单占位）+ 右详情空壳。
    DEFERRED（spec §9）：触发入口（⌘, / sidebar 头像，G3-002 hide，v1 不渲染触发按钮）；
  ...
  <Dialog :open="open" @update:open="emit('update:open', $event)">
    <DialogContent
      aria-modal="true"
      class="flex max-w-[900px] gap-0 overflow-hidden p-0 sm:rounded-lg"
    >
      <!-- modal-head：标题 + 关闭（内置搜索 ⌘K + 保存状态 pill DEFERRED） -->
...
import { Settings, Sparkles, Bot, Blocks, SlidersHorizontal } from '@lucide/vue'
import {
  Dialog,
  DialogContent,
  DialogHeader,
