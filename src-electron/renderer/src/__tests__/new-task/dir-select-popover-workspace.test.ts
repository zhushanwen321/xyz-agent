...
describe('T4.5: 删派生函数后零残留', () => {
  it('DirSelectPopover 不再 import recentWorkspaces 或旧 RecentWorkspace 类型', () => {
    ...
    const src = fs.readFileSync(
      '/Users/zhushanwen/Code/xyz-agent-workspace/refactor-arch-render-runtime/src-electron/renderer/src/components/new-task/DirSelectPopover.vue',
      'utf-8',
    )
  ...
  it('useNewTaskFlow 不再 import resolveDefaultCwd', () => {
    ...
    const fs = require('fs')
    const src = fs.readFileSync(
      '/Users/zhushanwen/Code/xyz-agent-workspace/refactor-arch-render-runtime/src-electron/renderer/src/composables/features/useNewTaskFlow.ts',
      'utf-8',
    )
  ...
  it('lib/utils.ts 不再导出 recentWorkspaces / resolveDefaultCwd / RecentWorkspace / MAX_RECENT_WORKSPACES', () => {
    ...
    const fs = require('fs')
    const src = fs.readFileSync(
      '/Users/zhushanwen/Code/xyz-agent-workspace/refactor-arch-render-runtime/src-electron/renderer/src/lib/utils.ts',
      'utf-8',
    )
