 ...
 * 本测试覆盖 applyChunk 在 message_start / text_delta / thinking_start / tool_call_start
 * 等 case 下正确 push contentBlocks（幂等 + 到达顺序），并验证后续 end/delta case 不改顺序。
 *
 * 运行：cd src-electron/renderer && npx vitest run src/__tests__/stores/chat-chunk-content-blocks.test.ts
 */
import { describe, it, expect, vi } from 'vitest'
import { ref } from 'vue'
import { applyChunk } from '@/stores/chat-chunk-processor'
import type { ChunkContext } from '@/stores/chat-chunk-processor'
...
function getMsgs(ctx: ChunkContext): Message[] {
  return ctx.messages.value.get(SID) ?? []
}

/** 取最后一条 assistant message */
function lastAssistant(ctx: ChunkContext): Message {
...
describe('applyChunk contentBlocks 填充（流式路径）', () => {
  // U1：message_start 初始化 contentBlocks: []
  it('U1: message_start（prev 空）→ 新 message 含 contentBlocks: []（空数组）', () => {
    const ctx = makeCtx()
    applyChunk(ctx, SID, msg('message.message_start', { messageId: 'a1' }))
  ...
  it('U2: content="" + contentBlocks=[]，收首个 text_delta "h" → content="h" 且 contentBlocks=[{type:text,refId:text}]', () => {
    const ctx = makeCtx()
    applyChunk(ctx, SID, msg('message.message_start', { messageId: 'a1' }))
    applyChunk(ctx, SID, msg('message.text_delta', { delta: 'h' }))
    const a = lastAssistant(ctx)
  ...
  it('U3: contentBlocks 已含 text 块，再收 text_delta "i" → content="hi"，contentBlocks 仍仅 1 个 text 块', () => {
    ...
    applyChunk(ctx, SID, msg('message.message_start', { messageId: 'a1' }))
    applyChunk(ctx, SID, msg('message.text_delta', { delta: 'h' }))
    applyChunk(ctx, SID, msg('message.text_delta', { delta: 'i' }))
    const a = lastAssistant(ctx)
  ...
  it('U4: 收 thinking_start(thinkingId="th1") → thinking 含 th1 且 contentBlocks 尾部 push {type:thinking,refId:th1}', () => {
    const ctx = makeCtx()
    applyChunk(ctx, SID, msg('message.message_start', { messageId: 'a1' }))
    applyChunk(ctx, SID, msg('message.thinking_start', { thinkingId: 'th1' }))
    const a = lastAssistant(ctx)
  ...
  it('U5: 收 tool_call_start(toolCallId="tc1") → toolCalls 含 tc1 且 contentBlocks 尾部 push {type:toolCall,refId:tc1}', () => {
    const ctx = makeCtx()
    applyChunk(ctx, SID, msg('message.message_start', { messageId: 'a1' }))
    applyChunk(ctx, SID, msg('message.tool_call_start', { toolCallId: 'tc1', toolName: 'read' }))
    const a = lastAssistant(ctx)
  ...
  it('U6: message_start→thinking(th1)→text_delta→tool_call(tc1)→thinking(th2) → contentBlocks 顺序=[thinking:th1, text, toolCall:tc1, thinking:th2]', () => {
    ...
    applyChunk(ctx, SID, msg('message.message_start', { messageId: 'a1' }))
    applyChunk(ctx, SID, msg('message.thinking_start', { thinkingId: 'th1' }))
    applyChunk(ctx, SID, msg('message.text_delta', { delta: 'hi' }))
    applyChunk(ctx, SID, msg('message.tool_call_start', { toolCallId: 'tc1', toolName: 'read' }))
    applyChunk(ctx, SID, msg('message.thinking_start', { thinkingId: 'th2' }))
    const a = lastAssistant(ctx)
    expect(a.contentBlocks).toEqual([
  ...
  it('U7: contentBlocks 已定，收 thinking_end + thinking_delta → contentBlocks 数组不变（仅 thinking 内容更新）', () => {
    ...
    applyChunk(ctx, SID, msg('message.thinking_delta', { delta: '推理' }))
    applyChunk(ctx, SID, msg('message.thinking_end'))
    const a = lastAssistant(ctx)
  ...
  it('U8: prev 无 assistant，收 text_delta → 不抛错（return early）', () => {
    ...
    expect(() => applyChunk(ctx, SID, msg('message.text_delta', { delta: 'x' }))).not.toThrow()
    // user 消息不变
  ...
  it('U11: a1 已有 contentBlocks，message_start(a2)→text_delta → a1.contentBlocks 原样；text 写入 a2', () => {
    const ctx = makeCtx()
    applyChunk(ctx, SID, msg('message.message_start', { messageId: 'a1' }))
    applyChunk(ctx, SID, msg('message.text_delta', { delta: 'first' }))
    applyChunk(ctx, SID, msg('message.message_start', { messageId: 'a2' }))
    applyChunk(ctx, SID, msg('message.text_delta', { delta: 'second' }))
    const list = getMsgs(ctx)
    // a1 不被改
  ...
  it('U11b: thinking_start 无 thinkingId → fallback id 在 thinking[].id 与 contentBlocks[].refId 一致', () => {
    const ctx = makeCtx()
    applyChunk(ctx, SID, msg('message.message_start', { messageId: 'a1' }))
    applyChunk(ctx, SID, msg('message.thinking_start')) // 无 thinkingId
    const a = lastAssistant(ctx)
  ...
  it('U11c: tool_call_start 无 toolCallId → fallback id 在 toolCalls[].id 与 contentBlocks[].refId 一致', () => {
    const ctx = makeCtx()
    applyChunk(ctx, SID, msg('message.message_start', { messageId: 'a1' }))
    applyChunk(ctx, SID, msg('message.tool_call_start', { toolName: 'read' })) // 无 toolCallId
    const a = lastAssistant(ctx)
  ...
  it('U11d: contentBlocks 已定，收 tool_call_end → contentBlocks 不变，仅 toolCalls[].status 更新', () => {
    ...
    applyChunk(ctx, SID, msg('message.tool_call_start', { toolCallId: 'tc1', toolName: 'read' }))
    const before = lastAssistant(ctx).contentBlocks
    applyChunk(ctx, SID, msg('message.tool_call_end', { toolCallId: 'tc1', status: 'completed', output: 'ok' }))
    const a = lastAssistant(ctx)
    expect(a.contentBlocks).toEqual(before)
