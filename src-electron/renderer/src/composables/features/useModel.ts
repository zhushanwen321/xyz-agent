 ...
 *
 * 与 useThinkingLevelSync 的联动：模型切换的乐观更新写 sessionStore.active.modelId，
 * useThinkingLevelSync 的 watch(currentThinkingLevelMap) 会在 modelId 变化后自动对齐思考
 * 等级档位（不可用档 → 重置到最高可用档，经 onReset → onThinkingSelect → setThinkingLevel
 * 回到此 composable）。本 composable 只负责单次 RPC + 乐观更新，不破坏该 watch 链。
 */
...
export function useModel() {
   ...
   * 立即同步 active.modelId（Composer 工具条显示跟随），不依赖 state_changed 广播。
   *
   * 调用方职责区分：
   ...
   * 乐观更新立即同步 active.thinkingLevel（Composer 工具条显示跟随）。
   *
   * 调用方职责区分：
