...
describe('PluginRegistry', () => {
  ...
  it('TC-1-01: scan() discovers valid plugin from fixture dir', async () => {
    ...
    assert.ok(hw.activationEvents.includes('onSlashCommand:hello'))
  })
  ...
  it('TC-1-02: scan() skips invalid manifest (no xyzAgent field)', async () => {
    ...
    assert.strictEqual(found, undefined, 'should skip plugin without xyzAgent field')
  })
  ...
  it('TC-1-03: scan() auto-infers activationEvents from contributes', async () => {
    ...
    assert.ok(desc, 'should find infer-test plugin')
    assert.ok(desc.activationEvents.includes('onStartupFinished'), 'should have declared event')
    assert.ok(
      desc.activationEvents.includes('onSlashCommand:foo'),
      'should infer onSlashCommand:foo from contributes',
    )
    assert.ok(
      desc.activationEvents.includes('onSlashCommand:bar'),
      'should infer onSlashCommand:bar from contributes',
    )
  })
  ...
  it('TC-1-04: cacheDescriptors / getDescriptor / getAllDescriptors', async () => {
    ...
    assert.deepStrictEqual(registry.getDescriptor('plugin-a'), descA)
    assert.deepStrictEqual(registry.getDescriptor('plugin-b'), descB)
    assert.strictEqual(registry.getDescriptor('nonexistent'), undefined)

    const all = registry.getAllDescriptors()
    assert.strictEqual(all.length, 2)
    const ids = all.map(d => d.pluginId).sort()
    assert.deepStrictEqual(ids, ['plugin-a', 'plugin-b'])
  })
  ...
  it('TC-1-05: reload() re-scans', async () => {
    ...
    assert.ok(first.some(d => d.pluginId === 'hello-world'))
    ...
    assert.ok(reloaded.some(d => d.pluginId === 'late-plugin'), 'reload should find newly added plugin')
    assert.ok(reloaded.some(d => d.pluginId === 'hello-world'), 'reload should still find old plugins')
  })
})
