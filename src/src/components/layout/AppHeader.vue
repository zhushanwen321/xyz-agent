<template>
  <header class="app-header">
    ...
    <div class="header-actions">
      <Button variant="ghost" class="h-btn" @click="settingsStore.toggleOverview()" title="窗口总览 (Cmd+J)">
      ...
      <Button variant="ghost" :class="['h-btn', { active: !settingsStore.focusMode && !settingsStore.splitMode }]" @click="setView('standard')" title="标准 (Cmd+1)">
        <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" style="width:16px;height:16px">
          <rect x="1" y="1" width="4" height="14" rx="1"/><rect x="6" y="1" width="9" height="14" rx="1"/>
        </svg>
      </Button>
      <Button variant="ghost" :class="['h-btn', { active: settingsStore.splitMode }]" @click="setView('split')" title="分屏 (Cmd+2)">
        <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" style="width:16px;height:16px">
          <rect x="1" y="1" width="6" height="14" rx="1"/><rect x="9" y="1" width="6" height="14" rx="1"/>
        </svg>
      </Button>
      <Button variant="ghost" :class="['h-btn', { active: settingsStore.focusMode }]" @click="setView('focus')" title="专注 (Cmd+3)">
        <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" style="width:16px;height:16px">
          <rect x="1" y="1" width="14" height="14" rx="1"/>
        </svg>
      </Button>
      <span class="h-divider"></span>
      <Button variant="ghost" :class="['h-btn', { active: settingsStore.currentView === 'settings' }]" @click="openSettings" title="设置 (Cmd+,)">
      ...
      <Button variant="ghost" class="h-btn" @click="toggleTheme" title="切换主题">
