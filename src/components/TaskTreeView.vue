  ...
  <div class="space-y-0.5">
    ...
    <div v-else class="flex flex-col gap-1.5">
      ...
      <template v-if="ancestorPath.length > 0">
        <div class="text-[10px] text-tertiary font-semibold tracking-wider py-1">PARENTS</div>
        <div
        ...
        >
          <div class="flex items-center gap-1.5">
            ...
            <span class="text-[11px] text-foreground font-semibold truncate flex-1">{{ ancestor.description }}</span>
            <span class="text-[10px] font-mono text-tertiary shrink-0">{{ formatTokens(ancestor.usage.total_tokens) }}</span>
          </div>
        </div>
      </template>

      <!-- CURRENT -->
      <div v-if="anchorNode" class="text-[10px] text-tertiary font-semibold tracking-wider py-1">CURRENT</div>
      <div v-if="anchorNode" class="rounded-md border-[1.5px] border-semantic-blue bg-surface p-2.5">
        <div class="flex items-center gap-1.5 mb-1.5">
          <span class="text-[11px] font-bold" :class="roleColorClass(anchorNode.role)">
            {{ roleLabel(anchorNode.role) }}
          </span>
          <span class="text-[12px] text-foreground font-bold flex-1 truncate">{{ anchorNode.description }}</span>
      ...
      <!-- CHILDREN -->
      <div v-if="anchorChildren.length > 0" class="text-[10px] text-tertiary font-semibold tracking-wider py-1">CHILDREN ({{ anchorChildren.length }})</div>
      <div
        v-for="child in anchorChildren"
        :key="child.node_id"
        class="py-1.5 px-2.5 rounded-sm bg-inset border border-border-default cursor-pointer hover:border-semantic-blue hover:bg-card transition-colors"
      ...
      >
        <div class="flex items-center gap-1.5">
          ...
          <span class="text-[11px] text-foreground font-semibold truncate flex-1">{{ child.description }}</span>
          <span class="text-[10px] font-mono text-tertiary shrink-0">{{ formatTokens(child.usage.total_tokens) }}</span>
        ...
        <div v-if="child.children_ids.length > 0" class="text-[10px] text-semantic-blue mt-0.5 pt-0.5 border-t border-border-default">
