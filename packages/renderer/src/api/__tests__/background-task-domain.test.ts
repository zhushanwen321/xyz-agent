 ...
 *
 * mock 边界：command 经 vi.mock 隔离（对齐 usage-forcequit-domains.test.ts 范式），
 * reply 样例按 shared protocol backgroundTask.tasks / outputResult / killResult payload 形状。
 *
 * 运行命令：cd packages/renderer && npx vitest run src/api/__tests__/background-task-domain.test.ts
 */
...
import { list, output, kill } from '@/api/domains/background-task'
