...
export function useSidebarSubagentActions(
  ...
  /** 选中 workflow → 视图 2 详情（sidebar 内，不切 Panel） + 打开 drawer 二级 tab */
  function onSelectWorkflow(runId: string): void {
    workflowStore.selectWorkflow(panelStore.activePanelId, runId);
    // [W3] 副作用：在 drawer 中打开该 run 的二级 tab + 切到 workflow tab（可视化执行进度）
    // useWorkflowDrawerTabs 是模块级单例（与 WorkflowDrawerPane 共享分区 Map）。
    useWorkflowDrawerTabs().openWorkflow(runId);
    useSideDrawer().open("workflow");
  }
  ...
  async function onSelectAgentCall(
    agentCallSessionId: string | undefined
  ): Promise<void> {
  ...
  async function onWorkflowAction(payload: {
