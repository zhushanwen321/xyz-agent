 ...
 *
 * @param focusedSessionId 焦点 panel 绑定的 session（来自 useSidebarNew，驱动 ⌘H 快捷键的 handoff 源）。
 *   注入而非内部派生：focusedSessionId 是 useSidebarNew 的派生状态（代理 core use-session），复用避免重复定义 + 单一来源。
 */
