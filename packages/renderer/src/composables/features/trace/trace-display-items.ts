...
export type TraceDisplayItem =
  ...
  | { kind: 'block'; parent: TraceRow; index: number; block: TraceContentBlock; headline: string }
  | { kind: 'divider' }
  ...
  for (const row of rows) {
    ...
    if (isOpen) {
      extractContentBlocks(content).forEach((block, index) => {
        out.push({ kind: 'block', parent: row, index, block, headline: blockHeadline(block) })
      })
    }
  }
  return out
