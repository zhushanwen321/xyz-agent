 ...
 * actions 由调用方（useSidebarNew.initApp）注入，避免与 useSidebarNew 循环 import
 * （useSidebarNew.initApp 调本函数，本函数若再 import useSidebarNew 会成环）。
 *
 ...
 * 不依赖 api，不依赖 useCommandRegistry（本模块直接调 commandStore.registerApp，无需 session 上下文），不依赖 useSidebarNew（actions 注入破环）。
 */
...
/** 应用命令依赖的 actions（由调用方注入，打破与 useSidebarNew 的循环 import） */
export interface AppCommandActions {
  /** 新建任务（useSidebarNew.newSession） */
  newSession: () => void
  /** 进入概览（useSidebarNew.goOverview） */
  goOverview: () => void
}

/**
 * 构建并注册应用内置命令。
 * @param actions newSession/goOverview（调用方 useSidebarNew.initApp 注入）
 */
