 ...
 *
 * 链路：dispatchGlobal（真实 events 全局通道）→ onGlobal → source → MessageBusBridge → bus。
 * 不 mock events 层——AC1 明确要求经 dispatchGlobal 全链路验证。
 *
...
describe('createWsPluginMessageSource 过滤条件（FR1/AC1）', () => {
  ...
  it('TC3: extension.error 非白名单 → bridge 零感知（bus 零事件）', () => {
    ...
    dispatchGlobal({ type: 'extension.error', payload: { sessionId: 's1', code: 'boom' } })
  ...
  it('TC5: EXTENSION_BRIDGE_TYPES 字面量 5 项 + 每项行为级验证（进 bridge 产出非 error 事件）', () => {
    ...
    // （运行时 dispatchGlobal 只按 type 路由 + 透传 payload，形状正确性由 core parser 校验）。
    ...
    for (const s of samples) {
      ...
      bridge = b
      const { emitted } = spyEmit(bus)
      dispatchGlobal({ type: s.type, payload: s.payload } as never)
      expect(emitted).toHaveLength(1)
