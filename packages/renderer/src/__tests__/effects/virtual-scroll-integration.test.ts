 ...
 *
 * 设完属性后 dispatchEvent('scroll')：MessageStream.handleScroll 监听 @scroll.passive，
 * 调 virtualList.onScrollUpdate 把 DOM scrollTop/clientHeight 同步进响应式 ref，
 * 驱动 visibleRange（真 computed）失效重算（纯改 DOM scrollTop 不触发响应式，需 scroll 事件搬运）。
 */
...
describe('虚拟滚动集成 · 场景 2：DOM 只渲染可见窗口（核心收益验证）', () => {
  ...
  it('totalHeight 随 turn 数线性增长（25 turn × 200px = 5000，未上报实测前全估算）', async () => {
    ...
    await nextTick()
    // totalHeight 是真 computed：Vue setupState 代理自动解包 ref，vm.totalHeight 直接是值（25 * 200 = 5000）
    const totalHeight = (wrapper.vm as unknown as { totalHeight: number }).totalHeight
...
describe('虚拟滚动集成 · 场景 3：空态（renderItems 为空显示欢迎语不崩）', () => {
  it('空消息列表 mount 不崩 + totalHeight=0（SR12/INVAR-9 空态守恒）', async () => {
    ...
    // 基本存活
    expect(wrapper.find('.message-stream').exists()).toBe(true)
    // totalHeight = 0（空态 SR12/INVAR-9，reduce 初值 0 防 NaN）。真 computed 经 vm 自动解包。
    const totalHeight = (wrapper.vm as unknown as { totalHeight: number }).totalHeight
  ...
  it('空态 visibleRange = {0, -1}（空循环语义，无越界）', async () => {
    const chat = useChatStore()
    chat.hydrate('empty', [])
    const wrapper = mountStream('empty')
    await nextTick()
    const range = (wrapper.vm as unknown as { visibleRange: { startIndex: number; endIndex: number } }).visibleRange
    // allEntries() 为空 → n=0 分支返回 {0, -1}（调用方 `i <= -1` 自然空循环）
