...
describe('E1: 提交后空窗期 spinner running（核心 bug 回归）', () => {
  it('addPendingSend 后 isActive=true → derivedStatus=running，渲染转菊花', () => {
    ...
    // mount SessionItem 传 running status → 渲染转菊花（running/waiting 态用 spinner 替代脉冲圆点）
    const wrapper = mount(SessionItem, {
      props: { session, active: true, status },
    })
    const spinner = wrapper.find('[data-testid="sidebar-session-spinner"]')
...
describe('E2: 非焦点 session 提交后 spinner running（activeId 限定已移除）', () => {
  it('activeId=A + addPendingSend(B) → derivedStatus(B)=running', () => {
    ...
    // mount SessionItem(B) 传 running → 渲染转菊花（非焦点 session 也正确显示执行态）
    const wrapper = mount(SessionItem, {
      props: { session: sessionB, active: false, status: statusB },
    })
    expect(wrapper.find('[data-testid="sidebar-session-spinner"]').classes()).toContain('animate-spin')

    chat.clearPendingSend('B')
  })
})

describe('E3: compact 期 spinner running + Panel 渲染 Composer（isCompacting 分支）', () => {
  it('setCompacting(s1,true) → derivedStatus=running；Panel 渲染 Composer', () => {
    ...
    // 但 derivedStatus 应为 running（compact 视觉态属 running，走 isCompacting 独立分支 → spinner）
    const status = derivedStatus('s1').value
    expect(status).toBe('running')
...
describe('DOT_CLASS SSOT 一致性（附加 sanity）', () => {
  it('running 态 DOT_CLASS 仍含 animate-pulse-accent（静态圆点 fallback 映射未被意外修改）', () => {
    // DOT_CLASS 是 5 态 → 圆点 class 的 SSOT。running/waiting 在组件层改用 spinner，
    // 但 DOT_CLASS.running 映射保留（done/stopped/error 仍走圆点渲染，SSOT 表不可漂移）。
