...
export default {
  ...
  menu: {
    ...
    appearance: '外观',
    appearanceDesc: '主题、配色与分区字号偏好',
    usage: '用量',
    usageDesc: '查看 Token 用量与会话统计',
  },
  // ── Appearance 页（AppearancePage）──
  appearance: {
    regionFontTitle: '分区字号',
    regionFontHint: '按界面区域独立调整字号（×0.9 / ×1.0 / ×1.15 / ×1.3），立即生效',
    region: {
      sidebar: '左侧边栏',
      chat: '对话流',
      drawer: '侧边抽屉',
    ...
    tierLarge: '大',
    tierXLarge: '特大',
    terminalFontDesc: '终端字体大小（px），与「终端」设置页共用',
    terminalFontSaved: '终端字号已保存',
    tokenGroup: {
      bg: '背景层级',
      text: '文字',
      accent: '主色 / 状态',
  ...
  providerEdit: {
    ...
    // B-1 凭证区条件化（oauth 形态显示状态区，api_key/env_var 维持输入框）
    credentialLabel: '凭据',
    credentialOauthLoggedIn: '已登录（OAuth）',
    credentialOauthNotLoggedIn: '未登录（OAuth）',
    credentialOauthLogin: '登录',
    credentialOauthRelogin: '重新登录',
    credentialOauthLogout: '退出登录',
    credentialOauthLoggedOut: '已退出登录（{name}）',
    credentialOauthLogoutFailed: '退出登录失败',
    credentialOauthHint: 'OAuth token 存入 auth.json，由 pi 自动刷新',
    switchToApiKey: '改用 API Key',
    switchToOauth: '改用 OAuth 登录',
    switchToApiKeyConfirmTitle: '改用 API Key',
    switchToApiKeyConfirmDesc: '改用 API Key 将退出 OAuth 登录：保存后输入的 API Key 会替换 auth.json 中的 OAuth 凭证。',
    switchToOauthConfirmTitle: '改用 OAuth 登录',
    switchToOauthConfirmDesc: '改用 OAuth 将替换现有 API Key 凭证，完成授权后自动生效。',
    switchConfirmBtn: '确认切换',
    ...
    customModelsLabel: '自定义模型',
    catalogModelsMixedHint: '内置模型只读（pi catalog 升级自动刷新）；自定义模型可增删，保存后与内置模型合并生效。',
    modelSourceBuiltin: '内置',
    modelSourceOverride: '自定义',
    collapse: '收起',
    manualAdd: '+ 手动添加',
    modelLabel: '模型',
    modelNameLabel: '模型名称',
  ...
  system: {
    ...
    renameMode: '自动重命名时机',
    renameModeHint: '标题自动生成的触发时机；自动生成模式需开启上方自动重命名开关，agent 自主命名不受该开关限制，由 agent 在对话中调用 rename_session 工具改名',
    renameModeFirstPrompt: '首次请求时',
    renameModeFirstStop: '首轮回复完成',
    renameModeAgentTool: 'agent 自主命名',
    renameModeSwitched: '自动命名时机已生效；rename_session 工具面对新会话生效',
    renameModeSwitchedAutoDisabled: '时机已保存；需开启上方自动重命名开关才会自动生成标题',
    renameModel: '重命名模型',
    renameModelHint: '用于生成会话标题的模型（下个新会话生效）',
    renameModelFollow: '跟随会话模型',
    renameModelUnavailable: '（不可用）',
    smartContextTitle: '智能上下文压缩',
    smartContextEnable: '启用智能压缩',
    smartContextDesc: 'Agent 自决压缩时机 + 分档阈值提醒；关闭后回到原生压缩行为',
    smartContextModelLabel: '压缩模型',
    smartContextModelHint: '生成压缩摘要的模型；与当前会话模型一致时走同模型模式（KV 缓存命中，成本最低、摘要质量最高）',
    smartContextModelFollow: '跟随当前会话模型（KV 缓存优化）',
    smartContextThresholdLabel: '提醒阈值（3 档）',
    smartContextThresholdDesc: '上下文 token 越过某档时提醒 Agent 一次（单位 K，自行判断是否压缩）',
    smartContextThresholdInvalid: '阈值必须为正数（单位 K）',
    smartContextExcludedLabel: '排除模型（精准匹配）',
    smartContextExcludedDesc: '命中列表的会话模型完全关闭本功能，按完整 provider/modelId 精准匹配',
    smartContextExcludedAdd: '添加模型',
    ...
    llmRetryMaxRetriesLabel: '重试次数上限',
    llmRetryMaxRetriesDesc: '单次失败请求最多自动重试的次数（0-20）',
    llmRetryBaseDelayLabel: '基础等待',
    llmRetryBaseDelayDesc: '首次重试前的等待时长（秒，0-600；后续按指数退避逐次翻倍）',
    llmRetryAdvancedTitle: '高级（provider 层单请求重试）',
    llmRetryAdvancedHint: '与上层重试次数叠乘（总尝试 ≈ (1+本值)×(1+上层值)）',
    llmRetryProviderMaxRetriesLabel: '单请求重试次数',
    llmRetryProviderMaxRetriesDesc: 'HTTP 请求层的单次重试次数（0-10，留空 = 0）',
    ...
    llmRetryEffectiveHint: '保存后对新会话生效，运行中的会话不受影响',
    llmRetryUnitSec: '秒',
    llmRetryUnitMin: '分钟',
    llmRetryUnitHour: '小时',
    llmRetryUnitTimes: '次',
  ...
  usage: {
    ...
    // ── 每日消耗 ──
    sectionDaily: '每日消耗',
    sectionDailyMeta: '按 provider 堆叠 · 悬停查看明细',
    tooltipNoUsage: '无消耗',
    // ── 热力日历 ──
    sectionHeat: '节奏 · 近 16 周',
    sectionHeatMeta: '墨色越深，当日消耗越高',
    heatLess: '少',
    heatMore: '多',
    heatDayMon: '一',
    heatDayWed: '三',
    ...
    // ── 模型谱 ──
    sectionModel: '模型谱',
    sectionModelMeta: '点击行可单看该模型',
    // ── 项目谱 ──
    sectionProject: '项目谱',
    sectionProjectMeta: '按 session 工作目录聚合 · 色段为 provider 构成',
    // ── 缓存构成 ──
    sectionCache: '缓存构成',
    sectionCacheMeta: '命中 = cacheRead ÷ (cacheRead + input)',
    // ── 明细台账 ──
    sectionDetail: '明细台账',
    sectionDetailMeta: '按 provider 分组 · {groups} 组 · {range}',
    sectionDetailMetaRangeDays: '近 {n} 天',
    sectionDetailMetaRangeAll: '全部',
    // ── 空态 ──
    emptyTitle: '还没有会话记录',
    emptyDesc: '开始一次对话后，这里会出现你的用量台账。',
    emptyScanned: '已扫描 {count} 个会话文件',
    // ── 脚注 ──
