    ...
    >
      ...
      >
        ...
        <Button
          ...
          :title="
            docked ? t('panel.sideDrawer.unpin') : t('panel.sideDrawer.pin')
          "
        ...
        >
          ...
          <span class="font-mono text-[10px] text-neutral-fg">{{
      ...
      <div class="min-h-0 flex-1 overflow-auto">
        ...
        <!-- Workflow tab：workflow 可视化抽屉（二级 tab + DAG/Gantt view toggle，WorkflowDrawerPane 内自管状态） -->
        <WorkflowDrawerPane
      ...
      >
        ...
        >
          ...
          <div v-if="entry.textRaw" class="min-w-0 flex-1 overflow-hidden">
            <AnsiText
              :content="entry.textRaw"
              class="block truncate text-neutral-mid"
            />
          </div>
          <span v-else class="min-w-0 flex-1 truncate text-neutral-mid">{{
            entry.text
          }}</span>
        </div>
      </footer>
...
import WorkflowDrawerPane from "./workflow/WorkflowDrawerPane.vue";
...
const activeTabMeta = computed(
  () => tabs.value.find((tab) => tab.key === props.activeTab) ?? tabs.value[0]
);
 ...
 */
const { activeGuiComponent, activeLines, activeLinesMeta, statusEntries } =
  useDrawerWidgetBuffers(toRef(props, "sessionId"), toRef(props, "activeTab"));
/**
