  ...
  >
    ...
    <div v-if="isBatch" class="flex flex-col gap-1">
      <div
        ...
        :key="record.id"
        class="flex items-center gap-1.5 font-mono text-[var(--text-xs)]"
    ...
    <div v-else class="flex flex-col gap-1">
      <div
        class="flex cursor-pointer select-none items-center gap-1.5 font-mono text-[var(--text-xs)] transition-opacity hover:opacity-80"
      ...
      <p v-if="summaryLine" class="pl-4 text-[var(--text-sm)] leading-snug text-neutral-mid line-clamp-1">{{ summaryLine }}</p>

      <!-- 展开详情：完整 content + patchFile 提示 -->
      <template v-if="expanded">
        <div v-if="patchHint" class="ml-4 mt-0.5 rounded-sm border border-info/30 bg-info-soft px-2 py-1 font-mono text-[var(--text-xs)] text-info">
          {{ patchHint }}
        </div>
        <!-- fullContent 走 MarkdownRenderer（thinking variant，同为次要过程信息语义）。
             subagent 返回的 LLM 生成文本含 md 语法（bold/列表/标题），纯文本不渲染。 -->
        <div v-if="fullContent" class="ml-4 max-h-[200px] overflow-y-auto rounded-sm bg-surface-2/50 px-2 py-1 text-[var(--text-xs)] leading-relaxed text-neutral-mid">
