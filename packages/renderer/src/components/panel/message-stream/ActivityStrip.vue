<!--
  ...
  executing bash 行），按优先级纵向堆叠（compacting > bash > thinking）：
  ...
  - settling「收尾中…」行暂不实现（P-1 探针 V8 校准后补），rows 按行数组堆叠，结构上
    直接支持后续加行。
...
/** 活动条行（单一变化轴：行类型 + 文案）。优先级 = 数组序（compacting > bash > thinking）。 */
interface ActivityRow {
  kind: 'compacting' | 'bash' | 'thinking'
  text: string
  /** bash 行专属：mono 命令文本 */
  command?: string
}
...
const rows = computed<ActivityRow[]>(() => {
  ...
  if (!compacting && !props.executingBash && chat.sessionPhase(props.sessionId).turn === 'dispatching') {
...
/** v-for 行的函数 ref 分发（thinking 行不参与高度断言，el 丢弃） */
