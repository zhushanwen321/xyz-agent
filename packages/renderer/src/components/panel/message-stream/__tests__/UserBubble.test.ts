...
import UserBubble from '@/components/panel/message-stream/UserBubble.vue'
...
describe('W4TC3: UserBubble pending 态', () => {
  ...
  it('follow-up 模式 pending → info 青配色', () => {
    const wrapper = mountBubble({
      turn: makeTurn({
        user: { id: 'u1', role: 'user', content: 'following', status: 'pending', sendMode: 'followUp', timestamp: NOW } as Message,
      }),
    })
    const bubble = wrapper.find('.border-dashed')
...
describe('W4TC3: UserBubble 编辑态', () => {
  ...
  it('编辑态取消 → emit edit-state-change false', async () => {
    ...
    const cancelBtn = editButtons.find(b => b.text().includes('取消'))
