<template>
  ...
  <Popover v-model:open="controlledOpen">
    ...
    >
      <!-- 标题行：basename + 匹配数 -->
      <div class="border-b border-border px-2.5 py-1.5 text-[11px] text-neutral-dim">
      ...
      <div class="max-h-[220px] overflow-y-auto py-1">
        ...
        >
          ...
          <div class="min-w-0 flex-1">
            <div class="truncate font-mono text-[12px]" :class="i === activeIndex ? 'text-accent' : 'text-neutral-fg'">{{ node.name }}</div>
            <div v-if="dirPathOf(node.path)" class="truncate font-mono text-[10px] leading-tight text-neutral-dim">{{ dirPathOf(node.path) }}</div>
          </div>
        </Button>
      </div>
    </PopoverContent>
