<template>
  ...
  <Popover v-model:open="controlledOpen">
    ...
    >
      <!-- 标题行：basename + 匹配数 -->
      <div class="border-b border-border px-2.5 py-1.5 text-[var(--text-xs)] text-neutral-dim">
      ...
      <div class="max-h-[220px] overflow-y-auto py-1">
        <Button
          ...
          :key="node.path"
          variant="ghost"
          class="flex w-full items-center gap-2 rounded-none px-2.5 py-1.5 text-left text-[var(--text-sm)] leading-[1.4] transition-colors"
        ...
        >
          ...
          <div class="min-w-0 flex-1">
            <div class="truncate font-mono text-[var(--text-sm)]" :class="i === activeIndex ? 'text-accent' : 'text-neutral-fg'">{{ node.name }}</div>
            <div v-if="dirPathOf(node.path)" class="truncate font-mono text-[var(--text-2xs)] leading-tight text-neutral-dim">{{ dirPathOf(node.path) }}</div>
          </div>
        </Button>
      </div>
    </PopoverContent>
