<template>
  <!--
    ...
    - win/linux：自绘 3 彩色圆点 mimic mac（mac 原生位置 {8,8}；aside 顶在窗口 y=0，故 left-0=窗口x8 / top-[8px]=窗口y8），hover 整组显 close/min/max 符号，点击 IPC 控窗口
    全屏态 isFullscreen=true 时 opacity→0（响应式 :class 绑定，替代旧 [data-fullscreen] 祖先选择器），
    mac 系统 hover 浮层独立不参与。
  -->
  <div
    class="traffic-light absolute left-0 top-[8px] flex gap-2 z-10 transition-opacity duration-[var(--duration-slow)] ease-[var(--ease)] group"
