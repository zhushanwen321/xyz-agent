...
// 「宿主侧引擎 stderr」行同款契约；pi 侧 tee 落点 = 任务子进程 stderr（W11 前无消费面）。
//
...
beforeEach(() => {
  dir = mkdtempSync(join(tmpdir(), "pi-stderr-rot-"));
});
...
describe("路径与参数", () => {
  it("实例维度路径：<engineDataDir>/logs/pi-task-stderr-<pid>.log", () => {
    expect(stderrLogPathFor("/data", 1234)).toBe(
      `/data/logs/${PI_STDERR_LOG_PREFIX}1234.log`,
    );
    expect(PI_STDERR_LOG_PREFIX).toBe("pi-task-stderr-");
  });
  ...
  it("pid 解析：本前缀形态（含轮转副本）；非本前缀 undefined", () => {
    expect(stderrLogPidOf("pi-task-stderr-42.log")).toBe(42);
    expect(stderrLogPidOf("pi-task-stderr-42.log.1690000000000")).toBe(42);
    expect(stderrLogPidOf("other.log")).toBeUndefined();
    expect(stderrLogPidOf("pi-task-stderr-abc.log")).toBeUndefined();
  });
});

describe("尺寸轮转", () => {
  it("超阈值 rename 副本返回 true；未超 / 文件不存在返回 false", () => {
    const over = join(dir, "pi-task-stderr-1.log");
    writeFileSync(over, "x".repeat(100));
    expect(rotateStderrLogIfNeeded(over, { maxBytes: 10, keepDays: 7 })).toBe(true);
    expect(
      readdirSync(dir).some((n) => n.startsWith("pi-task-stderr-1.log.")),
    ).toBe(true);

    const under = join(dir, "pi-task-stderr-2.log");
...
describe("过期清理三判据", () => {
  it("pid 已死 + mtime 过期 → 删；任一不成立 → 保留；双实例互不删对方文件", () => {
    ...
    const deadOld = join(dir, `${PI_STDERR_LOG_PREFIX}${deadPid}.log`);
    const deadFresh = join(dir, `${PI_STDERR_LOG_PREFIX}${deadPid + 1}.log`);
    const aliveOld = join(dir, `${PI_STDERR_LOG_PREFIX}${process.pid}.log`);
  ...
  it("keepDays env 覆盖生效（1 天删 2 天旧件；7 天保留）", () => {
    ...
    if (isPidAlive(deadPid)) return;
    const f = join(dir, `${PI_STDERR_LOG_PREFIX}${deadPid}.log`);
    writeFileSync(f, "x");
