...
// node_modules 外，回退判据见 bin/pi-subagent-cli.mjs）。
//
...
//      in-flight 反向请求计时——ack 排除面见 SDK spawn.ts，R9-2/R9-2b）；
...
//
// 进程退出面：宿主 stdin EOF → 自灭守卫杀进程组（任务子进程同组连带收割——
// spawnEngineChild 硬编码 detached:false）；正常 dispose 不退进程。
