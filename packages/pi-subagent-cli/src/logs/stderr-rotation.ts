...
//
// pi 任务子进程 stderr tee 的实例维度文件名 + 轮转 + 过期清理（W11，impl-plan
// §2.11 / 设计 §3.9 写入面表同款契约；与 zcode 包 logs 模块同构——两引擎包互不
// 依赖，各自单源）。
//
...
//
// 清理三判据（三者同时成立才删，缺一不删）：
//   1. 同前缀（pi-task-stderr-）；2. pid 已死（process.kill(pid,0) 跨实例探测，
//   存活实例的文件一律不删）；3. mtime 过期（XYZ_LOG_KEEP_DAYS，缺省 7 天）。
...
export function stderrLogPathFor(engineDataDir: string, pid: number): string {
  return join(engineDataDir, "logs", `${PI_STDERR_LOG_PREFIX}${pid}.log`);
}

/** 从文件名解析实例 pid（非本前缀形态返回 undefined——调用方跳过）。 */
export function stderrLogPidOf(fileName: string): number | undefined {
  if (!fileName.startsWith(PI_STDERR_LOG_PREFIX)) return undefined;
  const rest = fileName.slice(PI_STDERR_LOG_PREFIX.length);
