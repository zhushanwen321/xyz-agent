/**
 * PS-22 探针：pi /skill: 展开格式 vs xyz-agent skill-injector 展开输出 golden diff
 * （composer-multi-skill-injection D5 守卫 / 验收场景 8）。
 *
 * 登记条目（docs/pi-semantics.json PS-22）：pi 0.84.4 的 /skill:name 展开为
...
describe.skipIf(!REAL_PI_READY)(
  `PS-22 探针：/skill: 展开格式 golden diff（真实 pi RPC${REAL_PI_SKIP_REASON ? `｜skip：${REAL_PI_SKIP_REASON}` : ''}）`,
  () => {
      ...
      async () => {
        ...
        expect(
          ...
          `注入器报 skill_missing：生产标记形态（name="${SKILL_NAME}"，无前缀）在 get_commands 映射中未命中。` +
            ...
            `修复面：skill-injector 映射/插值用剥离前缀后的 skill 名（PI 锚点 PS-22）`,
        ).toBeUndefined()
