 ...
 *
 * 巨消息豁免不在本类处理（由 broker 在 append 前用 `payload.length <= maxBytesPerSession` 过滤，
 * 避免单条 >maxBytes 入桶瞬间清空整桶）。本类只保证 append 进来的条目在双限内驱逐。
 *
...
export class SessionBuffer {
  ...
  /** 当前桶累计字节数（entries 各条 data.length 之和）。 */
  bytes = 0
   ...
   *
   * 边界：单条 data.length > maxBytes 时（理论上 broker 已豁免不入桶，但兜底防护），
