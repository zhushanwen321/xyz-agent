...
describe('ConfigService authMethod 透传与推断（I6，wave-quick-setup-c TC7）', () => {
  it('setProvider authMethod 写 providers.json（A1-5 写侧切换），不再透传 models.json', async () => {
    ...
    const svc = new ConfigService('/tmp/project', mockStore, undefined, extrasStore as unknown as Pick<XyzProviderStore, 'modify' | 'getExtrasSync' | 'readAllSync' | 'getScopedModelsSync' | 'modifyScopedModels' | 'cleanScopedModelsResidue'>)
  ...
  it('listProviders 回填 authMethod：providers.json 标注值优先；无标注 $开头→env_var，非空→api_key，空→undefined', () => {
    ...
    } as unknown as Pick<XyzProviderStore, 'modify' | 'getExtrasSync' | 'readAllSync' | 'getScopedModelsSync' | 'modifyScopedModels' | 'cleanScopedModelsResidue'>
