...
//   - T2③/LC-1 + D9：deliverMessage 热路径 armMidRoundNoProgress（挂载中段无进展检测
//     + 到期 onMidTimeout 处置：kill + 失败终态化 + error 含 'settled watchdog' 标记
//     与恢复指引；agent_end 交棒收尾段的接线在 session-runner stdout pump，两路径共用）；
