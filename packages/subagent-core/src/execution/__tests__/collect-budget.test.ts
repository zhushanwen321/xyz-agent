...
describe("buildBatchLlmContent 预算截断组装（设计 §3.1.2）", () => {
  ...
  it("6 成员各 3000（Σ=18000）：不触发第二段——每条全量在场、零指针行", () => {
    const records = Array.from({ length: 6 }, (_, i) => filled(`bg-${i}`, 3000));
    const content = buildBatchLlmContent(records);
    const parts = content.split("\n\n---\n\n");
  ...
  it("n=125：纯清单退化——每条只剩头行 + 指针行，正文零残留", () => {
    const records = Array.from({ length: 125 }, (_, i) => filled(`bg-${i}`, 1000, "B"));
    const content = buildBatchLlmContent(records);
    const parts = content.split("\n\n---\n\n");
  ...
  it("长短参差（1 短 500 + 6 长 6000）：统一收紧 3428，短条目足额保留——非瀑布分配", () => {
    ...
    const shortOne = filled("bg-short", 500, "S");
    const longs = Array.from({ length: 6 }, (_, i) => filled(`bg-long-${i}`, 6000));
  ...
  it("totalChars 口径：仅计条目正文之和——Σ=23898 < 24000 时包装开销（批头/头行/指针行/分隔符）不触发收紧", () => {
    ...
    // 但预算判定只看正文和 → 不收紧
    const records = Array.from({ length: 6 }, (_, i) => filled(`bg-${i}`, 3983));
    const content = buildBatchLlmContent(records);
  ...
  it("Σ=24000 恰好等于 totalChars：不算超（> 才收紧），6×4000 全量在场零指针行", () => {
    const records = Array.from({ length: 6 }, (_, i) => filled(`bg-${i}`, 4000));
    const content = buildBatchLlmContent(records);
    expect(content).not.toContain("[truncated");
    const parts = content.split("\n\n---\n\n");
    for (let i = 0; i < 6; i++) {
  ...
  it("失败成员不入正文预算：failed error 全文原样，且不计入 Σ——成功条目 kept=4000 而非收紧 3428", () => {
    const failedOne = member("bg-bad", { outcome: "failed", error: "boom".repeat(2000) });
    const longs = Array.from({ length: 6 }, (_, i) => filled(`bg-${i}`, 6000));
    ...
    expect(content).toContain(`Subagent "worker-bg-bad" (bg-bad) failed: ${"boom".repeat(2000)}`);
    const failedPart = parts.find((p) => p.includes("(bg-bad)")) ?? "";
