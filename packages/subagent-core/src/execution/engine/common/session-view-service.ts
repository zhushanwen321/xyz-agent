...
/**
 * zcode ①级 reader：sessionRef 的 dbPath/sessionId 重定位 + 池目录白名单 +
 * readZcodeSessionView（extension/runtime 双端复用的同一份 reader）。
 * 语义复刻被收敛前的 runtime readZcodeNativeTier：dbPath 锚池目录 resolve；
 * 越界路径（../ 逃逸 / 跨池）拒绝降②级；读取失败（结构化错误）降②级。
 */
async function readZcodeNativeTier(
  ...
  // 白名单（record 来自 JSONL 文本不可信）：dbPath 必须严格落在该引擎自己的池目录内
  // ——防 ../ 逃逸与跨池读取
