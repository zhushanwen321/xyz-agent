...
// [H2 W2 迁移步⑥] mergeRunSignals 公共 helper（原定义在本文件，提取落点 engine/common）
...
// [H2 W2 迁移步⑥] mergeRunSignals/MergedRunSignalHandle 已提公共 helper
//（engine/common/run-signals.ts——SubagentService.executeWorkflowAgent 接管 workflow
// 派发后两调用点共用同一实现，本文件内部消费经顶部 import）。此处 re-export 保持
// 既有 import 路径（测试消费面零改动）；W4 掏空 run() 后以 common 为唯一权威落点。
