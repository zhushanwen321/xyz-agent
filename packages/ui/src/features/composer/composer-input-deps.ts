 ...
 * ChipCallbacks.getSlashIcon / ChipCallbacks.t）：
 * - pasteImage：粘贴/拖入图片处理（走 writeSessionImage IPC 落盘，core 不 import IPC）
 * - getSlashIcon：slash 命令 chip 图标组件查找（SLASH_ICON_COMPONENTS 映射，core 不 import lucide 资源）
 * - t：i18n 文案（chip × 按钮 aria-label，core 不 import i18n）
 */
export interface ComposerInputDeps {
  ...
  /** 取 slash 命令对应的图标组件（iconKey 如 'terminal'/'star'/'wrench'） */
