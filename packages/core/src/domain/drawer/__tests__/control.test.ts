 ...
 *
 * 状态隔离：drawer 域是模块级单例（controlState/瞬时参数跨用例残留），
 * beforeEach 调 _resetDrawerForTest()（coordination 导出，组合清 control 分区 + 瞬时参数）。
 * 绑定目标 ref 每个用例新建（bindDrawerSessionId 新 ref 覆盖语义）。
 */
