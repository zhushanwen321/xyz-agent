...
describe('settings-store fail-fast', () => {
  ...
  it('transport 未注入 → setSkillDirs throw 含 getSettingsTransport', async () => {
    ...
    providePlatform({ kind: 'mock', storage, webSocket: { create: () => ({}) as never }, ipc: null })
    const store = createSettingsStore()
    await expect(store.setSkillDirs(['/a'])).rejects.toThrow('getSettingsTransport')
  })
})
...
describe('settings-store 路径配置经 IF1 transport 转发', () => {
  it('setSkillDirs/setAgentDirs/setExtensionDirs 调用对应 transport 方法', async () => {
    ...
    const store = createSettingsStore()
    await store.setSkillDirs(['/skills'])
    await store.setAgentDirs(['/agents'])
    await store.setExtensionDirs(['/ext'])
    expect(transport.setSkillDirs).toHaveBeenCalledWith(['/skills'])
    expect(transport.setAgentDirs).toHaveBeenCalledWith(['/agents'])
    expect(transport.setExtensionDirs).toHaveBeenCalledWith(['/ext'])
  })
})
