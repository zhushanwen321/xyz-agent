 ...
 *
 * [W3 迁移] 迁自 renderer composables/panel/useComposerForkMode.ts。改动：
 * - 跨域能力（vue-i18n t / GitFork icon / forkSessionAsk / toastError / forkEnterSignal channel）
 *   去掉 renderer import，改为 ForkDeps 字段注入（core 零 renderer 依赖）
 ...
 * - handleForkEsc 参数 KeyboardEvent → KeyboardEventLike（core 零 DOM 约束，真实 KeyboardEvent 兼容）
 * 逻辑 byte-level 保持。
 */
...
export function useComposerForkMode(
  ...
  watch(deps.forkEnterSignal, (req) => {
    if (!req) return
  ...
  /** fork 模式 placeholder 文案；非 fork 模式返回 null（调用方回退到普通 placeholder） */
  const forkPlaceholder = computed(() => (forkMode.value ? deps.t('panel.composer.forkHint') : null))

  /**
   * Esc 处理：fork 模式下清空输入 + 退出。
   * @returns true 表示已消费（composer 聚焦时优先于全局 Esc handler）
   */
  function handleForkEsc(e: KeyboardEventLike): boolean {
  ...
  async function handleForkSend(text: string): Promise<boolean> {
    ...
    try {
      ...
      await deps.forkSessionAsk(srcSessionId, fromMessageId, text, staging)
    } catch (e) {
      deps.restoreInput(text)
      const msg = e instanceof Error ? e.message : String(e)
      deps.toastError(deps.t('panel.panel.sendFailed', { error: msg }))
    } finally {
  ...
  function asStagingAction(): StagingAction {
    return {
      ...
      enter: (source) => {
        ...
        const forkSourceArg = source as { srcSessionId: string; fromMessageId: string }
        enterForkMode(forkSourceArg.srcSessionId, forkSourceArg.fromMessageId)
      },
      exit: () => exitForkMode(),
      /**
