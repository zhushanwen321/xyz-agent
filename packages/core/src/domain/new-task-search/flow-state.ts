/**
 * flow-state —— 新建任务状态机（core 域迁移版，IF4/DM4）。
 *
 ...
 * 真调用 + 业务守卫，留 flow/branch/dir-select）。
 *
 * 依赖方向：vue reactivity（ref/readonly）+ @xyz-agent/shared（SessionSummary）。core 零跨包 import。
 */
 ...
 * 父编排器按需把具体 setter 作为参数传给子 composable（见 branch 签名）。
 */
