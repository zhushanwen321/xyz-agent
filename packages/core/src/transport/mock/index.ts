...
export const session = {
  ...
  async importCandidates(_payload: import('@xyz-agent/shared').ImportCandidatesRequest): Promise<import('@xyz-agent/shared').ImportCandidatesReply> {
    await sleep(TIMING.ack)
    return { total: 0, items: [], dirs: [] }
  },
  /** Mock importSession：恒 reject（空候选集下不可达；与 fetchCurrentSystemPrompt 同形，供 UI 错误态演示）。 */
  async importSession(_payload: import('@xyz-agent/shared').ImportRequest): Promise<import('@xyz-agent/shared').ImportReply> {
    await sleep(TIMING.ack)
