...
//
// zcode 引擎 CLI 入口（manifest bin）。加载顺序 dist-first（MF-5 修复，约束 C-proc-11）：
...
if (existsSync(distEntry)) {
  ...
  console.error(
    [
      `[zcode-subagent-cli] engine entry not found: ${distEntry}`,
      "",
      ...
      "    (npm i @zhushanwen/zcode-subagent-cli@latest)",
      "  - workspace checkout: pnpm --filter @zhushanwen/zcode-subagent-cli build",
    ].join("\n"),
  );
  process.exit(1);
}
