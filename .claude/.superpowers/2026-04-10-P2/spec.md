...
6. **混合持久化** — TaskNode 元数据存主 JSONL，SubAgent 对话存独立 JSONL（sidechain）
7. **结构化通知** — XML 格式回传结果，含 usage 统计
8. **反馈 + 干预** — feedback 工具（子→父中途通信）+ 用户暂停/恢复/终止
...
### 修改 Rust 文件
- `src-tauri/src/types/event.rs` — AgentEvent 新增 4 变体 + 更新 session_id()/variant_name()
- `src-tauri/src/types/transcript.rs` — TranscriptEntry 新增 TaskNode 变体 + 更新 uuid()/parent_uuid()
- `src-tauri/src/engine/tools/mod.rs` — Tool trait 增加 ctx 参数 + 注册 dispatch_agent
- `src-tauri/src/engine/tools/read.rs` — call 签名增加 ctx 参数
- `src-tauri/src/engine/tools/write/mod.rs` — call 签名增加 ctx 参数
- `src-tauri/src/engine/tools/bash/mod.rs` — call 签名增加 ctx 参数
- `src-tauri/src/engine/loop_/mod.rs` — run_turn 新增 budget_guard + 构建 ToolExecutionContext
- `src-tauri/src/engine/context/prompt.rs` — PromptManager 新增 new_with_prompt()
- `src-tauri/src/lib.rs` — AppState 新增 Arc<Mutex<TaskTree>>, Arc<AgentTemplateRegistry>
- `src-tauri/src/store/jsonl.rs` — LoadHistoryResult 新增 task_nodes 字段
- `src-tauri/src/api/session.rs` — get_history 返回 task_nodes
