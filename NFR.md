...
- **前端直接发 key 给 pi，不做 key→value 映射（S-12）**：`ThinkingLevelPopover.onSelect` emit `opt.level`（pi 档位名），`sessionApi.setThinkingLevel(sid, key)`。**禁止调 `resolveThinkingValue` 做 key→value 映射**——pi 内部 provider 层自查 `thinkingLevelMap[key]` 取 value 发 API，前端越权会导致 pi 存 value 而非 key，provider 层按 key 查不到。前端枚举对齐 pi（off/minimal/low/medium/high/xhigh，去掉 max）。验证：`ThinkingLevelPopover.vue` grep `emit('select', opt.level)` 无 `resolveThinkingValue`。`[from: 2026-07-02-thinking-level-and-model-select §nfr N1]`
