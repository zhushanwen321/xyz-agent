...
- 2026-09-12 Gate B 发现登记（fail 级为零，以下均边角形态无数据丢失、非 H4 回归，待用户裁决是否立项修复）：**F1**（S2-B）mid-flight cancel（`.record-binding` settle 才落盘 → 盘上无锚）→ `markCancelled` warn 分支 `.state` 缺席 + `.alive` 残留（pid 死不阻塞探针，manifest/entry 承接终态）——潜在改进 = cancel 路径锚点缺失时反查子文件；**F2**（S5）entry-only record（zcode 引擎无子 session 文件）不在 rebuild 源（reconstructAll），缓存全删后 manifest 投影永久缺失（3/67）——潜在改进 = rebuild 补扫主 session custom entry 腿；**F3**（S1-adopt）设计 D8 第五行「one-shot 纳管态×shutdown = closed(parent-shutdown) 可重连」**结构性不可达**：纳管态仅 zcode 引擎可产生（pi 引擎 kickOffChatRound catch 无 adopt 分支），zcode record 恒 entry-born，`recoverEntryOnlyOrphans→finalizeEntryOnlyOrphan`（no child session file）先于 bootPartition 直断 closed/gc——实际行为 = closed/gc 干净终态（对照设计声明少了「可重连」），潜在修复 = finalizeEntryOnlyOrphan 补 resumable-无-result 保留分支（镜像 finalizeOrphanRecord），或设计文档按现实修订 D8 第五行作用域（阶段 6 同步处理）
