...
# CW_HOME= (空串) 是有意的隔离逃逸: cw verify 给验收子进程注入隔离 CW_HOME(mkdtemp),
# 那里没有账本; getCwHome 把空串视为未设置回落 ~/.cw, 才能读到真实 closure 状态。read-only。
out=$(cd "$CANONICAL_REPO" && CW_HOME= cw status 2>/dev/null)
fail=0
