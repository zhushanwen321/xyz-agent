...
describe('R2 豁免表放行', () => {
  it('豁免文件整文件跳过（file 级，对齐 R1 D-10 先例——行号键随编辑漂移永不生效）', () => {
    // 本分支（方案 B 未合并）：migrate-pi-layout-v2 两交付物不在仓，豁免-放行 fixture
    // 换本分支既有豁免文件（方案 B 合并时随 sweep 恢复原 fixture 并回收过渡豁免）
    expect(scanFile('packages/runtime/src/services/session/workflow-extractor.ts', "join(dd, 'pi', 'sessions')")).toEqual([])
    ...
    expect(scanFile('docs/troubleshooting.md', "const DIR = '/Users/x/.xyz-agent/pi/sessions'")).toEqual([])
  })
