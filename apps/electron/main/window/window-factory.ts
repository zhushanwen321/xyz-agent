...
export async function createWindow(
  ...
  const win = new BrowserWindow({
    ...
    show: false,
    title: 'TaiJi',
    // 跨平台窗口装饰（shell spec §五方案 X + v6-spec-shell.html SSOT）。
    // mac：hidden + trafficLightPosition 把红黄绿放到 macOS 原生左上角位置 {16,26}（圆点中线 y=26+6=32）。
    //   不用 hiddenInset：inset 模式强制红黄绿水平内缩，trafficLightPosition.x 被系统忽略。
    //   hidden 模式下红黄绿仍由 OS 绘制（点击/全屏 hover 行为不变），位置可控。
    //   原生位置：圆点 12px，顶 y=26 / 中线 y=32；左缘 x=16 / 右缘 x=68（红 16~28 / 黄 36~48 / 绿 56~68）。
