...
- **Manual-name race guard**: the session name is re-checked immediately before persisting, so a name set via pi (`set_session_name` / rename RPC) during the 2-30s LLM call window is not overwritten (`skip: name exists`); out-of-band JSONL edits (e.g. xyz-agent runtime rename) are not visible to the guard.
