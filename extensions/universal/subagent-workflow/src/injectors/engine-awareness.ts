...
// lastEngine diff → 变更时按硬约束顺序编排：reload 路由缓存（D2，先于一切——
...
// （readConfig / reload / sendMessage）由 index.ts engine handler 装配注入。
...
/**
 * defaultEngine 缺省归一：空白 / undefined 归一到缺省引擎（'pi'）。
 * 与 model-prompt.ts 的 `defaultEngine?.trim() || CORE_ENGINE_ID` 同一 normalize——
 * 检测 diff 与状态段渲染必须对同一读取结果给出同一引擎 id，否则两处会说谎。
...
export interface EngineAwarenessDeps {
  ...
  /** 变更生效：reload 路由缓存（D2；生产 = ModelConfigService.reloadGlobalConfig）。 */
  reload(): void;
 ...
 *
 * 分派（D5 三态 × D1b 基线）：
 *   - read failed → 保持 lastEngine 不动、不 reload、不通知（防 torn write 瞬间伪通知）；
 ...
 *   - last ≠ target（含 ENOENT=合法缺省）→ reload 先行 → 通知 → 记账，三步同 turn。
 *
 * 抛错面：不主动 try-catch——readGlobalConfig 三态返回不抛，reload/sendMessage 的
...
export function runEngineAwarenessTurn(deps: EngineAwarenessDeps): EngineAwarenessOutcome {
  ...
  // D2 顺序硬约束：先 reload（本 turn 路由 + 渲染同源生效）→ 再通知 → 再记账。
  deps.reload();
  deps.sendMessage({
