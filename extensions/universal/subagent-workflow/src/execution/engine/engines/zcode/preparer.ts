...
//   - 凭据源从「仅 v2 config」泛化为「v2 config + cli config 双源合并」——本机实测
//     （2026-08-25 验收前置门）cli/config.json 可能只含局域网网关 provider（不可达），
//     桌面登录态在 v2/config.json；cli/config.json 独有的 provider（如 router）也该可用；
...
interface SourceConfig {
  /** provider 注册表（可能来自两源，逐条运行时 guard）。 */
  providers: Map<string, ZcodeProviderEntry>;
  /** 主模型（仅 cli config 携带；v2 实测无 model 顶层键）。 */
  mainModel?: string;
...
function readSourceConfig(absPath: string): SourceConfig {
  ...
  const main = isRecord(model) ? model["main"] : undefined;
  if (typeof main === "string" && main.trim() !== "") conf.mainModel = main.trim();
  try {
...
export interface ZcodeSourcePaths {
  /** 桌面登录态 config（主凭据源）。缺省 ~/.zcode/v2/config.json。 */
  v2ConfigPath?: string;
  /** CLI 自身 config（次凭据源 + model.main）。缺省 ~/.zcode/cli/config.json。 */
...
function defaultCliConfigPath(): string {
  return path.join(os.homedir(), ...ZCODE_CLI_CONFIG_PATH_SUFFIX);
}
 ...
 *
 * 解析链（zsub 同构，requested > 默认链）：
 *   1. 显式 requested（task.model）；
 *   2. cli config 的 model.main（CLI 自己选定的主模型）；
 *   3. ZCODE_FALLBACK_DEFAULT_MODEL（zsub 同构兜底）。
 *
...
export function resolveZcodeModelRef(requested: string | undefined, sources?: ZcodeSourcePaths): string {
  ...
  const cli = readSourceConfig(sources?.cliConfigPath ?? defaultCliConfigPath());

  // 双源合并：同 id 时 v2 优先（桌面端刷新凭据的落点），cli 独有 provider（如 router）补充
  const merged = new Map<string, ZcodeProviderEntry>([...cli.providers, ...v2.providers]);
  const withKey = [...merged.entries()].filter(([, e]) => hasApiKey(e));
  const wanted = requested?.trim() || undefined;
  const target = wanted ?? cli.mainModel ?? ZCODE_FALLBACK_DEFAULT_MODEL;
  ...
  if (withKey.length === 0) {
    ...
    throw new ZcodePrepareError(
      "engine_credential_missing",
      `zcode 引擎找不到任何带 apiKey 的 provider（${hint}）。已读源：${sources?.v2ConfigPath ?? defaultV2ConfigPath()} 与 ${sources?.cliConfigPath ?? defaultCliConfigPath()}。` +
...
export function prepareZcodeHome(opts: {
  ...
  const cliPath = opts.sources?.cliConfigPath ?? defaultCliConfigPath();
  const v2 = readSourceConfig(v2Path);
  const cli = readSourceConfig(cliPath);
  const provider = providerOf(modelRef);
  const v2Entry = v2.providers.get(provider);
  const cliEntry = cli.providers.get(provider);
  const entry = v2Entry ?? cliEntry;
  if (entry === undefined) {
    // provider 在两源注册表都不存在：模型引用不可解释（resolve 期漏网或直接调用）
    throw new ZcodePrepareError(
      "model_not_available",
      `未知 provider "${provider}"（两源 config 均无该条目：${v2Path} / ${cliPath}）。` +
        `恢复指引：先经 resolveZcodeModelRef 校验模型引用，或改用已配置的 provider 后重试。`,
    );
  }
  if (!hasApiKey(entry)) {
    ...
    throw new ZcodePrepareError(
      "engine_credential_missing",
      `provider "${provider}" 存在但未配置 apiKey（${v2Path} / ${cliPath} 重读无凭据）。` +
        `恢复指引：确认 ZCode 桌面端登录态有效后重试；凭据配置说明见 docs/research/agent-engine-zcode.md。`,
    );
  }

  // 比对基准取「实际命中该 provider 的源」的 mtime（另一源刷新不相关条目不应触发重写）
  const hitSourceMtime = v2.providers.get(provider) !== undefined ? v2.mtimeMs : cli.mtimeMs;
