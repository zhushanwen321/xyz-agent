...
describe("package.json xyz-agent.subagentEngines 静态声明一致性（U7b 冷启动回退源）", () => {
  ...
  it("与组合根注册的引擎一致（registerPiEngine/registerZcodeEngine 后 listEngines 等值）", async () => {
    ...
    await import("../engines/pi/registration.ts").then((m) => m.registerPiEngine());
    await import("../engines/zcode/registration.ts").then((m) => m.registerZcodeEngine());
    const { listEngines } = await import("../registry.ts");
