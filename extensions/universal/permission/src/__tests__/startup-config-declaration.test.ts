 ...
 *
 * 运行：cd extensions/universal/permission && npx vitest run src/__tests__/startup-config-declaration.test.ts
 */
...
describe("startupConfig 声明守护", () => {
	it("声明 content 与代码 DEFAULT_CONFIG 深相等", () => {
		const entry = pkg["xyz-agent"]?.startupConfig?.find(
			(e: StartupConfigEntry) => e.path === "config/permission-ext-config.json",
		);
