...
首次使用需要生成 `<agentDir>/config/model-switch-ext-config.json` 配置文件（`<agentDir>` = pi agent 目录，`PI_CODING_AGENT_DIR` 覆盖，默认 `~/.pi/agent`）。
...
按照下方配置格式手动创建 `<agentDir>/config/model-switch-ext-config.json`。
...
`<agentDir>/config/model-switch-ext-config.json`：
...
自动生成 `config/model-switch-ext-config.json` 的流程：
...
- 运行 `/setup-model-policy` 可自动生成配置（写入 `<agentDir>/config/model-switch-ext-config.json`）
