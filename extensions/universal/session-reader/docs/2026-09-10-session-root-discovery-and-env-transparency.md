     ...
     b. agent/ 已存在（先升后迁：新版启动过、agent/ 含窗口期增量）：
        逐项搬移 pi.backup-v2-<ts>/agent/* → <dataDir>/agent/，按域分三种规则：
          ...
          · provider 三件套（auth.json 凭据 / models.json 自定义 provider 定义 /
            ...
            **域外顶层键**（providers.json 的 version/scopedModels、models.json 未来
            新增顶层键）→ 偏好旧赢并进冲突清单。**防御式降级**：任一侧 JSON 解析失败、
            域路径解析不到、或域内值非 object 形态（pi 未来改 schema）→ 该文件整体回退
            「旧赢 + 新避让」并进冲突清单——宁可方向性已知损失，不做坏数据合并。
...
- v9（2026-09-10）：用户两问驱动的修订（迁移触发判断简化 + 双仓漂移升级为执行范围）：
  ...
  6. **第 9 轮修复**（影面审 1 MF + 1 SG，实机核证）：①**keyed union 域锚点勘误**——v9.2「三件同为顶层 keyed map」前提与实机不符（仅 auth.json 顶层 keyed；models.json 顶层单键 `providers` 嵌套；config/providers.json 顶层含 version/scopedModels 非域键），按字面实施 models.json 交叉失配复活、providers.json 恒定降级；v9.3 锚定真实域路径（auth 顶层 / models 与 providers 在 `.providers`），域外顶层键偏好旧赢进冲突清单，降级判据改「域路径解析失败或域内值非 object 形态」，V9⑦ 场景 X/Y 用例分别构造在三个文件上，形态实测入事实表；②union 写回补 tmp+rename **原子写**声明（对齐 §6.12 写语义）+「旧侧完整文件留备份、不产中间态」。
