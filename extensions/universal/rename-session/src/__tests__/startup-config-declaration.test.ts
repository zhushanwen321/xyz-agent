 ...
 * content 与代码 DEFAULT_RENAME_CONFIG 常量深相等，防止两侧任一改动未同步（漂移）。
 * 机制：runtime 启动序列统一 ensure（extensions-startup-config.ts），声明即首建内容。
 *
 * 运行：cd extensions/universal/rename-session && npx vitest run src/__tests__/startup-config-declaration.test.ts
 */
...
import { DEFAULT_RENAME_CONFIG } from "../pure.js";
...
describe("startupConfig 声明守护", () => {
	it("声明 content 与代码 DEFAULT_RENAME_CONFIG 深相等", () => {
		const entry = pkg["xyz-agent"]?.startupConfig?.find(
			(e: StartupConfigEntry) => e.path === "config/rename-session-ext-config.json",
		);
		expect(entry).toBeDefined();
		expect(entry?.content).toEqual(DEFAULT_RENAME_CONFIG);
	});
});
