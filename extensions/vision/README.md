...
- **多模型支持**：从 `<agentDir>/config/vision-ext-config.json` 读取模型配置（`<agentDir>` = pi agent 目录，`PI_CODING_AGENT_DIR` 覆盖，默认 `~/.pi/agent`）
...
创建 `<agentDir>/config/vision-ext-config.json` 指定视觉模型：
