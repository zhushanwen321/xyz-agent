...
function registerSessionLifecycle(pi: ExtensionAPI): void {
	...
	pi.on("message_end", (event, ctx: ExtensionContext) => {
		...
		// message_end 的 message 是 AgentMessage 联合。role 已收窄到 "assistant"，
		// 再经 unknown 中转断言为 AssistantMessage 取 usage（Pi 的 AssistantMessage 与 pi-ai 同源）。
...
function makePalette(theme: Theme): Pallet {
	// theme.fg 现在需要 ThemeColor（Pi SDK 收窄了签名）。此处 c 用 ThemeColor，
	// 调用方只传字面量 token（"dim"/"text"/...），全部是 ThemeColor 的合法成员。
...
function buildLines(
	...
	// theme.fg 现在需要 ThemeColor。format.ts 的回调签名保持 string（不引入 Pi 依赖），
	// 这里把 token 单步断言为 ThemeColor：调用方只传 pctColor() 的字面量结果
	// ("error"|"warning"|"accent"|"success")，全是 ThemeColor 合法成员，运行时安全。
	const themeFg = (token: string, text: string) => theme.fg(token as ThemeColor, text);
