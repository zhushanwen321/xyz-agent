/**
 * [HISTORICAL] permission 配置迁移脚本（幂等，可长期保留）
 *
 * 迁移：<agentDir>/permission-config.json → <agentDir>/config/permission.json
 *
 * 触发时机（迁移在 npm 安装时自动完成，运行时不双读旧路径）：
 * - 原生 pi：`pi install npm:@zhushanwen/pi-permission` 走真实 npm CLI → postinstall
...
/** permission 配置迁移入口：permission-config.json → config/permission.json。 */
