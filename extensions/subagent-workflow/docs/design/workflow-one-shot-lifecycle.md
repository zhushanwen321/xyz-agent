...
- **C（冲突）**：pause/resume 是**只有手动入口、没有任何程序化消费方**的能力（cw 编排不经它；嵌套 workflow 只用 signal 继承不用 pause），却持续征税：①「terminate worker + 重跑脚本 + callId 缓存 replay」机制要求**用户脚本逐字节确定性**——LLM 生成的脚本写个 `Date.now()`/`Math.random()` 重跑就走分叉、replay 错位；② paused 态在 error-recovery 里散布 6+ 处守卫，是一类「暂停期间消息乱入」的误伤面；③ `discardInFlightCalls`（lifecycle.ts:288）是 pause 竞态的补丁，代码里挂着 `MUST_FIX (round-4 #1)` 注释（lifecycle.ts:265）。
- **Q（问题）**：能否在**不损失任何真实使用能力**的前提下，把 pause/resume 连同它的机制族整体剔除，让 workflow 生命周期收敛到与 subagent 同构的「一次性」语义？
- **A（答案）**：能。关键事实：**崩溃恢复现状本就只到 `done,failed`**（jsonl-run-store.ts:211-213 的 D-4 注释：kill-9 后残留 running 在 session_start 转 failed，不存在真正的跨进程 resume）——pause/resume 是唯一真实的 resume 路径，而它无消费方。删除它是「现状诚实化」而非「能力削减」。worker 崩溃重试（error-recovery 的 3 次指数退避 + rebuildRuntime + replay）是另一个独立机制，保护已完成的 agent 调用投入，**保留**。
...
|---|---|---|
| G1 | **功能不回归**：run/status/abort、嵌套 workflow、token/time 预算、worker 崩溃重试、kill-9 残留检测全部保持 | 现有 5 个内置 workflow 与自定义脚本行为完全不变 |
...
**本章结论：pause/resume 的三个机制（terminate+重跑 replay、paused 散布守卫、discard 竞态补丁）服务一个没有消费方的能力；且「跨进程恢复」在现状中本就不存在——一次性化是诚实化，不是削减。**
...
2. **paused 态的散布成本**：error-recovery.ts 里 `isTerminal(run) || run.state.status === "paused"` 的守卫出现在至少 6 处（:169/:177/:218/:236/:346/:414/:453/:622/:667/:698—— stale 消息丢弃、replay 判断、live record 清理、stale 完成守卫、error 计数污染防护），每一处都是「暂停期间异步消息乱入」的误伤面；lifecycle.ts:116 的时间预算 timer 要为 paused 重排；gui-mappers.ts:34 的状态映射、WorkflowsView 的按键分支（:435-446/:696-698）都带 paused。
3. **「跨进程恢复」本就不存在**：jsonl-run-store.ts:211-213 注释自述——`WorkflowRun.reconstruct` 跳过 I1 校验（running ⟺ runtime 存在）因为「进程被杀后 worker 不可能还活着」，D-4 在 session_start 把残留 running 转 `done,failed`。**kill-9 / 崩溃后的 run 从来不能续跑**；pause/resume 是唯一的 resume 路径，而它只服务手动场景。
...
| P2 | **paused 态在错误处理层散布 6+ 处守卫** | 每新增一个异步消息处理点都要记得补 paused 分支，漏一个就是「暂停期间消息乱入」类 bug | paused 是一个与异步事件流正交的状态，守卫只能点状分布 |
...
| P4 | **能力无消费方** | cw 编排不经 pause；嵌套 workflow 用 signal 继承；唯一入口是 `/workflows pause` 手动命令与工具 action | pause 是「run 挂 session 生命周期」逼出的跨 session 存续补救，而非真实用户需求 |
...
[重启] session_start → loadAll 读到残留快照（status:"running" 无 runtime）
  → 直接转 done,failed（reason:"process_crashed" 或现有等价 reason）
[用户] /workflows → 列表显示该 run failed，含崩溃时间；无 running 幽灵
```
...
|---|---|---|---|---|
| **A 一次性化（本方案）**：删 paused 态 + pauseRun/resumeRun + pause 场景 replay；崩溃重试及 replay 保留；接口面清除 | ✅ 状态机两态与 subagent record 模型同构（active/closed + 派生态）；守卫清零；无消费方能力不征税 | 中：净删 ~400-500 行（lifecycle 两个函数 + error-recovery 守卫分支 + 接口面 + 视图），无新增机制 | 低：无程序化消费方；崩溃重试独立保留；风险集中在「漏删某处 paused 引用」→ 编译期类型 + grep 验收兜底 | ✅ **推荐** |
| **B 保留 pause 并修复**：内容寻址 callId（hash 替代自增）+ 确定性原语（$NOW/$RANDOM 入日志）+ 事件日志替代快照 | ⚠️ 修复了 P1/P3 但 paused 散布守卫（P2）仍在；为一个无消费方能力新增三个机制（hash、原语注入、日志格式），方向与「减法优先」相反 | 高：三个新机制 + 迁移 | 中：机制增多、契约仍在（内容寻址容忍分叉但 replay 命中率下降时 token 重复消耗） | ❌ 否决 |
...
**被否方案的具体后果**：用方案 B，§2.1 的旅程变成「pause 后 resume 时 $NOW 重放同值、hash 命中缓存」——功能对了，但 error-recovery 的 6+ 处 paused 守卫一处不少，且新增的三个机制每个都要长期维护；用户得到的却是一个没人用的能力。用方案 C，§3.1 场景 E 变成「worker 崩溃 → 已完成的 5 个 review 调用（~20 万 token）作废，用户重新跑」——省钱机制变烧钱机制。
...
- **依据**：两态与 subagent record 的 active/closed 模型同构（V4 B-1 方向在编排层的镜像）；「是否挂起」不再是状态——一次性语义下不存在挂起。
- **被否**：保留 paused 但废弃入口（半吊子：状态还在，守卫还得留）。
- **探针**：⛔ S8 grep 断言：`src/` 下无 `pauseRun|resumeRun|"paused"` 运行时引用（允许 CHANGELOG/本文档/迁移注释提及）。

#### D-2：session 切换 / kill-9 的统一语义 = 转 failed（现状诚实化）

- **选择**：session 切换（dispose）或进程重启后，任何非 done 残留 run → `done,failed`。**不再区分「running 残留」与「paused 残留」**——paused 快照格式随状态机删除。loadAll 的恢复逻辑（jsonl-run-store.ts:211-213 的 D-4）从「session_start 时转」前移/合并为「loadAll 时直接转」（实现时二选一，以不引入新 hook 为准）。
- **依据**：现状对 running 残留本就如此（§2.1 承重事实 3）；paused 残留此前是「可 resume」，删除 pause 后语义统一。跨 session 存续需求明确降级为 out-of-scope（§1）。
- **探针**：⛔ S3：kill -9 真实场景断言列表无 running/paused 幽灵。
...
- **选择**：`SNAPSHOT_VERSION` 从 `wf-run-v1` bump `wf-run-v2`（快照 status 枚举收窄为 running/done）；旧 v1 文件 loadAll 静默跳过——沿用现状 D-5 的不兼容策略（jsonl-run-store.ts:61-65 注释已确立「升级格式时 bump + 旧文件跳过」先例）。**快照内容本身不瘦身**（calls/trace 明细保留——TUI 运行中观测用；磁盘瘦身无收益，run 目录随 session-file-gc 的 30 天 TTL 清理）。
...
- **选择**：① `workflows/README.md` 与工具 promptGuidelines 补一次性语义；② lifecycle.ts 头部模块注释（:8-9 的 pauseRun/resumeRun  API 说明）重写；③ error-recovery.ts:151 的「时间预算静默失效（直到 pause/resume 才重排）」等 paused 相关注释清除；④ CHANGELOG 记录 breaking change（action enum 收窄是 API 变更）。
- **依据**：项目规则——文档-代码漂移是认知风险源（V4 P5 的同类教训）；action enum 收窄影响 LLM 可见 schema，必须在 CHANGELOG 声明。
...
| S3 | **kill -9 诚实化**（G2/G3） | run review-fix-loop → 进行中 `kill -9` 主进程 → 重启同 session | 残留 run 显示 done/failed（无 running/paused 幽灵）；含失败原因；list 可打开详情 |
...
| S7 | **worker 崩溃自愈**（G1/G2，D-3 核心） | 构造 worker 崩溃（实施期定注入点：如测试脚本内触发不可达分支使 worker 抛基础设施错误）→ 观察重试 | ≤3 次重试后完成或 failed；**已完成的 agent 调用不重复消耗 token**（对照子进程 session 文件数量/内容）；在飞被 abort 的 call 重跑而非 replay 假失败 |
...
|---|---|---|---|---|
| **U1 状态机核** | WorkflowRun 删 paused；transition/assignRuntime 合并创建即 running；删 pauseRun/resumeRun；error-recovery 6+ 处 paused 守卫简化为 isTerminal 单判据；discardInFlightCalls 挪入崩溃重建路径 + 注释重写；时间预算 timer 删 paused 重排分支 | `orchestration/lifecycle.ts`、`orchestration/workflow-run.ts`（状态定义所在）、`orchestration/error-recovery.ts` | S1/S2/S7 + S8 grep | 核心语义变更，单独成 commit 便于回滚；replay 机制在此单元内验证保留（S7） |
| **U2 持久化** | SNAPSHOT_VERSION bump v2；快照 status 枚举收窄；loadAll/deserializeRun 适配（非 done 残留 → failed 的落点）；旧文件跳过 | `orchestration/jsonl-run-store.ts` | S3 + S8（旧文件） | 依赖 U1 的状态定义；格式变更是独立可验证单元 |
| **U3 接口面** | tool-workflow action enum + description；commands verb/补全/case；command-actions 纯函数；WorkflowsView ViewActions/按键；gui-mappers 状态映射；类型收窄 | `interface/tool-workflow.ts`、`interface/commands.ts`、`interface/command-actions.ts`、`interface/views/WorkflowsView.ts`、`interface/gui-mappers.ts` | S4 + S1 | 依赖 U1 的函数删除；对外 API 变更单独成 commit 便于 CHANGELOG 对应 |
...
1. `WorkflowRun` 状态定义的确切文件与 `pending` 瞬态的处理（grep 显示快照格式含 `"pending"` 值——实施时确认它是创建瞬态还是持久态，合并进两态收敛）。
2. S7 的 worker 崩溃注入点（区分「worker 基础设施错误」与「脚本错误」两类重试路径，分别注入）。
3. loadAll 恢复语义落点（loadAll 时直接转 failed vs 保留 session_start hook 转——以不引入新 hook 为准）。
4. `executeNestedWorkflow` 的轮询路径是否引用 paused 态（grep 未命中，实施时编译期确认）。
...
| 「挂起」语义 | 派生态：进程死 + record 活 = resumable | **不存在**（无需求；未来需求走事件日志 checkpoint 独立方案） |
| 崩溃诚实化 | 跨重启重建矩阵 | kill-9 残留转 failed |
