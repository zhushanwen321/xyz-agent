...
export function buildWorkerScript(userScript: string): string {
 ...
 // ── agent global — CC-compatible multi-signature ──
    ...
    '    const _knownFields = new Set(["prompt", "description", "schema", "model", "scene", "label", "task", "agent", "phase", "skill", "timeoutMs", "cwd", "fork", "worktree"]);',
    '    const _unknownFields = Object.keys(opts).filter((k) => !_knownFields.has(k));',
    '    if (_unknownFields.length > 0) {',
    '      _pushWorkerLog("warn", ["[workflow] agent() received unknown fields: " + _unknownFields.join(", ") + ". Known fields: prompt, description, schema, model, scene, label, task, agent, phase, skill, timeoutMs, cwd, fork, worktree"]);',
    '    }',
    '',
