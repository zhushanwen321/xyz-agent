...
/** recall_context 工具的 details 形状。两个分支共用同一结构（level 缺省时 undefined），
...
function recallResult(id: string, store: RecallStore): { content: { type: "text"; text: string }[]; details: RecallDetails } {
...
export default function contextEngineeringExtension(pi: ExtensionAPI): void {
  ...
  pi.on("context", (event, ctx) => {
    try {
      ...
      // context 事件 handler 返回 ContextEventResult（{ messages?: AgentMessage[] }）：
      // 压缩后的 messages 经 unknown 中转回 AgentMessage[]，结构与 Pi 契约一致。
  ...
  pi.registerTool({
    ...
    execute: async (_tcId: string, params: { id: string }, _sig: AbortSignal | undefined, _upd: unknown, _ctx: ExtensionContext) => recallResult(params.id, store),
  });
