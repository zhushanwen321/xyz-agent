...
| `infra/git-executor.ts` | **新建**，execFileSync 封装 | git 命令增删 | ~100 |
...
| 工作目录 .git | 客户-供应商 | spawn（execFileSync） | 自有（本地 git） |
...
### AC-3: git 命令防注入
- 验证：`grep -rn "exec\|spawn\|execSync" src-electron/runtime/src/infra/git-executor.ts | grep -v "execFileSync"` 无输出
- 说明：git 命令必须用 execFileSync 数组参数，禁止 shell 拼接
