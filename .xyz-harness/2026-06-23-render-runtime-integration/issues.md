...
git-zone（Panel zone ⑤）需要完整 git 操作能力：status/stage/unstage/commit。当前 runtime 只有 `services/git-info.ts` 的 `readGitInfo`（轻量缓存查询，分支名 + worktree 标记），不支持全量 git 状态查询和写操作。

需要新建：
- **Runtime**: `services/ports/git-executor.ts`（IGitExecutor 接口）+ `infra/git-executor.ts`（execFileSync 实现）+ `services/git-service.ts`（业务编排）+ `transport/git-message-handler.ts`（消息路由）
- **Frontend**: `api/domains/git.ts`（domain 层）+ protocol.ts 补 `git.status/stage/unstage/commit` ClientMessage + `git.status:result` ServerMessage
...
##### 方案 A: 完整 IGitExecutor（推荐）

**改动**:
- 架构: 新建 4 个 runtime 模块（port + infra + service + handler）+ 1 个 frontend domain
- 模块: `git-executor.ts`(port) / `git-executor.ts`(infra) / `git-service.ts` / `git-message-handler.ts` / `git.ts`(domain)
- 模型: GitStatusResult = {isRepo, branch, stagedCount, unstagedCount, stats, hasConflict, files: GitFileStatus[]}
- 流程: 前端调 git.status → runtime handler → git-service → git-executor（execFileSync）→ 返回结果

**优点**:
- 完整的三层架构，port 边界清晰
- stage/unstage/commit 与 status 共享 IGitExecutor，复用 execFileSync 封装
- 安全约束内置：execFileSync 数组参数防注入，timeout 防卡死
...
**选择**: 方案 A（完整 IGitExecutor）

**理由**: 长期架构优先。D-9 明确 readGitInfo（缓存查询）和 IGitExecutor（实时操作）职责分离。方案 B 会破坏这一边界，后续补 stage/commit 时需要重构。一次性建完整，避免二次返工。
...
- [ ] execFileSync 数组参数（`grep -rn "execFileSync" infra/git-executor.ts` 有输出）
- [ ] timeout 防卡死（spawn 设 timeout）
- [ ] vue-tsc 0 错
...
- 流程: onMounted → git.status() → 渲染四态；操作后 → 刷新
...
FileView 当前直读 `import { fixtureFileChanges }` 硬编码 mock fixture。需要切到 `chat store` 里 active session 末条 assistant 的 fileChanges 聚合（跨回合合并）。
