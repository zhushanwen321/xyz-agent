...
**Review 要点：**
- [ ] domain list() 返回类型与后端 `{groups: SessionGroup[]}` 对齐
- [ ] useSidebar 的 derivedStatus/sessionDigest 等扁平消费是否未被破坏（它们要的是 SessionSummary[]）
- [ ] SessionList 分组渲染是否正确（组标题 + 组内项）
- [ ] mock 数据分组是否合理（fixtureSessions 的 cwd 是否有 ≥2 组）
...
**Review 要点：**
- [ ] shiki 主题是否对接 design-tokens（不硬编码颜色，taste/no-hardcoded-colors 规则）
- [ ] 流式光标（Turn.vue 的 streaming 态）在 markdown 渲染下是否正常
- [ ] 代码块高亮语言是否覆盖常见（ts/vue/json/bash/md）
- [ ] XSS 安全（shiki 默认转义，确认 v-html 只用于 shiki 输出）
...
**Review 要点：**
- [ ] model.switch 是否真调 runtime（非仅本地 ref）；model.switched 订阅是否回写
- [ ] thinking 等级切换是否持久（setThinkingLevel + thinkingLevel prop 同步）
- [ ] commands/context 订阅是否改 session 通道（非 onGlobalType）；sessionId 是否正确透传
- [ ] context.update 字段映射是否正确；cacheHit/modelId 是否保留占位（非崩溃）
- [ ] 静态 fallback 是否删除（slashCommands 不再硬编码）
...
| Wave | 内容 | 状态 | Commit |
|------|------|------|--------|
| W01 | ServerMessageMap + ModelInfo 统一 | ✅ 完成（待提交） | _待提交_ |
| W02 | session.list 分组 UI | ⬜ 待执行 | — |
| W03 | Markdown shiki 渲染 | ⬜ 待执行 | — |
| W04 | Composer 接线 4 项 | ⬜ 待执行 | — |
