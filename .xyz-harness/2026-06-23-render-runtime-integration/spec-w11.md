...
### FR-11 契约裂缝
shared/src/protocol.ts ExtensionInfo 补 **tools 字段**（dirName 已在 protocol.ts:346，无需补；前端 SettingsModal 已用本地 ExtensionItem 桥接 tools，统一回 shared）；FileChangeStatus 补 'unmerged' 枚举（与 git.status 共用枚举；**unmerged 由 runtime 推送**——git.status 输出 hasConflict + files[].status=unmerged，前端据此渲染冲突态，见 G-R2-03 / requirements C15）。
