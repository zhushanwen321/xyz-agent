...
| `gitOverlay`（state，独立 ref） | `Ref<Map<string, GitFileStatus>>` | — | D-012 树/标注分离；git.status 变化只更新 overlay 不触发树重建 | #3 AC-3.2 |
| `nodeStatus`（state） | `Ref<Map<string, LoadStatus>>` | — | 每节点独立加载态（unloaded/loading/loaded/error/invalidated，D-011/D-017） | #3 AC-3.3/3.4 |
...
| `invalidate`（action，**K-9**） | `(sessionId: string, paths: string[]) => void` | void | 标相关节点 loaded→invalidated（D-017）；**供 composable 派发，store 不自行监听** | #3 AC-3.11 |
...
**异常路径 alt/else**：
- `alt 越界`：listTree 内 isUnderOrEqual 失败 → FileError('out_of_cwd') → error envelope → store nodeStatus=error
...
sequenceDiagram
    ...
    alt loaded（折叠复用缓存）
        ...
        alt 越界（NFR-AC-S2，T2.10）
            ...
            UF->>ST: nodeStatus.set(error)
        else 在途切 session
            API-->>UF: 响应回来
            UF->>UF: 校验 sessionId 不匹配→丢弃 stale（AC-3.7）
        else 正常
            API-->>UF: FileNode[]
            UF->>ST: tree merge + nodeStatus.set(loaded)
        end
        deactivate API
...
sequenceDiagram
    ...
    participant ST as fileTreeStore
    Note over ST: gitOverlay 独立 ref（不触发树重建）
...
sequenceDiagram
    ...
    activate ST
    ST->>ST: 相关节点 nodeStatus loaded→invalidated（D-017）
    deactivate ST
    deactivate UF
    deactivate CCP
...
#### 数据流链
agent_end→chat-chunk-processor.applyFileChanges→chat store messages 更新→useFileTree watch（composable 层）→过滤+路径定位→fileTreeStore.invalidate→nodeStatus invalidated
...
| stores.fileTreeStore.invalidate | renderer/stores/fileTree.ts:60 | ✅ 接线完整 | K-9：供 composable 派发（nodeStatus invalidated） |
| composables.useFileTree.loadTree | renderer/composables/useFileTree.ts:43 | ✅ 接线完整 | fileApi.tree + store.setTree + rehydrate |
| composables.useFileTree.expandNode | renderer/composables/useFileTree.ts:58 | ✅ 接线完整 | nodeStatus 状态机 + fileApi.expand（5 异步竞态） |
...
- [x] 无 `❌ 未定义`（check 脚本 ③f + 本表双向核对）
- [x] 接线状态标注准确（叶子标叶子，非叶子标接线完整，adapter 标真引 SDK）
