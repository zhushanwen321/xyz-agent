...
> **一句话结论**：不新建任何 mailbox 通道——pi 0.84.1 的投递原语（`sendMessage`/`prompt(streamingBehavior)`）已经完整；要统一的是散落在 5 处的「投递时状态冲突处理」策略，收敛为一个零 pi 依赖的策略内核包 `@xyz-agent/session-delivery`（packages 层），extension 与 runtime 两侧各做薄装配。session-manager 的 send 排队与子 session 完成回流是第一批新消费者。
...
- **Question**：如何给「向 session 投递消息」提供一个统一入口：调用方声明投递意图（steer / followUp），内部处理与目标 session 运行状态的冲突，idle 时带 triggerTurn 唤醒？
- **Answer（本设计的）**：策略内核包放 packages 层（两侧可消费），窄端口注入运行时能力（isIdle / hasPendingMessages / send），extension 侧注入 `pi.sendMessage`、runtime 侧注入 `RpcClient.prompt(streamingBehavior)`；现有 5 处实现逐个切换。
...
| # | 目标 | 使用者体验 |
|---|------|-----------|
| G1 | session-manager 的 `send_to_session` 在目标 busy 时排队而非拒绝 | agent 调 send 不再收到 `rejected: true` 后自己琢磨重试；消息保证在目标 session 的下一 turn 边界注入 |
...
| 5 | `MessageDispatcher.sendMessage` | runtime（`packages/runtime/src/services/session/message-dispatcher.ts:123-128`） | `client.prompt()` | busy 预检（`isGenerating \|\| isCompacting \|\| isBashRunning`）**直接拒绝**，返回 `{blocked: true, rejected: true}` + `send.rejected` 广播 |
...
|---|---|---|---|
| `prompt`（支持 `streamingBehavior` 参数，rpc-mode.js:305 透传） | 立即开新 turn | 按 `streamingBehavior` 入队，不抛错 | **主动唤醒** |
| `steer` | **纯入队**（agent-session.js:986-993 只 `_queueSteer`，不判 idle） | 入 steer 队列，turn 边界 drain | 惰性 |
...
【extension 通路】（同进程，custom message 可达）
  subagent-workflow notifier / scheduler / workflow / goal
    └→ delivery.deliver(msg)                          # 统一入口（内核）
         └→ [内核] gate(isIdle?) → 退避/合批/去重
              └→ port.send(msg, {triggerTurn, deliverAs})
                   └→ pi.sendMessage(custom msg)       # extension 装配注入
                        └→ idle: 新 turn / streaming: steer|followUp 队列

【runtime 通路】（跨进程，RPC）
  SessionManagerHandler（send 排队 / 完成回流）
    └→ delivery.deliver(msg)                          # 同一内核
         └→ [内核] gate(isIdle?) → 退避/合批/去重
              └→ port.send(msg, {deliverAs})
...
**结论：采用方案 A——packages 层策略内核 `@xyz-agent/session-delivery` + 窄端口注入 + 两侧薄装配；复用 pi 既有词汇（deliverAs/triggerTurn），策略默认值从现有实现继承而非新发明。**
...
```ts
// 装配（extension index.ts，一次性）
const delivery = createDelivery({
  isIdle: () => ctx.isIdle(),                    // pi ExtensionContext
  hasPendingMessages: () => ctx.hasPendingMessages(),
  send: (msg, opts) => host.sendMessage(msg, opts),   // pi.sendMessage 原样透传
}, {
  deliverAs: 'steer',                            // 投递意图：turn 边界抢占
  triggerTurn: true,                             // idle 时立即唤醒
  mergeWindowMs: 60_000,                         // 完成通知合批（现状 MERGE_WINDOW_MS=60s，滑动窗口）
})

// 使用（subagent 完成回调处，一行；格式化在调用方完成——内核只拼接）
delivery.notify({ customType: 'subagent-bg-notify', content: buildLlmContent(record), display: true, details })
```

busy 窗口内的重试、合批、去重、丢消息防护全部由内核处理；`deliver()` 永不 throw（错误消化为内部 warn + 计数）。

**调用方 B——runtime（session-manager 的 send 排队）**：

```ts
// SessionManagerHandler 内按 sessionId 惰性装配
const delivery = getOrCreateDelivery(sessionId, () => createDelivery({
  ...
  hasPendingMessages: () => false,               // RPC get_state 有 pendingMessageCount（rpc-mode.js:357），
                                                  // 但端口是同步签名拿不到异步 RPC 结果——一期保守 false，见 §5 待验证
  send: async (msg) => { await client.prompt(msg.content, { streamingBehavior: 'steer' }) },
}, { deliverAs: 'steer', triggerTurn: true }))

// handleSend 改为（deliverChecked：入队 + ensureActive 同步确认，失败同步抛给 select 通道）：
try {
  await delivery.deliverChecked({ customType: 'session-manager-send', content: prompt, display: true })
  return { queued: true }
} catch (e) {
...
parentDelivery.deliver({
  customType: 'session-manager-notify',
...
| 失败 | 现象 | 恢复 |
|------|------|------|
| 目标 pi 进程已死 | `port.send` 内 ensureActive 抛错 | 入口即拦：`deliverChecked` 同步抛给调用方（U5 的 agent 立即看到错误 + hint）；已入队消息由内核 send 重试（达上限 settle rejected，经 onSettled 上报） |
| 退避达上限仍 busy | 强制发送（fallthrough 到 pi 队列） | **预期**（⛔ 实施期探针门，见 §3.3 探针表 row 4）：pi 的 steer 队列在 run 循环内 drain；探针若证伪按降级路径收紧为丢弃+计数 |
...
**D2 端口注入，内核零 pi 依赖**
选择：内核自定义窄端口（`isIdle` / `hasPendingMessages` / `send`），不 import pi 类型。被否：直接依赖 pi 的类型定义（内核会随 pi 版本漂移，且 runtime 装配侧拿不到 ExtensionContext）。端口签名刻意对齐 pi 词汇（triggerTurn / deliverAs），团队已熟悉，不发明新概念层。

**D3 投递意图直接复用 pi 三值（steer / followUp）+ triggerTurn 布尔**
选择：调用方声明 `{deliverAs, triggerTurn}`，与 pi sendMessage options 同形。被否：自创 intent 抽象（`'wake' | 'queue-turn' | 'queue-run'`——多一层翻译，且与两侧底层参数一一对应时是纯冗余）。实战结论内化为**默认值**：内核默认 `deliverAs: 'steer', triggerTurn: true`（F1 的教训），followUp 场景显式声明。

**D4 策略项与默认值**（从现有实现继承，不新发明）：

| 策略 | 默认值 | 继承自 | 作用 |
|------|--------|--------|------|
| busy 策略 `busyPolicy` | `'retry-force'`：退避 `{backoffMs: 100, max: 50}`（≈5s 上限）后强制发送 | notifier FLUSH_BACKOFF | F2 窗口内重试；达上限强发（pi 队列兜底，⛔ 探针门验证） |
| | 备选 `'park'`：busy 时入队不重试，等下一次 `notify()`/`flush()` 外部触发 | scheduler tick 模式（busy 跳过延迟到下个 30s tick） | scheduler 迁移用它保行为等价（避免 5s 强发提前注入正在进行的 run） |
...
| 合批格式归属 | **调用方预格式化**：每条消息的 content 已是终态文案（notifier 的 buildLlmContent 在调用方）；内核只拼接——多条以 `"\n\n---\n\n"` join，details 包装为 `{batch: true, items}` | notifier doSend（notifier.ts:215-221） | 字节锁测试锚定的输出由「调用方格式化 + 内核固定拼接规则」共同决定，归属清晰 |
| 去重 | 关；workflow 场景显式开（keyOf + 窗口 1000） | workflow MAX_NOTIFIED_RUN_IDS | F3 类双派发抑制（scheduler 任务级 dispatchesInFlight 保留在其调用方） |
| 立即投递触发 | `notify()` 入口先探「无排队依赖即 flush」——notifier 的 `!hasRunningBackground() → flush` 语义由「`notify()` 时 `isIdle() && !hasPendingMessages()` 则跳过合批窗口立即投」承接 | notifier notify 入口（notifier.ts:150-152） | 空闲时通知零延迟，不为合批空等 60s |
| 投递结果信号 | `deliverChecked()` 返回 Promise（resolve=已入队且可达性确认，reject=入队失败）；异步终态经 `onSettled` 回调（delivered / rejected） | scheduler 的 await 失败记账 + at-least-once 语义 | 依赖投递结果的消费者（scheduler once 任务）不丢任务；不关心的调用方（notifier）忽略即可 |
...
选择：一期 `RpcClient.prompt` 文本直投（通知文案对齐 notifier 的 buildLlmContent 模式：label/status/摘要/sessionFile 指针）。被否（一期）：经 session-manager extension 的 marker 通道（runtime → extension_ui_request → extension 内 `pi.sendMessage`）投 custom message——能拿到结构化渲染（customType + details 走前端 bg-notify-render 类渲染器），但要扩展 marker 协议 + 走一圈 UI 请求通道，一期收益不抵复杂度。触发升级条件：前端需要对回流通知做专门渲染（折叠/图标/点击跳转子 session）时再上。

**D6 与 pending-notifications 的关系：组合，不合并**
pending-notifications 是记账层（register/unregister 差集 = 谁在等），本内核是投递层（把消息送进 LLM）。组合协议：通知场景 = `pending.register`（等待态对 goal 可见）→ 终态 → `delivery.deliver`（唤醒投递）→ `pending.unregister`。两包互不依赖，编排发生在调用方（session-manager 回流场景）。合并的诱惑存在（「通知」一词撞名），但记账要持久化（appendEntry 跨重启）、投递要实时（内存队列跨毫秒），生命周期完全不同。

**D7 U5 接管面：dispatcher 链路副作用的逐项处置（防静默绕过）**
`handleSend` 现走 `sessionService.sendMessage` → `MessageDispatcher.sendPrompt` 完整链路，含 6 个内部步骤；改为 `delivery.deliver` 后逐一声明：
...
| 错误广播（`send.rejected` / `message.error`） | **替换**——agent 路径失败不走前端 banner | 同步失败经 `deliverChecked` reject → select 通道返回错误 + hint（agent 立即可见）；异步失败（queued 后 port.send 抛）经 U6 回流通知告知父 agent；U6 未落地窗口期内仅 console.warn（诚实标注的已知缺口，见 §5 待验证） |
| `handleCreate` 初始 prompt（同文件第二个 sendMessage 调用点，session-manager-handler.ts:139-145） | **直投不走内核队列**——新 session 必然 idle 无竞态，用 port 层同款「ensureActive + prompt」直发；失败照旧 throw（错误对象挂 sessionId + hint） | 保留 create+send 原子性契约（外层 catch 依赖 throw 组装恢复路径，session-manager-handler.ts:73-84）；内核 never-throw 模型不适合此同步确定性场景；与 row1 一致不过 BeforeSend hook |

**运行时断言清单**（探针状态）：

| 断言 | 探针 | 状态 |
|------|------|------|
| RPC `prompt(streamingBehavior)` streaming 时不抛错且入队 | pi CLI `--mode rpc` 手发 streaming 期间 prompt+streamingBehavior | ✅ 已测（pi dist 代码路径核实 + rpc-mode.js:305 透传确认；实施期 U1 落地时补真机探针） |
| RPC `steer` 对 idle session 纯入队不唤醒 | 同上，idle 时发 steer 观察 turn 不启动 | ✅ 已测（agent-session.js:986-993 只 `_queueSteer`，无 idle 分支） |
| extension `sendMessage({triggerTurn:true})` idle 时立即开 turn | subagent-workflow 现有 e2e 已覆盖 | ✅ 现网行为 |
...
export interface DeliveryMessage {
  ...
  // 注：runtime 侧装配只消费 content（user prompt 形态），customType/details 被忽略——
  // 两侧同形不同义是 D5 声明的已知边界，接口注释须标注
}
export interface DeliveryPort {
  isIdle(): boolean
  hasPendingMessages(): boolean
  send(msg: DeliveryMessage, opts: { triggerTurn?: boolean; deliverAs?: 'steer' | 'followUp' | 'nextTurn' }): Promise<void> | void
}
export interface DeliveryConfig {
  deliverAs?: 'steer' | 'followUp'          // 默认 'steer'（D3）；scheduler 显式 'followUp'（现状语义）
  triggerTurn?: boolean                     // 默认 true（D3）
  ...
  backoff?: { ms: number; max: number }     // 默认 {100, 50}（D4；retry-force 与 send 错误重试共用）
  dedupe?: { keyOf: (msg: DeliveryMessage) => string; window: number }
  onSettled?: (msg: DeliveryMessage, outcome: 'delivered' | 'rejected') => void  // 异步终态信号（D4）
}
export interface DeliveryHandle {
  notify(msg: DeliveryMessage): void        // 常规入口（合批窗口 + 空闲时零延迟立即投，D4）
  deliver(msg: DeliveryMessage): void       // 无合批直入（等价 notify 但绕过窗口语义微差，供回流等单条场景）
  deliverChecked(msg: DeliveryMessage): Promise<void>  // 入队 + 可达性同步确认；reject = 入队失败（U5 用）
  flush(): void                             // 强制投递尝试（shutdown / park 模式外部重触发共用）
...
约束：
- 内核无 timer 依赖注入以外的全局状态（vitest fake timers 可测）；单一投递循环天然 in-flight 防重（deliver 入队 → 至多一个 flush 在途）。
...
| S3 | subagent-workflow 迁移后后台 subagent 完成通知（含**多任务合批**：60s 滑动窗口内两个后台 subagent 先后完成） | 跑 subagent-workflow 现有后台通知 e2e（extensions/universal/subagent-workflow 的 bg-notify 用例）+ 新增多任务合并场景 | 用例全过；单条通知文案与迁移前逐字节一致（notifier 有字节锁测试锚定）；合批场景两条通知合并为一条消息（`---` 分隔、details 为 batch 结构），与迁移前合并输出一致 | G4 |
...
| U2 | 内核包 `@xyz-agent/session-delivery`（从 notifier 抽取 enqueue/gate/退避/合批/flush 骨架 + 单测搬迁） | 抽取而非重写：notifier 是竞态知识最全的实现，搬迁测试即继承 F1/F2 教训 | — | S5 |
| U3 | subagent-workflow notifier 切换内核（装配 + 删私有逻辑，字节锁测试不动） | 第一个消费者，验证内核抽象对最复杂场景的覆盖度 | U2 | S3 |
| U4 | scheduler 切换内核（gate 交内核 + `busyPolicy: 'park'`（tick 重触发）+ `deliverAs: 'followUp'` 保持现状语义 + `onSettled` 承接失败记账；dispatchesInFlight 保留） | 第二消费者，验证 park 模式 + followUp 配置形态与 at-least-once 语义等价 | U2 | S4 |
| U5 | session-manager send 排队（SessionManagerHandler 装配 runtime Delivery + `handleSend` 改 `deliverChecked`；工具返回 `{queued: true}`；`handleCreate` 初始 prompt 按 D7 末行直投） | G1 主体；同时补 create 带 prompt 的工具 schema 暴露（前序分析的独立缺口，一并落地） | U1, U2 | S1, S6 |
...
| U7（评估项） | workflow helpers 切换内核 / compact 队列并入 | 非本设计目标必需；U3/U4 落地后评估迁移成本 | U2 | — |
...
- ⛔ runtime 装配的 `hasPendingMessages` 一期固定 `false`：`get_state` 虽有 `pendingMessageCount`（rpc-mode.js:357）但端口同步签名拿不到异步 RPC 结果。残余影响：runtime 侧 F2 窗口判定少一个信号（isIdle gate 已覆盖主路径）——S5 真机观察后再评估 get_state 预询。
...
- F2 死区在 pi 侧物理存在（run 循环最后一次 `hasQueuedMessages` 检查之后入队的消息无人 drain）——两侧都只能靠退避压概率，⛔ 探针（强发后 drain）是唯一大门，**必须在 U2 动工前先跑**。
