...
backfed_from: [nfr, code-arch, execution]
---
...
graph LR
  ...
  subgraph "P1 核心（Wave2，依赖 P0）"
    #4[#4: useSearch composable 编排]:::investigating
    #5[#5: api门面清理]:::investigating
...
| §7: useSearch composable（新建） | 模块 | #4 | ✅ 已覆盖 | `[BACKFED from code-arch on 2026-06-30] D-026：search real domain → useSearch composable` |
| §7: 命令注册表（store 扩 + composable 新） | 模块 | #2 | ✅ 已覆盖 | — |
| §7: 匹配引擎（提取纯函数） | 模块 | #1 | ✅ 已覆盖 | — |
| §7: 跳转编排（新建） | 模块 | #6 | ✅ 已覆盖 | — |
| §7: recents composable（新建） | 模块 | #3 | ✅ 已覆盖 | — |
| §7: api/index.ts（接线改造） | 模块 | #5 | ✅ 已覆盖 | `[BACKFED from code-arch on 2026-06-30] D-026：接线→门面清理（删除 search 导出）` |
...
| §5: error 态——查询失败（useSearch catch） | 状态 | #8 | ✅ 已覆盖 | AH-S2：查询路径因 allSettled 吞单源错误，error 实际仅跳转可达，见 #8 AC `[BACKFED from code-arch on 2026-06-30] D-026` |
...
| §10: 特化决策「符号占位不建 port」 | 挑战 | #4 | ✅ 已覆盖 | 符号占位逻辑在 useSearch composable 内处理（占位提示，不调 api）`[BACKFED from code-arch on 2026-06-30] D-026` |
| §10: 特化决策「recents 用 localStorage 不走 runtime」 | 挑战 | #3 | ✅ 已覆盖 | — |
| §11 AC-1: search 不再常驻 mock | 兜底 | #5 | ✅ 已覆盖 | grep 验收 AC 落在 api/index.ts 门面清理 `[BACKFED from code-arch on 2026-06-30] D-026` |
...
| §12 BC-9 乱序响应保护 loadSeq | 兜底 | #4 | ✅ 已覆盖 | useSearch 重写须保持不变式 `[BACKFED from code-arch on 2026-06-30] D-026` |
...
2. useSearch composable（matchFilter 前端过滤候选）`[BACKFED from code-arch on 2026-06-30] D-026`

两消费者共享同一子串核心算法，提取避免逻辑重复。关联 system-architecture.md §7（匹配引擎模块）+ §11 AC-4（grep 验收纯函数）。

#### 为什么是这个 P 级

- **P0（基础设施先行，D-017）**: 无前置依赖，且被 #4（useSearch 的 matchFilter）+ #7（SearchModal 渲染）依赖。匹配引擎不稳，上游依赖方返工。`[BACKFED from code-arch on 2026-06-30] D-026`
...
- 流程: matchFilter 在 useSearch composable 调用（候选过滤）；segments 在 SearchModal 模板调用（高亮渲染）`[BACKFED from code-arch on 2026-06-30] D-026`

**优点**: 职责单一——matchFilter=过滤（返回候选），segments=渲染（返回高亮段）。两消费者各取所需，无耦合。符合 §7 边界声明「ME 不含分组、不调 api、无副作用」。
**缺点**: 两个导出函数，调用方需知道用哪个。
**适用场景**: 过滤与渲染是两个独立关注点（本例正是）。

> 红队审查注：提取由**本期两个真实消费者**驱动（SearchModal 渲染 segments + useSearch 过滤 matchFilter），DRY 成立。原稿「未来复用（如 composer 候选过滤）」论据夸大——composer 走 file-candidates.ts DTO 映射无 segments 消费，已删去该论据，不依赖假设性复用。`[BACKFED from code-arch on 2026-06-30] D-026`
...
- 模型: 单一返回形态，调用方一次调用拿全
- 流程: useSearch 与 SearchModal 都调同一函数 `[BACKFED from code-arch on 2026-06-30] D-026`

**优点**: 单一入口，调用方简单。
**缺点**: 强耦合过滤与渲染——useSearch 只需过滤不需高亮段（matchFilter 场景），却被强制计算高亮段（性能浪费）；且返回形态不符合 SearchItem DTO（多包一层 segments）。
**适用场景**: 过滤与渲染总是同时发生（本例不是）。

##### 方案 C: 内联不提取（保持 SearchModal 私有）

**改动**: 无新模块，segments 留在 SearchModal，useSearch 另写过滤逻辑。`[BACKFED from code-arch on 2026-06-30] D-026`
**优点**: 零改动成本。
...
**选择**: 方案 A（双函数分离）
**理由**: 过滤与渲染是两个正交关注点——useSearch 只过滤（matchFilter），SearchModal 只渲染（segments）。方案 A 职责单一、无强耦合、符合 §7 边界。本期两消费者驱动 DRY 成立（非依赖假设性未来复用）。`[BACKFED from code-arch on 2026-06-30] D-026`

**放弃方案的理由**:
- 方案 B: 强耦合过滤与渲染，useSearch 被迫计算不需要的高亮段，违反单一职责 `[BACKFED from code-arch on 2026-06-30] D-026`
...
- **P0（基础设施先行，D-017）**: 无前置依赖，被 #4（useSearch 聚合命令源）+ #6（跳转编排执行命令）+ #10（Sidebar keydown 复用）依赖。注册表不建，上层无处取命令。`[BACKFED from code-arch on 2026-06-30] D-026`
...
- **P0（基础设施先行，D-017）**: 无前置依赖，被 #4（useSearch 空查询返 recents）+ #6（跳转后写 recents）依赖。recents 读写机制不建，空查询无数据源、跳转后无持久化。`[BACKFED from code-arch on 2026-06-30] D-026`
...
### #4: useSearch composable（编排 4 数据源查询）`[BACKFED from code-arch on 2026-06-30] D-026：search 编排归 composable 非 domain`
...
架构 §7 + D-026 要求新建 `composables/features/useSearch.ts`（编排型 composable，**不新建 api/domains/search.ts**——search 编排跨 commandStore+fileSearchStore 违反「domain 严格只调 transport+pending」铁律，归 composable 与 useSidebar/useFileSearch 同层），编排 4 数据源查询：
...
useSearch composable 合并全量候选 → 交匹配引擎 matchFilter 前端过滤 → 按类型分组成四类 section（输出整形，GAP-E1 归 useSearch composable）。符号类占位（不调 api，D-001）。

**关键不变式（BC-9）**: useSearch composable 重写 loadResults 时必须保持 loadSeq 序列号守卫（乱序响应不覆盖新结果），架构 §12 BC-9 明确这是正确性不变式。loadSeq 守卫在 useSearch.query() 内部维护，SearchModal 只 await，不重复守卫（AH-C1）。

**错误冒泡链（AH-E1/E2，关键）**: useSearch composable 编排 file 源**必须直接调 `composer.getFileCandidates`**（经 pending reject 透传 error envelope），**不经 `useFileSearch.load`**（其在 :39-43 静默 catch 吞错降级空数组）。若经吞错层，file 源失败永不冒泡到 useSearch composable catch，#8 AC-8.2「不静默」假性 PASS。useSearch composable 可复用 useFileSearch 的 **session 级缓存**（缓解 AH-C2 性能），但缓存未命中时直调 composer.getFileCandidates 不经吞错层。

关联 system-architecture.md §7（search 模块，D-026 反哺后为 useSearch composable）+ §9 swimlane + requirements G2 + §12 BC-5/BC-9。`[BACKFED from execution consistency-final on 2026-06-30] D-026 措辞清扫`
...
##### 方案 A: 单 composable 函数 query(q) 内编排 3 源（Promise.allSettled）+ 分组

**改动**:
- 架构: 新建 `composables/features/useSearch.ts`，导出 `query(q): Promise<Section[]>`
...
**优点**: 单一入口，调用方简单（SearchModal 只调 query）；allSettled 容错（单源失败不阻塞其他）；分组逻辑内聚（GAP-E1 归 useSearch composable）。符合架构 §9 swimlane。
**缺点**: useSearch composable 内职责稍多（编排+合并+分组），但同属「查询编排」轴（GAP-E1 已决策归编排层）。
**适用场景**: 4 源查询+分组是紧耦合的编排流水线（本例正是）。

##### 方案 B: useSearch composable 只编排查询，分组下沉 SearchModal

**改动**: useSearch composable 返回扁平 SearchItem[]（不分 section），SearchModal 内分组
**优点**: useSearch composable 职责更纯（只查询）。
**缺点**: 违反架构 §7（useSearch composable 定位含「按类型分组 输出整形」）+ §9 swimlane（useSearch 做分组）；分组逻辑散落到 UI 层，未来其他 search 消费者需重复分组。
**适用场景**: 分组是 UI 关注点（架构已决策不是）。

##### 方案 C: 每数据源独立 composable 函数（queryCommands/queryFiles/querySessions），SearchModal 聚合

**改动**: useSearch composable 导出 3 个独立函数，SearchModal 调 3 次聚合
**优点**: 每函数单一职责。
**缺点**: 违反 api 门面模式（composable 对外是聚合接口，调用方不应感知多源）；SearchModal 承担编排逻辑（UI 层不该有数据编排）；3 次调用无法 Promise.allSettled 容错。
**适用场景**: 3 源完全独立无聚合需求（本例需统一分组渲染）。

#### 取舍决策

**选择**: 方案 A（单 composable 函数 + allSettled + 分组）
**理由**: 单一入口符合 composable 聚合接口模式；allSettled 容错独立数据源（AGENTS.md 规则）；分组内聚符合架构 §7/§9 决策（GAP-E1 归编排层）。长期架构优先：useSearch composable 封装编排，未来其他 search 消费者（如命令面板独立入口）直接复用 query。`[BACKFED from execution consistency-final on 2026-06-30] D-026 措辞清扫`
...
- [ ] AC-4.4 [并发]（trace: architecture §12 BC-9 + AH-C1）: 乱序响应保护——快速连续查询，旧响应不覆盖新结果（loadSeq 守卫在 useSearch.query() 内部维护，SearchModal 只 await 不重复守卫）`[BACKFED from execution consistency-final on 2026-06-30] D-026 措辞清扫`
- [ ] AC-4.5 [异常]（trace: architecture §9 allSettled + AH-E1/E2）: error 冒泡链——file 源错误必须从 runtime reject 一路冒泡到 useSearch composable catch（useSearch composable 直调 composer.getFileCandidates，不经 useFileSearch.load 的吞错层），中途不得有 catch 降级空数组的吞错；到 useSearch composable catch 后转 toast，不静默 `[BACKFED from execution consistency-final on 2026-06-30] D-026 措辞清扫`
...
- [ ] AC-4.10 [数据/竞态]（[BACKFED from nfr on 2026-06-30] nfr K-3）: useSearch composable 消费 session 级缓存时须自绑 `useFileSearch.setupInvalidation` watch（不依赖 CommandPopover 挂载），或 query() 前校验缓存新鲜度——防 CommandPopover 未挂载时 agent 改文件不触发 invalidate → stale cache（用户搜不到刚改的文件）`[BACKFED from execution consistency-final on 2026-06-30] D-026 措辞清扫`

---

### #5: api/index.ts 接线（删除 search 导出）`[BACKFED from code-arch on 2026-06-30] D-026：search 编排归 composable，无 WS domain`
...
`api/index.ts:42` 当前 `export const search = mockApi.search`（硬编码常驻 mock）。G2 要求消除 mock 常驻。

**[BACKFED from code-arch D-026]** 原 #5 设想「search 接 real domain，三元切换对齐其他 9 个 domain」。但 D-026 [REVISIT of #4] 确认：search 编排归 `composables/features/useSearch.ts`（跨 store+domain 编排，与 useSidebar/useFileSearch 同层），**不新建 api/domains/search.ts**（domain 严格只调 transport+pending 的铁律不变，search 编排跨 commandStore+fileSearchStore 违反铁律）。因此 search 无 WS domain，#5 改为「删除 api/index.ts 的 search 导出 + SearchModal 改调 useSearch」。

关联 system-architecture.md §7（api 接线模块，D-026 反哺后 search 不再是 domain）+ §11 AC-1（grep 验收）+ requirements G2 + decisions.md D-026。

#### 为什么是这个 P 级

- **P1（核心关键路径）**: G2 grep AC-1 硬验收点。依赖 #4（useSearch composable 存在才能切换消费方）。改造极小（删 1 行导出 + SearchModal import 改向）但价值关键（消除 mock 常驻）。

#### 方案对比

##### 方案 A: 删除 search 导出 + SearchModal 改调 useSearch（D-026 后）

**改动**:
- 架构: `api/index.ts` 删除 `export const search = mockApi.search`（search 无 domain，门面不再导出）；SearchModal 改 `import { useSearch } from '@/composables/features/useSearch'`，调 `useSearch(activeSessionId).query(q)` 替代 `api.search.query(q)`
- 流程: useSearch 内部判 VITE_MOCK（mock 轨走 mockApi.search fixture，real 轨走真实 4 源聚合）；mock 模式仍可用（mock fixture 保留在 api/mock/search-data.ts）

**优点**: 分层正确归位（编排归 composable，domain 保持纯净）；与 useSidebar/useFileSearch 跨层编排模式一致；不引入「编排型 domain」特例破坏一致性。
**缺点**: SearchModal 消费方从 `api.search` 改为 `useSearch`（真实改动，非纯门面切换）。
**适用场景**: search 编排本质是跨 store+domain 聚合（本例正是，D-026 confirmed）。

##### 方案 B: 仍建 api/domains/search.ts 三元切换（D-026 前原方案，已弃）

**改动**: 新建 api/domains/search.ts 作编排型 domain，import commandStore+fileSearchStore，api/index.ts 三元切换
**优点**: api 门面模式完整（search 与其他 9 domain 同构）。
**缺点**: 违反「domain 严格只调 transport+pending」铁律——search domain 需 import commandStore+fileSearchStore+composer/session domain，是唯一的非纯净 domain，破坏一致性。D-026 已弃。

#### 取舍决策

**选择**: 方案 A（删除 search 导出 + 改调 useSearch，D-026）
**理由**: D-026 [REVISIT of #4] 已 confirmed——编排归 composable 是分层正确归位（长期方案），不引入破坏一致性的 domain 特例。useSearch 与 useSidebar/useFileSearch 同属「跨 store+domain 编排」层，模式 100% 一致。

**放弃方案的理由**:
- 方案 B: 违反 domain 纯净铁律，D-026 已弃

#### 验收标准

- [ ] AC-5.1 [正常]（trace: architecture §11 AC-1，D-026 后修订）: `grep -n "export const search" api/index.ts` 无输出（search 导出已删除）；`grep -n "useSearch" components/overlays/SearchModal.vue` 有输出（改调 useSearch）
- [ ] AC-5.2 [模式]（D-026 后修订）: useSearch 内部判 VITE_MOCK——mock 轨走 mockApi.search fixture，real 轨走真实 4 源聚合；SearchModal 消费方对两种模式无感（都调 useSearch.query）
- [ ] AC-5.3 [类型]: `export type { SearchItem }` re-export 保留（类型源仍是 api/mock/search-data.ts，SearchItem 形态 SSOT 不变，useSearch DTO 映射目标）
...
- segments → 提取至 #1 匹配引擎
- 数据加载 → 改调 #4 useSearch composable `[BACKFED from execution consistency-final on 2026-06-30] D-026 措辞清扫`
...
**改动**:
- 架构: SearchModal 骨架不变（template 结构、props/emits、Dialog 容器），内部 script 替换：loadResults 改调 useSearch.query（#4）；segments 改 import match-engine（#1）；confirmSel 改调 useSearchJump.confirm（#6）；recents 改 useRecents（#3）
- 流程: 行为契约逐条对照保持（BC-1~BC-12），loadSeq 守卫迁移到 useSearch composable 内（BC-9） `[BACKFED from execution consistency-final on 2026-06-30] D-026 措辞清扫`
...
- [ ] AC-7.6 [等价]（trace: §12 BC-9 + AH-C1）: loadSeq 乱序响应保护保持（守卫在 #4 useSearch composable 内部，SearchModal 只 await） `[BACKFED from execution consistency-final on 2026-06-30] D-026 措辞清扫`
...
- **P1（核心闭环质量契约，D-018）**: error 态缺失违反 no-silent-catch lint 强制规则（lint 不过无法提交，硬阻断）；loading 态是 UC-3 AC-3.4 明确要求。依赖 #4（error 挂载点在 useSearch composable 的 catch）。 `[BACKFED from execution consistency-final on 2026-06-30] D-026 措辞清扫`
...
**改动**:
- 流程: loading——query 触发后 setTimeout 200ms 才显示加载条（<200ms 返回则 clearTimeout 不显示，避免闪烁）；error——useSearch composable catch 错误→toast 反馈 + 对应分组显示空态提示 `[BACKFED from execution consistency-final on 2026-06-30] D-026 措辞清扫`
...
- [ ] AC-8.2 [异常]（trace: AGENTS.md no-silent-catch + AH-E1/E2）: useSearch composable catch 错误→toast 反馈，不静默失败（lint 通过）。**前置**：file 源须直调 composer.getFileCandidates 不经 useFileSearch.load 吞错层（见 #4 AC-4.5），否则 error 永不到 useSearch composable catch 致假性 PASS `[BACKFED from execution consistency-final on 2026-06-30] D-026 措辞清扫`
...
**[BACKFED from nfr on 2026-06-30]** ④NFR Step2 追踪（F-1）发现关键稳定性漏洞：runtime WS 断连时，`ws-client.ts` 的 onclose 不 reject in-flight pending（`pending.ts` 无 clear/flush），file 源/session 源的 pending **永远不 settle**。而 `Promise.allSettled` 只在所有输入 settled 后才 resolve → #4 useSearch composable 的 `query()` 永久 await → **浮层永久 loading 挂死，无 toast 无降级**。 `[BACKFED from execution consistency-final on 2026-06-30] D-026 措辞清扫`
...
- **P1（UX 阻断级，D-023）**: 浮层永久挂死是 UX 阻断级 bug（用户看到永久 loading 无反馈），与 no-silent-catch lint 精神一致（lint 强制 catch，却在 transport 层漏了超时）。依赖 #4（useSearch.query() 是挂载点）。被 #8（error 态协同）依赖——#8 AC-8.2「不静默」的真正完整需要 #17 闭合（否则 WS 断连路径仍是静默挂死）。 `[BACKFED from execution consistency-final on 2026-06-30] D-026 措辞清扫`

#### 方案对比

##### 方案 A: useSearch.query() 对 WS 源加超时 race（Promise.race + setTimeout）`[BACKFED from code-arch on 2026-06-30] D-026：domain query() → useSearch.query()`

**改动**:
- 架构: `[BACKFED D-026]` useSearch composable 的 `query()` 内对 file 源/session 源的 WS 调用包一层 `Promise.race([wsCall, timeout(10s)])`，超时→reject
...
**适用场景**: WS 源调用无内置超时，需 useSearch composable 层兜底（本例正是）。 `[BACKFED from execution consistency-final on 2026-06-30] D-026 措辞清扫`
...
**优点**: 根因更彻底——所有 WS 消费者受益（不止 useSearch composable）。 `[BACKFED from execution consistency-final on 2026-06-30] D-026 措辞清扫`
...
**选择**: 方案 A（useSearch.query() 超时 race）
**理由**: 本期 search-modal topic 范围内的根因修复——在 useSearch composable 层加超时 race 兜底 WS 断连，不改动跨 topic 共享的 transport 层（方案 B 影响面过大，应另开 topic）。与 allSettled 容错语义自然衔接。长期架构优先：方案 B 是更彻底的全局修复，但应作为独立 transport topic 处理，不混入本期。 `[BACKFED from execution consistency-final on 2026-06-30] D-026 措辞清扫`
...
- [ ] AC-17.1 [异常/稳定]（trace: nfr F-1 + D-023）: useSearch.query() 对 file/session WS 源加超时 race（10s），WS 断连时 pending 在 10s 后超时 reject，allSettled settle，浮层显示分组空态（不永久 loading 挂死） `[BACKFED from execution consistency-final on 2026-06-30] D-026 措辞清扫`
...
- **D-020 debounce 提前到 P1**（REVISIT of D-019）：异常猎手 AH-C2 发现 #4(P1) useSearch composable 用 allSettled 每次按键发 3 源全量拉取，debounce 在 #10(P2) 造成 P1 阶段性能炸点。修订——debounce(120ms) 升 P1（#7 SearchModal 改造，与 watch query 同 PR），#10 剩 Sidebar keydown + scrollIntoView 两项搭便车。 `[BACKFED from execution consistency-final on 2026-06-30] D-026 措辞清扫`
...
> - #4 新增 AC-4.10：缓存失效竞态防护（K-3，useSearch composable 消费须自绑 setupInvalidation watch） `[BACKFED from execution consistency-final on 2026-06-30] D-026 措辞清扫`
