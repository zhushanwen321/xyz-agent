...
export class GitMessageHandler {
  ...
  async handleGitMessage(msg: ClientMessage, ws: WsType): Promise<void> {
    switch (msg.type) {
      ...
      // ── #6 选分支 / #7 创建分支（ack 复用 'message.status'，status 区分语义）──
      // 数据流：client git.checkout/createBranch → handler → gitService.checkout/createBranch →
      // IGitExecutor.exec(checkout,...) → reply 'message.status' {status:'switched'|'branch_created'}
      case 'git.checkout': {
