...
/**
 * 创建新 session（#1 cwd 透传）。
 * 位置参数 create(cwd?, label?)（③#1 方案 A 用户批准）：新建任务几乎总 label=undefined，实为 create(cwd?) 主导。
 *
 * 边界：cwd=undefined → payload 不含 cwd 键（runtime 回退 process.cwd()，即「上次上下文」/默认）；
 * 非法 cwd → runtime reject → 本 Promise reject（§4.1 E2）。runtime session.create handler 负责 cwd
 * 校验 + pi spawn，失败回滚 session 实体不留僵尸（NFR④#1，§3.8 骨架约束）。
...
export async function create(cwd?: string, label?: string): Promise<SessionSummary> {
  ...
  // cwd=undefined 时不写入 payload 键（与 ClientMessageMap['session.create'] 可选语义一致），
  // 让 runtime 回退；传 label（原 title 语义）。
  transport.send({ type: 'session.create', id, payload: { cwd, label } })
