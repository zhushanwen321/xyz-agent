...
| 编号 | 目标 | 对应 |
|---|---|---|
| G4（继承父文档） | 开发者冷启动应用到可交互时间显著缩短：首屏 gzip JS 从 684KB 降到 <400KB；TTI 提前 | D-8 |
...
- **Out**：任何状态层/渲染层改动（01/02 范围）；终端输出模型（D-6，03 范围）；文件树（D-7，03 范围）；runtime 跨层（D-9，03 范围）；pi 进程与功能需求变更。
...
- ⛔ **坏的部分**：`markdown.ts:23-25` 的 katex 是**静态 import**（消息渲染是首屏必经路径，katex 也需首屏，但它值得单独分组而非合进主 chunk）；`TerminalView.vue:60-64` 的 xterm + 4 addon 静态 import 且 TerminalView 被 PanelContainer 静态 import → xterm 进首屏图；`AppShell.vue:34` 的 SettingsModal 静态 import → 设置页树进首屏图；双 locale 全量静态 import → 3057 行 message 启动时全编译。
...
| Q1-3 | `useCompletionSound.ts:39-64` win 每次 `new Audio(base64)`；mac/linux 每次 IPC `spawn` 系统进程；`detectPlatform`/`resolveName` 每次重探测（`sound-platform.ts:33-41`） | 平台探测与默认音解析是无状态纯函数却被反复重算；win 端每次新建 Audio 对象 |
...
| Q1-7 | `useTurnElapsed.ts:69-71` streaming 期间每秒 `setInterval` 无失焦节流 | 窗口失焦/切 tab 时，不可见的计时器仍每秒 tick 重算 + 触发渲染 |
...
| **DetailPane** | 否——`PanelContainer.vue:87` 是 `v-else-if="drawerTab === 'detail'"` | 相对轻，但作为抽屉非首屏面板，拆出可让 DetailPane 与 MainPanel 解耦 | ✅ 拆 |
...
**为什么 DetailPane 也拆（即使相对轻）**：它和 TerminalView 同是 `drawerTab` 的 `v-else-if` 分支、互斥挂载，且 DetailPane 静态引 MarkdownRenderer/DiffView 等组件，把它一并拆出可让「抽屉 tab 面板」整体从首屏 chunk 剥离，边界更整齐（不因它轻就留在首屏图里形成「半拆」状态）。
...
| 组名 | 成员 | 理由 |
|---|---|---|
| **vendor** | `vue` / `pinia` / `vue-i18n` / `reka-ui` / `@lucide/vue`（+ 其余稳定第三方 UI/状态库） | 框架与 UI 基座低频变更、稳定缓存，与业务 chunk 分离后升级不连带重缓存 |
| **xterm** | `@xterm/xterm` + `@xterm/addon-fit` / `-web-links` / `-search` / `-unicode11` | 单一大依赖（~数百 KB），且只在 TerminalView 触发时下游请求，独立 chunk 让首屏彻底不碰 |
| **shiki** | `shiki`（core + 13 语言） | 高亮引擎体积大 + 已有按需语法 chunk，core 单独一组；首次渲染代码块时异步加载 |
| **katex** | `katex` + `markdown-it-katex` | 公式渲染体量大但被 `markdown.ts` 静态 import（消息必经），单独一组不并进主 chunk |
...
| **Q1-2** | `i18n/index.ts:2-3`、:30-38 双 locale 全量静态 import（3057 行） | 初始只传 zh-CN；`setLocale('en-US')` 时动态 `import` + `setLocaleMessage` | 启动首屏不再含 en-US 3057 行；切英文即时显示 |
...
| **Q1-5** | `pending.ts:22` 无上限；:50-56 真实 setTimeout | size 上限（如 256）超限最老 reject + 惰性超时清理 | 高压下 pendingMap 不超 256，无 timer 泄漏 |
| **Q1-6** | `useAppUpdate.ts:465` 无 visibility 判断；:462 force 绕缓存 | hidden 时顺延 + visible 时补查 | 后台隐藏期间不发 20min 联网检测请求 |
| **Q1-7** | `useTurnElapsed.ts:69-71` streaming 每秒 interval 无失焦节流 | `visibilitychange` 停表 + 恢复时 `Date.now()` 补算 | 失焦时无每秒 tick；恢复时耗时连续正确 |
...
|---|---|---|---|
| **A1 冷启动 TTI 对比** | G4 | ① `pnpm build` 产生产物；② 打包版（或 `pnpm dev` 的 build 产物 loadFile）冷启动；③ DevTools Performance/Network 记录「加载 → 可交互（侧栏可点击）」 | 首屏 gzip JS 总量从基线 684KB 降到 <400KB；TTI 较改动前基线明显提前 |
| **A2 首屏体积断言** | G4 | `pnpm build` 后 `du -sh apps/electron/renderer/dist/assets/` 逐个 chunk 看 | 主 chunk <400KB gzip；存在独立 `xterm-*`/`shiki-*`/`katex-*`/`vendor-*` chunk；xterm 不进主 chunk |
| **A3 语义不回归（三个懒加载点）** | G4/G5 | ① 切到终端 tab → 终端正常渲染、PTY 可用；② 点文件 → DetailPane 正常预览；③ 打开设置 → 设置页树正常 | 三处的功能与改动前一致；chunk 均**首次触发时**才在 Network 出现 |
| **A4 切英文即时性** | 本子文档附加 | 设置 → 语言 → en-US | 切英文即时（无启动期编译等待），全部文案变英文；切回 zh-CN 即刻还原 |

### 4.3 验收场景（Q1-1，后台多 session 完成，代表性）

| 场景 | 回溯目标 | 步骤 | 通过标准 |
|---|---|---|---|
| **A5 后台多 session 完成无长任务** | 本子文档附加（G1 相关抖动消除） | ① 开 4-5 个后台 session 并行让 AI 干活；② 切到别的应用等它们几乎同时完成；③ DevTools Performance 录完成瞬间 | 完成瞬间主线程无 >50ms 的 `localStorage` 同步 IO 长任务；markers 写入合并/走内存 cache |
...
| `packages/renderer/src/i18n/index.ts` | 初始只 zh-CN，setLocale 动态 import + setLocaleMessage | Q1-2 |
...
| `packages/renderer/src/composables/panel/useTurnElapsed.ts` | visibilitychange 停表 + Date.now 补算 | Q1-7 |
...
| 首屏主 chunk gzip <400KB | `pnpm build` 后对主 chunk 做 gzip 测量 vs 基线 684KB | ⛔ build 产物断言（对应 F6，§4 A2） |
