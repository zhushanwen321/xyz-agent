...
- **A（答案）**：D-6 终端分两步——第一步 rAF 写队列合并 xterm.write（scrollback 仍是唯一累积点，隐藏 tab 不丢输出），第二步 scrollback 脱离响应式、改命令式 buffer + 版本回放（**不做 attach 源头过滤**，理由见 §3.3.1）；D-7 文件树分两步——第一步徽章预聚合 + 过滤防抖，第二步扁平可见列表 + virtua 虚拟滚动（virtua 是既有依赖，无新增打包面）；D-9 overlay 回写（renderer 侧）——共享失效 helper 的**两个职责分离**：目录/搜索缓存失效改用 `message.file_changes` ready 帧的变更清单直接触发（不再 deep watch 整 messages），git 角标刷新用 ready 后 debounce 300ms 调 `git.status` RPC 回写 `setGitOverlay`；runtime 侧 git 异步化与帧序保证见 `03-git-state-service.md`（D3/D4）。三项各自成章、各自可独立验收。
...
依赖关系：D-6/D-7 相互独立（父 00 §3.4：D-6/D-7 第一步属阶段 1、第二步属阶段 4）。D-9 依赖 runtime D3/D4（`03-git-state-service.md`，阶段 2 联动）——renderer 侧的 overlay 回写以 runtime 侧「异步 git + ready 帧序」为前置。失效链（`useFileChangeInvalidation` 每 token 全量重扫）与 07 的关系分两段：阶段 1 的 D-1 落地后，整 `chatStore.messages` 的 watch 触发源被切断（外层 Map 恒等稳定），**必须按 07 文档 U7 把依赖源迁移到 per-sid 分区 ref**（保住触发）；阶段 2 的本文 D-9 进一步把该 helper 改为「ready 帧订阅 + 职责分离」（§3.3.3），彻底移出 token 路径。
...
- **In**：
  ...
  - D-9 overlay 回写 + 目录/搜索失效归位（renderer 侧）：`useFileChangeInvalidation.ts`（共享 helper `watchFileChangesForInvalidation` 的职责分离：目录失效/搜索缓存失效改 ready 帧驱动、overlay 回写改 debounce + RPC）、`useFileTree.ts`/`useFileSearch.ts`/`useSearchModalDeps` 三消费方的归位。runtime 侧（`file-change-reconciler.ts` execSync→execFile、`event-interpreter.ts` 帧序）见 `03-git-state-service.md`（D3/D4），不在本文范围。
...
- **下一层 = 可实现的接口/数据模型/代码任务**：D-6 的 `outputQueue/flush/replay(version)` 接口与命令式 buffer 语义、D-7 的 `VisibleRow` 数据结构与 `projectVisibleRows` 投影函数签名、D-9 的 renderer 侧 debounce 与 `git.status` RPC 复用契约，每章落到「文件改动地图 + 待验证检查点」（§5）。
...
**根因**：① runtime 侧 `snapshotGitStatus` 用 `execSync`（`file-change-reconciler.ts:92`），每个 mutating tool（write/edit/bash）+ agent_end 各同步执行一次 `git status --porcelain`，0.35s/次（F7）同步阻塞 runtime 主线程（事件循环停滞期间所有 WS 广播、其他 session 处理都被卡住）；② renderer 侧「角标永不刷新」的**断点**是 `setGitOverlay` 只有 `loadTree`（`useFileTree.ts:49-80`）一个调用点，而 `loadTree` 只在 session 首加载/重开跑，`useGitStatus.ts:162` 虽在 `message.complete`（agent_end）后刷新 git 抽屉数据却不写 tree overlay——于是 AI 写文件后树角标 stale 直到重开 session；③ `useFileChangeInvalidation` 是**共享 helper**（`watchFileChangesForInvalidation`），被 3 个消费方使用：`useFileTree.setupInvalidation`（目录失效）、`useFileSearch.setupInvalidation`、`useSearchModalDeps`。其 `{deep:true}` watch 整 `chatStore.messages`，每个 token commit 全量重扫该 session 所有消息的 fileChanges（根因 1 失效扇出在文件树的副作用），产出两个动作：**目录内容失效**（`onInvalidate(sid, changedPaths)` → `store.invalidate`：loaded→invalidated，AI 写文件后下次展开重拉新文件）与 stale 角标候选（实际无产出）。**该 helper 承担两个职责，任何改造都必须两个职责同时归位**（§3.3.3 D-9 的分离方案），否则「新文件不出现」或「搜索缓存不失效」。
...
| 终端输出队列背压（E6-a） | `outputQueue` 在 rAF 被长时间推迟（后台 tab 节流）时超防御上限 | **合并（join 成单块）而非丢弃**——输出侧语义是保序全量，丢 chunk 会在历史留空洞；命令队列 `pendingWrites` 的 drop-oldest 契约不适用于输出侧（两队列语义分离） | 
...
interface TerminalPartition {
  scrollback: string[]          // 保留（第一步不动模型；**唯一累积点**，见下）
  outputQueue: string[]         // 新增：一帧内待刷的 chunk 暂存（命名避开 terminal-write-queue.pendingWrites 命令队列）
  rafPending: boolean           // 新增：rAF 是否已置位（防重入）
  // ... 其余不变
}

// 写队列核心（3 个函数）
function appendChunk(sid: string, chunk: string): void   // terminal.data handler 只 push 到 outputQueue + requestAnimationFrame 置位
function flushPending(sid: string, xterm: Terminal | null): void
  // rAF 回调：① 先把 outputQueue 全部 append 进 scrollback（含 SCROLLBACK_LIMIT 裁剪）——scrollback 是唯一累积点；
  //           ② 若 xterm 非 null，再把本帧合并块写一次 xterm；若 xterm null（tab 隐藏/切走），跳过 xterm 写入，内容已在 scrollback，mount 回放补齐。
  // 任何分支都不得丢弃 outputQueue 内容。
function writeToTerminal(data: string): void            // 用户输入：直连绕过队列（terminalApi.write，保持现状）
```

- **终端数据**（PTY 输出）走 `appendChunk` + rAF `flushPending`：每帧合并为**一次** scrollback 批量 append（触发一次 `watch(scrollback.length)`）+ 一次 `xterm.write`。**scrollback 是唯一累积点——`flushPending` 无论 xterm 可见与否都先写 scrollback，xterm 写入只是副产物**（隐藏 tab 期间输出照常累积，与现状 `useTerminal.ts:68-76` 的「无条件进 scrollback」保证一致；初稿「xterm null 直接 return」会丢输出，已修正）。
...
- **attach 流量控制不做（审查修正，初稿在第二步落位 attach 过滤，已撤回）**：初稿计划「runtime 按 attach sid 集合过滤 `onData` 广播」，但该过滤会**在源头丢弃隐藏 tab 的输出**——scrollback 靠 runtime 全量广播在隐藏期间持续累积，源头过滤后切回重放必然断档，且需 runtime 侧新增 per-sid 缓冲 + reattach 重放协议来补齐，复杂度过高。定案：**`terminal-service.ts:159-161` 的 `attach` 保持 no-op 预留，不落地过滤**。理由：① 全连接盲发问题已由 02 文档 D1/D5 解决（terminal.data 归 transient、经 bus 只发订阅该 sid 的连接，通常 1 个）；② attach 过滤的增量收益（隐藏 tab 少收几条消息）远小于「runtime 缓冲 + 重放协议」的复杂度与双缓冲漂移风险（G5 反面：scrollback 是唯一累积点，runtime 再存一份就是第二权威）；③ 若未来实测隐藏 tab 的 WS 流量仍构成瓶颈，再以「runtime per-sid 缓冲 + reattach 重放」独立立项。**被否记录**：attach 过滤（源头丢弃，破坏「切走不丢」）；仅降帧（隐藏 tab 低频摘要——摘要内容本身是失真，违背「历史完好回放」）。
...
**renderer 侧（两个职责分离，审查修正——初稿只做 overlay 回写、连同目录失效一起删掉，会酿成「AI 新建文件后已 loaded 目录不重拉 / 搜索缓存不失效」的功能回归）**：

共享 helper `watchFileChangesForInvalidation` 现由 3 个消费方共用（`useFileTree.setupInvalidation` 目录失效、`useFileSearch.setupInvalidation`、`useSearchModalDeps`），其 `{deep:true}` watch 整 messages 承担**两个职责**：
- **职责一（目录内容失效）**：`onInvalidate(sid, changedPaths)` → `store.invalidate`（loaded→invalidated，AI 写文件后下次展开重拉新文件）。**必须保留**——移除后，AI 新建文件时已 loaded 的父目录展开命中 `expandNode` 的 loaded 早退分支（`useFileTree.ts:94`）复用过期缓存 → 新文件不出现（功能回归，非性能取舍）；同时 `useFileSearch`/`useSearchModalDeps` 的搜索缓存失效也被静默破坏。
- **职责二（git 角标刷新）**：现状零产出（watch 只推导 changed 集交给 onInvalidate）；本决策补上「ready 后 debounce + `git.status` RPC 回写 `setGitOverlay`」。
...
onMessage('message.file_changes', (msg, sid) => {
  if (msg.payload.changeSetStatus !== 'ready') return   // 只消费 ready（turn 权威终态，isFullSet=true）
  const changedPaths = (msg.payload.fileChanges ?? []).map(f => f.filePath)
  if (changedPaths.length > 0) onInvalidate(sid, changedPaths)  // 职责一：目录/搜索缓存失效（保留）
  scheduleOverlayRefresh(sid)                                    // 职责二：debounce 后 RPC 回写角标
})

// 职责二（原 overlay 回写设计，不变）：
async function scheduleOverlayRefresh(sid: string): Promise<void> {
  // debounce per-sid 300ms（trailing）后：
...
- **职责一用 ready 帧路径清单而非消息扫描**：比现状更准——现状从全部消息累积推导 changed 集，ready 帧的 `fileChanges` 是权威全集（isFullSet=true）；且 ready 帧在 `changes.length === 0` 时不推送（event-interpreter.ts:434）——无变更时无失效需求，语义自洽。**目录失效不依赖 RPC 结果**（本地路径清单直接 `onInvalidate`），RPC 失败不阻断失效。
- **三消费方归位**：helper 对外签名不变（`watchFileChangesForInvalidation(onInvalidate, sessionIdRef)`），内部实现换触发源——`useFileTree`（目录失效 + overlay 入口）、`useFileSearch`、`useSearchModalDeps`（搜索缓存失效）三处**零接口迁移**，各自回调语义不变。
...
- **`useFileChangeInvalidation` 的成本转正**：原 `{deep:true}` watch 每 token 全量重扫的 wasteful 成本改为「只在 file_changes ready 后按路径清单失效 + debounce 一次 RPC」——既消除根因 1 在文件树的扇出副作用，又把刷新动作接上「产出 UI 价值」（角标真刷新）的断点。
- **被否方案记录（减法优先复核）**：复用 `useGitStatus` 现有 `message.complete → refresh()` 时机顺带 `setGitOverlay`——不选的理由：`useGitStatus` 是 git 抽屉 composable，panel 未挂载抽屉时订阅空窗；且 `message.complete` 不区分「是否写过文件」，无写文件的回合也触发 RPC；ready 帧是精确的「本 turn 有文件变更」信号。且职责一（目录失效）绝不能挂进 `useGitStatus`（其生命周期随抽屉），必须由 helper 自身订阅 ready 帧。
- **与 07 U7 的衔接**：阶段 1 的 07 U7 先把两个整-ref watcher 依赖源迁到 per-sid 分区 ref（保住触发）；阶段 2 本文 D-9 用 ready 帧订阅替换之——U7 是过渡、D-9 是终态，中间无回归窗口（07 §2.3.5 已声明「目录失效/角标刷新的改造属 D-9」）。

**错误规格（D-9）**：

- **E9-a 高频连续完成**：多 session 快速连续 ready → debounce 窗口内多次触发只执行最后一次（trailing 语义），不会产生 RPC 风暴。
- **E9-b renderer git.status RPC reject**：非 repo / 越 cwd / 超时 → 职责二 catch 降级为不写 overlay（角标保持旧值），不打断 renderer 主循环；**职责一（目录失效）用 ready 帧本地路径清单，不依赖 RPC 结果，照常执行**；用户可手动重开抽屉 git 面板刷新（`useGitStatus` 既有路径）。
...
| 场景 | 步骤 | 通过标准 | 回溯目标 |
|---|---|---|---|
| V-P2-1 高频终端输出流畅 | ① 开终端 tab 跑 `npm run build`（或 `find / -name x` 高频输出）；② 输出期间滚动消息流、点侧栏文件；③ Performance 录输出密集段 | 终端输出流畅、界面可交互；**量化 bound：输出密集段 Performance longest task < 50ms、CPU 峰值相对基线（改造前同命令录制）显著下降**；每个 rAF 帧合并多次 write（探针 P-D6-1）；切 tab 回来历史完整（探针 P-D6-2） | G3 |
...
|---|---|---|---|
| D-6.1 rAF 写队列 | `TerminalPartition` 增 `outputQueue/rafPending`；`terminal.data` handler 改 `appendChunk` + rAF；`flushPending` **先写 scrollback（唯一累积点）再写 xterm**；用户输入 `writeToTerminal` 直连不变 | 快赢消除 `watch` 回调 × `xterm.write` 频次，零模型改动，可独立验收 V-P2-1/V-P2-4 | `useTerminal.ts`（分区 + append/flush）、`TerminalView.vue`（挂 rAF flush 调用） |
| D-6.2 命令式 buffer + 版本回放 | `scrollback` 改 append-only 非响应式 buffer + version；组件用 `replay(version)`；删 `watch(scrollback.length)` + `replayedScrollbackLength`；**attach 流量控制不做**（`terminal-service.ts:159-161` 保持 no-op，理由见 §3.3.1） | 目标架构，G5 结构正确（scrollback 不该是响应式列表，且是唯一累积点）；attach 过滤被否（源头丢弃破坏「切走不丢」+ 双缓冲漂移） | `useTerminal.ts`（buffer 结构）、`TerminalView.vue`（回放重写） |
...
| D-7.2 扁平可见列表 + virtua | `VisibleRow` 结构 + `projectVisibleRows`；`FileTreeRow` 改纯行组件（收 `VisibleRow`）；`FileView` 用 virtua 挂扁平行（**virtua `^0.50.0` 已是 renderer 既有依赖**，renderer/package.json:43，无新增打包面） | 目标架构，G5（树 SSOT + 投影），统一虚拟化/过滤/排序/徽章 | `fileTree.ts`（投影函数 + dirChangeCounts 暴露）、`FileView.vue`（virtua + 扁平行）、`FileTreeRow.vue`（递归→单行） |
...
| 单元 | 内容 | justification | 文件改动 |
|---|---|---|---|
| D-9 renderer 失效归位 + debounce + RPC 回写 overlay | `useFileChangeInvalidation` 改「`message.file_changes` ready 帧订阅 + 职责分离」：目录/搜索缓存失效用 ready 路径清单调 `onInvalidate`（保留），角标刷新 debounce 300ms → `gitApi.status` → `setGitOverlay`；移除 `{deep:true}` watch messages | 修复 F4 stale 角标断点；消除根因 1 在文件树的 wasteful 全量重扫；**两职责同时归位**（只删 watch 不补目录失效 = 新文件不出现/搜索不失效的功能回归）；复用 git.status RPC（经 D4 GitStateService 缓存） | `useFileChangeInvalidation.ts`（helper 触发源替换 + 职责分离）、`useFileTree.ts`/`useFileSearch.ts`/`useSearchModalDeps`（三消费方归位确认，签名不变） |
...
**检查点**：D-9 后 `setGitOverlay` 有两个调用点（loadTree + ready 回写）；`useFileChangeInvalidation` 不再挂 `chatStore.messages` deep watch，**且目录失效仍工作**（ready 路径清单 → `onInvalidate` 三个消费方照常响应）；runtime 侧探针（git 无同步阻塞）在 03 文档的探针清单中验收。
...
| D-9 失效触发归位（token 路径清零 + 双职责走通） | P-D9-3（useFileChangeInvalidation 触发点在 token vs ready；目录失效 + overlay 均自 ready 驱动） | ✅ token 路径零触发；ready 后 `onInvalidate`（目录失效）与 `setGitOverlay`（角标）均走通 | ⛔ token 路径仍触发；或目录失效/角标刷新缺一 |
...
| P-D9-3 | `useFileChangeInvalidation` 移除 deep watch 后，在 `message.file_changes` ready 事件 handler 打印触发点与 `onInvalidate` 路径清单 | renderer console | 触发点只在 ready、token 无触发，且 `onInvalidate` 以 ready 路径清单调用（目录失效保留）即 ✅ |
