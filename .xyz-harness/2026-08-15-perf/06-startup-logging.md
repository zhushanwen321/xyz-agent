...
- **[实施定案，2026-08-17 回写] 实际实现为全串行链（有意收敛，非上文「相互可并行」的并行编排）**：`startup-background-init.ts` 的后台初始化块为 ① `migrateProviderConfig`（setMigrationGate，D8-3 gate）→ ② `migrateBuiltinExtensions` → ③ `checkAndAutoUpgrade` → ④ `getPiVersion`（mutate appInfo + 补发 app.info，D8-2）→ ⑤ `skillRegistry.initGlobal` → ⑥ `pluginService.initialize` → ⑦ `ensureAutoRenameDefault`（auto-rename 默认初始化，本段上文未列的实现新增段），逐步 await 顺序执行。上文本条「『migrateProviderConfig`、`getPiVersion`、`skillRegistry.initGlobal`、`pluginService.initialize` 相互可并行』是设计阶段的可行性判断，**保留为过程记录，实施未采用**。串行收敛的定案依据：① 唯一硬序约束 `migrateBuiltinExtensions` → `checkAndAutoUpgrade` 在串行链中自然满足（且比「并行块 + 局部串行」更严格——全部偏序约束均无竞态面）；② listen 提前的主目标（端口先就绪、/health 即 ready、迁移/探测不阻塞 listen）不受影响——串行发生在 listen 之后的后台块内；③ 每步独立 try/catch 的 best-effort 语义在串行链中最易保持。**代价：后台初始化总完成时间 = 各段耗时之和**（并行可压缩为最长段），以换取编排简单与约束零心智负担——对各段量级（迁移 <100ms、探测 1-2s 等）而言，总完成时间仍在秒级，与 D8-2「1-2s 内补全版本串」的 V2 验收口径兼容。
