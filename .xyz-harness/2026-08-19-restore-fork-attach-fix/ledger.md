...
- 2026-08-19 深夜 **实施后差距复审（reviewer 对抗审查，报告 review/design-impl-gap-review.md）**：结论 = 核心修复与护栏实现一致、验收证据链等强度、无越界实现；**1 must-fix + 6 suggestion 全部处置**。must-fix（ADR-0063 I1 未如实记录跳过分支 ③）+ 5 条文档回写（设计 §3.4 I1/D3 跳过分支语义、§5 W2 行 R1 豁免/helper 落点偏差、§4 V4 覆盖边界、附录实施期裁决 5 条、attach-lifecycle 头注释、ADR-0063 与登记表 ⑩ helper 路径）随复审 commit 落地；reviewer 独立实跑 R1 exit 0 + pi 锚点抽查 8 条吻合。**新增观察项 2 条（不实现，按最小代码原则记录）**：① helper 跳过分支 ③ 收紧为可观测 error（构造性失效窗口，W2 verifier 已评估）；② `.tmp-migrate-` 残留无清理机制（rename 失败无回滚删除 / delete 链不清扫 / 崩溃残留永久孤儿——scanner 已排除不附着错位，纯磁盘垃圾，触发窗口 = 写临时名后崩溃）。
