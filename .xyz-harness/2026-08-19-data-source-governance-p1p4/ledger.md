...
| W12 | 5 个 state 话题切实例快照发布 | verifying | 996063a6f | 依赖 W7/W8✓；builder 交付 11 文件 +420/-262：5 阶段全切换（session-service 三阶段 + event-interpreter 两包装类 SubagentsState/WorkflowUpdatesState）+ 删三旧直写路径（fetchAndBroadcastCommands/broadcastSessionState/fetchAndBroadcastContext 收敛）+ switchModel markDirty 三实例 + w12 等价性测试 14 用例（两层：等价层同值 + 数据源层证伪影子）+ W8/W10 minor 顺带。自报 3150 全绿 + message-bus 零 diff。**关键设计细化待 verifier 裁决**：publish 形态 = 「fetch 落定 → applySnapshot → setTimeout 0 挂钩发布」非原地换读（论证：原地换读滞留防抖窗口旧值）。死代码移交：resolver 注入链 + setThinkingLevelCache 留 W13/W15。主 agent 待 PASS 后按 hunk 分 5 次 commit |
