...
| wave | 名称 | 状态 | 验收基线 commit | 备注 |
|------|------|------|----------------|------|
| W13 | session store applySnapshot 单入口 + DTO | verified (PASS 待 commit) | 996063a6f | verifier PASS（报告 w13-report.md：10 检查点全过——防篡改 0 diff/11 源 + 38 测试与 git status 一一对应/**useChat 6 hunk 机械改写 + registry 恰 3 行注释与 W14 零重叠实证**/mergeViewSnapshot D1b + W15 挂点锚定/DTO 与 W12 payload 逐字段对上/三入口 grep 清零/R2 WATCHED_MUTATIONS 单键化/红性两组——falsy 跳过 TC-4b 红 + 直呼 lint 实弹，字节还原/core 994 + renderer 3054 全绿/三冲突裁决全支持 builder——chat 域以 plan「全部调用点改」为准）。minor 5 条（W15 落盘字段确认/usage 丢弃存量行为/index.ts 混 W18 导出/store 残余写方法归 W24/行超长）。**commit 顺序锁定 W18→W13→W14**（shared/index.ts 双侧依赖 W18 constants 先入库保每快照可编译；registry.ts 归 W14） |
...
| W18 | runtime 消费管线（entry_appended + get_entries） | verifying | 9382ccb57 | 依赖 W12✓/W16✓/W17✓；builder 交付：生产 8 文件 + 2 删除 + 测试 2 新增 7 改写（3160 全绿）——entry_appended 移出 NULL_EVENTS + customType 过滤失效信号；事件直写全链删除（包装类退役）；RecordEntriesCache 游标三路径（初始全量/增量/Entry-not-found 两轮自愈）；两 extractor 重构 scan（自描述优先 + legacy 兜底）；混沌 2 用例；死代码连删 -139 行（含 model-context-cache.ts 孤儿）。两偏差上报（interfaces.ts 实际路径；Map 物理位置在 session-service——plan 文件分工支持）。登记表 #8/#9 撤销草稿已交 |
