...
| W18 | runtime 消费管线（entry_appended + get_entries） | building | 9382ccb57 | 依赖 W12✓/W16✓/W17✓；builder 后台运行中（entry_appended 移出 NULL_EVENTS + customType 过滤 + getEntries(since) 游标增量 + extractor 重构 entry 扫描器 + 混沌收敛用例 + W12 移交死代码顺带删；完成后撤销登记表 #8/#9 过渡例外——草稿制） |
