...
| W24 | R2 调用图收紧 | rejected→修复中 | ed26b3da8 | 依赖 W2✓/W13✓；builder 交付 34 文件（verifier 实测，自报 35 差 1）：R2 留 mjs 定案 + 形参/方法引用/工厂包装三检测面 + 超集证明 + 许可表运行时解析登记表（13 条全命中 §1 主表 + stale 实测红）+ R3 扩围 61 处（60 规则标记 + 1 口径外保守补标）+ attach 修复 5 探针全对 + node --test 38/38 + lint 0 errors + renderer 3054/core 999。**verifier PASS 带 1 major 打回修复中**：工厂包装检测面漏「表达式体箭头函数」（ReturnStatement 访问器不触发，Vue 惯用形态静默绕过——探针 `const grab = () => useSessionStore(); grab().applySnapshot({})` 0 报错，对象方法包装 + MemberExpression receiver 双重逃逸）+ 顺带 i18n loadedLocades 重新分类（Set 被 add() 变异非字面量）。5 minor 中 3 记录（stale 门控/计数差/detachedMethodRef 文案/paramOwnerFn 同名） |
