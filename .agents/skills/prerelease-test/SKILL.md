...
创建 `-beta` 后缀的预发布版本，触发 CI 构建产物（DMG/EXE/AppImage），
供本地安装测试。测试通过后自动还原代码版本号，不占用正式版本号。

## 核心流程

1. 确认工作区干净（可在 main 或开发分支运行）
2. 从最新正式 release 计算 beta 版本（固定 `-beta` 后缀）
...
1. 打开 CI 链接查看失败日志
2. 修复问题后重新运行脚本（会自动清理旧 beta release 并重建）
...
- **Q: 为什么要用 `-beta` 而不是直接 bump？**
  直接 bump 会占用 `v0.4.7` 的 tag，导致正式发布时版本号被跳过。

- **Q: 重复测试时 tag 冲突怎么办？**
  脚本在阶段 2 自动清理旧的 beta release 和 tag，然后重建。
