...
**Step 1 — 确认变更范围**（主 agent 自己跑）：`git diff main...HEAD --stat`（与维度选择同一次 diff）
...
- focus（见「维度 → Agent 映射」表对应审查焦点）+ agent 定义文件路径（`<repo>/.agents/skills/pr-cr-fix/agents/review-<维度>.md`，subagent 须复读原文获得完整 checklist）
