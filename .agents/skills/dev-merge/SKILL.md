...
**调用约束**：cwd 必须在待合并 feat worktree 根目录（脚本靠 `git rev-parse --show-toplevel` 定位源 worktree）。脚本调用用 worktree 根目录的相对路径（`.agents/skills/dev-merge/dev-merge.sh`），因为 skill 始终从当前 feat worktree 调用，脚本就在该 worktree 内。
...
```bash
cd ../<dev-branch> && git status --short   # 全量冲突态
# 逐个解决冲突文件后：
cd ../<dev-branch> && git add <files> && git commit
```
...
3. 这是**会话级永久状态**：本会话后续所有 bash 调用都会得到同样错误。收尾判断——脚本输出过 `OK:` → 全部完成，正常输出总结；脚本在删 worktree 之后输出过 `ERROR: 分支 ... 删除失败` → 分支是唯一残留，在总结里写明一条待执行命令 `git -C ../<dev-branch> branch -D <feat-branch>` 交给用户/下次会话
