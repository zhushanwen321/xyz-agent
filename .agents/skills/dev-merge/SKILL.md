...
### 第 4 步：清理源 worktree（用户确认后）

合并成功**且用户确认清理**后执行（删除 worktree + 分支，破坏性操作须用户点头）：
...
**[关键] cleanup 成功后**：原 worktree 目录已不存在，而 bash 调用的默认 cwd 可能仍指向它（目录悬空）。后续所有 bash 命令必须显式 `cd` 到其他 worktree（如 dev worktree），否则会莫名失败。
