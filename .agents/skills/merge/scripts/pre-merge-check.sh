...
#
# 用法: bash .agents/skills/merge/scripts/pre-merge-check.sh [worktree-dir]
#   worktree-dir: 可选，默认当前目录
#
# ⚠️ 自包含副本：来源于全局 merge-worktree skill，
#    属于 xyz-agent merge skill，不依赖全局 agents 安装。
#
# AI 行为约束：
#   - 此脚本的每一项检查都不可跳过
#   - 失败时必须修复后重新运行，直到全部通过
...
while IFS= read -r tsconfig; do
    ...
    # 所有 tsconfig（含前端目录）统一在此循环处理：
    # 有 typecheck 脚本的目录走 npm run typecheck（vue-tsc 项目通常用此方式），
    # 否则走 npx tsc --noEmit。不再为 frontend 单独硬编码分支。
    ...
    TSC_RAN=true
done < <(find . -maxdepth 3 -name "tsconfig.json" -not -path "*/node_modules/*" -not -path "*/dist/*" 2>/dev/null | sort)

# 前端目录（含 vue-tsc）已在上面的 tsc 主循环统一处理：
# 有 typecheck 脚本走 npm run typecheck，否则走 npx tsc --noEmit。
# 不再为 frontend/ 硬编码单独分支（xyz-agent 前端在 packages/renderer 等位置）。
...
if git rev-parse --is-inside-work-tree >/dev/null 2>&1; then
    ...
    # 未推送 commits 检测（动态检测 upstream remote，不硬编码 origin）
    # bare repo 项目 origin 可能指向本地 bare，GitHub remote 可能叫 github
    _BRANCH=$(git branch --show-current 2>/dev/null || echo "")
    UNPUSHED=""
    if [[ -n "$_BRANCH" ]]; then
        # 优先用 @{upstream}（最准确，反映 git 实际追踪状态）
        UNPUSHED=$(git log --oneline '@{upstream}..HEAD' 2>/dev/null || echo "")
        # @{upstream} 未设置时，枚举 remote 找哪个有该分支
        if [[ -z "$UNPUSHED" ]] && ! git rev-parse --verify '@{upstream}' >/dev/null 2>&1; then
            TRACKING_REMOTE=""
            for _remote in $(git remote 2>/dev/null); do
                if git rev-parse --verify "$_remote/$_BRANCH" >/dev/null 2>&1; then
                    TRACKING_REMOTE="$_remote"
                    break
                fi
            done
            if [[ -n "$TRACKING_REMOTE" ]]; then
                UNPUSHED=$(git log --oneline "$TRACKING_REMOTE/$_BRANCH..HEAD" 2>/dev/null || echo "")
            fi
        fi
    fi
    if [[ -n "$UNPUSHED" ]]; then
