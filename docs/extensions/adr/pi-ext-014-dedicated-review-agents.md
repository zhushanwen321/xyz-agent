...
pi-ext-011 决定所有 subagent 统一使用 `general-purpose` + task prompt 注入方法论。当时理由是 harness 只有 2 种 subagent（review + retrospect），不值得独立维护。
...
pi-ext-011 的决策在以下条件下仍然成立：
- subagent 数量 ≤ 2
- prompt 内容简短（< 20 行）
- 不需要跨 skill 复用

本 ADR 限定了 pi-ext-011 的适用范围，不推翻其核心原则。
