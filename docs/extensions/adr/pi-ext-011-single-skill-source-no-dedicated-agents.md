...
Retrospect 模块原同时存在于 `skills/harness-retrospect/SKILL.md`（源仓库路径）和 `agents/harness-retrospect/agent.md`（源仓库路径，已删除），内容几乎相同，维护两份。决定：删除 agent 版本，保留 skill 版本。所有 subagent（包括 review、retrospect）统一使用 `general-purpose` agent，通过 task prompt 注入方法论（让 subagent read 对应的 skill 文件）。
