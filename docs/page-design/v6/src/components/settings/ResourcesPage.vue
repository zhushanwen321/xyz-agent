...
type RpSource = 'pi' | 'claude' | 'agents' | 'piinstall' | 'muted'
...
const rpItems = ref<RpItem[]>([
  ...
  { name: 'merge', sources: ['pi', 'claude'], desc: '合并分支并发布（多来源链示例）。' },
...
/** 4 源（spec §4：skill 显全部 4 源）；pi 已在共享池生效、zcode 目录无资源 → 两行禁选 */
const IMPORT_SOURCES: ImportSource[] = [
  { id: 'claude', name: 'Claude', dir: '~/.claude/skills', count: 12 },
  { id: 'codex', name: 'Codex', dir: '~/.codex/skills', count: 8 },
  { id: 'pi', name: 'Pi', dir: '~/.pi/skills', count: 5, shared: true },
  { id: 'zcode', name: 'Zcode', dir: '~/.zcode/skills', count: 0, empty: true },
]
const importableCount = computed(() => importSources.value.filter((s) => !s.shared && !s.empty).length)
...
function confirmImport() {
  ...
  importTimer = setTimeout(() => {
    for (const [i, dir] of dirs.entries()) {
      if (!projectPaths.value.some((p) => p.path === dir)) {
    ...
    <section class="lp-card">
      ...
      <div class="lp-import-actions">
        ...
        <button v-else class="btn btn-secondary btn-dense" @click="closeImport">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="17" y1="7" x2="7" y2="17"/><polyline points="8 7 17 7 17 16"/></svg>
          收起导入
        </button>
      ...
      <div v-if="importOpen" class="imp-panel">
        ...
        <div v-if="importStep === 'detecting'" class="lp-row imp-row">
          <svg class="imp-spinner" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 12a9 9 0 1 1-6.219-8.56"/></svg>
        ...
        <template v-else-if="importStep === 'select'">
          <div v-for="s in importSources" :key="s.id" class="lp-row">
            <input
              ...
              :disabled="s.shared || s.empty"
              :aria-label="'导入 ' + s.name"
              @change="toggleSource(s.id)"
            />
            <span class="lp-src-label" :class="{ dim: s.shared || s.empty }">{{ s.name }}</span>
            <span class="lp-src-path" :class="{ dim: s.shared || s.empty }">{{ s.dir }}</span>
            <span class="lp-count">{{ s.count }} skills</span>
            <span v-if="s.shared" class="lp-tag shared">共享池已生效</span>
    ...
    <div v-if="importStep === 'confirm'" class="imp-confirm-stage" @click.self="cancelConfirm">
      <div class="imp-confirm" role="dialog" aria-modal="true" aria-label="导入目录到 xyz-agent？">
        <div class="imp-confirm-head">
          <svg class="imp-confirm-ico" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" x2="12" y1="15" y2="3"/></svg>
        ...
        <ul class="imp-confirm-bullets">
          <li><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg><span><b>来源：</b>其他 Agent 的 skill 目录配置（<code>{{ selectedDirs.join('、') }}</code>）</span></li>
          <li><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg><span><b>去向：</b>写入当前 xyz-agent 的 <code>projectPaths</code>（项目目录组）</span></li>
          <li><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg><span><b>去重：</b>已存在的路径会自动跳过</span></li>
        </ul>
      ...
      <template v-else>
        ...
        <div v-if="filteredRp.length === 0" class="rp-empty">
          ...
          <template v-else>
            ...
            <span class="rp-empty-text">{{ TAB_LABEL[sourceTab] }} 来源下暂无 Skill</span>
