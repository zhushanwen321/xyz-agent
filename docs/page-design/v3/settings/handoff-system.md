...
**▶ 已裁决待落地（ADR-0021-B，2026-06-20）** — ✅ 2026-06-27 已落地（stores/settings.ts 重构为单一真相源）：

- `settingsStore` 初值：`theme='light'` → `'dark'`、`themePreset='neutral'` → `'cold-blue'`；`SystemPage.vue` `mutedSwatches/colorfulSwatches` 含 `cold-blue` 品牌默认。
- `setSystem({theme})` 同步 `<html data-theme>` 到 DOM（theme='system' 按 prefers-color-scheme 解析），主题切换从「死设置」变为实际生效。
- CSS 变量迁移：`--section-bg`→`--surface`、`--divider`→`--border`、`--accent-light`→`--accent-soft`（真身代码无残留 impl 自造名，全用 SSOT 名）。
- locale 真相源统一：i18n 不再单独持久化 `xyz-agent-locale`，改读 system 偏好 key；`setSystem({locale})` 调 `i18n.setLocale` 切换实际语言。
- ⏳ themePreset（palette）CSS 切换暂未实装：SystemPage 选中态落地、`data-palette` 属性 + `--accent` 覆盖待做（见 design-tokens.md 待办）。
...
- [x] 语言 select 即时切 i18n，不重开 modal（zh-CN / en-US 两项，母语显示）— store.setSystem({locale}) → i18n.setLocale
- [x] 外观模式三态（light/dark/system）经 `data-theme` 属性正确 apply；system 跟随 prefers-color-scheme — store.applySystemToDom 已实装
- [ ] 配色主题 11 swatch（Muted 5 + Colorful 6，含 draft 新增 cold-blue 品牌默认），oklch/hex 色块 + label，选中态 `accent ring + accent-light 底` 清晰 — 选中态已落地，实色切换 ⏳ 暂缓
- [ ] palette 切换经 `data-palette` 属性驱动，非 JS 逐变量赋值 — ⏳ themePreset 暂未实装 CSS
- [ ] ★draft 决议：「聊天显示」整块不进 draft 验收（compactStreaming/standaloneTools 等真身保留，draft 不展示）
- [x] 默认值正确：theme=dark / themePreset=cold-blue（ADR-0021-B 裁决已落地，2026-06-27）
