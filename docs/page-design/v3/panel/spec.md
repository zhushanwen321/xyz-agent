...
## 核心裁决 · Panel 内部结构（4 个固定 zone + git 下沉抽屉）

无论单 / 双，一个 Panel 内部由固定 4 个 zone 自上而下排列；git 状态不再占底部 zone，下沉为 Side Drawer 的 Git tab（入口为 header 右侧 git 图标按钮 + 脏状态点）：
...
└──────────────────────────────────────────┘
   ⑤ git（原底部 zone）→ 下沉 Side Drawer Git tab，入口在 panel-header 右侧
```

**panel-header 含 split + 新建会话两按钮**（同槽位互斥）— 单 Panel 时显「分屏」（开第二 session），双 Panel 时分屏隐藏、改显「新建会话」（替换待机侧为新 session 并聚焦）。不允许多于 2 个 panel。无工作区级横跨 header（每 panel 只有一个 header，所有操作落 per-session header）。**panel-header 右侧另含 git 图标按钮**（非 git 仓库隐藏）：点击打开 Side Drawer 的 Git tab，按钮带脏状态点（conflict→danger / 有改动→warning / clean→无点）。
...
**Side Drawer（归 Panel 联动，header 多 tab 通用容器）**：抽屉 = 一个 header 多 tab 容器，tab 承载不同实体（文件×N / 终端 / 子Agent / 浏览器 / **Git**…），**而非单实体的视图切换**。**Git tab 承载 cwd 全量 git 状态**（分支 / stats / 文件列表 / 暂存 / 提交，GitPanel.vue），原 Panel 底部 git-zone（zone ⑤）下沉于此；**Diff/预览 下沉为文件 tab 内部 view-toggle**（detail-pane 范式为准）。类型枚举 Git / Browser / Terminal / 文件×N / ChangeSet Detail / SubAgent Detail 是 tab 内容分类，不是独立浮层。与 Panel 数据强耦合，从触发它的 Panel 内浮起，**固定挂该 Panel，不跨 Panel 覆盖对侧**（跨 Panel 行为 v1 不做）。Git 数据由 Panel 经 `GIT_STATUS_KEY` provide、GitPanel inject（单实例：header 脏状态点与抽屉内 stage/commit 共享同一份数据）。
