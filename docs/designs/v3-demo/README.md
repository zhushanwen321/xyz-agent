...
xyz-agent v3 重构的设计探索文件集合。**暂存区**，非最终归宿。按"设计单元（文件夹）"组织，方法 `recursive-skeleton`（L0→L4 递归骨架）。

## 全局基础件（在 docs/designs/ 根，非本目录）

| 文件 | 位置 | 层级 | 说明 |
|---|---|---|---|
| `PRODUCT.md` | **项目根**（`docs/designs/PRODUCT.md` 为 symlink） | 产品 | 定位 + 品牌人格 |
| `design-tokens.md` | `docs/designs/` | 原子 | 值 SSOT（色/字/距/影/动效） |
| `design-system.md` | `docs/designs/` | 原语 | 组件原语层（tokens 之上如何用） |
...
3. **ADR `adr-NNNN-<主题>.md`** 平铺根；基础件语义名放 `docs/designs/` 根。
...
## 归位到真实仓库（sandbox 外 · 仅备忘）

用户已明确「只管 demo 目录」。真实仓库治理另行处理，本表仅备忘：`adr-*.md` → `docs/architecture/adr/`；`design-tokens.md`/`design-system.md` → 真实 design-system 区；`PRODUCT.md` → `docs/project/`；设计稿 → 治理后的 `docs/designs/`。

## 归位到真实仓库（2026-06-20 已执行）

v3-demo 设计稿 + 基础件已从 sandbox 归位到真实仓库：
- `adr-*.md` → `docs/architecture/adr/0018-0022`（重编号接续项目既有 0001-0017，全量更新交叉引用）
- `design-tokens.md` / `design-system.md` → `docs/designs/`（根）
- 设计稿（spec.md + draft HTML）→ `docs/designs/v3-demo/`（本目录）
