...
| 文件 | 改动 | 单元 |
|------|------|------|
| `packages/runtime/src/services/provider-config-helper.ts` | 防线载体共享纯函数 `applyProviderWritePolicy`（provider 级空串转译收口于此：apiKey 分支见 `applyProviderCredentials` 删键、其余=不写键；setProvider 与 importer 共用）；`applyModelRoutingFields` 模型级转译（name/api/baseUrl 空串=不写键 + 模型级空 id 整条丢弃）+ catalog 分体系（type 忽略、baseUrl 网关写入/清除 + extras `gatewayBaseUrl` 标记同步 + 不物化空壳）；`resolveCatalogDisplayFields` 网关优先派生改造；listProviders 内联判定改 resolver 批量 sync 版 | M1/M4/M2 |
