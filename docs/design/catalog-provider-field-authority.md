...
**基线声明**：本章行号与符号基于实施前基线 commit 0d8b6cbbc，实施后可能漂移——现状以 §3.3 与代码为准。
...
5. runtime applyProviderLevelFields（实施前函数名，现已收口为防线载体 applyProviderWritePolicy）：`if (data.baseUrl !== undefined) merged.baseUrl = data.baseUrl`（`provider-config-helper.ts:538`）——`''` 非 undefined，空串写入；`:539` 快照 api 固化进 override。`upsertProvider` 落盘（`:765`）。
...
[runtime] setProvider：
  ...
  │     收到空串 → 剥除或删键（apiKey:'' = 清除语义 delete；模型级空 id = 整条丢弃该模型；其余 = 未指定不写键），对齐 pi「空串无语义且拒载」
...
- **采用**：六层防线，每层拦截一类路径。**防线作用的字段全集 = pi `minLength:1` 约束全集**（provider 级 `name/baseUrl/apiKey/api` + 模型级 `id/name/api/baseUrl` + modelOverrides 级 `name`，`model-config.js:137-140/:170-173`）——空串值对 pi 任何 provider 都是「写入即拒整个文件」的毒药，防线不按「历史上谁写过空串」挑选字段：
  ...
  - **② runtime 空串值转译**：provider 级空串转译收口于防线载体 `applyProviderWritePolicy`（apiKey 分支见 `applyProviderCredentials`），`applyModelRoutingFields` 保留模型级转译——全集字段收到空串（**trim 后空串同视**——拦 CLI 发纯空白串的极边缘）——`apiKey` 空串 = **清除语义**（`delete merged.apiKey`，与 pi-provider-store.ts 的 `clearProviderApiKey`（`index.ts:572-574` 装配处引用）同构——「清除」的正确落盘形态是删键而非写空串）；其余字段（provider 级 name/baseUrl/api 与模型级 name/api/baseUrl）空串 = 未指定（不写键）+ warn；**模型级 id 单列（实施回写 D-1）**：空/纯空白 id → **整条丢弃该模型** + warn——id 是 pi 必填字段（minLength:1），「不写键」会留下无 id 条目，同样被 TypeBox 拒载整个 models.json。custom 分支显式接受语义：**用户清空 custom baseUrl 保存 = 不变更**（custom 无「默认」可回退，删除 provider 用删除功能）。
  - **③ catalog 分体系语义**（`isCatalogProvider(id)` 时，provider 级字段的空串由本层接管——②的通用转译只管 custom 与模型级字段，层间分工：catalog provider 级字段全部走③的分体系语义）：`type` 键忽略 + warn（provider 级 api 对 catalog 无用户语义——只影响 override 自定义模型的协议缺省，撤输入框后新增 override 模型的协议缺省回落派生值）；`baseUrl` 非空 = 用户网关——写入 `merged.baseUrl` 的**同时**在 providers.json extras 写显式网关标记 `gatewayBaseUrl`（authMethod 先例同域：显式标注区分「用户显式设置」与「历史冻结」，判定力不依赖任何会漂移的外部数据）；`baseUrl` **显式空串带键** = 清除网关——删除 override 既有 `baseUrl` 键 + 清除 extras 的 `gatewayBaseUrl`（场景 A' 回退通道；**未带键（undefined）= 不变**，对齐既有 merge 协议——OAuth 收尾等不带键调用方天然不动网关）；**写序契约**：设置网关 = 先写 extras 标记、后写 models.json（崩溃中间态 = 多余标记，D2 自愈、重试即恢复）；清除网关 = 先删 models.json 键、后清 extras 标记（崩溃中间态同样是多余标记——两个方向对称取「标记多余可自愈」侧，反向序会静默丢网关）。**extras 写入点唯一**（setProvider 路径经既有 `extrasStore.modify` 通道；importer 不产生网关写入故无 extras 写；`extrasStore.modify` 无内容 diff 守卫——清除分支对不存在的标记 no-op 短路，避免无谓写盘）。**gatewayBaseUrl 判定为仅存在性**（清洗只看键在不在，禁止按值比对——标记值 stale 不影响判定，防复活 v2 值比对问题）；**不物化空壳**——剥除/清除后 merged 若不含八字段（models/baseUrl/headers/compat/modelOverrides/apiKey/oauth/authHeader）任何键，产出跳过 `upsertProvider` 信号 + warn（对齐 M5-01「宁丢不写错位」先例，`provider-config-helper.ts:519-523`）。注意（实施回写 D-9，措辞修正）：「不物化空壳」**只约束新建**（`existingConfig === undefined` 且空壳 → 跳过 upsert，不产生条目）；**既有条目一律 upsert**——剥除/清除必须落盘，否则用户「清空网关」成为静默 no-op（盘上旧 baseUrl 残留，展示回内置端点但 pi 仍打旧网关，违反 G1）。「对既有条目是 no-op 而非删除」的准确含义 = **不删除既有条目本身**（用户全清空后旧条目仍在盘上，由 D2 清洗顺序契约中的既有空壳修复分支在下次启动时接管，MF-5 语义：catalog 空壳从快照合并 models 修复），不是「既有条目跳过写盘」。
...
- **证据**：毒化链 §2.1（P-poison 实测 + 审查复核 apiKey/model 级空串同毒化）；pi 语义 `model-config.js:137-140/:170-173`（minLength 全集）；网关覆盖语义 `provider-composer.js:98`；既有先例 M5-01（`provider-config-helper.ts:519-523`）、QuickSetup truthy 守卫（`use-quick-setup-form.ts:175-176`）、pi-provider-store.ts 的 `clearProviderApiKey`（`index.ts:572-574` 装配处引用）、authMethod 显式标注修复推断歧义（extras 同域先例，`provider-config-helper.ts:319-320`）。
...
- **采用**：扩展 `sanitizeInvalidProviders`（`pi-provider-store.ts:532-573` 同族），启动时对 models.json 逐 provider 条目做两类清洗（先①后②，再做既有空壳判定——顺序见下）：
  - **① 空串键剥除**：全集中任一字段值为 `''` → 删该键（幂等，有剥除才写盘），日志 `[provider-repair] stripped empty-string schema keys on "<id>": <keys>`；**模型级 id 例外（实施回写 D-1）**：空/纯空白 id 的模型**整条丢弃** + warn——id 是 pi 必填字段，删键会留下无 id 条目，同样毒化文件（三条写路径与防线②同口径）。
...
- **采用**：新建 `packages/runtime/src/services/auth/provider-credential-resolver.ts`，双形态共享同一份源优先级定义（单点模块，优先级常量一处声明）：
  ...
  - **分层与注入设计**（链 3 可实施性）：接口 `IProviderCredentialResolver` 下沉 `services/ports/`（与 IModelSource 同模式）；实现驻 `services/auth/`（消费 AuthService + configStore port）。链 3 消费点（`pi-provider-store.ts:353/:384`，infra/pi/）**不 import 实现**，只 type-only import 接口（豁免 C-comm-03 的 import type 形态）。注入通道按 PiConfigStore 实际装配形态二选一（均为模块级函数而非实例方法持有逻辑的薄委托类，构造参数到不了消费点）：**首选**模块级 init 注入（组合根装配期调用 `initProviderCredentialResolver(resolver)`，模块级变量持有；调用先于任何 `findValidDefaultModel`——组合根装配序保证）；**备选**调用链显式传参（`findValidDefaultModel` 签名加 resolver 参数逐层透传）。resolver 构造无 IO——读取全部懒发生，装配早期触发 `findValidDefaultModel` 无时序风险。**实施已定（检查点 5 结论，M2c）**：模块级 init setter 形态，落点 `pi-provider-store.ts`（initProviderCredentialResolver + 模块级变量——与唯一消费点同模块；经 pi-config-store 转发会构成反向 import 循环、纯转发多一跳零收益，被否），装配序（迁移/清洗之后、全部 service 装配与 server.start 之前）由 runtime-wiring 静态断言锁定；备选传参形态未采用。
  - 迁移 5 条散落链（§2.3 表）：链 2 `handleDiscoverModels:714` 改 async 版；链 1 `QuotaService.getCredential` 的 auth.json/models.json 两段替换为 resolver（保留 quota 专属 secrets 第一段——那是 Coding Plan 专属 key 语义，见 `ProviderInfo.quota.apiKeySet`，不属于 provider 凭据）；链 3 经注入消费 sync 版（顺带消除绕过 AuthStorage 的私有裸读 `readAuthCredentials`）；链 5 `listProviders` 内联判定改批量 sync 版；`index.ts:533-538`（实施前行号）clearApiKey 闭包**实施修正（D-26）**：不走 resolver sync 版——sync 形态只有「任一源有凭据」的并集判定，而 clearApiKey 是 models.json 单文件删键 RMW（用并集判定当 guard 会改变语义：auth.json-only provider 触发无谓写盘、空串毒化条目被并集放行），落地 = 保留纯删键 RMW（不写空串）并提取为 `pi-provider-store.ts` 具名函数 clearProviderApiKey（index.ts import 即执行 main() 不可单测，提取后删键语义可落盘断言）——D6 姊妹守卫白名单的 clearApiKey 条目相应记 pi-provider-store.ts。
...
- **采用**：
  - **测试连接**（有模型的 provider 可点）：runtime 按模型 api 分组、每组取**第一个 `enabled !== false` 且生效 baseUrl 非空**的模型作代表（双过滤：禁用模型不代表、生效 baseUrl 空的模型不冒充网络错误；**「生效 baseUrl 非空」按含回落链的生效值判定而非模型级字面值**（实施回写 D-20）——否则「模型发现」拉回的模型（只有 id/name/contextWindow）永远无法当选代表，与回落链自相矛盾），发**协议真实最小请求**（anthropic-messages：POST `{baseUrl}/v1/messages`，body 含 `max_tokens:1`；openai-completions：POST `{baseUrl}/chat/completions`，body 含 `max_tokens:1`；openai-responses：POST `{baseUrl}/responses`，body 含 `max_output_tokens:16`——**实施修正（D-18，P-test-req 实测）**：OpenAI Responses 拒绝低于 16 的值，pi-ai 自身也在发请求前 clamp 到 16（常量 OPENAI_RESPONSES_MIN_OUTPUT_TOKENS，`pi-ai/dist/api/openai-responses.js:12-19`，issue #6265）；写 1 会对全部 responses provider 稳定误报 400。anthropic-messages / openai-completions 维持 1）。**baseUrl 生效顺序（实施修正 D-19，pi 实装核实——catalog 与 custom 不同类）**：catalog 是**网关优先**（`config.baseUrl ?? model.baseUrl`，`provider-composer.js:98` applyModelsJson 对全部内置模型执行——网关覆盖模型级端点，优先级高于模型级）；custom 才是模型级优先（`definition.baseUrl ?? providerConfig.baseUrl`，`:53`）——本设计原稿的统一四级回落链对 catalog 方向反了（照原稿实现会「测模型级端点、聊天打网关」，违反 G3「成功 = 真的能聊」）。落地为两分类：catalog = 网关（override 非空 baseUrl）→ 模型级 → 全缺报错；custom = 模型级 → provider 级（「模型发现」拉取的模型只有 id/name/contextWindow，必须回落）→ 全缺报「模型未配置 baseUrl，无法测试连接」（引导查 runtime 日志，不冒充网络错误）。凭据经 D3 resolver。结果按协议分组返回 `{ api, modelId, ok, error? }[]`。
...
- **采用**：`resolveCatalogDisplayFields` 改造为两级语义（对齐 pi 真实生效顺序——网关覆盖在先、模型级在后）：
  - **baseUrl**：override 非空 `baseUrl`（用户网关）→ 原值下发（展示「自定义网关：{url}」，与派生值视觉可区分）；无网关 → 对合并模型集（`getMergedCatalogModels`，快照 ⊕ overlay 单点）计算「全模型 baseUrl 非空且同值 → 该值；存在 >1 种非空值 → undefined（展示『按模型分发』）；全部空/缺省 → undefined（展示『内置目录未提供』）」。空/缺省视为无信息、不参与同值性判定（≥1 模型空 + 恰 1 种非空值 → 该值）——与 `deriveUniformModelField` 实现对齐。
  - **api**：无用户语义（防线③撤编辑入口）——对合并模型集计算「全模型同 api → 该值；>1 种 → undefined（展示『按模型分发（各协议 ×模型数）』）」。
  - runtime 聚合层计算（前端零推导，对齐 supportedLevels 的 view-ready 原则）。前端 `ProviderEditBody`：catalog 时「类型」渲染为只读派生文案（不渲染输入框）；「Base URL」改为**「端点（自定义网关）」可选输入框**——placeholder「留空使用内置端点；填写后覆盖该 provider 全部模型的请求地址」，框下方展示当前派生值/网关标注；custom 照旧。快照脚本的 provider 级 artifact 字段**保留生成**（见被否 b），但 runtime 展示聚合不再消费；`BuiltinProviderTemplate.api/baseUrl` 类型注释标记「构建期 artifact，禁止新代码消费——展示用派生值」。QuickSetup 模板卡的 api/baseUrl 展示（`ProviderQuickSetup.vue:110-112`）顺带改为「混合 provider 显示『按模型分发』、空串显示『—』」的简单聚合（模板 models 数据一行 computed，消除 G1 在 settings 第一步入口的泄漏；**实施细化 D-23**：协议空串 = `emptyValue`（「—」），**端点空串 = `endpointNotProvided`（「内置目录未提供」）**——端点为空给更具体的信息量，验收场景按此判据）。UI 侧「网关 vs 派生」归类**实施注记（D-22，受控例外）**：shared `ProviderInfo` 无来源判别字段，归类用启发式（派生值恒等于模型集唯一非空 baseUrl → 判「内置端点」，否则判用户网关）——只归展示类别、不重算下发值；边界（用户网关恰等于派生值时被判「内置端点」）两种状态下 pi 生效端点相同，无行为差异；彻底解 = `ProviderInfo` 加 `baseUrlSource: 'gateway' | 'derived'` 字段（长期方案，impl-plan 残留风险已登记）。
...
| `ProviderEditBody.vue:517` → `ModelListSection.vue:269-272` compat 编辑器的 providerApi 回落 | M4 定义：form.api 对 catalog = 派生值（混合 → undefined），override 模型 compat 字段集判断回落模型自身 api | 可接受：compat 字段集对 undefined api 走通用集（现状对未知 api 已有该路径）。**实施注记**：实施现状 form.api 对混合 catalog 持三值 Select 历史兜底 'anthropic-messages'（零消费方，清理候选见 impl-plan 残留风险 13）；compat 判定经 ProviderEditBody 直读 provider.api 派生值，不经 form.api |
...
- **采用**：① 新增 `scripts/check-provider-credential-reads.mjs`（照 `check_services_infra_import.py` / `check-extension-dependencies.mjs` 模式：纯 Node 零依赖 ripgrep 扫描 + 白名单 + fail 收集 + 退出码 0/1）：禁 `packages/runtime/src/**` 在白名单文件外出现 `getApiKeyForProvider` / `readAuthCredentials` / `getProvider(*).apiKey` 直查模式；白名单 = resolver 模块自身 + 迁移基线文件（初始即清零）。**同族姊妹守卫：`upsertProvider` 直调清单**（**实施回写 D-29：白名单落地为 7 文件经验枚举**——`services/provider-config-helper.ts`（setProvider 载体）/ `services/migration/provider-importer.ts`（importer 主路径，载体托管）/ `services/migration/legacy-provider-migration.ts` 与 `services/migration/provider-extras-migration.ts`（两个启动迁移写入点，本设计原稿漏列）/ `infra/pi/pi-config-store.ts`（IConfigStore 实现类自身）/ `infra/pi/pi-provider-store.ts`（模块函数定义文件本体 + clearProviderApiKey）/ `services/ports/config.ts`（IConfigStore 接口方法签名声明——命中「标识符+(」扫描模式但非调用），每条带一行理由注释入脚本——堵防线载体被旁路的复发通道，审查影响 R2-5 的机器化收口）。接入 `.githooks/install-hooks.sh`（按 `packages/runtime/src/**` 路径触发）+ `scripts/preflight-check.sh`（CI）。② constraints.json 登记新约束（id 按 C-proc 域取号，authority = 本文档），`render-constraints.mjs` 重生成 md——进 CR 动态加载视野。③ `check-doc-symbol-drift.mjs` 的 `DOC_MODULE_MAP` 登记本文档与映射源码模块——**实施回写 D-14：实际登记 6 模块**（provider-config-helper / provider-catalog / provider-credential-resolver / pi-provider-store + `packages/core/src/domain/settings/use-provider-edit.ts`（解 API_KEY_CLEAR_SENTINEL / THINKING_PRESETS）+ 脚本自身（解 DOC_MODULE_MAP）），设计文档与 impl-plan 两份各映射同 6 模块，防文档符号漂移。
...
- **已接受代价**：白名单维护——新增合法读取点（经 review 认定的例外）需改白名单。**实施回写后口径**：守卫 A（凭据直查）白名单 = resolver 模块**仅 1 个文件**（初始清零达成，这是「收口失效 >8 文件」信号的唯一适用对象）；守卫 B（upsertProvider 直调）的白名单不是例外通道而是**合法写点全集清单**（7 文件，见采用段）——新增写点必须经 review 进清单，清单膨胀 = 新写点逃逸 setProvider 载体的信号；恢复路径：改白名单即放行（通道存在）；重审触发：守卫 A 白名单 >1 文件或守卫 B 出现清单外绕行；显式判定：可接受。
...
- **采用**：修正 pi-provider-repair.ts 注释的漂移锚点（实施前位于 `:34/:45`，表述「zod minLength:1, model-config.js:168」→ 0.84.4 实装为 TypeBox、provider 级 baseUrl 在 `:171`、校验器装配在 `:184`；修正后锚点段现位于 `pi-provider-repair.ts:44-49`）；pi-provider-store.ts「为什么修复路径不用合并视图」注释（实施前位于 `:495-498`，现位于 `:508-525`）的论证前提（「overlay 条目归一化 baseUrl 缺省填 ''」）随 D7 改变，同批更新该注释的依据表述；本文档全部 pi 断言标注实装行号（已随文标注）。
...
**setProvider 全部调用方枚举**（防线③的作用面核对）：① 编辑体 save（`use-provider-edit.ts:465`，M1 改 payload）；② OAuth 收尾（`useProviderPageOauth.ts:122-127`，D1⑤ 改造）；③ QuickSetup（`use-quick-setup-form.ts:175-176`，D1⑥ 修复：catalog 模板不写 baseUrl/api + 死键清理）；④ importer（`provider-importer.ts:251/:322` 主路径直调 upsertProvider + fallback——经防线载体共享纯函数拦截，见 D1 防线载体段）；⑤ CLI `xyz-settings set-provider`（`commands.ts:130-148`，payload 经 config.setProvider → 防线生效，审查核实兜底成立）。**非 setProvider 的 models.json 写入方**：pi-provider-store.ts 的 `clearProviderApiKey`（`index.ts:572-574` 装配处引用）直调 upsertProvider 但为纯删键 RMW（无新值写入），不产生违规值（审查核实不构成绕过）；quota 写入不落 models.json。
...
// 实现：services/auth/provider-credential-resolver.ts，组合根装配后经模块级 init setter 注入
// ——实施已定此形态（检查点 5 结论）：initProviderCredentialResolver 落点 pi-provider-store.ts，
// 备选传参形态未采用；禁选构造参数形态（薄委托类实态到不了消费点）
```
...
**错误规格（每个失败配恢复指引，准则 6）**。**实施回写（D-15）：线协议权威 = 结构化错误码而非文案**——行级错误码 `http_error|<status>|<body≤200>` / `network_error|<msg>` / `unsupported` / `no_base_url` / `no_enabled_model`，顶层失败 `success=false` 携带 `provider_not_found` / `no_models` / `no_api_key` / `test_unavailable`；行级失败不改变顶层 success（部分协议成功仍算该 provider 可测）；下表文案为 **i18n 展示层职责**（zh/en 由 locales 维护，文案演进不改协议）：
...
| P-cred | xyz `AuthService.getCredential` 对 $ENV_VAR 引用 / command 配置值凭据的解析行为（是否返回明文） | 读 `services/auth/auth-storage.ts` 与 pi `dist/core/auth-storage.js:213-216 resolveConfigValue` 比对 + 构造 env 引用凭据实测 resolver 输出 | ✅ 已验（M2-r，2026-09-10）：xyz 不展开 env 引用、pi 展开（`resolve-config-value.js:71-73` 逐字核对）→ resolver 按降级路径自实现 pi 同语义 env 展开；command 配置值形态实施期未出现于任何链路（降级路径未触发，登记保留） | 同左 |
| P-test-req | 3 协议最小请求体（max_tokens=1 级）在真实端点可调通且响应可区分 401/网络错/协议错；空 baseUrl 回落链与「不可测」分支行为 | 实施期对真实 provider（opencode-go 或等价测试端点）发 3 协议最小请求 + 构造无 baseUrl 模型组验证分支 | ✅ 已验（M3a）：anthropic-messages / openai-completions 用 max_tokens=1 真实调通、401 与网络错可区分；**openai-responses 最小值实测为 16 非 1**（设计已按 D-18 修正）；无凭据场景以 stub 验响应形状；带真实凭据的全协议端到端归验收场景 3 | 某协议调不通 → 该协议从支持集剔除（本轮无需——三协议全支持） |
| P-sanitize | 剥空串键 + catalog provider 级键处置（api 一律剥 / baseUrl 按 extras 网关标记）后同一 models.json 经 pi `ModelConfig.load` 通过（覆盖 8 字段矩阵 + fireworks 冻结实例 + 有/无标记对照） | 复用 P-poison 探针脚本：剥键 → load → 断言 providers.size 恢复；有标记网关保留 / 无标记剥除的对照断言 | ✅ 已验（M1cd）：毒化 `providers.size` 0 → 清洗后 5 / error=undefined；有标记保留 / 无标记剥除 / 错位标记进待清清单 / 合法条目零触碰四路断言全过 | 未触发 |
| P-oauth-shell | D1⑤ 后 OAuth 收尾不再物化 models.json 条目（`{authMethod:'oauth'}`-only 调用 → 无条目/无空壳） | 实施期模拟 OAuth 收尾调用 setProvider 后读 models.json | ✅ 已验（M1cd）：零条目（不物化成立） | 未触发 |
| P-presets | D9③ 修正后的预设实际输出档位数与命名语义一致（on-off 两档 / high-max 三档）；discover 合并 + 行级联动后 reasoning 落盘显式 boolean | 实施期在隔离数据目录跑 §4 场景 10 全流程 + 对修正后预设逐个调 pi `getSupportedThinkingLevels` 断言档位数 | ✅ 已验（M6，P-presets 实测）：on-off 2 档 / high-max 3 档 / all-levels 5 档，与命名语义一致；场景 10 端到端归阶段 5 验收 | 未触发 |
...
|---|-----------------------------------------------|------|----------|----------|
| 1 | **P0 拆雷**：用户在 dev 环境已导入 opencode-go（auth.json 有真实凭据）+ 已有一个 custom provider（**实施修正 D-12：对照 id 必须用不在 catalog 快照内的 provider**——快照 39 个 provider 实测含 deepseek/fireworks，deepseek 的 models.json 条目实为 catalog override（provider 级 api/baseUrl 被 D2② 剥除、凭据归 auth.json），不能作 custom 对照；用 zhipu / kimi / mimo 类或自建 id 如 test-custom，models.json 有 apiKey），展开 opencode-go 改 headers 保存 | dev 环境操作；保存后 `cat ~/.xyz-agent-dev/pi/agent/models.json`；发起一次该 custom provider 聊天 | models.json 中 opencode-go 条目（若存在）不含 `baseUrl`/`api` 键；该 custom 条目原样；聊天正常返回 | G2（失败模式 A 修复） |
| 2 | **存量毒化恢复（空串形态）**：模拟已踩雷用户——手工往 models.json 写入 `{"opencode-go": {name, api:"anthropic-messages", baseUrl:"", models:[]}}`（custom 对照条目同文件，id 同场景 1 修正），重启 runtime | 写入毒化条目 → 重启 dev → 看 runtime 日志与 provider 列表 | 日志含 `[provider-repair] stripped empty-string schema keys on "opencode-go"`；该 custom provider 在列表中且可聊天；models.json 毒化条目的空串键已被剥除 | G2 恢复通道（D2） |
...
| 4 | **派生展示**：展开 opencode-go（混合）与 anthropic（单协议 catalog）、zhipu 类 custom 对照（**实施修正 D-12：deepseek 在 catalog 快照内非 custom，对照换快照外 provider**）三个编辑体对比 | dev 环境逐个展开 | opencode-go：无类型输入框，显示「按模型分发（3 协议分布）」；端点框留空显示「内置端点（按模型分发）」；anthropic：显示派生单值 `anthropic-messages`；custom 对照：输入框照旧可编辑可保存 | G1（失败模式 B 修复） |
...
| 8 | **防复发拦截**：试验 commit——在 `settings-message-handler.ts` 白名单外新写一行 getApiKeyForProvider(id) 调用（该符号已随 M2fg 收口删除，反证时临时重写一行即可） | 本地 stage 该改动跑 pre-commit（或直接跑 `node scripts/check-provider-credential-reads.mjs`） | 检查红、报错指向文件行号与 resolver 模块；撤销后转绿 | G4（结构约束生效） |
...
| 文件 | 改动 | 单元 |
|------|------|------|
| `packages/runtime/src/services/provider-config-helper.ts` | 防线载体共享纯函数 `applyProviderWritePolicy`（provider 级空串转译收口于此：apiKey 分支见 `applyProviderCredentials` 删键、其余=不写键；setProvider 与 importer 共用）；模型级转译（name/api/baseUrl 空串=不写键 + 模型级空 id 整条丢弃——由 translateModelSchemaFields / mergeProviderModel / applyModelRoutingFields 转译族分担）+ catalog 分体系（type 忽略、baseUrl 网关写入/清除 + extras `gatewayBaseUrl` 标记同步 + 不物化空壳）；`resolveCatalogDisplayFields` 网关优先派生改造；listProviders 内联判定改 resolver 批量 sync 版 | M1/M4/M2 |
...
| `packages/runtime/src/index.ts` | 组合根：resolver 装配 + 模块级 init setter 注入（检查点 5）；clearApiKey 闭包提取为 pi-provider-store 的 `clearProviderApiKey`（`index.ts:572-574` 装配处引用，D-26）；启动 async 阶段编排 D2 待清标记清单的 `extrasStore.modify` 执行 | M2/M1 |
...
| 测试（vitest，各包既有配置） | resolver 优先级、清洗剥键（空串矩阵 + 模型级空 id 整条丢弃 + 网关标记判定/待清清单）、网关优先派生函数、setProvider 防线（含 apiKey 删键/不物化空壳/source 维度分流）、use-provider-edit payload、OAuth 收尾、overlay 归一化、D9 用例（discover 合并 reasoning 显式 / pickStrategy 联动含显式 false 不覆盖 / 两预设经 pi 同源 getSupportedThinkingLevels 断言档位数——挂 use-provider-edit / thinking-levels 既有测试文件）；受 THINKING_PRESETS 修正影响的 renderer fixture 测试同批更新（实施时 grep 定位）——全部 `mkdtemp` 隔离，不碰真实数据目录 | 各单元 |
| `docs/troubleshooting.md` | D9⑤ 存量恢复指引（models.json 补 reasoning:true / 重新设置策略）+ D2 网关剥除恢复指引（识别 stripped unmarked 日志、网关输入框重设） | M6/M4 |

### 待验证检查点（设计阶段无法确定，实施期必验）——全部已验（2026-09-10 实施完成，结论同步 §3.6 探针表）

1. **P-cred**（M2 内）：xyz AuthStorage 对 $ENV_VAR / command 配置值凭据的解析现状——决定 resolver 是否要自实现 env 展开（降级路径见 §3.6）。**✅ 已验**：xyz 不展开、pi 展开 → resolver 自实现 pi 同语义 env 展开；command 形态未出现于任何链路。
2. **P-test-req**（M3 内）：3 协议最小请求体在真实端点的可调通性——决定支持集与「暂不支持」文案的覆盖范围。**✅ 已验**：三协议全支持；openai-responses 最小值 16（D-18 修正已回写 D4）。
3. **P-sanitize**（M1 内）：剥键清洗（空串矩阵 + 冻结 artifact）对毒化文件的充分性——复用 P-poison 脚本验证。**✅ 已验**：四路对照断言全过（§3.6 行）。
4. **P-oauth-shell**（M1 内）：OAuth 收尾不物化条目的端到端验证。**✅ 已验**：零条目。
5. **链 3 注入接线**（M2 内）：resolver 注入按 PiConfigStore 实际装配形态落地——**模块级 init setter（首选）/ 调用链显式传参（备选）**；禁选构造参数形态（薄委托类实态下构造参数到不了模块级消费点，主审第 3 轮 S5），任一落地形态均需满足「组合根装配完成先于任何 findValidDefaultModel 调用」的时序约束。**✅ 已定**：init setter 形态，落点 pi-provider-store.ts（非 pi-config-store 转发——反向 import 循环，见 D3 实施回写）；装配序由 runtime-wiring 静态断言锁定。
6. **派生展示对 overlay-only 模型的覆盖**（M4 内）：`getMergedCatalogModels` 在 overlay expired/never-seen 时退化为纯快照——派生值在 overlay 刷新前后可能变化（如 pi.dev 目录新增模型改变同值性），实施时确认这是预期行为（派生值跟随数据权威变化）而非缺陷，并在类型注释写明。**✅ 已确认**：预期行为成立——派生数据源与校验视图同点（getMergedCatalogModels 单点），退化语义已在 `provider-config-helper.ts` 的 resolveCatalogDisplayFields 注释写明（overlay 缺席时退化为纯快照、派生值跟随数据权威变化）。
...
- 2026-09-10 v3.4（**实施回写批**：dev-flow 全单元落地后，按 impl-plan 偏差登记 D-9/D-12/D-14/D-15/D-18/D-19/D-20/D-22/D-23/D-26/D-29 + 检查点 1-6 结论回写；证据与裁决全文见 `docs/design/catalog-provider-field-authority.impl-plan.md` §5）。关键修订：① D1③「不物化空壳」措辞收窄——只约束新建，既有条目一律 upsert（剥除/清除必须落盘，否则清网关成静默 no-op，实施第 2 轮曾按原措辞实现出该缺陷）；② D4 两处事实纠错——openai-responses 最小 `max_output_tokens:16`（pi-ai clamp 常量实锚，写 1 稳定 400）；baseUrl 生效顺序对 catalog 是**网关优先**（`config.baseUrl ?? model.baseUrl`）非统一四级链，落地为 catalog/custom 两分类（本批最实质纠错）；代表模型过滤按**生效** baseUrl 判定（否则模型发现拉回的模型永无法当选）；③ §3.5 错误规格——结构化错误码为线协议权威（行级 http_error/network_error/unsupported/no_base_url/no_enabled_model + 顶层 provider_not_found/no_models/no_api_key/test_unavailable），文案归 i18n；④ §4 场景 1/2/4——deepseek/fireworks 实在 catalog 快照内，custom 对照改用快照外 id（zhipu/kimi/mimo 类）；⑤ D3/D6——clearApiKey 不走 resolver 并集判定（语义不可行），提取为 pi-provider-store 具名删键函数；守卫 B 白名单落地 7 文件（设计原列举漏两迁移写入点等）；DOC_MODULE_MAP 实际 6 模块；守卫 A 白名单 = 1（清零达成）与守卫 B 写点全集清单口径分离；⑥ D5——QuickSetup 端点空串显示「内置目录未提供」（D-23）；UI 网关/派生归类为受控启发式（D-22，长期解 baseUrlSource 字段登记残留）；⑦ 检查点 1-6 全部 ✅（§5/§3.6）；⑧ D-21 修复项：tester 接口随端口化驻 services/ports（IModelConnectionTester），handler ctx 恒注入，transport→infra 值导入让步闭合。
- 2026-09-10 v3.4.1（**design-code-sync 第 1 轮修复批 `f15838cea`**：三区一致性审查 8 must-fix + 12 suggestion + 2 info 全修，聚焦复审 22/22 真修复 + 0 新增 must-fix；本条目由复审追加的 4 条收尾编辑补登）。要点：① 模型级空/纯空白 id 处置在 D1②/D2①/§3.1/§5 拆出单列为「整条丢弃该模型」（D-1 回写——「不写键」会留无 id 条目，同样被 TypeBox 拒载整个 models.json）；② §2 加实施前基线声明（commit 0d8b6cbbc）+ 清除已删函数悬空引用 + D8 锚点行号校准；③ troubleshooting 新增 §12（catalog 网关被启动清洗剥除的识别与恢复——D2 已接受代价在设计与 impl-plan 的双登记兑现）；④ clearApiKey 全部锚点统一为 clearProviderApiKey（index.ts:572-574 装配处）；⑤ 守卫 A 阈值文案对齐设计（>1 文件）；⑥ D5 form.api 实施注记（三值 Select 历史兜底、零消费方）与三态枚举空值语义补句；⑦ 代码侧 6 处注释口径修正（哨兵删键语义 / QuickSetup 守卫理由 / quota 头注释与失败态措辞 / pi 锚点归属词），全部零行为变更。
