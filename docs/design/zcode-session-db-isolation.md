...
- **显式判定**：存量行**可接受**（不阻塞本设计），因为：① 它们不再增长（新污染已被隔离）；
  ...
  **重审触发条件**：用户报告「GUI 侧边栏出现历史 subagent 会话」→ 立即执行 W5 清理工具。
...
（说明 GUI 有第二条发现通道），回退 = 撤 W1 的 env 注入（一行），handle/白名单改动向后兼容
（隔离路径不再出现，存量分支不受影响），并按方案 B 补 `titleGenerationEnabled:false` 减害。
