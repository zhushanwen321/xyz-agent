...
- 2026-09-02 阶段 5 双级验收（入口门三机械核验过：6 单元 committed / 清零 commit 可见 / 首轮验收）：
  ...
  - **验收期新发现（非本重构范围，登记残留风险；2026-09-03 深入调研完成根因闭环）**：dev worktree 残留 `apps/electron/resources/pi/pi-darwin-arm64`（prepare-pi-resources.sh 产物经 git-cwt 缓存 symlink 注入，Bun 1.3.14 compile 单文件 binary）时，**fork / restore（switch_session）后 pi 子进程必死**。pi 定位逻辑（find-pi-executable.ts）两分支逐字一致、`git log main..HEAD` 该文件零改动——非重构回归。
    - **根因（对照实验组 E~K 完全钉死，100% 复现）**：三元组缺一不可——① pi 以 Bun compile 单文件 binary 运行（node 运行 node_modules/.bin/pi 完全正常）② 磁盘 extension 排在 --extension 列表第 2 位及以后（排第 1 不炸：pi loader 对首个 extension 复用主 realm，后续走隔离加载路径）③ switch_session 触发 extension 重新加载后执行 proper-lockfile 锁。机制：首次 lock 成功时 mtime-precision 在 fs 上 `Object.defineProperty(fs, cacheSymbol, {value})` 烙下 non-configurable/non-writable 缓存；Bun compile 产物第 2+ 个磁盘 extension 重载后的 `require('fs')`（经 graceful-fs clone 透传）是 Proxy，第二次 probe 首行 `fs[cacheSymbol]` 时 handler.get 返回值与 target 上的 non-configurable 属性不一致 → V8 Proxy invariant TypeError → **异常从 Bun 内部抛出，穿透 JS try-catch（实验证实 catch 分支不执行）** → 进程 exit 1。触发者是 base-tool-enhance 的 session_start reaper（@zhushanwen/pi-file-lock → proper-lockfile，挂 fire-and-forget 异步链，排在 xyz-agent 真实 spawn 列表第 7 位）。
    - **影响面**：普通新会话（startup 路径，第一次锁）不炸但已烙缓存——**同一 pi 进程后续任何第二次锁都可能炸**（bash 后台任务 spawn 侧 writeRegistryEntry 走 lockSync 同款 probe）；fork / restore（switch_session 后 reaper 第二次锁）必炸。打包版 app（isPackaged）用同款 Bun binary + 磁盘 builtin extensions → **fork / restore 功能在打包版同样必炸，需独立修复**。node pi（dev 无 resources 残留时的 fallback）完全正常。
    - **修复方向（按优先级）**：短期 = base-tool-enhance reaper / pi-file-lock 对 Bun compile 运行时做防御（探测 Bun 单文件形态则跳过锁或降级无锁扫描——锁是并发优化非正确性前提，孤儿判定自带幂等）；根本 = pi binary 侧修 Bun compile 的模块加载（属 pi-mono 上游，本项目不改 pi 源码，可提报）；或 xyz-agent 打包链路放弃 Bun compile 产物改用 node 分发。dev 环境处置：验收期已移除该残留 symlink 使 dev 回退 node_modules pi（缓存本体 /Users/zhushanwen/Code/xyz-agent-workspace/.pi-binary-cache 完好，恢复命令 `ln -s <缓存>/pi-darwin-arm64 apps/electron/resources/pi/pi-darwin-arm64`，修复前不建议恢复）。
    - 复现器（K 组最小形态）：dummy extension（无 hook）排第 1 + clone extension 排第 2（session_start 里 try-catch 包 `lockfile.lock()`）→ stdin 发 `switch_session` → 第二次锁触发 TypeError 且 catch 不执行、exit 1。附带环境动作：main worktree 补跑 pnpm install（pi-file-lock 新包链接缺失致基线 runtime 首次启动失败，装后正常）；两轮测试 session 已全部删除清理。
- 残留风险登记（阶段 3 产出）：
