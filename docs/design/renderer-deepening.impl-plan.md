...
| 17 | dev-merge dev-0.9.14 语义裁决：D5「删除路径放弃 startFlow 兜底」被 main 侧 D7 空态承接（enterEmptyChatState = push + startFlow，af96fa94c）取代——D5 声明基于 fork 时点 core 现状，main 2026-08-31 以 D7 修复 flow=idle 死态且经其验收，属更新的产品决策；D7-U 系列测试断言已对齐合并终态（idle→landing）。同一裁决下壳不注入 main 的 selectSessionFallback 端口（D3 路线 core 链即完整链，端口保留为 headless 特化扩展点） | 合并语义冲突裁决（采纳更新基线） | 本表留痕；设计文档附录 B v6 后记同步 |
...
- ~~useSidebarNew 临时接缝名重命名未排期~~——**已闭环（dev-merge dev-0.9.14）**：main 侧 2026-08-31（af96fa94c）已独立完成同一 strangler 收尾并把 useSidebarNew 重命名回 useSidebar，合并时采纳其终态命名（resetSidebarNewForTest 同步复位为 resetAppBootstrap），全仓引用已改指
