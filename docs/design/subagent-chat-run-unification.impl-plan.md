...
| U7 | 开发完成（待硬核验流转 commit） | 1 | 工作区 docs/ 15 文件 +51/-26：constraints.json C-proc-13 五段逐段改写（authority 改挂 subagent-chat-run-unification.md#33-关键决策）+ constraints.md 再生 98 条 / chat-domain-v1x 两文档 superseded 横幅 / protocolization 两文档 H1 退役注记 / extensions/subagents/architecture.md 协议面段（方法 10→9、roundLifecycle 删）与机制表 4 行更新 / architecture/subagent-engine-abstraction.md EnginePort interact 契约块退役注记 / subagent-service-decomposition.md R0 行 unregisterChatRoundRoute 跨聚合边消亡清理 / troubleshooting §12 H1 修订 + H1 后续聊排障要点 / 本设计 S6 行绑定工件前提（UF-1）+ §5 U7 行 stale 措辞修正 / 本 impl-plan §5 偏差行 + §6/§7 状态回写；验收 = check-doc-symbol-drift 绿（9 映射文档零悬空）+ render-constraints 幂等重跑零 diff + 残余 grep 清点活文档零悬空 |
