...
| 19 | 条目从分区消失（registry LRU 淘汰/损坏自愈清表）时以最后已知快照兜底渲染元信息；输出跟随持续至终态/drawer 关闭/组件卸载（快照冻结语义——审查期修正本条初版「跟随以分区条目消失为停止信号」的登记失实，实际实现无该停止条件且不违背 D7 三停止条件语义） | 本表 | 设计未定义的罕见场景防御，防 drawer 内容空白 |
...
| 21 | D4② NATIVE_VIEWS 形态修正：ui 包静态 Record 会使 ui 反向依赖 renderer（依赖方向 renderer→ui）成环 → 落地为 l2-tab-item.ts 注入契约 NATIVE_VIEWS_KEY（ui「契约在 ui、真实实现由壳 provide」既有范式，views-source.ts 同款）+ PluginViewContainer inject 命中渲染原生组件；未注入时全部 view 走 ViewHost 原路径（既有 TC1 回归绿） | 本表 | 设计原文形态不可编译（依赖方向），注入契约是仓内既有范式等价物 |
...
| 27 | kill 矩阵补「身份比对失败（pid 复用）」子分支：mismatch → 按③为原条目收尾，③锁内判活重查发现复用者存活时拒绝为 identity-unverifiable（宁不杀勿误杀，可重试） | 设计 §3.3 D6 表格已补 ④b 行（审查期同步）；service 实现 + 2 用例覆盖 | 设计矩阵初版无「比对失败」行（只有「不可判定」④），实现路径与 reaper 防复用语义一致且保守正确 |
| 28 | EventAdapter 旁路钩子先于 translate 触发 + try/catch 防护（畸形 raw 事件不炸事件流） | 本表 | 优于 D2「纯旁路不改翻译」的最小要求，畸形事件用例已覆盖 |
| 29 | output RPC lost 语义覆盖「taskId 条目不存在」（同归 lost:true + 空串） | 本表 | §3.1 仅定义「文件清理/丢失」；条目不存在同属「输出不可用」用户语义，handler 注释已登记 |
| 30 | list RPC 先 listTasks 后 markWatched（mark 以当前 mtime 为基线防重播） | 本表 | 读-mark 间瞬时变更延迟到下次变更广播，属「基线不广播」固有边界，窗口极窄 + C6 拉取兜底 |
