...
| R2 | PS-24 探针落点从 scripts/check-pi-semantics.mjs 改为 runtime REAL_PI vitest 池 + pi-semantics.json 登记 | 登记 schema 强制 guard.test 指向 .test.ts；防漂移效果等价（审查中真实 pi 实跑逐字 diff 绿） | 设计 D5 守卫段/文件改动地图已同步，注明 CI skip 边界 |
...
| u2 | committed | 1 | 增量 27/27 绿（主 agent 重跑一致）；typecheck/守卫/lint 过；runtime 全量 2 存量失败=runtime 测试锁旧解析行为（u3 连带），已由 u3 follow-up `529cb950c` 补修闭环（见变更历史 W2 收尾条目） |
| u3 | committed | 1 | segments 39 绿 + core apply-entry/equivalence/store 144 绿（主 agent 重跑一致）；三包全量见 subagent 证据；runtime message-converter.test.ts 2 用例锁旧行为已由 follow-up `529cb950c` 补修闭环（29/29 绿 + 消费链 87 绿，见变更历史 W2 收尾条目） |
...
- ~~待验证检查点 2/3/6（RPC 缓存、降级文案对齐、CJK 校准）分别落在 u2/u5/阶段 5~~ → **归属修正（审查 B-U2）与结论回写**：检查点 2 结论 = 实现采「无标记零 RPC；含 chip 消息每条两次往返（get_commands + get_session_stats）；无缓存」——仅 chip 消息发生往返、量级可接受，重审条件 = 用户感知发送延迟时评估缓存（结论回写自审查 A-U1）；检查点 3 归属修正为 u1/u2 领地（指引行 SSOT 在 skill-marker.ts，非 u5），主 agent 已核实关闭：pi `<available_skills>` 指引（skills.js formatSkillsForPrompt）为英文条件式「Use the read tool to load a skill's file when the task matches its description」+ 相对路径按 skill 目录解析指引；SKILL_FALLBACK_GUIDANCE「请使用 read 工具加载上述 skill 文件后再继续任务」核心动词语义对齐，指令式（必读）系用户显式插入意图的正确表达（照抄条件式反而弱化意图），降级块 location 为绝对路径使 pi 的相对路径解析指引不适用——**无需改动**（**2026-09-07 更新：该结论被 adversarial-review-fixes §3.4 C5 推翻——指引行改英文「Use the read tool to load the skill files above before continuing the task」；指令式语义、read 工具名/路径/时机三要素、绝对路径边界均维持，仅语言对齐 pi 英文环境；同批 skill-marker 测试新增历史中文块用例——块本身仍可识别，旧中文指引行残留为正文**）；检查点 6 在阶段 5 场景 2⑤ 回填
...
- 2026-09-06（W2 收尾）：u3 follow-up `529cb950c`——u3 领地缺口的 runtime `message-converter.test.ts` 2 用例锁旧行为（首 block 截断 + 尾随文本截断，均为升级前缺陷断言）对齐 D7 parse-back 新行为（多 block 完整还原 + body 保留优先，尾随空白保留为文本段），29/29 绿 + 消费链 87 绿无涟漪；§6 状态表 u2 行「2 存量失败」与 u3 行「待补修」随之闭环。
- 2026-09-06（W3 后）：u6 PS-24 真实 pi 探针实证两处 u2 生产缺陷并打回修复（R1，28/28 + 探针绿）：① get_commands skill 项 name 恒带 `skill:` 前缀（pi agent-session.js:1996 实装），注入器映射与 block 插值改为剥前缀归一（设计未明说该形态，属设计盲区补齐）；② References 行 baseDir 恒用 `dirname(path)` 弃用 `sourceInfo.baseDir`——后者按 source 分链语义可变（.pi/skills 来源下为扫描根），与 pi 实装 `skill.baseDir=dirname(filePath)`（skills.js:236/:260）漂移。**设计 §5 检查点 1 的「sourceInfo 含 baseDir，D5 所需数据齐全」断言对 baseDir 字段不成立**——登记为设计文档待修项（阶段 3 doc_errors 预登记，设计文档 D4/D5 的 sourceInfo 表述与检查点 1 措辞需同步修正）。
...
- 2026-09-06（阶段 3 清零）：定向复审 pass（`a3470a8a6` 影响面逐项核验：注释归因对照 pi 实装逐点相符、设计文档修订与实现一致、R1-R7 无遗漏、零逻辑变更确认）；复审唯一 low 观察项（PS-24 登记行号 :1998→:1996 + baseDir 措辞收敛）已修（`41fc179c2`）。**审查循环 R1 收敛，unreasonable/doc_errors 双清零，转入阶段 5 双级验收。**
- 2026-09-06（阶段 5 双绿）：**Gate A PASS**——6 改动包 10569 用例 0 失败（唯一 skipIf = 探针 REAL_PI 门控登记例外且实跑 passed）、lint 零警告、7 项 typecheck 绿、real-pi 池 39 用例实跑绿、零容忍绕过扫描干净、六单元覆盖矩阵全覆盖（5 豁免为纯类型/接线文件）。**Gate B PASS**——12 项场景全 pass（真实 pi 进程 + 真实模型 mimo-v2.5-pro + dev app 端到端）：多 skill 注入/预算降级/fail-safe/失效可见/hook 破坏/重开恢复（含 sidecar 丢失兜底正文保留 + 存量格式回归）/steer/路径文本负面/手打回归/探针篡改变红/LF 构造帧；临时实验全部还原、git 干净。**检查点 6 关闭**：CJK 校准三类样本偏差 中文 +15.7% / 英文 +24.9% / 代码密集 −7.6%，绝对值均 <30% 按设计判据**无需调整系数常量**（中文高估方向符合 D6 保守声明；代码密集为最弱面但低估 <8%）。**残留观察项（非本 feature 缺陷）**：浮层开启时 Enter 在 CDP 合成事件下双触发（选中+发送），行首命令浮层存量通道同样复现（2026-06/07 存量代码），建议独立排查真实键盘行为。
- 2026-09-07（adversarial-review-fixes u5 文案批）：C5 落地——SKILL_FALLBACK_GUIDANCE 改英文（§7 检查点 3 结论同步更新）；degradeBudget toast 补恢复动作；skill chip 补 tooltip（composable.skillChipTitle）；连带 4 个测试文件 fixture 同步 + 历史中文块解析行为登记用例。
- 2026-09-06（design-code-sync R1-F2）：设计 §3.5-⑤② 的「实施期优化项」（编辑重发从 Segment[] 重建 chip 而非文本回填；实现现状 `UserBubble.vue:221` 仍为 normalizeContent 文本回填）处置为**经评估延期未实施**（编辑重发为低频路径，文本回填后 runtime 再展开仍生效，仅视觉退化；重审条件=用户反馈编辑重发体验）；设计 §3.5-③「该升级路径由 ⑤② 优化项消除」同步收敛为「可由……消除（优化项延期未实施，升级路径仍开放，重审条件不变）」。
