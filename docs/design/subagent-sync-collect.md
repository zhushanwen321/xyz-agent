...
| # | 场景 | 行为 | 恢复指引 |
|---|------|------|---------|
| E1 | 批等待中主进程崩溃，重启 | session_start 恢复钩子扫描**主 session 文件「每 id 末条 subagent-record entry」**（collectLastRecordEntries 同构 + 投影扩展含 collectMode/batchFinalized **及终态五字段**（补发判定与内容依赖）；不走 collectRecords light 路径——它只读子文件 identity 头，主 session 落标 entry 不可见）：只收 collectMode=sync 且无 batchFinalized 标记的成员（排除已通过批 flush 或 E9 转换离场的成员）重建缓冲与 running 集；若全员终态且账本中无该批 notifyId 的已送达记录 → 立即 notifyBatch 补发（内容 = 末条 entry 终态快照）；仍有 running → 等其自然终态走正常流；补发尝试后统一补 batchFinalized 标记（补标崩溃重入幂等收敛） | 无需人工干预；账本幂等键防重发；E9 跨键残余重复窗见 §3.1.3（PS-17 同族）；〔注 2026-09-05 探针裁决 → 同日 v2 修复兑现订正〕v1 探针（A6）实测 kill 形态可构造、补发 240s 零到达，当时归因「worker 与宿主 SIGKILL 共亡、orphan 兜底未证实」；v2（[subagent-sync-collect-v2.md](subagent-sync-collect-v2.md)，W1-W3 commit 8b679519b / e2238ce4c / bbcd95c1c）定谳真根因并修复——**非「孤儿自行跑完」前提缺口**：orphan 覆写 entry 抹 collectMode（E1 候选恒空）+ E1 running 口径与协调器分岔（轮终 running+resumable 成员被误判仍在跑）+ E1 等待分支无再驱动（v2 §2.4 已证 pi appendEntry 对已 attach 文件同步落盘，不存在 stale-read）；v2 探针 V2/V3 实跑：SIGKILL 含成功成员批与 kill -9 全灭批重启后均单条补发可达 + 二次重启零重发（留痕 `scripts/probes/subagent-sync-collect/RESULTS.md` V2/V3 节，2026-09-05） |
...
选择：隐式（批 = 全部未通知的 sync record）。被否：batchId——LLM 需自己管理 id 的生成/复用，孤儿批（id 拼错永不闭合）是必然出现的失败模式；隐式集合下"批"只是投递时机，无身份可错。代价：①同轮混派多批语义不可表达（后派 sync 并入 pending 批）——v1 接受并文档化；②**批饥饿**：主 agent 持续不 STOP、不断派新 sync，pending 永不为空 → 最早结果被无限扣留——逃生 = `list` 观察仍在跑的成员 + 对已有条目 cancel/close 逼闭合（pendingSyncCount 回显仅在 start 响应，list 不投影 collect 信息；投影列为 v2 候选，不作为 v1 逃生依赖）；③**批缓冲无上限（2026-09-07 登记，adversarial-review-fixes SC-9 维持）**：协调器缓冲为内存态、无成员数/字节上限，量级 = 未闭合 sync 成员数 × record 引用（非 result 全文复制——正文在 record/entry，缓冲持引用），受并发池 6 与派发节奏自然限流，极端派发风暴下线性增长；逃生通道与批饥饿同款（list + cancel 逼闭合）；崩溃一致性由 E1 主文件末条 entry 重建兜底（缓冲丢内存无害）——维持不加上限（真实上界受「派完即 STOP」的工具 prompt 契约约束，加上限反而引入新截断语义）。探针 ✅：单测覆盖"分两轮派发 2+1 sync → 仍单条通知含 3 段"。
...
| A6 | 崩溃恢复不丢不重 | ① 2 个 sync（sleep 60s）派发后 kill -9 主 pi；重启同 session；② 正常 `/exit` 于批未闭合时（2 已终态 + 1 在跑）再 resume | ① 恢复后补发单条批通知（若已全终态）、二次重启零重发（账本幂等键生效）——〔注 2026-09-05 探针裁决 → 同日 v2 修复兑现订正〕v1 探针实测 FAIL（补发 240s 零到达，`diag-survive.mjs` 证 worker 与宿主 SIGKILL 共亡），当时归因「产线前提缺口、orphan 兜底未证实」；v2（[subagent-sync-collect-v2.md](subagent-sync-collect-v2.md)，W1-W3 commit 8b679519b / e2238ce4c / bbcd95c1c）定谳真根因——**orphan 覆写 entry 抹 collectMode 致 E1 候选恒空**（非「孤儿自行跑完」前提缺口；v2 §2.4 已证 appendEntry 同步落盘，无 stale-read），覆写保标记 + E1 resumable 口径 + settled 有界重扫三修复落地后，kill -9 重启补发可达：v2 探针 V3 复验 11 PASS（批头 2 finished 0 failed + 二次重启零重发；成员正文空——sleep 中 kill 无 assistant 输出，gc 判 finished + 覆写无 result 可 merge 的第三形态），V2 含成功成员批 15 PASS（补发含 result 全文）——留痕 `scripts/probes/subagent-sync-collect/RESULTS.md` V2/V3 节；② resume 后 E9 转换的两条经 async 重放送达、**零重发**，恢复补发只含在跑成员（batchFinalized 排除生效） | G4 |
