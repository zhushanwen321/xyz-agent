...
2. **zsw 是第二宿主**：zsw 消费的 subagent-core 是 **vendored 副本**（zsw 仓 `lib/vendor/subagent-core/`，由 zsw 仓 `scripts/vendor-subagent-core.js` 刷新 + `VENDOR-MANIFEST.json` 溯源；zsw `lib/core-ref.js` 头注明确「三形态都没有 node_modules 解析面」）。因此隔离**不是自动继承**：zsw 必须 re-vendor 含本改造的 core 版本并发版，其新会话才落隔离库。
...
xyz-agent 合入本改造 → core 版本 bump → zsw re-vendor（**版本门槛 = 含 `db-path.ts` 的 core 版本**，即本合入批次；以 zsw 仓 `VENDOR-MANIFEST.json` 溯源核对）→ zsw 发版 → zsw 新会话才落隔离库。在此之前 zsw 继续写宿主库（`lib/config.js` 硬编码）。
...
- 本仓工具 = `scripts/zcode-session-db-cleanup.mjs`（清理对象 = **xyz-agent 侧**三库：宿主库 + GUI 索引库 + xyz-agent 隔离库）。**截至本文投递时点 W5a 尚未产出**（排期独立，不阻塞止血合入）——本节契约以 impl-plan §2.5 为权威文本；工具落地后，以脚本自身 `--help` 与实现为实现级钉死点。
...
## 4 消费方式钉死（impl-plan §2.6，边界声明）

**zsw 侧唯一受支持形态 = vendored 整包源码深导入 `db-path.ts`**：

```
<zsw 仓>/lib/vendor/subagent-core/src/execution/engine/engines/zcode/db-path.ts
```

（vendored 副本内的源码深路径；zsw 侧路径解析权威 = 其仓 `lib/core-ref.js` 单一解析点 + `VENDOR-MANIFEST.json` 溯源。安装态发布产物只含 `dist/`，源码级 vendored 副本存在于 zsw 源仓、经其构建管线进入发布形态。）

**npm 安装形态不可达**，实态依据（2026-09-09 于本仓 grep 核实）：

1. `packages/subagent-core/package.json` 的 `exports` 无 `db-path` 子入口（现有子入口仅 `.` / `./engines/zcode/reader` / `./engines/zcode/constants` / `./engine/paths` / `./relay-env` 等，无 db-path）；
2. 根 barrel `src/index.ts` 不导出 `zcodeSessionDbPath`（全包 `export.*zcodeSessionDbPath` 检索仅 `db-path.ts` 本体命中）；
3. `zcode-engine.ts:89` 的 `export { hostZcodeDbPath }` 是模块级 re-export（存量兼容用途），既不在 package.json exports 暴露、也不经 barrel 透出——同样不可经 npm 消费。

**禁止**：按 npm subpath 接线（会直接 resolve 失败）；绕过构造函数手拼路径字面量（§1.1 纪律）；只拷公式不拷函数实现（丢失权威源单一性）。

此边界与 zsw 现状天然一致：`lib/core-ref.js` 头注自证 zsw 插件三形态（inline 直载 / marketplace 副本 / npm 包内容）都没有 node_modules 解析面，core 只能以构建期 vendored 副本形态存在——即 zsw 本来就不存在「npm 安装 subagent-core」的接线方式。

**版本门槛**：vendored 副本须刷新到含 `db-path.ts` 的 subagent-core 版本（本合入批次；以 `VENDOR-MANIFEST.json` 溯源核对）。
...
- [ ] **4. 消费方式合规（补充项，对照 §4）**：zsw 侧对 `zcodeSessionDbPath` 的引用经 vendored 整包源码深导入，无 npm subpath 接线残留、无手拼路径字面量。判定方式：zsw 仓内检索 `db.sqlite` 硬编码新路径为零（存量 `lib/config.js` 的宿主库硬编码除外——那是面 B 的存量兼容锚点）。

## 6 引用锚点（本仓）

| 内容 | 位置 |
|------|------|
| 路径构造函数实现（权威源） | `packages/subagent-core/src/execution/engine/engines/zcode/db-path.ts` |
