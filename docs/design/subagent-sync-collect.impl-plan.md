...
- 变更历史：
  ...
  - 2026-09-05 同日 v2 定谳订正：上一条的旧归因已被 v2 修复轮推翻——A6 真根因是 orphan 恢复覆写 entry 抹掉 collectMode 致 E1 候选集恒空（叠加 E1 running 口径分岔与无再驱动），非「worker 共亡前提缺口」（pi 实装已证 appendEntry 对已 attach 文件同步落盘，无 stale-read 窗）；v2（subagent-sync-collect-v2.md，commit 8b679519b/e2238ce4c/bbcd95c1c 系列）修复后探针 V2/V3 复验全 PASS——见本节上方已划线的残留风险两条与 RESULTS.md V1-V3 节。
