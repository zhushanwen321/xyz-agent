...
| G2 | 选中不误发送 | 浮层**打开（`open`）时按 Enter 是「选中候选」，绝不触发消息发送**；Tab/Enter 语义等同；浮层未打开时 Enter 正常发送。消费条件 = 浮层 `open`（**轮 4 取代**轮 3 第 4 轮 RC-A-1 的「实际可见才消费」，决策因果见 §3.3 D2；**轮 5 按键位写准**：`Enter`/`Tab` 消费（**IME 组合中例外**——放行给输入法确认候选词），`↑`/`↓`/`Esc` 以候选非空为前提——空候选时放行，见 §3.3 D2 消费矩阵）——前提是**每个 open 态都有可见渲染**，不存在「open 但不可见」的状态类。逐态可见性（轮 4 更新）：① 非空候选五路（slash/session/subagent/skill/`$` file——file 候选非空指 panel 态已加载 cwd 候选或 landing 拉取成功且有文件）⇒ 列表渲染、消费；② slash/session/subagent/skill 空候选、landing `@` 无 sessionId（`buildSubagentCandidates` 无 sessionId 时返回 `[]`）、landing `$` 无 cwd、landing `$` 候选源非空但 query 无匹配（`fileNoResultsVisible` 要求候选源为空）⇒ **「无匹配项」行**、消费；③ landing `$` 有 cwd 但候选未到（`cwdFileStatus === 'idle'`——open 边沿把状态复位为 idle 后**本次 open 不发请求**：无 cwd 直接返回，或 1s 节流命中跳过；节流命中时上一轮 open 的在途回执到达即写回 success/error）⇒ **「加载中」行**、消费；④ landing `$` 错误态 ⇒ 加载失败行（可点重试）、消费；⑤ landing `$` 空结果态（拉取成功且目录无文件）⇒ 无结果行、消费。**行为变化（轮 4 显式登记，取代轮 3 第 5 轮的「已接受代价」）**：②③ 两类的 Enter 从「放行 ⇒ 发送含触发符字面量的消息（landing 首发还会创建 session）」变为「被消费 ⇒ 不发送」；方向是**修正而非回归**——这批状态此前什么都不渲染（用户既看不出处于什么态、也没有任何反馈，这正是 RC-A-1 当初否决「open 即消费」的理由），现在每个态都有可见行，且 `Escape` 仍是显式关闭入口（关闭后 Enter 正常发送） |
...
v1 把 liveInEditor 检查放进 `insertChipAtSelection` 是**无效落点**：六类 `insertXxxChip` 的调用序都是 `restoreSelection()` → `el.focus()` → … → `insertChipAtSelection()`（设计时点六处：`chip-commands.ts:121/145/172/200/219/245`；轮 3 补：D4-a 后命令通路 `insertSlashChip` 也收编进同款调用序——现为七处，即 `insertSlashChip`、`insertSkillChip`、`insertFileChip`、`insertImageBadge`、`insertMentionChip`、`insertSessionChip`、`insertSubagentChip` 各调一次 `restoreSelection()`；**轮 5 勘误**：原写的「HEAD 为七处 `:91/147/173/202/232/253/281`」实为 `1fbc9c3aa` 时点坐标，HEAD 已漂移，按本文口径改符号引用），活选区在检查点之前就已被 `restoreSelection` 用 savedRange 覆盖，检查恒走「活选区」分支但读到的已是 stale 位置。r1 把优先级判定**上移到 `restoreSelection` 本体**（`contenteditable.ts:278`；design-code-sync 轮 1 同步：终态本体经偏差 #14 提取至 `packages/dom-core/src/composer/input/selection-restore.ts` 的 `restoreSelection`，contenteditable.ts 经 useSelectionRestore 工厂消费——`contenteditable.ts:278` 为设计时点行号）：
...
function restoreSelection(): void {
  ...
  //    savedRange 永不应用，比现状还回归）（轮 5 修订：判据为双端 isSelectionInside）
  const sel = window.getSelection()
  // 活选区两端点仍在编辑器内（键盘路径焦点从未离开 / Chromium blur 后 selection 对象常保留）→ 原样使用，不应用 savedRange
  if (sel && sel.rangeCount > 0 && isSelectionInside(el, sel)) {
    el.focus()   // 键盘路径本有焦点，no-op；Chromium 选区保留型 blur 路径回拉焦点不动选区
    return
  }
  // ② 活选区失效（无选区 / 任一端点被移出编辑器，含跨边界选区）→ 应用 savedRange（blur 时刷新）
  if (!sel) return
  if (!savedRange) {
    el.focus()   // 对齐现状（r3 SG-r3-1）：savedRange 为空仍回焦，防 chip 插入未聚焦编辑器（可达性极低但属修复引入的行为差异）
    // 轮 5 补：回焦后活选区若仍不在编辑器内（引擎未把跨边界选区归一化进编辑器），显式清掉——
  ...
  // （此处只校 anchor：savedRange 由 saveSelection 按单端 anchor 判定捕获，与捕获口径一致）
  if (!(sel.rangeCount > 0 && el.contains(sel.anchorNode))) placeCaretAtEnd(el)
}
```

- **等价性论证（r1 修正 F4 的未验证断言；轮 5 判据改双端）**：不依赖「blur 后活选区必不在编辑器内」——两个分支都收敛到编辑器内的正确选区：键盘路径活选区即光标（savedRange 不再覆盖）；点击路径若 Chrome 保留编辑器内 selection（与 blur 前 caret 同值）则原样使用，若被移出（点击落到浮层文本）则应用 savedRange（blur 时 `saveSelection` 刚刷新为 caret 同值）——两种浏览器行为产出一致结果。**轮 5 补充（判据前提与实现对齐）**：这里「活选区」的定义是**双端点都在编辑器内**（anchor + focus，方向无关），不是单看 `anchorNode`——**跨边界选区（锚点在编辑器内、焦点在编辑器外，如鼠标从输入框拖选到消息区）不得当活选区使用**，否则下游 `insertChipAtSelection` 的 `range.deleteContents()` 会删到编辑器外 DOM；同理 `!savedRange` 分支回焦后若活选区仍在编辑器外，必须显式 `removeAllRanges()` 再返回。两处均为实现（`selection-restore.ts` 的 `isSelectionInside` 与 `!savedRange` 分支）对原草案的收紧，本论证的「编辑器内」一律按双端判据理解。
...
`CommandPopover.vue` `handleKeydown` 的 Enter/Tab 分支改造为三步（**轮 5 落点修正**：终态宿主为 `packages/renderer/src/composables/panel/command-popover-keyboard.ts` 的 `handleKeydown`，见下注与 §5 P2）：
...
if (e.key === 'Enter' || e.key === 'Tab') {
  ...
  if (len > 0) onSelect(list[activeIndex.value])  // 轮 5 补空列表守卫：v8③「空态行 + open 即消费」下 len===0 时 list[activeIndex] 为 undefined，按草案裸调 onSelect 会在 CommandPopover.vue 的 item.selected 读点上抛 TypeError（实现：`if (len > 0) opts.onSelect(...)`）
  return true
}
```
（`composingRef`：CommandPopover 模块内监听 `compositionstart/compositionend` 维护的 boolean，与 contenteditable.ts 的 `composing` 同款范式。design-code-sync 轮 1 同步：CommandPopover 侧终态经偏差 #5 提取为 `packages/renderer/src/composables/panel/composition-flag.ts` 的 useCompositionFlag；AmbiguousFilePopover 保留同款范式的自建 composingRef 未迁移——改其 IME 守卫需单独动该文件副本。**轮 5 同步落点**：本段草案的宿主体不是 `CommandPopover.vue` 而是 `packages/renderer/src/composables/panel/command-popover-keyboard.ts` 的 `handleKeydown`（Enter/Tab 分支 + `activeIndex` 收敛单点），组件只留「消费 composable 返回 + `defineExpose({ handleKeydown })`」——本段为设计时点形态，终态见 §5 P2。）

- **消费前提 = 浮层 `open`（轮 4 取代 RC-A-1 的「实际可见才消费」）**：`handleKeydown` 只在 `!open` 时返回 false（此时全部键放行 ⇒ Enter 正常发送）；open 态**按键位**消费（**轮 5 按实现写准**，实现见 `command-popover-keyboard.ts` 的 `handleKeydown`）：**`Enter`/`Tab` 消费**——有候选时选中并截断传播；空候选时仅消费事件、**不关闭浮层、不发送**（length 判据对本分支不生效的唯一例外）；**IME 组合中例外**（`composingRef` / `e.isComposing` 时 Enter 放行，交输入法确认候选词）；**`↑`/`↓` 以候选非空为前提**——非空时消费**并截断传播**（**本轮修复**：此前只 `preventDefault` 不 `stopPropagation`，事件穿透到 composer 的方向键导航，单视觉行 at-edge 触发历史召回、当前草稿被上一条历史消息替换——这是本轮 must-fix，见下方边界声明③），空候选（`len === 0`）时放行（无项可移）；**`Esc` 以候选非空为前提**——非空时 `preventDefault` + 关闭，空候选时放行、交 reka DismissableLayer 兜底关闭（**勿写成「恒消费」**；空态组由 `composer-keydown.test.ts` 的「浮层可见但候选为空」用例锁定）。**决策因果**：[HISTORICAL] 轮 3 第 4 轮曾把条件收紧为「浮层实际可见（`items.length > 0 || fileFallbackVisible`）」，理由是当时存在一批「open 但什么都不渲染」的状态（slash/session/subagent/skill 空候选、landing `@` 无 sessionId、landing `$` 的 `idle` 与 query 无匹配），吞键会让消息发不出且无任何提示。**轮 4 为每个 open 态补了可见反馈行**（候选列表 / 「无匹配项」/ landing `$` 「加载中」/ 加载失败行），「不可见」状态类消失 ⇒ 该理由不再成立 ⇒ 条件回到「open 即消费」的判定框架（具体按键位边界如上）。逐态可见性同见 §1 G2。**实现落点**：键盘路由与 `activeIndex` 收敛下沉到 `packages/renderer/src/composables/panel/command-popover-keyboard.ts`（组件只留候选组装与渲染），消费条件与模板 `v-if="open"` 同源；回归锁见 `composer-keydown.test.ts` 的「浮层可见但候选为空」空态用例组，以及 `composer-slash-trigger.test.ts` 的 N-2/N-2a/N-2b 用例组（**轮 5 勘误**：原把 N-2 组挂在 `composer-keydown.test.ts` 名下，该文件全文无 N-2）。**行为变化**：②③ 两类的 Enter 从「发送含触发符字面量的消息」变为「被消费不发送」，Escape 仍是显式关闭入口（关闭后 Enter 正常发送）。
...
- **同款模式顺带修复**：`packages/ui/src/features/chat/AmbiguousFilePopover.vue:113-150` 是同款「window capture + preventDefault + 无 stopPropagation」双入口模式（r2 影响面审核实：其选中链路仅 emit('select') → selectFile + drawer.open，无 chip 插入/无 composer 参与/关闭不依赖 target 阶段 handler，加 stopPropagation 安全）——P2 一并加 **stopPropagation + IME 双保险守卫**（同款问题：其 capture 劫持 IME 确认 Enter），并在两处登记时序契约注释（防未来复制复发）。**轮 5 勘误**：该文件落地的是 **Enter/Tab** 的截断，其 ↑↓ 分支**有意保持不截断**（理由见本节边界声明⑤）——勿按本句推断 ↑↓ 也被截断。
- **边界声明**：① split mode 双 CommandPopover 同时 open（理论可达）时，先到的实例消费后由**幂等守卫 `e.defaultPrevented`** 阻断第二实例（`command-popover-keyboard.ts` 的 `handleKeydown` 首部「已消费则不再处理」），即先注册者独裁消费——**不是** `stopPropagation`（它不拦同一节点（window）上已注册的其他 listener，只拦向上层节点的传播）；现状双消费同样按注册序，行为差异可忽略，登记为已知限制（**轮 5 勘误**：原把阻断机制归因于 `stopPropagation`，机制写错；组件内注释与 `e.defaultPrevented` 守卫为准）；② 浮层 **open** 期间 Tab 被完全消费（焦点遍历不可用）——期望行为，属工作流变更显式登记（r1 SG-6；轮 4 起消费条件统一为 open）；③ **方向键穿透**（**轮 5 修复**）：修复前 ↑↓ 只 `preventDefault` 不 `stopPropagation`，window capture 消费后事件继续到达 contenteditable 的冒泡监听、被 composer 的方向键导航二次消费，单视觉行 at-edge 触发 dom-core history 的 `setText`，把当前草稿替换为上一条历史消息——根因是「capture 消费未截断传播」，结论是 ↑↓ 与 Enter/Tab 同款补 `stopPropagation`（候选非空时），空候选仍放行。④ **全局截断的焦点域**（**轮 5 登记**）：截断发生在 window capture，对任何焦点位置生效——浮层 `open` 期间 ↑↓/⏎/Tab 不再向下传播，「浮层仍 open 但焦点已在其它输入区」时该区域的应用层方向键监听收不到事件。可达性低：`Composer.vue` 以 `v-model:open="cmdOpen"` 受控，`CommandPopover.vue` 的 reka Popover 默认 `onFocusOutside/interactOutside` 会把 `open` 置假（鼠标路径基本不可达）；仓内其它全局方向键消费者不在该窗口（`PanelContainer.vue` 的全局 keydown 只处理 Escape）。注：修复前虽未截断，但 `preventDefault` 已终止浏览器默认行为，故「浮层 open 期间方向键在本窗口失效」的既有面本就存在，本轮只是把「应用层监听也收不到」补齐。⑤ **同款站点的 ↑↓ 有意不截断**：`AmbiguousFilePopover.vue` 的 ↑↓ 仍只有 `preventDefault`（Enter/Tab 已按本设计加 `stopPropagation`）——与该浮层的宿主场景有关：它在消息区，方向键在彼处可能承担滚动等正常职责，截断会误伤，故有意保留差异。
...
| ui | `features/composer/__tests__/`（`composer-input-get-text` / `composer-injection-real-dom`——本分支有改动；`file-chip` / `useComposerChipCommands.image` / `composer-input-trigger-forward` / `useComposerDragDrop`——**回归运行，预期零改动**，`git log --oneline 135c1dbab..HEAD` 实测为空） | 真实选区链路去 mock（restoreSelection mock 盲区——bug 存活根因；实施收窄见偏差 #15：仅 composer-injection-real-dom 去 mock，位置覆盖由 dom-core chip-commands.test 真实链路区段承接）；触发转发回归（零改动文件仅回归运行） |
...
| ui | `lib/__tests__/segment-rebuild.test.ts`（轮 3 新增） | 编辑重发：slash 前缀剥离 + 其余非 text 段（skill/file/mention/session/handoff）序列化剥离、subagent/未知类型恒保留；**image 段剥离语义（轮 5 改行为层登记，不写实现细节）**：首位/末位 image 编辑重发幂等（prompt 与草稿逐字相同）、未命中（用户删掉路径）仍丢弃段、中位 image 属位置近似但逐轮收敛不累积（D6）；本文件 40 → 46 例（轮 4）→ **52 例（轮 5：+4 条 image 剥离矩阵用例 + 幂等不变量 CASES 纳入 image 首/末两例）** |
| renderer | `composer-keydown.test.ts`（D2 改动本体，含 capture/bubble 时序锁用例）；`composer-slash-trigger.test.ts`（U11c + N-2/N-2a/N-2b 索引收敛组）；`composer-slash-injection.test.ts`（未修项清零轮 `d4b964220` 新增 U19/U20 SearchModal ⌘K 注入分流组）；`composer-hash-trigger / composer-compact-queue / composer-dispatch-route / composer-bash-mode / composer-send-button-states / composer-fork-mode / composer-landing-skill-reload`（**回归运行，预期零改动**，`git log --oneline 135c1dbab..HEAD` 实测为空） | 浮层 open 时 **Enter/Tab 恒被消费**、不触发 onSend（时序锁）；**↑↓ 候选非空时消费并截断传播**（轮 5 修复的方向键穿透，回归锁）+ 每个 open 态都有可见渲染（列表 / 「无匹配项」/「加载中」/ 加载失败——消费条件与模板 `v-if="open"` 同源，**轮 4 取代** RC-A-1 的「不可见即放行」）+ 空候选时 ↑↓/Esc 放行、Enter/Tab 仍消费（`composer-keydown.test.ts` 的「浮层可见但候选为空」空态用例组）+ 浮层未 open 时 Enter 正常发送（保留契约锁）+ 候选源缩短的同一拍 activeIndex 收敛（模板高亮行 = Enter 选中行、↑↓ 起点不再归 0——`composer-slash-trigger.test.ts` 的 N-2/N-2a/N-2b）；强制最前断言改写为就地断言（**仅 `composer-slash-trigger.test.ts` U11c**——`composer-slash-injection.test.ts` 在 u5 实施期未改动该文件（轮 3 收窄依据成立）；未修项清零轮（`d4b964220`）因第三入口合流新增 U19/U20 用例组，见 v8①，该文件计入变更点）；D4-c 判定源迁移 |
| core | `domain/composer/dispatch/send.test.ts` / `submit.test.ts`；`domain/chat/` 的 `mutations.test.ts` / `useChat.test.ts` / `submit-queued-entry.test.ts`（场景 10/11/12 对应：staging/defer 重放/编辑重发） | 判定源迁移 + staging/defer 载荷。**轮 3 第 5 轮同步实施实况**：`mutations.test.ts` / `submit-queued-entry.test.ts` 两者**未实施**（`git log --oneline 135c1dbab..HEAD` 对二者输出为空），用例归并至 `send.test.ts` / `useChat.test.ts`（本分支实际改动的两个文件），见实施计划 u6 领地登记。**轮 5 勘误**：本条原把 `submit.test.ts` 一并列为「未实施」，实误——该文件在计划基线 `c06edd2bf` 即已存在（`git cat-file -e` 可验），本分支未改动它，计划用例归并至 `send.test.ts`（实施计划 u6 行同此口径） |
...
| P2 | D2 capture 截断 + IME 守卫 + 同款模式修复 | `renderer/composables/panel/command-popover-keyboard.ts`（键盘决策 + activeIndex 收敛单点；Enter/Tab：isComposing 双保险 + preventDefault + stopPropagation；↑↓ 候选非空时同款截断——**design-code-sync 轮 5 修正落点**：原写 `CommandPopover.vue` 的 handleKeydown，HEAD 已将该分支提取至本 composable）；`renderer/composables/panel/composition-flag.ts`（useCompositionFlag，IME 组合态标记）；`renderer/components/panel/CommandPopover.vue`（只剩「消费 composable 返回 + `defineExpose({ handleKeydown })`」，无键盘分支）；`ui/features/chat/AmbiguousFilePopover.vue`（同款模式顺带修，自建 composingRef 未迁移）；两处时序契约注释 | 根因 2；IME 确认劫持与 AmbiguousFilePopover 是同根双入口模式，合并修复防复发（r1 F3/SG-1）；独立可验收（场景 1「无新消息」+ 场景 13） |
...
- v6（design-code-sync 轮 3 收尾文档修复，2026-09-10）：G2 口径与实现对齐（RC-A-1）——原表述「浮层**开着**时按 Enter 是『选中候选』」比实现宽：实现（当时在 `CommandPopover.vue` 的 `handleKeydown`，轮 3 第 4 轮）先判 `overlayVisible`（= `items.length > 0 || fileFallbackVisible`，与模板 `PopoverContent` 的 `v-if` 逐字同条件），只在浮层**实际可见**时消费 Enter/Tab（**轮 5 勘误**：`overlayVisible` 已随未修项清零轮 `d4b964220` 删除，全仓 grep 无此标识符；本轮另有修复删掉已无消费者的 `fileFallbackVisible` 本身——该判定整体被「消费条件 = 浮层 `open`」取代，落点即 `command-popover-keyboard.ts` 的 `handleKeydown`，见 §1 G2 与 §3.3 D2）；不可见态（候选为空的 slash/session/subagent/skill 路）浮层不渲染任何内容，吞掉 Enter 会让消息发不出且无提示。① §1 G2 收紧为「实际可见（有渲染内容）时…」并补不可见态语义（不可见 ⇒ Enter 正常发送属预期；landing `$` 错误/空结果态浮层可见 ⇒ 仍被消费不发送）；② §3.3 D2 补「消费前提 = 浮层实际可见」bullet，同节两处与 §4 回归测试清单 renderer 行的「浮层 open 时」措辞同步改为「实际可见时」（并补不可见态放行的回归锁条目）。实现侧修复与回归锁：当时的 `overlayVisible` 判定（**轮 5 勘误：符号已删除，见上**）+ `composer-keydown.test.ts` 的 RC-A-1 用例。
- v7（design-code-sync 轮 3 第 5 轮收尾文档修复，2026-09-10）：① §4 回归清单 ui 行等价锁范围按用例实情收窄为「**段序仅含 slash/text 段（无 badge 段）**」，并把含 badge 段的非等价归因补为**两条并存**——(a) badge 边界空格不显式渲染（`UserBubble.boundarySpaceBefore` 只对 `prev.type === 'slash'` 渲染空格，其余 badge 走 CSS `mr-1` 间距、不产文本节点，而 `segmentsToText` 经 `needsBoundarySpace` 对 chip→text 补空格）；(b) badge 段展示投影 ≠ 序列化（file 显 `fileBasename(path)` + 行范围、session 显 `label`、skill/subagent 显 name/slug、image 显缩略图）。(b) 的登记面改引 `composer-multi-skill-injection.md` §3.5-⑤ 的实有标题「`normalizeContent` 纯文本投影面（P0-12 登记）」——原引「展示投影 ≠ 序列化」为该文档不存在的措辞（`UserBubble.test.ts` 的 MF-1 组注释与 `[slash, file, text]` 反例用例为证）。（**轮 3 第 6 轮复审勘误**）本条曾于第 5 轮把归因写成单因 (b)，并判定 (a) 的旧表述「失实」而删除——该判定本身失实：(a) 经代码与段序实测成立（`UserBubble.vue` 的 `boundarySpaceBefore` 与该文件注释「其余 badge 类型沿用自身 `mr-1` 间距，不引入额外文本节点」为证；取投影无差异的段序 `[file('a.ts'), text('正文')]`，序列化 `a.ts 正文` 与气泡文本 `a.ts正文` 的唯一差异即该空格），单因结论被证伪。v5 ⑦ 记录的同款措辞随之被本轮取代；② §1 G2 与 §3.3 D2 的不可见态枚举补 landing `@`（无 sessionId）与 landing `$` 两态（`idle` 拉取中/无 cwd、候选源非空但 query 无匹配），并显式登记后果（Enter 放行 ⇒ 首发送出含触发符字面量的消息并创建 session；判定已接受）；（**轮 3 第 6 轮勘误**）① 行「非空候选四路」补 `$` file 路为**五路**（原枚举漏了最常见的可见态：panel 态 cwd 候选已加载 / landing 成功有文件）；⑥ 行「永不进入 success/error」改「**本次 open 不发请求**（无 cwd，或 1s 节流命中）——节流命中时上一轮 open 的在途回执到达即写回 success/error，idle 是瞬态窗口」；③ §4 回归清单 core 行补括注（`submit.test.ts` / `mutations.test.ts` / `submit-queued-entry.test.ts` 三者未实施，用例归并 `send.test.ts` / `useChat.test.ts`）；④ 同清单 segment-rebuild 行 image 项补「无单测覆盖」标注；⑤ §3.3 D1 收口范围 bullet 的 `useCommandPopoverTrigger.ts:192` 改符号引用（`onAddSelect` 的 slash 分支——基线 `:192` 实为 attach/image 分支收尾，slash 分支起于 `:193`）；⑥ 头部口径补第三类例外（显式标注 HEAD 者，当时 1 处；**该例外已于轮 5 清零**——原「HEAD 行号」实为 `1fbc9c3aa` 时点坐标，已改符号引用）；⑦（**轮 3 第 6 轮补登记**）第 5 轮代码侧改动进本文登记面：键盘 Enter/Tab 读点对 `activeIndex` 的越界处理（防候选源缩短后 `list[activeIndex]` 为 `undefined` 抛错）——**轮 5 勘误：`onSelect(list[Math.min(activeIndex.value, list.length - 1)])` 的读点兜底已随 v8④ 删除**，改为 `packages/renderer/src/composables/panel/command-popover-keyboard.ts` 的 sync watch 单点收敛 + Enter/Tab 分支的 `if (len > 0)` 空列表守卫；原落点 `CommandPopover.vue` 亦已提取至该 composable（组件只留 `defineExpose({ handleKeydown })`）；`command-popover-symbols.ts` 的 `displayName` 与 `isSkill` 判据同源（`skillDisplayName` 与 `displayName` 消费同一 `isSkill`）；S-2「已选禁选」的落地——**轮 5 勘误：原写「`insertSlashChip` 的 slash 路消费 `selectedSkillNames`」是悬空引用**（`insertSlashChip` 签名历来是 `(command, icon?)`，全历史无 `selectedSkillNames` 参数），实际落点是 `CommandPopover.vue` 的 `onSelect` 守卫与 `command-popover-symbols.ts` 的候选构建（`buildSlashCandidates` 按 `selectedSkillNames` 标 `selected`）。**该修复的残余：已由 v8④ 关闭**，回归锁见 `composer-slash-trigger.test.ts` 的 N-2/N-2a/N-2b——它们锁的是**收敛后行为**（候选缩短的同一拍 activeIndex 收敛：模板高亮行 = Enter 选中行、首次 ↑↓ 从收敛值出发），**不是**「把残余固化为期望」；⑧（**轮 3 第 6 轮补**）image 未修项的方向枚举补齐——D6 编辑重发行 / §4 场景 12 / 回归清单 segment-rebuild 行原只记「翻倍」，补记第二方向「**删除后路径复活**」（用户删掉编辑稿里的路径后 prompt 仍出现该路径：image 段恒保留、不参与剥离），与 `composer-multi-skill-injection.md` §3.5-⑤② 的两方向登记同口径。
- v8（未修项清零轮，2026-09-10）：把设计与计划此前登记的 5 条未修项全部关闭（触发 = 用户要求「对这 5 条都做设计和修复」）。① **SearchModal ⌘K 第三入口合流**——字段链 `SearchItem.isSkill/location` + `PendingSlash.isSkill/location`，消费侧按项类型分流到 `insertSkillChip`（裸名/location/icon），`SearchModal.confirmSel` 显式搬运（§2.1 第 4 行与导语、§3.3 D3 段重写；原「已知不一致」登记关闭；原「location 数据面不存在」的成本论据经复核不成立）；② **image 段编辑重发改「裸路径 token 剥离」**（前/后边界判定；命中 ⇒ 剥该处文本 + 保留段；未命中 ⇒ 丢弃段），原「翻倍 + 删除后路径复活」两方向关闭（D6 行、§4 场景 12 与回归清单 segment-rebuild 行、`composer-multi-skill-injection.md` §3.5-⑤② 同步）；③ **浮层消费条件从「实际可见才消费」改回「open 即消费」**——先为每个 open 态补可见反馈行（「无匹配项」/「加载中」/ 加载失败），「不可见」状态类消失 ⇒ RC-A-1 的否决理由（吞键无反馈）不再成立（G2 行与 §3.3 D2 bullet 重写；②③ 两类的 Enter 由「放行发送」变为「消费不发送」，行为变化显式登记）；④ **候选源缩短的 activeIndex 越界窗口消除**——键盘路由与索引收敛下沉 `packages/renderer/src/composables/panel/command-popover-keyboard.ts`，候选长度变化的同一拍（sync watch）收敛，模板高亮 / Enter 选中 / ↑↓ 起点三处一致（v7⑦ 的残余关闭）；⑤ **runtime 钳制用例的宿主 `models.json` 依赖消除**——测试自注入 `e2e-reasoning-off-probe` 演员并按显式名断言，宿主无该文件时仍 2/2 通过（见实施计划 §7 的 runtime 登记）。测试锚点：ui `segment-rebuild.test.ts` 40 → 46 例（**该文件的计数在轮 5 进一步增至 52 例**，见 v9）；renderer 全量 4094 → 4108 例（`+3 skipped` 不变）；core 全量 1776 → 1781 例；runtime 该文件 2 例（自足化前后同数）。
- v9（design-code-sync 轮 5，2026-09-10）：触发 = 用户要求「总结本分支最初要解决的问题 + 当前代码是否已解决 + 再跑一次对抗式审查与修复」。三分区独立审查（A 设计↔代码 / B 现实↔ impl-plan / C 注释口径与机械信号）首轮 8 must-fix（两区重复命中，去重后 6）/ 12 suggestion / 6 info，全部当轮修完；代码侧复审 must-fix = 0，其提出的 2 suggestion + 4 info 一并当轮处理。**其中两条是真实功能缺陷（非文档问题）**：
  ① **方向键穿透**（修复：`command-popover-keyboard.ts`）：浮层 `open` 时 ↑↓ 只 `preventDefault` 未 `stopPropagation`，capture 消费后事件继续到达 contenteditable 冒泡监听 → `composer-keydown` 的 `handleBareArrowNav` 二次消费 → 单视觉行返回 at-edge → dom-core `input/history` 的 `setText` **把当前草稿替换为上一条历史消息**（同时高亮也在移动）。修复 = ↑↓ 与 Enter/Tab 同款 `stopPropagation`（候选非空时）；回归锁 `composer-keydown.test.ts`「open 且候选非空：↑/↓ 被完全消费」两条 + 「空候选 ↑ 仍放行」对照一条（回滚 ⇒ 2 红）。边界声明相应补 ③（方向键穿透根因）、④（全局截断的焦点域，可达性低）、⑤（`AmbiguousFilePopover` 的 ↑↓ 有意不截断：宿主在消息区，方向键可能承担滚动职责）。
  ② **image 编辑重发的换行无界累积**（轮 4「裸路径 token 剥离」的遗留；修复：`segment-rebuild.ts`）：只剥裸路径 token 时，image 序列化自带的换行留在 text 段内，而保留的 image 段重建时又补一遍 ⇒ 每轮重发净增一个换行（round0 → round4 共 5 次采样实测：`\n/data/a/1.png\n正文` → `\n/data/a/1.png\n\n\n\n\n正文`，即 roundN 相对 round0 净增 N 个换行）。修复 = 剥掉整个序列化单元 `\n<path>\n`（命中后向左右各扩一个紧邻换行；单调游标与未命中丢弃段语义不变）⇒ 首/末位 image 编辑重发幂等；中位 image 仍属 D6 已登记的位置近似，但逐轮收敛不再累积。回归锁 `segment-rebuild.test.ts` 46 → 52 例（新增剥离矩阵 4 例 + 幂等不变量 CASES 纳入 image 首/末两例；回滚 ⇒ 6 红）。
  文档侧修正要点：§4 renderer 行与实施计划 u5 行删除被证伪的「`composer-slash-injection.test.ts` 本分支零改动（`git log … 为空`）」（实为 `d4b964220` 新增 U19/U20）；实施计划偏差 #11 的「skill 分支行为逐行不变」按 `5b5d49f37` 的「委托 `insertSkillChip`」实现改写为行为已变；§5 P2 落点改为 `command-popover-keyboard.ts` + `composition-flag.ts`；§3.3 D1 草案与等价性论证的活选区判据由单端点改双端点 `isSelectionInside` 并登记「跨边界选区不得当活选区使用，否则下游 `deleteContents()` 会删到编辑器外 DOM」；§1 G2 与 §3.3 D2 的消费矩阵按键位写准（`Enter`/`Tab` 消费、IME 组合中例外；`↑`/`↓`/`Esc` 以候选非空为前提）；`segmentsToPrompt` 的 trim 自相矛盾注释、`fileFallbackVisible` 死导出、`chip-commands.ts` 能力清单、`UserBubble.test.ts` 头注释的单因归因等口径同步；实施计划 §8 新增「review finding id 登记」表（`S-3`/`S-4`/`N-3b`/`N-4`/`RC-A-2`/`RC-A-6`/`RC-A-5`/`RC-B-11`/`I-7` 等此前在本仓无定义处的引用面）。测试锚点：shared 335 例 / dom-core 232 例 / core 1781 + 6 todo / ui 650 → 656 例 / renderer 4108 → 4111 + 3 skipped / runtime 449 文件 5088 例（轮内零改动，复跑 junit `failures=0 errors=0`）；`check-doc-symbol-drift` 绿（该检查不覆盖本主题文档，见实施计划 §7 括注）。commit 锚点：代码侧 `05191ca89`、文档侧本 commit。
