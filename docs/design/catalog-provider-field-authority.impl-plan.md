...
| **M5b** | M5a 的登记部分（因依赖未就绪而拆出）：`docs/constraints.json` 的 C-proc-12（凭据读路径唯一通道）/ C-proc-13（models.json 写入点收口）已落条目 → 待 `scripts/check-provider-credential-reads.mjs` 存在后跑 `render-constraints.mjs` 生成 `docs/constraints.md`；`scripts/check-doc-symbol-drift.mjs` 的 `DOC_MODULE_MAP` 登记本文档与 impl-plan → 待文档反引号清理（外部 API / 占位 token / shell 变量不得带反引号，见脚本书写约定）后登记并验证 | `docs/constraints.json`<br>`docs/constraints.md`<br>`scripts/check-doc-symbol-drift.mjs` | M2g（hook 文件存在）· 主 agent 阶段 4（文档反引号清理） | plain | `node scripts/render-constraints.mjs --check` 退出码 0（`docs/constraints.md` 含 C-proc-12/13 生成段）；`node scripts/check-doc-symbol-drift.mjs` 退出码 0 且映射文档含本文档与 impl-plan |
...
| M5b | pending | 0 | —（M5a 拆出的登记部分；依赖 M2g 产出 hook 文件 + 主 agent 阶段 4 文档反引号清理） |
| M2c | pending | 0 | — |
| M2g | pending | 0 | — |
| M3a | pending | 0 | — |
| M3b | pending | 0 | — |
