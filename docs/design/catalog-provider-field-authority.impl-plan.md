...
| ~~M5b~~ | **已并入 M2fg**（constraints C-proc-12/13 登记 + render）——原定义见 §7 变更历史 | — | — | — | — |
...
| M2fg | committed | 1 | commit `92d761db6`（27 文件 +585/-205）；tsc 0；受影响域 6662 passed 0 failed（主 agent 复跑）；守卫脚本对树 exit 0（271 生产文件双守卫零违规）+ 反证 0/1/0（单元执行、主 agent 复跑守卫本体）；render-constraints --check 0（93 条含 C-proc-12/13 生成段）；eslint 0；共享 pre-commit 已重装同步。偏差 D-29/D-30/D-31 |
