...
| Gate B（真机） | round-1 FAIL 取证中 | 1 | 真机六链路（HEAD 12cdaf4c3，dev app + 真实 LLM，~47min）：**5 FAIL（链 1/2/4/5/6）/ 1 PASS（链 3）**。链 3 = 主 agent 经 subagents 工具派发（subagent-core 重构主链路）record 正常 closed 落盘——H3 核心面无恙。5 FAIL 全部经过 **GUI 命令派发链路**（@chip → runtime client.prompt('/subagents start') → pi 会话进程 extension → relay → 引擎子进程）：record 永不终态化（不落盘）、完成通知零发、任务完成瞬间 relay 被 pi 会话侧关闭（kill-on-disconnect 143）；附加 renderer 发送按钮点击静默失败 ×3（Enter 正常，renderer 面 H3 零触碰）与链 5 boot 孤儿不终态化。主 agent 取证：① relay 断开与 pi 会话 agent_start 同刻（02:13:25.048/.054）——turn 边界/收尾时 pi 会话侧主动关 relay、settle 链被打断 ② 每次 GUI 派发伴 `manifest=[] answered=null` 诊断 ③ **record 落盘布局翻转**：09-11 形态（H1，GUI 链路绿）record 落 `agent/subagents/`（新布局），今日落 `pi/agent/subagents/`（旧布局）——翻转发生在 H2 落地窗口 ④ **H2 Gate B 的 GUI 面是 SKIP-GUI**（用户占端口，CLI 降级证据覆盖，「GUI 视觉终验归阶段 6 后用户侧确认」从未执行）——GUI 链路自 09-11 凌晨 F6 补修验证后未再真机验过，H2/H3 改动在此链路均属盲区。**基线对照实验已派发**（git archive 37a53dbb7 → /tmp 基线树跑同形态 GUI 派发）：基线同样 FAIL → bug 归 H2（H3 G4「与拆分前一致」成立，bug 立项独立修）；基线 PASS → H3 回归实锤必须修完再交付 |
