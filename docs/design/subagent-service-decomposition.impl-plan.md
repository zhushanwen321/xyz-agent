...
| Gate B（真机） | **收口（round-1 重新定性，0 H3 回归）** | 1 | 真机六链路（HEAD 12cdaf4c3，dev app + 真实 LLM，~47min）：链 3 PASS（主 agent 经 subagents 工具派发 = H3 重构主链路，record 正常 closed 落盘）；原 5 FAIL（链 1/2/4/5/6）经**日志 + 代码直接定案**（用户质疑后跳过基线对照——实验 17.6min 零产出被停，教训入账：debug 优先于基线对照）：**链 1/2**「record 卡 running、无完成通知」= 「等待续聊」设计形态 + 通知链实证正常（主会话日志：主 agent 被唤醒并转述子代理结果——ledger 有 "finished a round"、对话流有「8 个文件」汇总；验收 agent 看漏主对话流）；**链 6**「kill 引擎无失败通知」= kill 打在任务完成后（12s 窗口内任务已完），无失败发生过——无效测试；**链 5**「重启不恢复」= record 本就该 running 等待续聊，turns/tokens 归零 = 磁盘重建投影细节；**链 4** 取消/发送按钮 = renderer 存量真问题，与 H3 无关（git 历史证实 GUI 链路代码 H2/H3 窗口零改动）。**定性依据（架构语义定案，验收判据须以此为准）**：① 每轮 = 新进程、完成即杀（kill 143 = SIGTERM = 设计行为，用户 R4 决策；权威注释 run-orchestration.ts:1271）② record 两态状态机（H1 v4 B-1）：status 仅 running/closed，「等待续聊」折叠进 running ③ record json 仅 closed 终态落盘（终态化三途径 = 手动 close / 30 天 idle-gc / 优雅退出批量收口；被 SIGKILL/快速 kill → 不落盘 by design）。**G4 结论成立：直接重构链路（链 3）真机全绿，GUI 链路「异常」全部为验收判据错位（one-shot「完成即终态」判据错套可续聊形态）——H3 交付。** 证据：`~/.xyz-agent-dev/logs/pi-2026-09-12-01a09358*.jsonl`（主会话 tee）+ `pi-relay-2026-09-12-sa-55fd243c*.jsonl`（relay 镜像，尾部 agent_settled 帧完整）。遗留（非 H3）：链 4 renderer 两按钮（取消第一击消失/发送点击静默失败）+ GUI「等待续聊」显示缺口（用户点名：进程已死应显「已完成·可续聊」）+ `manifest=[] answered=null` 派发诊断——均登记 backlog 独立立项 |
...
- 残留风险（计划创建时点登记，实施期追加）：
  ...
  - ⑪ **阶段 6 回写清单（阶段3 遗留，主 agent 裁决归档）——已兑现（2026-09-12 阶段 6，三项全落设计文档，标记 [终态回写 2026-09-12，阶段 6]）**：① 设计 §1.3 G1 注记——D-R4-1（run-orchestration 1506 物理/798 折算超限追认）与 D-R6-1（壳成员 63 AST 口径超限追认）的算术证伪与终值回写，**两把折算尺须对照定义**（eslint 口径 = skipBlankLines+skipComments 全文件折算；类体口径 = 减 imports/模块常量/接口声明），报告数字统一标注口径名（P3-8）；② 设计 §3.3 D3 静态清单划去 executeViaEngine（R4 已按删除判据移除，静态清单未跟——P1-2）；③ 设计 §4 S3 补「检查 4 可机械子集 = deps getter 直调 形态」注记（守卫注释已先行登记——P3-4）。
...
- 变更历史：
  ...
  - 2026-09-12 Gate B 收口：round-1 5 FAIL 经日志+代码直接定案重新定性（0 H3 回归，验收判据错位为主）+ 架构语义三条定案写入 §6 状态表；基线对照实验中止（17.6min 零产出，教训：debug 优先于基线对照）。遗留（非 H3）转 backlog：renderer 取消/发送两按钮、GUI「等待续聊」显示缺口、`manifest=[] answered=null` 派发诊断。
  - 2026-09-12 阶段 6 回写：设计文档三处终态注记落地（§1.3 G1 算术证伪终值 + 两把折算尺对照 / §3.3 D3 划 executeViaEngine / §4 S3 deps getter 直调机械子集注记），§7 ⑪ 核销。
