...
| R5 | D-R5-4 hooks 生效位置与安装器写入位置不一致（仓库既有形态，backlog） | `core.hooksPath` 指向 worktree 局部 `…/.bare/worktrees/<name>/hooks`，install-hooks.sh 按 `git-common-dir` 写 `.bare/hooks`——重跑安装器会写错位置。本单元手工同步生效实例并 diff 验证逐字节一致；统一动作超出 H3 范围，登记 backlog 待独立任务 |
| R6 | **D-R6-1 单类成员 ≤40 不可达（主 agent 已裁决：追认 52）** | 52 = 33 方法（R0 权威口径，含 9 个 baselines 单行转发 getter + collectCoordinator getter + queries/chatActions 聚合面 + 转发方法族）+ 19 字段（11 依赖 + 4 聚合实例 + queries/chatActions + notifyHost + getMainSessionFile）。超出的 ~12 个成员主体 = strangler 转发面——保持消费方零改动承诺（25 测试文件 + subagent-actions-core ×23 + SAR）必需；删转发让消费方直连聚合会把聚合公共面升级为跨包 API，破坏分层，更差。G1 成员数意图（防壳积累业务逻辑）实质达成：壳方法几乎全为单行转发/装配编排，「改一个域不再通读其余」已成立。与 D-R4-1 同性质（设计算术 vs 实测形态）。设计 §1.3 G1 注记待阶段 6 回写 |
...
| 阶段3（一致性对抗审查） | in_flight | - | 三分区并行审查派发（① 终态结构 vs 设计与偏差处置 ② 行为等价性与测试归位 ③ 守卫与门完整性）；七字段 finding schema，聚焦复审至 0 must-fix |
| Gate A（机器门） | pending | - | |
| Gate B（真机） | pending | - | |
