...
**K3② 真实 workflow 并发 prompt 头部区分度**：✅ **实测完成（2026-09-10 Gate B，V1 pi 链探针附带）**：6 路同模板并发（共享模板前缀 ≈257 字 + 主题词「山/海/风/月/雪/雨」追加在末尾，prompt 全长 260 字），**前 200 字符 6/6 完全同质**（distinct=1）——carbon 旗舰形态证实：M4 头部键在该场景必然多命中 → **安全放弃（不误配但无效）**，与决策 4 的失效面预判一致。**处置 = 按决策 4 预登记的升级路径登记为后续任务**（prompt 全文哈希或参数段取样；本实测同时证明全文 260 字六路互异 → 全文哈希可行），不在本 replay 内实现——安全底线（单命中才采纳 + warn 审计）已守住误配风险，升级只提升旗舰场景命中率。
