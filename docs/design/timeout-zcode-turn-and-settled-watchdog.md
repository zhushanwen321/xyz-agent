...
`shutdownRuntimeAndDisposeChannel`（`zcode-engine.ts:749-771`）：killChain 在 `exit` 事件 resolve 后，只等 1s（`HARVEST_GRACE_MS`，`constants.ts:172`）`close` 事件——channel 的崩溃收割（`failAllTurns`，`session-channel.ts:335,345-353`）挂在 onClose 上。若 `close` 迟到/永不到达，1s 后 `channel.dispose()` 退订——**onClose 收割被跳过，在途 turn 从「快速崩溃收割」退化为挂满自己的 turn 预算**。300s 时代退化终点是 300s 墙钟；§6 根修后（idle 30min / 上界 60min 先验值，⛔P-Z0 标定）这条退化路径的代价放大 6-12 倍——必须联动设计（§6 D7）。
...
```
[outcome] engine_run_failed: app-server 终态 status=error（会话 sess_xxx）。服务端返回尾部：
       …（read 兜底/delta 聚合的尾部内容）
       👉 恢复指引：错误内容来自模型/服务端；直接重跑，若持续出现核对 ZCode 桌面端凭据与模型配置（engine_credential_missing 同族排查）。
```

**F-4 瞬时失败 → 自动重试一次**：连接崩溃（onClose 收割，`ChannelClosedError`）或超时类失败 → 用新会话重跑一次（预算继承，不重置）；重试仍失败才终态化。文案附带「已重试 1 次」。**重试在止损链终局后启动**（stop 确认送达即启；stop 失败 → killChain 完成 + 连接惰性重建后再启——D6 时序，无新旧 turn 双跑窗）。
...
- **采用**：① channel 在 turn 上追加记录 `lastTerminalStatus`——`turn.terminal` 即使晚于 final-frame settle 到达（`session-channel.ts:569` 恒 settle success 的宽松终态先到、权威终态后到），也把 status 记下来随 `SessionTurnResult` 返回；② `parsedAppServerAttempt` 读 `r.terminal.status === "error"` → 合成 run-failed，message 附 read 兜底/delta 聚合的尾部内容（诊断信息不丢）+ 恢复指引（§5.2 F-3 文案）。
...
- **被否**：①多次重试/指数退避——app-server 不是单次 LLM 请求，重跑一轮=整任务重算，成本远高于 pi 的 agent-session 重试（那是对单次 turn 的续跑），一次封顶；②漂移类也重试——漂移有专属降级链（R5 降级 spawn 重跑，`constants.ts:139-152`），语义已完备不动；③status='error' 终态重试——v1 不做（错误内容可能非瞬时，重跑浪费；⛔P-Z2 数据落地后再评估），登记待验证。
...
- **采用**：`shutdownRuntimeAndDisposeChannel` 的 grace 到点、close 未到达时，先对 channel 在途 turn 主动执行**既有** `failAllTurns`（dispose 前），再退订——「退订 = 不会再有事件」在结构上蕴含「在途 turn 不应再等」。HARVEST_GRACE_MS=1s 维持（它守护的 exit→close 正常窗口是毫秒级，1s 合法回收层兜底）；联动修复后，close 永不到达的病态形态由 dispose 收割在 1s+杀链内闭合，不再依赖 turn 自身预算。`dispose()` 收割幂等（activeTurns 已空则 no-op），与 onClose 收割不冲突（先到者赢，`turn.fail` 的 settled 守卫已存在）。
...
| P-Z0 | 总上界 60min 先验值标定：真实任务**总时长**分布 P99 显著 < 60min（对照 pi 侧同类任务 spawn watchdog 实际生效值分布） | 扫 T001 34 任务 journal 总时长 + 新采样任务的总时长分布（非 gap 分布） | ⛔ M1 前必跑 | 合法任务 P99 总时长 ×2 仍 < 60min → 维持默认；P99 ≥ 60min/2 → 默认上调至 P99×2（env 语义不变）；分布双峰 → 取长任务峰 P99×2 |
| P-Z1 | zcode 真实任务事件流最大合法静默窗（工具执行期无事件）显著 < 30min | 扫 T001 34 任务 + 新采样的会话事件流，统计 inter-event gap 分布；＋ pi 侧真实会话日志采 chatMode 轮内 gap 分布（类比采样，标定 D9 中段「长工具无 stdout 事件」残余面） | ⛔ M1 前必跑 | 分布 P99 ≥ 阈值一半 → idle 默认上调至 60min（env 语义不变）；双峰分布 → 取长任务峰 P99×2；长工具形态真实存在 → 中段 fire 前 get_state 复核或阈值上调（S4） |
| P-Z2 | status='error' 终态事件序：final-frame 是否先到、read 是否携带错误信息 | 真实会话以失效凭据/错误模型触发一次 error 终态，记录帧序 | ⛔ M2 前必跑 | final-frame 先到且 read 无错误信息 → 只消费 source="turn.terminal" 的 status（覆盖收窄但不假成功） |
| P-Z3 | dispose 收割：close 缺失时在途 turn 在 grace 窗口内被收割 | fake app-server + 注入 close 事件吞没，断言 failAllTurns 经 dispose 路径触发 | ⛔ M2 | 失败 → HARVEST_GRACE 提至 5s 并保留 onClose 主路径（退化不变但窗口放宽） |
| P-Z4 | 重试预算继承：显式预算下重试轮不重置总预算 | 双超时注入（首轮耗尽大部分预算后注入崩溃），断言重试轮上界=剩余 | ⛔ M2 | 失败 → 显式预算存在时禁用重试（仅默认无限时配置启用） |
