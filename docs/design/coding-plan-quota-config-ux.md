...
| `packages/ui/src/overlays/__tests__/search-modal.test.ts` | 存量微修：与本次无关的 TS18048 编译红（运行时 guard 修复） |
| `docs/design/coding-plan-quota-ux.demo.js`、`demo-a.html` | 设计原型资产的存量 lint 红清理（行为等价由 vm 对比证明） |
...
**为什么 M0 必须包含 `useQuotaQuery.ts`、`protocol.ts` 与 `ProviderInfo` 字段**：`useQuotaQuery.ts:25-31` 的 `QUOTA_FAIL_REASON_KEYS` 是 `Record<QuotaFetchFailureReason, string>` 的**穷举映射**，漏 key 不是运行期静默 undefined，而是 `vue-tsc --noEmit` **编译错误**（pre-commit 在 `packages/renderer` 会跑 vue-tsc）；`protocol.ts:588` 引用 `QuotaConfigurePayload`（SSOT）——**注意守门的真实位置**：excess property 检查发生在 `useQuotaConfigure.saveAndTest` 里那个**带类型标注的对象字面量**（`payload: QuotaConfigurePayload`，renderer `vue-tsc` 覆盖），而不是 `command()` 的调用点（传的是类型化变量，TS 对非字面量不做 excess property 检查；`ClientMessageMap[K]` 只保证键名合法）。因此 **`protocol.ts` 必须继续引用 `QuotaConfigurePayload`、禁止内联字面量类型**，否则该门会静默消失。runtime handler 段在 v3 的 7 位置参数下漏改**不报错**（少传可选参数合法）；本次 payload 收敛后它变为**参数数编译错**（§7.1），守门从「清单纪律」升级为「编译器」—— U2 与端到端链清单仍保留，作实施导航。
...
- **行号锚定约定（v9 起）**：本文档正文大量使用 `file:line` 锚点，它们是**写作时（v8 及以前）的快照**。本 PR 自身的实现改动使其中相当一部分行号漂移（例：`quota-service.ts` 的 `getCredential` / `persistQuotaConfig` / `doFetch` 区间因顺序重排与守卫收尾整体位移；`CodingPlanSection.vue` 与 `useQuotaConfigure.ts` 为重写文件，旧行号全部失效）。**符号名（函数/类型/字段/i18n key）才是权威锚**，行号仅供定位参考；v9 及以后新增内容优先用「符号名 + 章节」锚定，必要时再补行号。复核者若按行号找不到内容，请按符号名 grep 后确认是否存在——**不要**据行号失准判定「符号不存在」。
- v9（2026-09-11）：实现期（dev-flow 阶段 3-5 + design-code-sync 第 1 轮）校准。关键变更：
  - **`credentialSource='exclusive'` 的不可回退收窄**（must-fix，行为修复）：`resolveCredential` 原本在 api-key 形态返回 null 后继续遍历 `fetcher.auth`，对 `auth: ['api-key','oauth']` 的 kimi-coding 会读到 auth.json 的 OAuth 并查询成功 —— 「界面显示用专属 Key、runtime 实际用 OAuth」，正是 D3 要消灭的背离。收窄为：`exclusive` 且 fetcher auth 含 `api-key` 时只允许 api-key 形态（缺失即 `no-credential`），并补 runtime 用例（kimi + exclusive + 文件缺失 + OAuth 可用 → no-credential，含 mutation 验证）。
  - **设置页失败文案的动作对齐**（must-fix，文案）：`settings.providerEdit.quotaFetchFailUnauthorized` 双语末句由「点击刷新重试」改为「点击『保存并测试』重试」（设置页区块内只有 `quota-save-test-btn`；「刷新」入口仅存在于对话页浮层——§5.2 路径 3 的订正段已先行更新，本次补齐 locale）。
  - **excess property 守门位置纠正**（§7.5 四处：protocol 行 / 端到端链第 3 段 / 守门分布清单 / 分段验收段 3，以及 §9.1）：真正的门在 `useQuotaConfigure.saveAndTest` 的**带类型标注对象字面量**（renderer `vue-tsc`），不在 `command()` 调用点（传的是类型化变量，TS 不做 excess property 检查；`ClientMessageMap[K]` 只保证键名合法）——故 `protocol.ts` 必须继续引用 `QuotaConfigurePayload`，禁止内联字面量类型，否则该门静默消失。
  - **出口覆盖补 force 维度**（§7.3 改动 4）：`lastFetchTime` 写点仅覆盖**非 force** 出口（基线 `if (!force)` 即存在，[W5]「refresh 不污染 fetch 的 throttle 锚点」）；不写的情形是三种（失配 / `!fetcher` 早退 / force 路径），照字面「全部出口都写」实施会让 refresh 污染 hover 节流。
  - **收尾 helper 结构描述与实现对齐**（§7.3 改动 4）：helper 收口「守卫 + `lastFetchTime` 单点写」；`cache.update` / `lastFailure` 由各出口以 commit 闭包传入（impl-plan §5 R11 已登记的形态）。
  - **徽标/占位与 readiness 同源 + 未知类型收敛**（§7.2 / §6.9 D8 / §7.4）：全部「已配置/必填」语义（徽标 + 专属 Key placeholder）由 `readiness.missing` 单一来源派生；D8 判据收敛为 `typeUndetermined = !fetcherId || readiness.missing.includes('type')`，preset 未命中态与「类型未选」共用同一 UI 形态（只渲染下拉 + 说明），使「未判定被读成已配置」结构上不可达。
  - **写侧推断理由纠正**（§7.3 改动 6 及四处实现注释）：`resolveQuotaCredentialSource` 是显式值优先，原反例（覆盖显式选择）不可达；真实危害是「把未设置的字段物化成推断值」（分别以 runtime 测试场景 C 为可证伪基线）。
  - 杂项：`ensureSecretsDir` 改为 persist 成功后无条件执行（表 A #6）；表 A #11 拆 `lastFetchTime`（写点=收尾 helper）与 `pending`（runFetch）；删除×在途 fetch 的缓存回写窗口登记（§7.3 改动 4/5 残余窗口）；§7.4 登记 oauth 凭证态行移除与**共 19 个**失去消费方的基线 quota key；§7.5 补新增测试基建（契约桩工厂 `quota-state-stub.ts`、i18n key 存在性守卫 `provider-edit-key-existence.test.ts`、`tsconfig.typecheck-test.json` 纳入）。
- v10（2026-09-11）：**残留清理批次**（impl-plan §7 残留 2/3/7/8/9/10/11 关闭）。与设计内容相关的两点：
  - **收窄判据抽成共享函数**（v9 那条 must-fix 的收敛形态）：`exclusive` 是否适用由 `shared` 导出的 `supportsExclusiveCredential(auth)`（判据 = `auth` 含 `api-key`）单点定义，**三处共用** —— runtime `resolveCredential` 的收窄、renderer `useQuotaConfigure.readiness` 的专属 Key 分支（不适用时落回 provider 凭据判定，与 runtime 忽略 `exclusive` 后按 auth 数组序解析的行为一致）、UI `CodingPlanSection.vue` 的分段控件与专属 Key 块门控（`exclusiveApplicable`）。这消除了 v9 遗留的「两端判据各自表述」耦合：若未来新增 `auth: ['oauth']`-only 的 fetcher，UI 不再显示一个 runtime 会忽略的「用专属 Key」选项。**注意**：§7.1 的「UI 与 runtime 必须调用同一个函数」原只针对 `resolveQuotaCredentialSource`（读侧推断），现同样适用于本判据。
  - **§8.2 验收场景已真实执行**（此前标记为待验收）：在隔离 dev 实例（真实 renderer + runtime + 真实凭证）跑完 S1-S16，S1 / S3-S10 / S12-S16 全部通过，S2 与 S11 各有一条凭证依赖断言不可达（明细与证据见 impl-plan §7 残留 3）；本轮**未修改**任何产品语义，故设计正文除上述判据收敛外无需改动。
  - 另：设计 §7.4 提到的 i18n key 面不受本轮 `panel.*` / `settings.providerEdit.*` 死键清理影响（被删的 97 个键在设计正文中零引用，已用全文档 grep 核对；`panel.context.quotaFailNoCredential` 等设计点名的键保留）。
