...
- v9（2026-09-11）：实现期（dev-flow 阶段 3-5 + design-code-sync 第 1 轮）校准。关键变更：
  ...
  - **excess property 守门位置纠正**（§7.5 / §9.1）：真正的门在 `useQuotaConfigure.saveAndTest` 的**带类型标注对象字面量**（renderer `vue-tsc`），不在 `command()` 调用点（传的是类型化变量，TS 不做 excess property 检查；`ClientMessageMap[K]` 只保证键名合法）——故 `protocol.ts` 必须继续引用 `QuotaConfigurePayload`，禁止内联字面量类型，否则该门静默消失。
