...
|------|------|--------------|------|
| I1 SDK 契约 | `InterceptorResult.injectedMessages?: string[]` + JSDoc（三分域语义 + D1 契约边界声明 + block 诚实描述 + observe 误用无运行时信号注记）；`HookResult.injectedMessages?: string[]`（runtime 内部塑形）——**注意 HookResult 双定义镜像**：权威源 = runtime 侧 `plugin-types/hook-types.ts`（hook-pipeline 实际 import 后者，经 plugin-types.ts re-export），plugin-sdk types.ts 是 `sync-types.sh` 生成物（头部声明请勿手动编辑）——**「两份同批改动」的实施形态 = 改 runtime 权威源后重跑 `bash packages/plugin-sdk/scripts/sync-types.sh` 再生，勿手编 SDK 文件**（实施期已验证：改前基线 sync 零漂移，再生 diff 仅含本次变更）（r1 审查 MF-3） | 契约先行——I2/I3 都依赖此形状；纯类型 + 文档，无运行时 | packages/plugin-sdk/src/types.ts（生成物）+ packages/runtime/src/services/plugin-service/plugin-types/hook-types.ts（权威源） |
...
- v4.1（2026-09-05）：**Gate B 端到端验收回写（W1-W5 全部 pass，§4）**。环境 = standalone runtime（:3321，隔离数据目录）+ 真实 pi（mimo-v2.5-pro）+ 8 个 sandbox 测试插件经真实发现机制加载，行为断言非日志断言，证据 /tmp/gate-b-i/。**W1** 注入生效：LLM 对话中原样复述注入 token（prompt/历史均不含该 token，唯一来源 = 注入；pi 0.84.4 convertToLlm core/messages.js:89-97 custom→user role 透传实锤）；**W2** 三域并存互不干扰：双插件注入段序 [BB, AA] 与 hook 执行序严格一致；**W3** 畸形丢弃 + 误用 warn（onBeforeToolCall 注入被忽略且 SHOULD_NOT_APPEAR 全程零出现）；**W4** blocked×注入闭环：turn 照常 + 已累积保留进 LLM 上下文 + pi 侧「blocked 只 log、注入照常评估」实测成立（u-i3 {content} 形态透传验证）；**W5** observe/工具回环零回归。偏差：W1 断言形态由「回复首行含 token」调整为「询问式复述」（mimo 对指令形态注入服从不稳定，链路职责已证明）；block 留痕落点 = pi 侧 extension log（D3 纯映射定案的派生事实）。⛔实施期门全部关闭。
