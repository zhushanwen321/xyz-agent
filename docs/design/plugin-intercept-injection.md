...
|------|------|--------------|------|
| I1 SDK 契约 | `InterceptorResult.injectedMessages?: string[]` + JSDoc（三分域语义 + D1 契约边界声明 + block 诚实描述 + observe 误用无运行时信号注记）；`HookResult.injectedMessages?: string[]`（runtime 内部塑形）——**注意 HookResult 双定义镜像（2026-09-16 已收敛为单一权威源）**：权威源 = `packages/plugin-sdk/src/types.ts`（runtime 经 plugin-types/hook-types.ts 薄壳 re-export 消费，sync-types.sh 生成链已删除）——「两份同批改动」的旧实施形态（改 runtime 后重跑 sync 脚本）作废，现只改 plugin-sdk 一处（r1 审查 MF-3 的镜像关切由 SSOT 收敛结构性消除） | 契约先行——I2/I3 都依赖此形状；纯类型 + 文档，无运行时 | packages/plugin-sdk/src/types.ts（生成物）+ packages/runtime/src/services/plugin-service/plugin-types/hook-types.ts（权威源） |
