...
- 2026-08-30 深夜 **Gate B 第二轮（待续场景全量实跑）全部完成**，含一项真实 bug 修复：
  ...
  - **环境事故记录（非本设计范围，待独立排查）**：AC-3b 首轮 kill -9 后，~/.xyz-agent-dev 的当日会话列表与 pi session 文件回退到 8/23 状态（今日文件消失、并行会话同窗活跃）；重跑未复现（全新 session 文件存活）。嫌疑方向：kill -9 竞态下 runtime DB/目录状态回滚或并行会话干扰，未定论。另观测：runtime supervisor respawn 68-91s（偏慢）；恢复旧 Stock 项目 session 时 pi 因 proper-lockfile/Bun 不兼容崩溃循环（Stock 扩展环境问题）。
