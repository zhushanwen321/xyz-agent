...
（执行期偏差统一登记于 §7 变更历史，本表保留为索引位；一致性审查 R1 后未新增独立条目——全部偏差条目见 §7 各 Wave 记录）
...
| u-wire | committed | 1 | 本波 core commit |
...
- 变更历史：
  ...
  - 2026-09-01 双级验收双绿收口：Gate A PASS（core 2803 tests / pi-sw 917 tests / extensions 全仓 24 包 3816 tests 全绿；覆盖矩阵 20/20；零新增 SKIP/disable；3 单元验收形态为探针脚本+typecheck，barrel 枚举冒烟无常驻 vitest 用例列为遗留风险）；Gate B PASS（S1/S2/S4/S5/S6/S7 独立复跑取证，S3/S8 姊妹文档承载如实；⛔1-⛔5 回填齐备，设计 §5.4 状态列已翻转）；一致性审查 R1 定向复审 clean。遗留风险登记：①barrel 枚举冒烟建议入常规 vitest 套件；②acceptance-report 为 .review/ 本地产物不入 git，S4③ 真机原始产物未留档（证据链上限=报告三要素+runId 时间戳自洽）；③用户真机 ~/.pi/agent 已装 npm 版 subagent-core exports 落后致已装 pi-sw 加载失败——core 0.4.0 发版后自然修复。
  ...
  - 2026-08-31 Wave3 4/5 committed（u-sw-injector 限流失败待重派）：①u-core-snapshot 发现设计未登记的第三处投影分叉（spec.budgetRef：core toSnapshot 剔除 vs pi serializeRun 直接落盘），codec 单源采 core 侧剔除（嵌套 run 落盘少一脏字段，性质同 strip live），已用例钉住。②u-core-exec-export：error-recovery.ts 实测不存在（计划预置偏差路径），错误类型族实测散布 9 文件；**交接 u-wire：worktree-manager.ts 与 worktree-git-ops.ts 存在同名 GitRunError 双类，barrel 全量 re-export 会命名冲突，需别名或选择性导出**。③u-sw-injector：discoverAgents 签名适配为壳内现取 hostRoots 后传第二参；warn/error 日志通道随装配移位 core（injector→agents-assembly，触发条件逐项等值）；parseAgentFrontmatter（壳内 export）生产消费清零但作为公开 export 面保留，留后续清理单元裁决。④u-core-actions：⛔4 快照期望值硬编码自 pi-sw 现实现实测输出（临时探针跑完即删，git 已核无残留）；六 handler 文案核查零 pi 专属词汇，零外置点。⑤u-sw-store：SNAPSHOT_VERSION 单源裁决删本地常量改消费 core（collectRecordRun 版本判定需要）；SO-DATA-2 warn 实现形态从「抛 TypeError catch」改为「codec 返回 undefined 显式 warn 分支」，消息模板与次数语义不变（原断言未改全过）。⑥u-sw-actions：3 个测试断言目标从壳源码文本锚改为 core 权威源（BG_MESSAGE 运行时 import / prompt fixture createRequire 深路径），守护语义保持；⚠ 交接一致性审查：args-meta 的 warnings→fatal 宿主升级（tool-workflow 消费面）未在本 wave 落地——tool-workflow.ts 不属任何单元领地，留审查阶段裁决。**→ 已闭环（2026-09-01，77a600d3d）：tool-workflow 切 core args-meta（本地双实现删除，reservedKeys=TOOL_TOP_LEVEL 显式注入——core 缺省空集，宿主必注入信封保留键为消费方契约），71 files / 917 tests 与基线一致，设计 D9 已回写闭环**⑦u-wire：原子写迁移实测 6 文件 8 写点（appserver-home 私有 writeAtomic 整函数删）；ensureDir 档位按旧行为修正（manifest/sessions-index/pidfile 三处 fail-fast）；manifest-store.recoverTmpFiles 未迁移（孤儿恢复逻辑非写点，迁移会改历史 tmp 兼容行为，登记）；SubagentServiceSessionInit/引擎错误族不进 barrel（D6 清单外，注释登记）。⑧一致性审查 R1（2026-08-31，core/pi-sw 双区独立 reviewer）登记：unreasonable 7 条 minor——已修 2 条代码（normalizeWorkflowRef 路径分支重放顺序对齐 normalizeRef 判序 + 新断言；parseStringListField 空数组保留 [] 与 tools 语义对齐 + 新断言，core 2803 绿），补录 5 条（args-meta.ts 头注释悬空引用→本条即其登记：TOOL_TOP_LEVEL→reservedKeys 参数化与 extension-logger→core logger 两处平台耦合剥离，pi 侧等值前提=消费时传回 TOOL_TOP_LEVEL；firstLineSanitized 口径统一为 \t→两空格属渲染层微观变更，同 formatElapsedSeconds 小时档性质；formatElapsedSeconds 小时档 views 消费点核实=detail-content.ts 两处与 WorkflowsView live elapsed 共 3 处纯展示插值、无字符串比较依赖；acceptance-report 补记 D-4 S5② 执行链降格为契约面可达性验证、D-5 S2 真机 watchdog 证据强度降级）；doc_errors 8 条已由主 agent 修正（设计文档 7 处 + 本计划 2 处）。②u-sw-misc 对 impl-plan 领地外的 robustness-medium-batch2-interface.test.ts 删 1 个死源码锚定 it（源码正则锚定已删的本地 boundedPrettySerialize 实现体；被删断言语义逐条核实由 core 测试覆盖，无覆盖净损失）——接受并登记。
