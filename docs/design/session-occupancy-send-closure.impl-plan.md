...
| u6a-p4-strip | ActivityStrip 展示统一（D7）：新组件 ActivityStrip（数据源 sessionPhase，纵向堆叠优先级 compacting > bash > generating > thinking，system-notice 形态）；TurnMeta dispatching「思考中…」占位迁入；compacting/executing bash 行迁入；fork notice 定位基线实测核对（`useNoticeStack.forkNoticeBaseTop` + `COMPACTING_NOTICE_HEIGHT` 常量同步 + dev 断言） | `packages/renderer/src/components/panel/`（ActivityStrip.vue 新增）；`packages/renderer/src/components/panel/MessageStream.vue`；`packages/renderer/src/components/panel/message-stream/`（TurnMeta 占位迁移）；`packages/renderer/src/composables/panel/useMessageStreamNotices.ts`（2026-09-11 改名 message-stream-layout.ts）；`packages/renderer/src/composables/panel/useNoticeStack.ts`（2026-09-09 v9 已删除）；`packages/renderer/src/i18n/`；对应测试 | u5b | plain | vitest 绿（三视角 DOM 断言）：① 单一活动条按优先级堆叠渲染四类状态；② TurnMeta 旧占位不再渲染、迁移后 fork notice 定位断言通过；③ V1-④ 阶段归属（压缩指示 = ActivityStrip）截图比对留档 |
...
- 转移 #10 respawn 后 occupancy 初值【已修复并重验 2026-09-06】：Gate B 反例根因 = renderer session.exited 兜底 unsub 抢先丢弃转移 #10 idle 帧 + registerSession 不发 idle 帧（「无帧 = idle 缺省」假设失效），restore 后 stale compacting 永无修正且 flush 无触发源。修复：registerSession 显式 publish occupancy idle 宣告帧（session-lifecycle.ts，写 state 快照 + 重订阅回放必达）；回归测试 occupancy-runtime.test.ts Part D 4 用例（快照回放/广播形状/create 一致性/null-safe）+ runtime-wiring TC8 等待条件同步；V6b 端到端重验 pass（kill 命中压缩中 elapsed=6.4s，重新打开后 idle 起步 + 队列自动续投），V5 切走切回三态一致不回归（切回窗口内 queue 按钮 + 压缩行 + pending 气泡 DOM 断言）。
...
- 2026-09-06：V6b④ 修复批次完成。根因：renderer session.exited 兜底 handler 同步 invalidateStreamSubscription 抢先于转移 #10 的 occupancy idle 帧派发（routeInbound 同步序），帧被丢弃后 occupancy 分区残留 stale compacting=true；registerSession 原设计不发 idle 帧、bus 快照已随 clearSession 清空，restore 重订阅回放无帧可修正 stale，flush 永无触发源。修复：session-lifecycle.registerSession 注册汇聚点显式 publish occupancy idle 宣告帧（绕过 updateSessionOccupancy 全等去重——初值即 idle 会短路；state 快照写入保证重订阅回放必达）。回归测试：occupancy-runtime.test.ts Part D 扩为 4 用例（restore 前置态快照回放 / create 一致性 / publish 形状 / null-safe），runtime-wiring TC8 轮询等待条件同步（宣告帧使「任意 publish 出现」不再等价播种链完成）；runtime 全量 4423 绿 + tsc 0 + eslint 0。端到端重验：V6b pass（kill 命中压缩中 compact failed elapsed=6384ms → dead → 重新打开 → 无压缩行 + 发送位 idle + defer 队列消息自动续投并获回复，截图 /tmp/gateB-fix/）；V5 三态一致不回归（切回窗口内 .queue-send-btn 存在 + 压缩行 + pending 气泡 DOM 断言；压缩完成后短暂 stop 为 settling 正确形态）。
