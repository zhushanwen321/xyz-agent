...
- 2026-09-11：Gate B 微修复 committed（dde26d7fe）：① forceQuit 编排清 queueStates 快照（G1 气泡残留；steer 文本草稿回收留用户裁决）② V5② 真断点 = Panel.vue AskUserOverlay/Composer 互斥挂载致观测条整体卸载（非接线断点），TurnProgressBar 提升至互斥对之外 + dead 态排除，CDP 实测等待期分型文案 + 回答后恢复计时（截图 /tmp/gateb-fix/）。受影响场景重验：V1 残留由 store 单测 + FQ 编排断言覆盖，V5② 由 CDP 实测 + U6 单测覆盖。- 2026-09-11：合并 `fix-session-reader-session-not-found` 时执行 C-pi-14 临时豁免销账（本表「基础设施」行登记的合并检查单）：守卫取该分支终版（199 行，无 [temp-branch-exemption] 段）取代 dev 侧含 518 行临时豁免的早期快照，复跑守卫通过（2731 文件扫描、豁免 11 文件）；随合并暴露的 7 处存量旧布局字面量按守卫恢复动作正面修复（pi-subagent-cli / subagent-engine-sdk / zcode-subagent-cli 三个 test/global-setup.ts + rpc-client-last-event-at / usage-stats-service / e1-e3-real-verify / session-restore-shortcircuit 测试）。install-hooks.sh 中本守卫段与 pi 直写守卫段合并入共享 hook 生成体。
