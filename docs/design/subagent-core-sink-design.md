...
| G4 | **pi 零回归**：pi-sw 全部既有行为（含注入清单、工具面、TUI）语义不变 | §4 S4 |
...
**例 2（安全校验分叉）**：用户（或被诱导的模型）传 agent 引用 `/proj/../../etc/passwd.md`。pi 侧：`subagent-tool.ts:115-128` `assertSafeStartPath` 拒绝 `..` 段。zsw 侧：`agent-discovery.js:202-212` 仅校验绝对路径 + `.md` 后缀，`..` 穿越放行。同一引用参数，一侧有防御一侧裸奔。
...
| index 重构（分域子 barrel + re-export） | 结构更清晰 | 高（zsw vendor 的 sha256 manifest、pi 深路径 import 双受影响） | 路径变更波及 pi 8 处消费文件 | ❌ 纯结构收益不值当回归面 |

**D-解析：宽容解析变体 API 形态**（A7）

| 方案 | 长期架构 | 短期成本 | 风险 | 裁决 |
|---|---|---|---|---|
| (a) agent-registry 提取面导出为 `parseAgentProfile`（宽松：缺 description 不拒、name 缺省 stem、返回全字段含 body），AgentMeta 扩可选执行字段（maxTurns/disallowedTools/skills） | 执行字段进 core 契约与 engine 字段同路径，双宿主 + 资产演进同源 | 中（core 扩类型 + 导出） | AgentMeta 面变宽需文档声明可选性 | ✅ |
...
| 方案 | 长期架构 | 短期成本 | 风险 | 裁决 |
|---|---|---|---|---|
| codec 下沉共享：core 出 `WorkflowRunSnapshot` 编解码器（toSnapshot/fromSnapshot + 版本 guard + live-strip 防御），FileRunStore 与 pi JsonlRunStore 共同消费，各自保留 IO 策略（append-only vs rewrite、去抖归属 store 层） | 投影单源、IO 策略宿主各异（合法差异显式化） | 中 | pi rewrite 模式切换行为需对照探针 | ✅ |
...
**D2：barrel 纯新增导出（选定）**
- **采用**：`src/index.ts` 增补导出，既有导出符号与路径零变更；导出按域分组注释（agent-ref 面/worktree 内核/运行时面/快照 codec/原语）。
- **被否**：分域子 barrel 重构——pi 8 处深路径 import 与 zsw vendor sha256 manifest 双受波及，结构收益不值回归面。
- **证据**：pi `interface/tool-render.ts` 等 8 文件消费深路径（RT1-F1 核实）；vendor 脚本按主入口单一解析（`scripts/vendor-subagent-core.js:137-141`）。
...
- **效果**：G1 的 block-scalar/maxTurns 分叉消解；zsw 手写 parser 退役。

**D4：快照 codec 下沉共享，IO 策略留宿主（选定）**
- **采用**：core 新模块 `orchestration/run-snapshot.ts`：`toRunSnapshot(run)` / `fromRunSnapshot(s)` / `RUN_SNAPSHOT_VERSION` + live-strip 防御内聚；FileRunStore 与 pi JsonlRunStore 改消费。rewrite/去抖/append 差异归属各 store。
- **被否**：pi 改用 FileRunStore——session 锚定与 GUI 消费面全动，违背 G4；双 codec+联动测试——锚定不等于单源。
- **证据**：两份投影字段集一致但语义细节分叉（strip live、去抖），`index.ts:209-212` barrel 注释自认双实现。
- **效果**：WorkflowRun 字段演进单点；G2 成立。

**D5：worktree 走 git-ops 纯函数内核 + 基线锚点抽象（选定）**
- **采用**：core 新模块 `execution/worktree-git-ops.ts`：保真读（stdout 不 trim）、GitRunError/SafeId、dirty 谓词、`collectWorktreePatch(anchor)`（统一 add -A + diff 基线机制）、三步容错清理、porcelain 列表；`anchor` 参数为基线锚点抽象——内存 baseCommit 或宿主持久锚点文件（zsw sidecar 语义）由宿主实现注入。目录布局、孤儿判定策略留宿主。
- **被否**：WorktreeManager 整类导出——内嵌注册表/写队列/pi 布局，参数化面不可控；维持双实现——patch 机制分叉持续。
...
- **效果**：git 语义单源；zsw 289 行收缩为锚点+布局适配（zsw 侧文档承载）。

**D6：动作层两步下沉——execution barrel 导出 + 领域内核下沉（选定）**
- **采用**：① barrel 导出 execution 运行时查询面（SubagentService 实例获取、record 状态查询、agent-registry 执行消费面——只含两宿主共同需要的查询/校验，不含 pi 专属）；② `subagent-actions.ts` 中零 pi-API 的领域内核（六 handler 的校验、守卫链、归属判定、终态映射）下沉 core `execution/subagent-actions-core.ts`，pi adapter 收缩为「参数提取 + core 调用 + TUI 渲染」。
...
- **证据**：`subagent-actions.ts:293-701` 六 handler 仅依赖 SubagentService + 纯数据变换（RT3-F3 逐模块核实）；zsw manager 815 行中约 60% 与 pi 动作层语义同构（RT3-F3 对照）。
- **效果**：最大一块双实现收敛；G3 的第三宿主「零复刻动作层」可达。
...
- **采用**：`recoverCrashedRuns(store, runs, reason)` 自由函数（lifecycle 域）；FileRunStore 增 `pruneStateFilesBeyondCap(max, envName)`（C1，语义对齐 pi jsonl-run-store 实现）；`runSummary(run)` 与 `isScriptRunning(runs, name)` 出 barrel（字段以 core WorkflowRun 为准，宿主可扩展投影）。
- **被否**：恢复/prune 留宿主——「两宿主都要且必须一致」的语义（G2 直接反例）；runSummary 双投影字段分叉已实锤（workflow vs name）。
- **证据**：pi `index.ts:579-627` 与 zsw `orchestration-host.js:566-580` 逐行同构；pi `jsonl-run-store.ts:359` prune 实现纯 fs 可平移。
- **效果**：zsw workflow-state 无限累积问题随 prune 消解；恢复语义单源。
...
| 失败 | 触发 | 恢复指引 |
|---|---|---|
| vendored 副本缺新导出符号 | 宿主未刷新即升级代码 | core-ref 符号守卫报错（模式已有）：重跑 vendor --local / --npm 刷新 + sha256 自检 |
| parseAgentProfile 遇不可解析 .md | 资产损坏 | 返回 `{ name: stem, body, warnings[] }` 宽容降级，不抛——与 zsw 现行为一致，pi 投影面不受影响（仍走严格层） |
| collectWorktreePatch 锚点丢失 | 宿主锚点文件损坏/缺失 | fail-fast GitRunError（错误消息含「基线锚点缺失，无法收集增量 patch；可用 listWorktreePorcelain 人工对账」），禁止静默全量 diff |
| codec 版本不匹配 | 快照文件版本 > 代码版本 | fromRunSnapshot 抛 `RUN_SNAPSHOT_VERSION` 错误，指引升级 core（读方向保持向后兼容） |

## 4 验收

以下场景全部使用真实依赖（真实 pi agentDir、真实 vendored 刷新、真实 git 仓），禁止 mock 替代。每个场景标注回溯目标。

**S1 资产行为一致性（G1）**：在 `~/.zcode/agents/` 放置测试资产 `t-sink.md`（block-scalar description + `maxTurns: 2` frontmatter + 无 `..`）。① pi 真机会话注入清单含该 agent 且 description 完整；② `zsw agents`（zsw 侧实施后）清单同一 description；③ 两平台对 `/x/../etc/passwd.md` 引用均拒绝（pi 现有 + zsw 收紧后）。通过标准：三步全过。回溯 G1。

**S2 预算一致性（G1）**：`maxTurns: 2` 的 agent 真实派发（pi 真机一轮 + zsw CLI 真机一轮），实测两侧 watchdog 均 ≥30min（floor）。通过标准：两侧实际超时一致。回溯 G1。
...
**S5 第三宿主模拟（G3）**：以 barrel 导出面清单为唯一依据（不看 core 源码内部），编写最小集成脚本完成：列 agents、run 一个内置 workflow（带参数）、触发一次崩溃恢复（kill -9 后 loadAll→recover）。通过标准：三步仅凭导出面完成。回溯 G3。

**S6 原子写统一（G2）**：kill -9 打断 core 内 3 处状态写（manifest/sessions-index/workflow-state）各 20 次，重启后无半截文件、残留 tmp 被统一扫描清理。回溯 G2。
...
| 单元 | 内容 | justification |
|---|---|---|
| U1 | barrel 批次一：agent-ref 面（normalizeRef/AGENT_REF_EXT/WORKFLOW_REF_EXT/displayAgentName）+ 报错文案工厂（invalidAgentRefMessage(ref, {howToList?})，`..` 段拒绝并入 normalizeRef）+ SLUG_MAX_LENGTH + isProcessAlive | 同域一次导出；`..` 是安全修复须最先可用（A1/A9/A10，RT1-F1/RT2-F2/RT3-F4） |
| U2 | `parseAgentProfile` 宽容解析 + AgentMeta 可选执行字段（maxTurns/disallowedTools/skills）| D3 裁决落地；zsw 第三份 parser 退役的前置（A7） |
| U3 | computeWatchdogMs 导出（语义化名 `maxTurnsToWatchdogMs`，floor 语义文档化） | 漂移已发生（floor 丢失），独立小单元先行（A3） |
| U4 | ConcurrencyPool 导出 + queuePolicy 参数化（缺省 priority 保 pi 行为） | 策略差异显式化，pi 缺省零回归（A4/D7） |
| U5 | worktree-git-ops 模块（D5 裁决全量：保真读/GitRunError/dirty 谓词/collectWorktreePatch(anchor)/cleanup/porcelain 列表） | 机制分叉收敛，锚点抽象使 zsw sidecar 语义可保留（A5） |
| U6 | `shared/atomic-write.ts` 原子写原语 + core 内部 6-7 处迁移 | 横切原语，tmp 命名统一后崩溃残留扫描可共享（B6） |
| U7 | recoverCrashedRuns + FileRunStore.pruneStateFilesBeyondCap + runSummary/isScriptRunning | 三个组装/投影函数同属「宿主各写一遍」域（B1/B5/C1/D8） |
| U8 | run-snapshot codec（toRunSnapshot/fromRunSnapshot/RUN_SNAPSHOT_VERSION + live-strip），FileRunStore 切换消费 | D4 裁决落地；pi JsonlRunStore 切换在 U11（B2） |
| U9 | args-meta 助手（argKeysFromMeta/findFlattenedArgKeys/normalizeArgsByMeta） | D9 裁决落地（B3/B4） |
| U10 | execution 运行时面 barrel 导出 + subagent-actions-core 领域内核下沉（六 handler 校验/守卫链/归属判定/终态映射） | D6 裁决；最大双实现收敛（D1） |
| U11 | pi-sw 消费改造：动作层切 core 内核、JsonlRunStore 切 codec、injector 改 discoverAgents（若 U2/U10 提供装配则装配循环删除）、boundedPrettySerialize 下沉 shared、format.ts/views 格式函数合并（宿主内）、死导出清理、workflow-list-injector 排序改 sortByCodepoint | 全部 pi 侧行为等值切换，U12 探针门验收（D6/D4/A6/B7/壳内清理） |
| U12 | 对照探针 + 回归保障：发现清单/注入 XML 前后快照、S4-S7 场景脚本化 | G4 的可证伪保障（D10） |

### 5.3 文件改动地图（core）

- `src/index.ts`：+6 组导出（U1-U10）
- `src/shared/agent-ref.ts`：normalizeRef 增 `..` 段拒绝（行为变更仅对新消费方可见，pi 现校验在其上、等值）
...
- `src/execution/concurrency-pool.ts`：queuePolicy 参数（U4）
...
- `src/models/workflow-run.ts`（或 lifecycle 邻位）：runSummary/isScriptRunning（U7）
...
| ⛔2 | `..` 拒绝对 pi 等值 | pi 现有 `..` 拒绝样例全数保持拒绝、合法 `~` 路径不误伤 | 实施期 |
| ⛔3 | patch 机制切换对照 | zsw sidecar 场景（新文件+已提交改动+跨重启）在 core 锚点抽象下产物等价（zsw 侧验收，此处出锚点 API） | 实施期（跨文档） |
| ⛔4 | 动作内核迁移守卫链等值 | pi 六 handler 行为快照（含错误文案锚）迁移前后逐项一致 | 实施期 |
| ⛔5 | codec live-strip 等值 | pi 侧含 live 字段 run 的快照往返与实施前逐字节一致 | 实施期 |
