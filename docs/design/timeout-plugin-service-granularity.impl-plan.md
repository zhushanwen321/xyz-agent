...
|------|------|----------------------|------|------|----------|
| U1 | 工具执行根修：`TOOL_EXECUTE_TIMEOUT_MS` → `DEFAULT_TOOL_EXECUTE_TIMEOUT_MS=1_800_000`（30min，设计 v2.2 勘误后） + `resolveToolTimeoutMs` 纯函数（合法正数/`<=0`或`Infinity`不限时/非法回落/clamp）+ 超时错误消息诚实化（§5.2 文案） | `packages/runtime/src/services/plugin-service/bridge-interop.ts`；测试 `packages/runtime/test/plugin-tool-execution.test.ts`（+同目录新增取值链测试文件可） | 无 | plain | V1；增量单测：取值链全分支 + 迟到回包 miss（P-9） |
...
| U1/U6 | formatDurationMs 双份 + declaredActive 内联复制 isDeclaredTimeoutActive 语义（假差异，语义同构）——收敛建议：后续清理 commit 从 bridge-interop 统一导出【已收敛（1dad3ce65）】 | bridge-interop.ts:33-66；commands-executor.ts:17-21/83 | 2026-09-04 A1 审查 |
...
| U3/U8 | expired 广播不经 core MessageBusBridge（bridge 无归一项），消费侧直订 WS onGlobal（沿 extension.ui_timeout 先例）；uiRequestExpired 经 runtime 侧广播回调分流直发 global（与 D3 permissionRequestExpired 直发对称——bus 装配 + sid 存在时原走 bus.publish(sid) session 级帧，onGlobal 不可达致撤窗生产常态失效，第 1 轮 design-code-sync 修复 R1-1 分流收口），无需 core 归一；permissionRequestExpired 本就直发。残留副作用：bridge 对 unknown type 各 emit 一条 bus error → console.warn（无用户可见异常，P-11 成立） | usePermissionRequest.ts；extension-host-dialog.ts；notification-host-controller.ts:97-99；plugin-service.ts（广播回调分流） | 2026-09-04 U8；2026-09-04 第 1 轮修复 R1-1 更新 |
...
| Unit | 状态 | 轮次 | 证据指针 |
|------|------|------|----------|
| U1 | committed | 2 | 15c772474+24184a1b8（guard 简化，第二轮）；20 tests 绿，typecheck 零错误；常量裁决 30min（设计 v2.2 勘误联动）+ opt-out clamp 近似 + guard helper（U2 落地后已简化）均登记 |
...
- §11 检查点闭环情况：P-8/P-9 **blocked**（端到端通路依赖 bridge extension，实测其与 pi 0.84.4 实装不兼容——export 形态旧 + `extension_ui_request` 不在 pi 0.84 UI context，插件工具经 pi 调用链路产品级不可达；**区间外既有缺口**，bridge/index.ts 在 7650690ac..HEAD 零改动；runtime 侧 D1 语义由 U1 单测 20 例 + 实读核实覆盖）；P-10 **已闭环**（入口 status=active 守卫满足 + E2E 三轮 rebuild 后同宿主插件自动重载恢复 ACTIVE + 工具重注册）；P-12 **已闭环**（clamp 值域安全由 U1 的 resolveToolTimeoutMs 取值链单测覆盖，含超大值 clamp 用例——plugin-tool-execution.test.ts）；D2 幂等单测已覆盖含 effective=MAX 边界；env 白名单无需登记（入站 XYZ_ 前缀放行 + 出站不注入）。
- **跟进项（本流水线范围外）**：①**bridge extension 重写适配 pi 0.84**（产品级：factory 形态 + UI context 无自定义 method 透传——插件工具经 pi 调用链路恢复的前提，V1/V2/P-8/P-9 端到端验收依赖此项）；②packages/plugin-sdk/src/mock.ts 存量 tsc 错误 2 处（w1 时期签名漂移，非本流水线引入）；③core MessageBusBridge 对两条 expired 广播补归一——已不再需要（第 1 轮 design-code-sync 修复 R1-1）：uiRequestExpired 经 runtime 侧广播回调分流直发 global（与 D3 permissionRequestExpired 直发对称），无需 core 归一；permissionRequestExpired 本就直发；④formatDurationMs/isDeclaredTimeoutActive 已统一导出（Gate A 修复落地），MAX_TIMER_DELAY_MS 双份（bridge-interop 模块私有/ui-api 导出，同值平台常量）可并入同批收敛；⑤sandbox 插件命令 invokeResult 一次疑似未闭环（Gate B 观察到一次，trusted 正常，独立于超时逻辑，待查）；⑥W3 crashCounts 衰减快照比对在连续崩溃耗尽重试后不收敛（exceeded 成不可逆终态）——crash-rebuild 既有机制，与超时修复无关；⑦迟到回包 debug 日志补点（plugin-rpc-server/PendingTracker——设计 v2.2 勘误②与 §5 U1 登记行「日志补点留后续」的闭环口）；⑧showInput defaultValue 主线程侧被丢弃（plugin-rpc-setup.ts:182-183 `_defaultValue` 不进 params，存量范围外）；⑨D2 撤窗帧为一次性 global 广播（不经 bus ring 回放），renderer 断连恰逢到期时帧丢失、DialogRequestQueue 无客户端自动过期 → 残留已到期弹窗滞留至用户操作（纯 UX 残留，runtime 侧迟到响应 noop 安全，与 D3 one-shot 语义对称）——design-code-sync 第 2 轮 N1 登记。
...
- v1.4（2026-09-04）：design-code-sync 第 1 轮校准修复（13 finding 全清，R1-1 为代码修复、其余文档/注释校准）：U1 常量勘误跟随（30_000_000 → 1_800_000，§2）；U1 证据指针补 24184a1b8（§6）；formatDurationMs 行加已收敛标记（§5）；跟进项③按 R1-1 分流收口改写 + 补⑦迟到回包 debug 补点 + ⑧showInput defaultValue 丢弃登记（§7）；P-12 闭环补记（§7）；U8 职责列补 plugin-sdk types.ts 转移（§2）；v2.2 条目版本号归属标注 + 1dad3ce65 双记去重（变更历史）。R1-1 代码修复（plugin-service.ts 广播回调 expired 分流直发 global + 单测）登记于 §5 U3/U8 行。
- v1.3（2026-09-04）：阶段 5 双级验收完成。**Gate A 绿**：4 包 5702 用例全绿 + typecheck 四包零错误 + 根 lint exit 0（lint 修复 1dad3ce65，明细见 v1.2 条目）+ 零 skip/零 SKIP_ + 覆盖矩阵 7 单元全认领。**Gate B 绿**（含 1 轮失败回流）：V3/V4/V4b/V5/V6①③ pass（真实等待 5min/9min37s/45s，无加速）；V6② 首轮 fail（load 超时连坐后同宿主插件状态被激活失败终态覆盖为 UNLOADED，rebuild 不重载——U7 引入回归）→ 修复 08dc7b4e1（settleActivationFailure CRASHED 让位，TDD 三连坐路径）→ 真实环境复验 pass（skip reload 0 次 + 重载自动发起 + crasher 形态 3 轮 rebuild 恢复弧线）；V1/V2/P-8/P-9 blocked 登记（bridge 与 pi 0.84 既有断裂，见残留风险①）。
...
- v2.2（2026-09-04；设计文档版本号，非本计划版本序列）：实施期勘误登记（与设计文档 v2.2 同步；dev-flow U1 发现）：§6.1 常量字面 30_000_000（500min）系设计初稿笔误，裁决收敛 1_800_000（30min）——U1 代码与测试已同步；登记实施偏差：opt-out clamp 近似、迟到回包静默 noop（debug 补点待后续）、U3 生产装配接线授权扩展（plugin-rpc-setup.ts / plugin-service.ts）。实质明细已并入本表 §5 登记表对应行。
...
- v1.4（2026-09-05）：**§11 检查点 V1/V2/P-8/P-9 回补（bridge 重写流水线 Gate B）**。bridge 重写（docs/design/bridge-rewrite-pi-0.84.md + impl-plan，7 commits e084a9ab7..84c1372bd）落地后真实环境回补：**V2 unblock pass**（timeout-tool-a 实测 10s isError，E3 文案逐字命中「timed out after 10s (declared; plugin handler may still be running, its result will be discarded)...」+ no-limit-tool-b 45s opt-out 正常 + 60s 迟到回包两侧无异常）；**V1 部分 pass**（V1b：sync 完成后 prompt，sleep-tool 90s 真实结果非 isError、runtime 无 timeout——D1 30min 默认端到端不误杀成立；主形态 fail-by-R2：~~pi 0.84.4 首 prompt 后 registerTool 的工具对该 session LLM 永固不可见~~ → **v1.5 撤销：R2 真相反转（上下文记忆污染非 pi 缺陷）+ 准入闸修复后 V1 主形态阻断解除**）；**P-8 部分实证**（miss 重同步真实触发——卸载后调用收 Tool not found + 第 3 个 sync 帧；~~重同步注册的工具本 session 不可见，恢复时点 = 下个 session~~ → **v1.5 复测 pass：恢复时点 = 下一 turn**）；**P-9 unblock pass**（abort 后迟到回包：pi 无 unhandled rejection + 同 session 后续 turn 正常 + runtime 正常消化 60s 迟到 tool_result）。证据：/tmp/bridge-gate-b2/（newsess/v2b/v3v4/v5/v8-result.json + runtime 日志 + pi tee）。
- v1.5（2026-09-05）：**R2 专项收口回写（v1.4 两处过时登记清账，C-proc-10）**。R2 动态实证（node loader hook 注入 + payload 级逐请求 tools 探针，bridge 设计 v4.4）：pi 0.84.4 无「永固不可见」缺陷——registerTool 后下一个 LLM 请求即携带新工具；原观测 = 上下文记忆污染（窗口内首 prompt 留下「无此工具」自述，弱模型锚定旧结论）。修复 = bridge 侧首 prompt 准入闸（8967db89a）：极速首 prompt 实测首个请求即带插件工具直接调用成功（延迟 ~3.1s）；P-8 复测 pass（miss 重同步新工具同 session 下一 turn 命中）。**V1/V2/P-8/P-9 四项至此全部闭环**（跟进项① bridge 重写依赖解除）。残留：V1 主形态 standalone runtime 全链复测并入 bridge impl-plan §7 残留⑦（prerelease 冒烟批）。
