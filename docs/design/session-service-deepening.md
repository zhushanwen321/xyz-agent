...
| 图片/附件/segment 存储 | `writeImage`:2292 / `migrateImage`:2346 / `writeSegmentsMetadata`:2381（三方法合计约 150 行；初版「约 310 行/12%」系 2292→EOF 区域口径，含 history 域读侧 `readSegmentsMetadataFile` 与尾部辅助代码，S1 实施实测修正） | — |
...
- **效果**：G1/G2 立得一个完整样本；SessionService 立减约 150 行（2603 → 2467；初版 ~310 行为区域口径高估，S1 实施实测修正）。
