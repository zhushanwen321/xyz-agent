...
| P4 | 缓存降级 + rebuildIndexes（触发/降级/量级三要素落地）+ manifest 词汇双写过渡（旧字段永久保留投影地位、新字段权威）+ 恢复路径归并（6→3 含 tmp 恢复退役 + extension boot 钩子触点）+ **`.alive` 探活读面退役（D3 ③四处穷举：record-store 判定移除 + worktree-manager 方向二活信号换 service 活跃 record 注册表（worktree 绑定路径匹配，核验点⑧）+ session-file-gc 判据换活跃注册表 + externalInstance 死字段链清理；护栏处置先于/同批于 boot 清理）+ boot 批处理清理全量残留 marker（D3 ④）** | 降级与重建最后做（依赖前面权威就位） | S5 + 恢复用例 + session-reader 视角 + **反向场景「注册表丢条目 + 属主存活 → 不删」（D3 ③(c)）**。**revert = 本 PR + P3（词汇双写先行使缓存格式前向兼容）；缓存本身可丢可重建 = 天然回滚通道** |
