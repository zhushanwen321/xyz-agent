...
| W11 | chat 域 inproc 临时保留面（engines/pi 11 生产文件 + ui-request-queue core 副本 + 3 测试文件），run/read/probe 等其余能力全部 cli 化 | 主 agent 裁决 2026-09-09：v1 协议 8 反向通道未含 HostBridge 载荷面（W7 登记缺口），chat 续聊协议化需协议 v1.x 扩展；设计「不允许内置例外」硬约束的**临时显式豁免**（非永久），深路径 import 收敛至 pi-host-binding/spawned-children 两 host 桥；协议 v1.x 排期待用户裁决 | 2026-09-09 |
