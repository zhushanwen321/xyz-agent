...
| u1-types | pending | — | M1；独立 commit（类型层先行）——按设计 v5：D4 类型归一不变（SDK SessionStartEvent 自带 previousSessionFile 字段，无需恢复 Like 接口） |
| u2-mechanism | pending | — | M2；单 commit；含 V1–V6 + P1–P3 终态段（按设计 v5.1 三档口径） |
