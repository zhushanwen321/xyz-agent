...
| D-11 | A13 验收 id 随目标 5 定案关闭，.cw-specs/trace-ext.json 同步（条目删除；A12 标题改三档口径、A11/A12 command 死路径修正为本轮 design-code-sync 修复补登） | 设计 §2 目标 5 | A13 使命（P2 探针实测 reason 映射）已于 M0 完成并定案（fork 档维持 resume，执行记录见设计 §6.5），代码与 README 的 A13 标记已全删，spec 条目是 A13 唯一的机检残留引用（docs/page-design/session-trace/ 规划文档中的历史记录按标注制保留，该文档头已补历史时点标注）；保留 manual gate 条目会被后续执行误判为待跑门 | 合理偏差，接受（按定案关闭，登记留痕） |
...
- v5（2026-09-12）：阶段 6 design-code-sync r1 修复（f280505bd，9 条 finding + 聚焦复审 must-fix==0）：§4 基线表 v4 残留行对齐终态、档位编号残留改直读档、§7 补代价 4 与检查点 4、状态表 hash/计数补全（c8014b8a3/57decee60/38e8b8095 六文件口径/b8a92f2ae/77cb01c93）、§5 表 schema 统一 5 列 + 新增 D-11（.cw-specs/trace-ext.json 同步：A13 按定案关闭、A12 三档化、死路径修正）；registry 反引号卫生、types.ts/README resume 措辞与实现对齐、行号锚点实读更新。复审引入的 2 条 suggestion（chat-domain 行锚点自致漂移、session-trace plan.md 涟漪残留）由编排方当轮收口。
