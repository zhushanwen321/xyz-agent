...
> **修订状态**：v3（审查修复第 2 轮，2026-09-11）。收敛轨迹：主审 6→3 must-fix、影响面审 6→3；v3 修复双报告合计 6 must-fix + 3 suggestion——核心修正 = 撤回 D8「纳入监督域」（被 2h 自毁链/无接线路径/语义静默收窄三面击穿，见附录 B#6）、单写者守卫升级为引擎退出链收割（附录 B#7）。被否谱系见附录 B。行号基线 = commit `64e379a7c` 附近（subagent-service.ts 行号随 L 批次有 ±23 漂移，实施期以 grep 锚定符号名为准）。
>
...
class ConversationContinuation {            // 每 chatMode record 一个实例
  ...
  onRunSettled(outcome):                        // run 应答回调（resolve = agent_settled，D7）
    ...
    失败/中断 → doFinalizeRoundToIdle(outcome=failed)（轮终簿记同上；result = 前值 ??
      ...
      不经 route(record)，正文 = 失败摘要 + 恢复指引，可达性迁移自 [T2-③/LC-1]）
...
| U7 文档与约束回写 | C-proc-13 **五段逐段**（①协议面删两通道 ②轮次活性权威：chatMode 纳入监督域 + 注销发射点②归属 Continuation ③⑤连带核对 ④不动）；`chat-domain-v1x-liveness-governance(.impl-plan).md` superseded 横幅；protocolization 文档族符号清扫（doc-symbol-drift 守卫全绿）；troubleshooting §12 | C-proc-10 纪律 | 守卫绿 |
