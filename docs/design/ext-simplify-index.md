...
| 12 | ext-simplify-12-pending-notifications.md | pending-notifications | C4 W4 死代码区 ~100 行 + registry 现算化（contested→设计裁决）+ 导出面收敛（14 具名导出仅 1 消费）+ 分档常量头注失实（该常量已随本设计删除，失实随之消解） | session-view-01a0907b-727b-*.md | 设计就绪（审查 2 轮至 0 must-fix，20260912） |
