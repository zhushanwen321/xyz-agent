...
> **⚠️ 2026-09-04 普查标定实锤（被否谱系登记，待清账）**：本决策的窗口标定被全项目超时普查击穿——P-T2c 探针测的是 agent_end→settled 收尾段（<2ms），窗口实际罩 prompt→settled 整轮执行，「余量」建立在错误被测对象上；>10min 合法 chatMode 单轮被误杀登记为普查 ❌4。修复设计见 [timeout-zcode-turn-and-settled-watchdog.md](timeout-zcode-turn-and-settled-watchdog.md) D9（两段式重锚定 + env 通道，含本决策的被否谱系与 U7 回写义务）——按 C-proc-10，修复落地同 commit 完成本节改写与 impl-plan §5 清账，本指针为衔接锚点。
