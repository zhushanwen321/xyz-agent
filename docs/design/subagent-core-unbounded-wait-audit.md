...
> **一句话结论**：weekly/monthly workflow 并发挂死（RC-1/2/3）不是孤立 bug，而是「默认无界」设计裁决的家族性发作——全仓普查共登记 **34 条同家族新缺陷**（13 条可致永久挂起/进程泄漏/宿主崩溃），修复按「正常路径逐点根修 + 回收层统一有界兜底」两层推进，兜底只许出现在回收层。
...
- **A（答案）**：本文登记全部 34 条发现（每条带 file:line 证据 + 触发→后果→兜底→断点四段式），归纳出 7 类失败模式与 1 个共同根因，给出「正常路径优先、兜底收敛到回收层」的修复方案与验收标准。
...
3. **兜底归位**：超时/watchdog 类兜底只允许出现在「回收」这一层，且默认有界（opt-out 而非 opt-in）。
...
**本章结论：34 条新发现中 13 条是一级缺陷（可致永久挂起/进程泄漏/宿主崩溃/通知丢失），全部带 file:line 证据；编排域的 OR-1 是全清单唯一「任何现有机制都不可达」的实例。**

编号规则：LC-=进程生命周期域，PS-=记录/通知/持久化域，OR-=编排域。严重度：**T0**=永久挂起/进程泄漏/宿主崩溃/数据丢失；**T1**=数据不一致/无限重投/状态损坏；**T2**=有界泄漏/竞态窗口/可观测性缺失。
...
| LC-7 | 安全兜底 env 非法值静默等价「关闭」【实锤】 | `session-runner.ts:209-215`（非数字/<=0 → undefined 不挂 watchdog，零日志）；`lifecycle-manager.ts:58-64` 同构 | 运维设 `XYZ_SUBAGENT_SPAWN_WATCHDOG_MS="30m"` 本意加兜底实际静默关掉——「以为有兜底、实际裸奔」（C+G） |
...
**本章结论：34 条发现归到一个共同根因——「默认无界」设计哲学：正常路径押注一次性尽力操作，异常路径落入保守等待，而回收层兜底默认关闭。**

把 34 条按模式归并后，根因不是 34 个疏忽，而是三条系统性裁决：
...
[观测] wf state 0 个 call 停 running；journal 最后一笔到 .finalized < 5s；600s 守卫触发 0 次
```

### 6.2 失败路径（带恢复指引）

- **子进程 wedged（连 get_state 都答不出）**：keep-alive 默认上限（30 分钟）到期 → SIGTERM → 30s 未退 SIGKILL → record 落 `closedReason:"watchdog"`，workflow 该 call 以明确错误失败。👉 恢复：错误消息含 sessionId 与 `subagents action:"list"` 指引，可人工复查后重跑该 call。
...
|---|---|---|---|---|
| A. 逐点修补：34 条各修各的 | 差——「默认无界」哲学还在，新代码继续犯同族错误（daily 守卫就是先例：修了一个点，家族还在） | 低（每点独立） | 低；但治标 | ❌ |
...
**被否若用**：方案 A 下，本次修完 34 条后第 35 条仍会以同样形态出现（模式目录无人防守）；方案 B 下，§6.1 里数小时的 wave keep-alive 会被默认上界误杀——kill 会连坐 dispose 的 killAllSpawnedChildren 杀全部子进程，L2 重派丢在途工作（MF-4 注释记载的真实事故形态）。
...
#### T2【决策】回收层统一默认有界 + kill 升级收敛（覆盖 RC-3/LC-1/LC-2/LC-3/PS-1/PS-3/PS-11）

- **采用**：①keep-alive 分支在 maxTurns/env 双缺省时挂**保守默认上限**（30 分钟，对齐旧 WATCHDOG_FLOOR 量级），显式 opt-out 而非 opt-in；②服务侧三条裸 SIGTERM（`subagent-service.ts:1320/1374/2034`）收敛到 `killChildWithEscalation`；③`killAllSpawnedChildren` 的 `child.killed` 跳过改为「killed 且已 close 才跳过」（或对 killed 进程补 escalation 检查）；④dialog 队列接线协议已有的 timeout 字段作为默认上界；⑤deliverMessage 非 EPIPE 失败路径 re-arm idle timer 或转 cold close（PS-3）。
- **被否**：给每个等待点各自加 env 开关——继续 C 模式的散布姿势，第 35 条还会犯。
- **证据**：`session-runner.ts:1380-1401`（不限时注释）、`dialog-queue.ts:267-283`（无上界 await）、`ui-request-queue.ts:182`（timeout 解析后无人消费）。
- **效果**：§6.2 的 wedged 进程回收路径成立；兜底触发率预期趋近零（正常路径已被 T1 修好）。
- **边界**：这是全方案中唯一「默认开启的上界」，默认值按最长合法任务标定——wave keep-alive 数小时是合法形态，故 keep-alive 默认上限只挂在「无 maxTurns 无 env」的裸缺省情形，且 30min 下限对齐旧 floor 语义，⛔探针 P-T2 验证真实 wave 场景不误杀。
...
- **采用**：①notify 门按 closedReason 白名单放行（parent-new/parent-fork 不注入新 session，落实 A-6 决策，PS-2）；②armIdleTimer 失败降级改为「挂默认 5min timer」而非「不挂」（保住通知放行门与回收两个不变量，PS-4）；③notify-ledger 重投加 attempts 上限 + 放弃语义（PS-6）；④shutdown flush 被 isIdle 门拦时落盘 pending 供重启 replay（修复 `subagent-service.ts:556` 注释承诺与行为的差距，PS-5）。
...
| P-T2 | 30min keep-alive 默认上限不误杀真实 wave 场景（数小时 keep-alive 合法） | 真实 cw wave 开发任务跑全程，统计 keep-alive 持续时间分布 | ⛔ T2 默认值定案前 | 失败 → 默认值改为 maxTurnsToWatchdogMs(估算) 对齐或仅对「无后代能力」调用挂默认上限 |
...
**本章结论：用 4 个真实场景验证——核心判据是「兜底触发率归零」（正常路径修好）与「挂死有界」（回收层兜底就位）。**
...
|---|---|---|---|
| S-A 生产复跑 | 目标 2（修正常路径） | carbon 上真实跑 weekly + monthly 各一轮（真实 LLM、真实 6 路并发、真实数据） | wf state 0 个 call 停 `running`；journal 最后一笔到 `.finalized` < 5s；**600s 守卫触发次数 = 0**；全程无 rpc-client 5400s 外层超时 |
| S-B 故障注入挂死有界 | 目标 3（兜底归位） | 真实 pi 环境跑一个 chatMode subagent，人为抑制其 agent_settled（或挂死 stdin），观察回收 | 默认上限内进程被 SIGTERM→SIGKILL 回收，record 落明确 closedReason，runSpawn 以错误返回而非永挂；错误消息含恢复指引 |
...
| S-D workflow 故障注入 | 目标 3 | 真实 run：`time: 1e12`（非法预算）与「worker 崩溃后重建失败」（可用 ulimit 限线程制造）两个子场景 | 前者在 worker 启动前 fail-fast 且错误含合法范围；后者 run 收敛 done,failed 而非卡 running；宿主进程全程不崩（无 unhandledRejection） |
| S-E 通知止损 | 目标 2 | 真实双 agent 场景：让回执 entry 不可匹配（模拟 compaction 清除），观察 watchdog 重投 | 重投达上限后放弃 + warn 日志；同一条完成通知不无限重复注入（修复前是每 120s 一次无限循环） |
...
| M1 | T1（agent_end 惰性回补）+ T2（回收层默认有界 + kill 收敛）——RC 家族根修 | §6.1 成功路径；验收 S-A/S-B |
| M2 | T3 剩余（worker 生命周期）+ T4（通知） | 验收 S-C/S-D/S-E |
| M3 | T5（多进程）+ T6（有界化） | 长跑稳定性 |

**依赖说明**：M1 是生产事故的根修，优先级最高，但 M0 先行是因为 OR-1 是一行级修复且当前无任何机制可达；T2 的默认上限值依赖 P-T2 探针结果，探针不过则按降级路径调整，不阻塞其余主题。

## 10. 下一层拆分

**本章结论：7 个修复主题即 7 个实施单元，单元间依赖仅 T2 依赖 P-T2 探针。**

| 单元 | 说明 | justification（为什么这么拆） |
|---|---|---|
| U-T1 | agent_end 惰性回补 + LC-4 守卫修复 + PS-9 反查 | 同一决策点（agent_end/finalize）的同一依赖（sessionFile），一次改动闭合 |
| U-T2 | 回收层默认有界 + kill 升级收敛 + dialog timeout 接线 + deliverMessage re-arm | 全是「回收/等待上界」同一语义层，需统一标定默认值，拆开会留下口径不一的上界 |
...
- LC-1..LC-10 = 生命周期域普查 F1..F10（其中 F6 与 PS 域 F10 并条为 LC-6，LC-10 为并条占位）。
- PS-1..PS-15 = 持久化/通知域普查 F1..F16（F10 并入 LC-6，余顺移）。
- OR-1..OR-8 = 编排域普查 S1..S8（S5→OR-5、S6→OR-6、S7→OR-7、S8→OR-8）。

## 附录 C：修复前的运营缓解（止血，非修复）

在代码修复落地前，生产环境可设 `XYZ_SUBAGENT_SPAWN_WATCHDOG_MS=1800000`（30 分钟）把「永久挂起」降级为「有界挂起」——注意这是兜底不是修复：daily 守卫 60% 触发率的教训表明，兜底的高触发率本身就是正常路径 broken 的信号，缓解期间应同步推进 M1。
