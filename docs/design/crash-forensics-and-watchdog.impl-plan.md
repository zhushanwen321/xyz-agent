...
| 20 | u7b 的 spawn 形态预置 0 生命周期接线分发：u4（session-service 钩子：新 session/respawn，fork 归属核实）/ u5（reattach/lazy restore）；u7b 只交付 mirror API + 事件适配 + 协议 | 领地互斥约束下的必要拆分；设计契约（五形态预置 0 + 对账 + errs 判别）不变，u4/u5 任务书承接（u4 本期 scope 已剔除该项，待 u7b 落地后另派）。**收口（随 u4 后小任务落地）**：attach 侧 = session-service onSessionRegistered 独立第二订阅 presetZero（单汇聚点覆盖五 spawn 形态——fork 经 registerForkedSession→registerSession 同点覆盖，fork 归属核实完成）；detach = removeSessionEntry 同点位 dropSession；reclaim = reclaimSession ok 分支同点位 dropSession。u5 侧待核实 reattach 是否走 registerSession（走则零额外接线）。errs 口径修正：新条目 injected=false → errsShape=null（判「无在途」），'absent-report' 仅呈现于 setInjected 后首报前 | 实施期登记（已收口 session-service 侧） |
