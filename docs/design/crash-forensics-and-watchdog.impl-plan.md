...
- v8（2026-09-12）：**over-engineering-audit 裁决执行批**（skill 五步审计 → 用户裁决「Worth exploring 探清楚后都优化」，报告见 tmpdir 快照；Speculative 两项 C7 schema 镜像断言收窄 / C9 inFlightSnapshot 三层槽维持——与偏差 #29 登记取舍对冲，未执行）。逐项：
  1. **C1 死代码删除**：subagent-core `size-rotated-stream.ts`（210 行 + 155 行测试）——头注自认「已无生产消费方」（zcode W5 自主化迁出后），仓内第 3 套轮转实现随删；execution 套件 1634 绿。
  2. **C3 runtime 台账 writer 改同步**（对齐 main 双胞胎 u1c 选型）：删 rotationInFlight 编排 / 10k pendingLines 回放队列 / endAwait close 序 / sink error 通道（endAndAwaitStream 退化为 logger 单消费者保留）；修复「崩溃台账在崩溃瞬间丢在途行」的 durability 语义倒置；接口不变（13 个 getCrashJournal 调用点零改动）；EISDIR 用例同步化改写 + 轮转窗口 close 用例同断言同步形态。
  3. **C4 watchdog 采样环删除**：环零读者（无 RPC / 不进诊断包 / 重启即丢；判定走 consecutiveAboveWarn 计数器；Gate W 复审走既有 5min 水位日志行——文件头自认）。「off 时采样环照跑（观测先行）」叙事与「环只存内存不落盘」自相矛盾，无出口的观测 = 无观测；删 ring/ringCapacity/WATCHDOG_RING_DURATION_MS/WatchdogStatus.samples，getStatus 保留 level/计数器/lastSample；武装后若需分钟级历史，加密水位日志采样即可。
  4. **C2 useMemoryPressure 生产装配**：App.vue setup 顶层窗口级单例挂载（refCount 订阅），关闭「hook 零调用方 + armed 默认 off」双重休眠；本表 §6 u7d 行「经 useRollingRestartStatus 引用链生产挂载」登记失实（该文件仅注释引用范式），随挂载落地成为事实。
  5. **C8 run 目录文件名族收敛 shared**：`runtime-checkpoint.json` / `main-running.marker` / failed 前缀 / 保留份数三处手抄（runtime-checkpoint.ts 导出零外部消费 + main.ts 本地副本 + bundle 第三副本）收敛 `@xyz-agent/shared/paths` RUN_* 常量族 + `getRunStatePaths()`；main `resolveRunStatePaths` 委托；runtime 仅 CHECKPOINT_VERSION 留本地。
  6. **C5 诊断解析层收敛**：新增 `diagnostics/journal-io.ts`（ENOENT 常态空 + 失败 warn 的设计 §3.1 口径 SSOT，patrol/bundle 共用——bundle 原全吞版违反 §3.1）；bundle 三消费方（detailPaths/piVersion/首屏表格）共享一次 `parseJournalLines`（docstring「导出侧复用」声明兑现），recentEventsTable 不再二次 readFileSync；C9 main 台账 retainedSegments 旋钮删除（零传参，展开定值三段对齐 runtime 形态）。
  7. 验证：四包 tsc 0（shared/electron/runtime/renderer）；受影响套件全绿（watchdog 19 / crash-journal 9 / infra+wiring 86 / diagnostics 19 / evaluator+run-state+journals 39+13 / checkpoint+reattach 56 / electron diagnostics 等 58 / execution 1634）；check-doc-symbol-drift 零悬空。
