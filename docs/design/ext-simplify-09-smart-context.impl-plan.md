...
| # | Unit | 偏差 | 定性 | 依据 |
|---|---|---|---|---|
| 1 | u1 | 测试 fixture 多补 settings 必填字段 | 合理偏差 | SDK 类型实测（§6 u1 证据指针；tsc 报错逐个补齐，§7 残留风险③闭合记录同源） |
| 2 | u2 | 逐消息 ceil 微差 + debugLog 多行格式化 | 合理偏差 | ceil 与 pi 核算同源语义（§6 u2 证据指针）；debugLog 为输出格式形态 |
| 3 | u3 | index.ts:113/114 判定为跨界通知对比、非门控双写 | 合理偏差 | 已核实设计基线（§6 u3 证据指针） |
