...
3. ~~**Gate B 未执行（阻塞，需用户决策）**：`pnpm run dev` 所需端口 **1420 / 3310 / 9222 全部被另一个工作区的 dev 实例占用**……~~ **已执行（2026-09-11，隔离 dev 实例实测；S1-S16 逐条结果见下）**。
   ...
   **唯一残留（凭证依赖，非代码问题）**：S2 的成功断言与 S11② 的原构造需要「当前有效」的平台凭证，无法自助获取 → 登记为**凭证依赖待验项**，不阻塞交付。**全程零 mock**。
