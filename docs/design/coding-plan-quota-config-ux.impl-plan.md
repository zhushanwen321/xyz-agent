...
3. **Gate B 未执行（阻塞，需用户决策）**：`pnpm run dev` 所需端口 **1420 / 3310 / 9222 全部被另一个工作区的 dev 实例占用**（实测：PID 7313 = `fix-subagent-no-notification/packages/renderer` 的 vite，PID 7345 = 该 worktree 的 dev Electron）。`packages/renderer/vite.config.ts:41-42` 为 `port: 1420` + `strictPort: true`，`apps/electron/main/window/window-factory.ts:23` 的 `VITE_DEV_URL = 'http://localhost:1420'` 是**硬编码常量**（非 env 可覆盖）→ 无法用备用端口并行起本工作区的 dev 实例。**另**：本工作区 dev 数据目录 `~/.xyz-agent-dev` 只有 `xiaomi-token-plan-cn`（mimo，cookie 文件在）具备可用凭证；`zai-coding-cn`（zhipu）**无任何凭证**，且该目录不存在 `auth.json` / `models.json` → 依赖真实凭证的成功路径（S1/S2/S3 的成功断言）与需要 opencode 凭证的场景不可执行。**恢复路径（用户决策）**：① 停掉另一工作区的 dev 实例后由本流水线起 dev 并跑可执行场景；② 用户自行按 §8.2 场景表验收；③ 接受为已登记延期项。**禁止**用 mock 变通冒充通过（acceptance.md）。
...
7. **三个 prop 已无消费方但仍声明**（阶段 4 徽标/占位同源化后的 deferred 项）：`cookieSet` / `workspaceConfigured` / `quotaApiKeyConfigured` 在 `CodingPlanSection.vue:295` 等仍有声明并带 `[保留]` 注释，但三者均已不在模板/computed 有任何消费位（`grep -n "props\.cookieSet\|props\.workspaceConfigured\|props\.quotaApiKeyConfigured"` 零命中）；调用侧 `ProviderEditBody.vue:217`（`quota-api-key-configured`）/ `:220`（`workspace-configured`）/ `:230`（`cookie-set`）仍在传值——移除需同批改该文件（跨领地），故本次保留。**恢复路径**：下一次触及 provider 编辑体时一并清理三个 prop 的声明与三处调用点（第 3 轮定向复审订正：初版只记两个，漏了 `quotaApiKeyConfigured`）。
...
9. **零引用 i18n key 的清理面**（阶段 4 扫描所见，未删、未逐个人工复核）：① `panel.*` 命名空间 460 个叶子 key 中约 98 个按字面引用零命中；② `providerEdit.*` 另有 6 个**基线即零引用**的死键（`editTitle` / `dialogDescription` / `unsavedTitle` / `unsavedDesc` / `unsavedConfirm` / `unsavedCancel`，zh-CN:310-311/418-421 与 en-US 同号），核实命令为**带命名空间的全路径 grep**：`git grep -n "providerEdit\.<key>\b" <rev> -- packages apps | grep -v /i18n/locales/`，HEAD 与基线 `62a9651e5` 均为空（裸 `git grep -w <key>` 会命中 `settings.dialogDescription` 等**同名不同命名空间**的引用，不可作为判据——第 3 轮定向复审订正）。**注意**：可能存在间接/动态组装引用，**不得按此数直接删**。**恢复路径**：独立批次逐 key 核实（含动态引用排查）后处理；另可考虑补一条「存在 → 是否被引用」的守卫（现有守卫只查「引用 → 存在」）。
11. **UI 与 runtime 对「专属 Key 是否适用」的判据不同（潜在耦合，当前触发面 0）**：UI 显示凭证来源分段控件的判据是 `!isCookieAuth`（即 `cookie ∉ auth`，`CodingPlanSection.vue:90`），而 runtime 的 `exclusive` 收窄判据是 `api-key ∈ auth`（`quota-service.ts:701-705`）。现有 5 个内置 fetcher 的 auth 形态只有 `['api-key']`（zhipu/minimax）、`['api-key','oauth']`（kimi）、`['cookie']`（mimo/opencode-go），**全部满足「api-key 或 cookie」**，故 0 触发面。**风险**：若未来新增 `auth: ['oauth']`-only 的 fetcher，UI 会显示「用专属 Key」而 runtime 忽略 `exclusive`（两端判据不一致）。**重审触发条件**：新增不含 api-key 的非 cookie fetcher 时，同步两端判据（或改为共用同一判据函数）。来源：第 2 轮聚焦复审（代码侧）登记，非本次修复引入。
