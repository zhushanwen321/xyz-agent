...
| R10 | cookie 空串清除 / apiKey 清除 / `clearProviderState` 三处删除语义收敛为单一 helper `removeSecretFile` | §7.3 改动 2「两处必须一致」+ 改动 5「幂等：ENOENT 视为成功，不做 existsSync 预检」 | 三处一致性由「清单纪律」升级为「结构保证」（ENOENT/非 ENOENT 分支只有一处实现），未改变任何设计声明语义 | `quota-service.ts:382-393`（定义）/ `:365` / `:405` / `:807-808`（三个调用点）；行号已按 `08036093f` 后的 HEAD 复核（第 3 轮定向复审订正） |
...
7. **三个 prop 已无消费方但仍声明**（阶段 4 徽标/占位同源化后的 deferred 项）：`cookieSet` / `workspaceConfigured` / `quotaApiKeyConfigured` 在 `CodingPlanSection.vue:295` 等仍有声明并带 `[保留]` 注释，但三者均已不在模板/computed 有任何消费位（`grep -n "props\.cookieSet\|props\.workspaceConfigured\|props\.quotaApiKeyConfigured"` 零命中）；调用侧 `ProviderEditBody.vue:216 / :224 / :230` 仍在传值——移除需同批改该文件（跨领地），故本次保留。**恢复路径**：下一次触及 provider 编辑体时一并清理三个 prop 的声明与三处调用点（第 3 轮定向复审订正：初版只记两个，漏了 `quotaApiKeyConfigured`）。
