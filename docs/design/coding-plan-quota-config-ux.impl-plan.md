...
| R10 | cookie 空串清除 / apiKey 清除 / `clearProviderState` 三处删除语义收敛为单一 helper `removeSecretFile` | §7.3 改动 2「两处必须一致」+ 改动 5「幂等：ENOENT 视为成功，不做 existsSync 预检」 | 三处一致性由「清单纪律」升级为「结构保证」（ENOENT/非 ENOENT 分支只有一处实现），未改变任何设计声明语义 | `quota-service.ts:382-393`（定义）/ `:365` / `:405` / `:801-802`（三个调用点）；行号已按 `ec6ea70ce` 位移后复核 |
...
9. **零引用 i18n key 的清理面**（阶段 4 扫描所见，未删、未逐个人工复核）：① `panel.*` 命名空间 460 个叶子 key 中约 98 个按字面引用零命中；② `providerEdit.*` 另有 6 个**基线即零引用**的死键（`editTitle` / `dialogDescription` / `unsavedTitle` / `unsavedDesc` / `unsavedConfirm` / `unsavedCancel`，zh-CN:310-311/418-421 与 en-US 同号，HEAD 与基线 `62a9651e5` 的 `git grep -w` 均为空——非本次改动丢失消费方）。**注意**：可能存在间接/动态组装引用，**不得按此数直接删**。**恢复路径**：独立批次逐 key 核实（含动态引用排查）后处理；另可考虑补一条「存在 → 是否被引用」的守卫（现有守卫只查「引用 → 存在」）。
