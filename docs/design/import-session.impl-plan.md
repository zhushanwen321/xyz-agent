...
| u1-scan-utils | committed | 1 | scan-external 9/9 + tmp-migrate 5/5 + tsc 绿；commit 80d7657bb（scan-external 阶段 3 终态 10/10：7be007a02 补「分批让出」用例；bc25decf3 后现 14（+4 name-tier 定位用例），见变更历史；scanExternalSessions 实现已于 6edc0a665 迁至 session-file-external-scan.ts，见 §2 领地注记/§5 偏差表） |
