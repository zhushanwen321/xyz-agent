...
- 2026-09-09：design-code-sync 第 2 轮（commit df1f8a77a）：双 reviewer 复审 4 组 findings 全修——composer-gen-stats D1 消费点双重过期改写（「:325 现只调 onContextUpdate」→ :465 + 设计时/实装后区分）、~14 处行号锚点一次性 grep 实测刷新（含 protocol.ts:1103→:1271 等设计期存量漂移）、§2.2 快照总注确立「行号=落笔时点快照、以符号内容定位为准，后续仅内容级失配立项」处置纪律、本表补 Gate A ledger 与第 1 轮 sync 条目、u4 行数 numstat 修正（65→63）；聚焦复审 cleared（11 处行号实击全命中、活引用零残留）。
