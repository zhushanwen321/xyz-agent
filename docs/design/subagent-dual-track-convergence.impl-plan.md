...
| 2026-09-03 | 一致性审查 r1 | **终态五 read 口径修正（已改设计 §3.1）**：EnginePort.read 生产调用方维持 conformance-only——「经 port 面生产调用」被 D1 被否②封死（runtime 跨进程不可达 pi 进程内 EnginePort），生产复用形态 = reader/journal-replay module 级（runtime 薄调用复用同一 module）；interact 半边已兑现（deliverChatMessage → chatPiEngine.interactRecord） | core 区 reviewer U1；设计终态五原文与 D1 被否②自相矛盾，module 级复用是唯一可行路径 | 已固化（设计文档同步） |
| 2026-09-03 | 一致性审查 r1 | **D1 双守卫收敛取侧 = 写侧严格语义**（subagent-extractor.ts:136-145）：runtime 读侧宽松分支（过滤非 string 继续读①②级）随收敛消灭，未经 extractor 投影的外部 record 含坏 handle 时改走整体拒绝降③级（outcome-only 安全降级，永不空数组）——extractor 是唯一写侧投影点且原本严格，宽松分支只对外部数据可达 | orchestration 区 reviewer reasonable；严格 > 宽松防御，未来第三引擎 guard 实现者按此取侧 | 已固化 |
| 2026-09-03 | 一致性审查 r1 | 合理演化登记（4 项，均不改设计声明目标）：①杀链参数化超出设计示意——unrefTimers/escalationNote 随 grace 一并参数化（A1 零回归必要维度，D3-① 终态二已补注）；②capability-gate 以 TaskShapeForGate 结构子集同时接受 ExecuteOptions（chat）与 AgentCallOpts（workflow），零适配服务两域；③killPiChild 移除 state.escalationTimer 记账，timer 生命周期收进 kill-chain（race-F4 三用例守护）；④V5③④ 消费侧测试经产出侧 classifyFailureKind 构造失败样本（词表演进测试自动跟随）+ 终态路径测试归属文件分工声明 | 三区 reviewer reasonable 精简合并；测试锚定技法供 failureKind 扩态沿用 | 已固化 |
| 2026-09-03 | 一致性审查 r1 | **doc_errors 修订 5 处（主 agent 亲为，设计文档）**：①终态五 read 口径（见上行）；②D2 采用段「统一走 executeViaEngine」→「统一经 EnginePort（chat 入口 executeViaEngine / workflow 入口 SAR）」——与 §3.5 及实现矛盾；③D6 采用段「TaskSpec → pi spawn 参数（不还原 ExecuteOptions 中间态）」→「AgentCallOpts → ExecuteOptions 一次性直出，消除 AgentTaskSpec 中间态与第二次映射」；④V3 通过标准改与 u-1c 门①裁决一致口径（手动/关闭类删除属 archive 保留不即时释放，回收走 idle-GC/pool TTL 兜底）；⑤output-collector 路径 ×2 处更新为 u-2a 下沉后位置（engines/pi/）+ 终态二补 kill-chain 参数轴注 | 三区 reviewer doc_errors；读者按旧路径/旧口径定位会落空或阶段 5 误判失败 | 已固化（设计文档同步） |
...
7. **桌面 workflow 发现空（独立排查项，非本重构引入）**：Gate B 桌面复测 workflow-script action:list 返回空（同一环境 agent 发现正常）。双形态交叉证据（Gate B 命令块）：非隔离 pi CLI（cwd=项目根）发现正常返回 5 个 [saved]；隔离形态空的根因=隔离 agent dir 缺 npm 安装目录（配置性）。桌面形态的特异性线索：桌面 runtime spawn 的主 pi 进程 cwd=home（lsof 实证），workspaceRoot 按 cwd 推导则 home 下无 .agents/workflows——疑似桌面 runtime spawn 未传项目 workspaceRoot，待独立排查。本区间（a2f87a515..HEAD）零触碰发现链代码。
8. **zsw CLI 缺失（环境限制）**：V1①②（zcode GUI 历史详情基线对照）与 V3 的 zcode 真实子代理实测在本机不可达（zsw not found + 无存量 zcode record）；V1 由 ③ golden 注入 + ④ 代码断言机器证据签收，V3 由 acquire 即时性测试证据 + u-1c 接线测试签收。
9. **Gate B 裁量项登记**：①chat 域 tool result 错误文本未拼入 EngineError.recovery 子句（errors.ts 构造函数全族既有形态 `${code}: ${detail}`，非本次引入；模型实测下轮自行恢复 + 工具 schema 预防说明兜底——改进项低优先）；②workflow 域 capability 拒绝后错误恢复层 attempts=3 重试（既有恢复矩阵行为，durationMs=1 × 3 无害）且 run 终态 reason=completed（拦截语义在设计即 call 层）；③--no-extensions 主进程形态下 structured-output 需随主进程显式挂载（argv-mirror 镜像行为，设计一致，错误 Recovery 文案已正确指引）。
10. **one-shot 成功 record 不写 manifest 是 SP-5 设计行为**：桌面实测 sa-6648b918 完成后无 sa-*.json 落盘——runAndFinalize one-shot 成功分支走 finalizeRoundToIdle（保持 running-resumable 等 message upgrade/idle-GC 30 天归档），manifest 是 finalizeRecord 终态诊断辅助非正确性依赖；探针直写同目录验证 ManifestStore 本身正常。
...
| 2026-09-02 | 八单元全部 committed（u-1a/1b/1c/2a/2b/2c/3a/3b，区间 5ae263fe8..73b097d60，181 文件 +7048/-4120） |
| 2026-09-03 | 阶段 3 一致性审查 r1（三区并行 reviewer：core execution / orchestration+runtime / 壳）→ 8 reasonable + 2 minor unreasonable + 5 doc_errors，无 must-fix；doc_errors 主 agent 修订设计文档 6 处，U-1（formatElapsed 小时分支）+ DE4（pi-engine 注释失真）组修复批次派 subagent |
| 2026-09-03 | 阶段 5 Gate A：9 命令全量验收——root lint 初检 exit 1（3 warning 本区间引入）修复后清零（commit 941679574：魔数命名常量 ×2 + eslint max-lines override 路径跟随 u-2a 迁移），core 2402/6、runtime 4095、extensions 三连全绿、renderer typecheck 0、覆盖矩阵无无人认领区；6 skipped 判定合理（live 手动门） |
| 2026-09-03 | 阶段 5 Gate B 双块签收（命令块 pi CLI + 桌面块 dev GUI）：V4③ build+validate 双 0；V4④ 拦截矩阵 4 场景（chat 同步拒绝无句柄/record/进程 + workflow call 层 durationMs=1 + pi 反向零拦截）；V4⑤ 桌面 maxTurns=3 正常执行 + 后台完成通知链路实证；V6 真实 workflow schema+model+maxTurns 全链路（parsedOutput 合 schema、record 23 字段、发现链双形态复测定性）；V5①② 终态四步+budget_limited 正常（③④测试签收）；V2② agent 清单注入实证；V1①② 与 V2① TUI 视觉受环境限制（zsw 缺失/TUI 不可自动化）由机器证据签收；双绿交付 |
