...
- 残留风险（验收后登记）：①runtime pi-settings-store D1a 满载 flake（预存独立技术债，非本区间引入）；②V1 manifest 时序口径 nuance——「先于通知送达落盘」在 D1 fire-and-forget 机制下毫秒级竞态方向非确定（两跑 +2ms/-3ms），验收门取「消费时点就位」口径（偏差 #11⑤），严格时序门须先改 D1 为同步写；③首批探针记录的 15/11 PASS 硬编码计数与实际 16/12 不符（区 2 插值化已修复，首批记录仅作历史留痕）；④根级 pnpm test first-fail 递归——flake 会中止掩盖尾部包（本轮人工补跑闭环，CI 注意）；⑤apps/electron 不在根级 test 扫描面（配置现状）。
