...
设置页新增「对话流式空闲超时」数字输入（单位分钟，合法域 1–60、默认 30——D3 单一权威口径，r2 复审 MF-3 修正 v1 残留 1–120），形态对齐既有「worktree 创建超时」输入域（`packages/renderer/src/components/settings/worktree/WorktreePage.vue` template :88-94 / script :161-194 / `config.setTimeout` 先例，r2 复审 SG-4 补全路径）：保存 → `config.setStreamingIdleTimeout` RPC → runtime 持久化到 `~/.xyz-agent/config.json` 的 `streamingIdleTimeout` 字段（实施期修正：实落 config.json 而非 settings.json，对齐 worktree timeout 同文件同模式；一致性审查 DE2）→ 生效时机（一致性审查 U2 演进）：进行中 turn 在变更后的下一活动帧起按新值重挂 idle 窗口，计时基线（已流逝时间）不清零——详见 §5.3。
...
- **采用**：(a)。协议形态对齐先例：`config.setStreamingIdleTimeout {timeout: 秒}` / `config.getStreamingIdleTimeout` → reply；runtime 持久化 config.json（实施期修正，见 §4.3）；**默认 1800s、合法域 clamp 60–3600s（1–60min）——单一权威口径（r2 复审 MF-3 定案，本文其余处引用此处；v1 残留 600s 已清）**。生效时机（实施期裁决修正）：`refreshStreamingTimer` 与挂载同为「读当前值」挂点——进行中 turn 在阈值变更后的下一活动帧即按新值重挂 idle 窗口（调大放宽/调小收紧，用户语义直觉一致）；真正「新 turn 才生效」的只有计时基线（进行中已流逝时间不清零）。一致性审查 U2 演进，§4.3 同步。store 侧：删 `readStreamingTimeoutMs` 死口，改为 store action `setStreamingIdleTimeoutMs(ms)`（app 启动时 renderer 经 `config.getStreamingIdleTimeout` 水合），`armStreamingTimer`/refresh 在挂载时读当前值。同时清除 store.ts:233-234/361 的悬空 D-016 注释（编号已被 command 注册表合法复用，防误导）。
...
- **采用**：删除 bash timer 整链：`BASH_TIMEOUT_MS`（timers.ts:15）、`armBashTimer`/`clearBashTimer`（timers.ts:71-100 区段 + bashTimers Map）、`finalizeBashOnly`（store.ts:872——唯一调用方就是被删的 bash timer 回调）、effect ctx 契约（effect-types.ts:53-60）、store 公开 API 导出（store.ts:1137-1138）；连带收窄 `markBashError` 签名（store.ts:1154 传入的 `clearBashTimer` 参数移除）。**保留**：`markBashError`（bash 错误路径 live 调用，store.ts:1154）、`findLastStreamingBashIndex`（其依赖）。测试同步改写（timers.test.ts / effects.test.ts / renderer 侧 chat-bash-effects.test.ts / chat-chunk-content-blocks.test.ts 中 armBashTimer 用例删除——前两文件在 core `__tests__/`，后两在 renderer；实施期另有标题级悬空引用清扫，一致性审查 DE4 勘误路径）。
...
| `packages/runtime/src/services/config-service.ts` | handler + config.json `streamingIdleTimeout` 持久化（对齐 worktree timeout 模式，实施期修正落点措辞） |
...
| S4 | 配置生效且持久 | G3 | settings 表单改阈值（如 5min）→ 保存 → 新 turn 触发 arm；重启应用后再查 | 新 turn 按新值计时（fake-timer 单测断言 arm 值 + 真实 UI 验证表单回显）；重启后值保留（config.json）；超范围输入被表单拒绝 |
