...
- **F2（已登记的长期债，非本批缺陷；2026-09-10 交付后调研定案）**：U-A6 复用了 `{type:"message_end"}` 作为工具活性信号载体。功能与安全均成立（零写入、1s 节流、不进正文槽、静默工具仍被回收），但**语义借用**是领地受限（core 为禁区）下的权宜。**交付后调研结论（用户纠正后复核：「core 禁区」是当时批次并行约束，非架构耦合）**：第一类 `activity` 变体改动集小且全部可编译期收口——SDK（union 定义 + journal-replay 穷尽 case + schema enum + 测试三处）+ core（`execution-record.ts` updateFromEvent 加 2 行 no-op case，唯一强制改动；其余消费点全部条件式判型或类型无关刷新，天然忽略/天然受益）+ pi（`TOOL_ACTIVITY_EVENT` 借用载体换真变体）；wire 盲传 JSON 双向前向兼容，协议维持 v1（additive）。配套决策：journal **豁免落盘**（双侧 reducer no-op，豁免不破坏 live≡reload 等价性；过滤点 `journal-wiring.ts` onEvent 包装，SAR 刷新在其上游不受影响）；1s 节流保留 translator 内。**实施序**：PR1 = F2 清账（单 PR 三包原子 + 文档同 commit 回写）；PR2 = F3（`roundLifecycle` 新增 `phase:"active"` 轮内心跳相位——宿主 `handleChatRoundPhase` 加一个 case 即闭合；红线：引擎侧不得经 `emitPhase` 发射，会误 resolve cancel 的 roundTerminalWaiters）；PR3 = zcode 生产者（zcode 宿主刷新面仅 text/thinking delta，与 pi 修复前同构失明；落点 `session-channel` refreshIdle 咽喉，但上游 app-server 工具执行期是否有推送属 A.2/A.5 未实证项，**真机探针 gate 后落地**）。代码注释已就地登记（`spawn-event-translator.ts` 的 `TOOL_ACTIVITY_EVENT` 段），此处同步登记防止只写在注释里丢失。
...
- **F6（范围外产品缺口，Gate B 发现，未修——修复归属需用户裁定）**：GUI（dev app）内**任何 pi 引擎 subagent 派发**（workflow 域与 chat 域全域）在 relay 拓扑下被阻断——relay 包装进程因缺 `XYZ_SUBAGENT_RELAY_SESSION_ID` 拒绝运行 → pi child **exit 13**。实测证据（2026-09-10 Gate B）：workflow 6 连败（每路 3 轮重试后失败）+ chat 域首轮失败，`~/.xyz-agent-dev/logs/pi-task-stderr-{11096..11101,24298}.log` 七份全为同一条 relay 拒绝信息；两次失败的通知均正常到达（G1 韧性面顺带得证）。根因链：spawn-runner 的 W8/H12 注入（`pi-subagent-cli/src/spawn-runner.ts` buildChildEnv，注释明示「旧实现 RECORD_ID 未送达 → 退出码 13」是已知形态）的 SESSION_ID 源 = `params.sessionRootId ?? env PI_SUBAGENT_ROOT_SESSION_ID`，但①协议 v1 ctx 无 sessionRootId 字段、②extension 的 engine client（`subagent-core/execution/engine/engine-inspect-package.ts` portFactory → `EngineClient`）不传现成的 `identityEnv` 选项、③根会话 pi 进程自身无该 env——**三个来源全空**。定性：协议化迁移的既有回归（dev-0.9.16 同形存在，已核对；旧架构 session-runner 的 env 注入未随「extension 内 engine client」拓扑迁移），**非本批 replay 改动引入**（本批未触碰 extension/engine-client/spawn 的 env 面）。修复方向二选一（需小设计再动）：engine client 创建时以宿主根会话 id 注入 `identityEnv`；或协议补 `ctx.sessionRootId` 字段（spawn-runner 注释已预告「协议补字段后收敛」）。**在 F6 修复前，GUI 内 pi 引擎派发不可用**（默认 zcode 引擎不受影响）。
- **F7（基础设施：staged 引擎构建滞后，Gate B 发现，未修——建议另行任务）**：dev app 实际加载的引擎 = `apps/electron/resources/engines/<id>/index.js`（gitignored 构建产物），pi 引擎副本构建于 2026-09-10 16:30（工作树当时状态：含 M2 代码、**缺 M4（17:28 commit）与 A 组修复（18:17）**）；**`pnpm dev` 不重建 staged 副本** → 存在「GUI 真机跑旧引擎代码」的系统性风险。本次 zcode 副本无影响（本批对 zcode-subagent-cli 的最后改动随 M0 合并，早于副本构建；且 zcode 引擎链路验收全过）。建议：dev 启动（或 pre-dev）纳入 staged 新鲜度校验/重建（staged 与源码内容哈希比对），或 dev 模式直接解析 workspace 源包。
...
- **附带定界**：首轮 zcode run 的 6 条「权威终态晚于落定结果到达」stderr = `zcode-subagent-cli/src/session-channel.ts:914` 的 P0-1 D5① 设计内诊断（final-frame 宽松终态先落定、权威 turn.terminal 迟到仅记录不改写）——预存、良性、非本批改动面、不影响完成。
