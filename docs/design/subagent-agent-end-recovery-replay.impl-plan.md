...
- **F1（待修，core 领地）**：`packages/subagent-core/src/execution/subagent-service.ts:2291` 的 `backfillEngineHandle` 幂等守卫是 `if (record.engineHandle !== undefined && record.engineHandle.sessionRef["sessionId"] !== undefined) return;`——**只要 sessionId 已落就直接 return**，于是「sessionId 已知 + 迟到携带 sessionFile 的 handleReady」这一形态被整条吞掉，`record.engineHandle.sessionRef.sessionFile` 永不落位。而这恰是本批让之可达且更有价值的组合（V2/M4 的 close 期兜底面都会发携 sessionFile 的 handleReady）。消费点实测两处：`subagent-service.ts:2810`（`anchor.sessionRef["sessionFile"]`）与 `:2921-2922`（携带给引擎 interact 定位）。修法：把「整条 return」改为「按字段补缺」（保留幂等：已有值不被覆盖，缺的字段补上），并补一条「sessionId 先落、sessionFile 后到」的用例。**排期**：等定向复审（组 B）回来后与复审条目合并为一组派修（避免在复审阅读期改动同文件）。
