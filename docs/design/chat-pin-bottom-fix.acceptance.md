...
- 修复：阶段 3 审查唯一 unreasonable 项，按设计 §4.5 P-timing 既有降级预案实施（RO 回调 → 外层 rAF → followIfStuck 内层 rAF = 双 rAF，消除 virtua 内部 viewportSize 更新时序缝隙）——预案原触发条件为 M1 探针失败，实际由本验收 V6 shrink 间歇残留触发（同一修复手段，语境差异已随设计 v8 回写对齐）；commit `2451a2036`。
