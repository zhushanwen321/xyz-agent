...
| 17 | session-pending.test / notifier-flush.test 两测试 P1 留壳（subject 模块在 core，但以 pi 真机制锚定回归面） | 两测试经 NotifyDomainPorts 注入 pi 真计数器 / session-delivery 真投递内核（头注自证「注入真函数保住回归面」），pi 机制是测试主体组成部分；迁 C 需给宿主中立包加 pi 协作件 devDeps，零覆盖增益（E 套件直接跑 C 源） | 一致性审查 r2 low，裁定登记处置（非迁移）；两测试头注释已标注 |
...
12. **E 壳测试套件依赖 C 测试树路径**（r2 登记）：chatmode-round-notify-real-chain.test.ts 经跨包深路径 import C 的 spawn-mock helper（单一权威源、避免双宿主两份 mock 漂移），C 侧测试基建重组时需同步该 import。
