...
- 2026-08-30（增量审查 r4 + 全量修复，0 must-fix / 2 suggestion / 4 doc_error 全处置；r4 结论已逐项记录本表，报告为会话临时文件不入库）：r4 确认三条闭合标注全部属实（self-test 独立复跑双段绿、逐字节核实、684 warnings 复测）。修复：① S1 检查项 0 改 matchAll 唯一性断言（多声明复制残留也拦，防第一匹配静默漏漂移——与检查项 2 fail-closed 哲学对齐）；② S2 self-test 扩双探针（新增检查点 5 面：workflows/ 注入 `require("../src/core/host-services.ts")` 探针，stderr 指名 host-services 定位），头注同步「双探针 = V6-① 原始两面」；③ 风险 8 数字刷新 706→684（r4 复测口径 `eslint src` 120 文件，0 errors / 674 fixable 不变）；④ 风险 12 精确化 7 文件 8 条语句（chatmode 同文件 :31 动态 + :101 静态，主 agent 亲验计数）；⑤ 设计 D9-① 补 r4 实现注记（版本双源断言 + --self-test）；⑥ install-hooks.sh 与 .bare/hooks/pre-commit 实装件守卫段注释同步（补 optionalDependencies + 版本双源，r3 遗留 optionalDeps 缺注一并补）。触发：用户指令（全等级修复 + 循环审查）。
- 2026-08-30（复审 r5，修复-审查循环收敛：**0 must-fix / 0 suggestion / 0 doc_error**）：r4 六条发现逐条对抗复核全部真实闭合，无修复过头、漏改或新引入副作用——matchAll 三分支完备（0/多/漂移各自 fail）；检查点 5 探针真命中检查 3a 核心分支（workerRoots 收集 .cjs → resolve 落 src/ 内 → HOST_SERVICE_MODULES 判定，非巧合）；双 hook 守卫段 19 行逐字节零差异；eslint 684 复测逐字一致；crash-recovery 4 passed；挂载面（ci.yml/release-npm.yml/install-hooks.sh 三处无参调用）零影响，检查项 0 只读 git 跟踪文件无 CI 误报面。3 条 INFO 处置：① 本表 /tmp 报告路径按「产物自包含」改注「会话临时文件不入库」（本行上方已改）；② ③ 为理论中断窗口类（探针残留经覆盖写自愈 + 正常跑转红暴露，fail-closed 方向），登记不动。**循环在 r5 收敛**。触发：用户指令（循环直到无 must-fix）。
