...
| T2③ 窗口起算口径裁决：设计正文「deliverMessage 发出 prompt 后同挂」vs 边界句「不触及 turn 执行期」张力——实施取**正文口径**（prompt 发出即起算，整轮含 turn 执行与收尾都在 10min 窗内），更强覆盖（续聊轮 turn hang 也能回收）；风险 = >10min 的 chatMode 单轮被误杀（P-T2c 实测正常轮次 <2ms/compact 40s，chatMode 对话形态单轮分钟级，风险罕见；如遇误杀调 SETTLED_WATCHDOG_TIMEOUT_MS） | u-t2a（主 agent 复核裁决） | **风险已兑现为普查 ❌4**（2026-09-04 超时普查实锤：P-T2c 测的是收尾段，窗口实际罩整轮，误杀非罕见）→ 按两段式重锚定修复（中段 prompt→agent_end 无进展检测 30min 有效事件刷新 + 收尾段 agent_end→settled 固定 600s + env `XYZ_SUBAGENT_SETTLED_WATCHDOG_MS`），代码落地 commit 0364b349e，设计依据 [timeout-zcode-turn-and-settled-watchdog.md](timeout-zcode-turn-and-settled-watchdog.md) §6-D9；unbounded-wait-audit 边界句已同步改写为两段式语义 | **已清账（2026-09-05）** |
