...
基线: <本计划 commit hash，基线提交后回填> | 来源设计: `docs/design/subagent-core-unbounded-wait-audit.md`（4 轮对抗审查收敛，commit 728679ba8；审查报告 `subagent-core-unbounded-wait-audit.review.md` 轮 4 must_fix=0） | 日期: 2026-09-01
...
| u-m0b | OR-1 顺序修复：scheduleTimeBudget 前移到 workerHost.start 之前 + tool-workflow 入口 time 上界校验 | `src/orchestration/lifecycle.ts`、`src/orchestration/interface/tool-workflow.ts` | 无 | plain | typecheck 绿；新增测试：time>2^31-1 在 worker 启动前 fail-fast 且 runs Map 无残留条目 |
