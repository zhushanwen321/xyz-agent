...
- **Gate A**：PASS；**Gate B**：V1-V4 单测可覆盖部分全绿 + P1-2 探针证伪（端点 404 确定性，降级路径生效零损失）/ P2-2 通过（0.070ms << 100ms）——⛔门全部关闭（设计 §5.5 回写）。
- 一致性审查：1 轮收敛（11 reasonable / 5 unreasonable：P1-2/P2-2 已补跑闭环、canonical 偏离已登记、G1 文案闭环归 u-y1 链完成、P4 形态已登记 / 1 doc_error 已修）。
