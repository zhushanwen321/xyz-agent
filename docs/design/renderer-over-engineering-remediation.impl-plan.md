...
- 变更历史：
  ...
  - 2026-09-11 Gate B 残余风险（未闭环项，交付时如实呈报；状态以本行为准，与上文三条【已闭环】不重复计）：① V1/V3 GUI 交互**已于阶段 6 补验通过**（2026-09-11 17:40-18:00 本 worktree `pnpm dev` + CDP 实测：⌘K 唤起/空查询 recents/命令源/文件源（真实 runtime FileService，16 文件命中）/会话源/跨会话跳转/空态与 stale-recents 错误路径；侧栏会话列表×2 + 设置页 12 分区渲染 + provider 配置全链路；重载采样 **0 console error**，仅 2 条与基线同源的 onScopeDispose warn 与设计内 finalizeSession 诊断 warn）；**V2 仍 blocked**——dev 隔离数据目录（`~/.xyz-agent-dev`）无真实 provider 凭据，哑 provider 只能建会话（session create 不调 LLM）不能产出 markdown 回复，mock 罐头回复为纯文本且 V2 场景表未许可 mock，按 Gate B 规则不以变通冒充通过；补偿证据 = ui MarkdownRenderer 本批零生产改动（仅 1 行注释）+ ui 668 tests 含真实点击路径 + Gate A 绿；复验条件 = dev 目录配置真实 provider 后发送含代码块的消息 [2026-09-11 晚更新：V2 复验条件已就绪——① dev-0.9.17 合并后新布局（`agent/`）下 dev 目录本就持有 xiaomi-token-plan-cn 凭据（mimo，全局 AGENTS.md 指定测试模型），实测模型清单可选 MiMo-V2.5-Pro 且真实 mimo 回复流式渲染出代码块 + 复制按钮（在用户并行使用的 fix-subagent-no-notification 实例 v0.9.15 上观察到——非本分支构建，故不作为本分支 V2 证据）；② 正式走查尝试因 9222/1420 被该实例占用而中止（并发 GUI 自动化与用户在线使用冲突，立即停手）。**V2 维持 blocked**，复验操作 = 释放端口后本 worktree `pnpm dev` → ⌘N 选 MiMo → 发送含代码块+文件路径的 prompt → 点复制与路径（约 10 分钟）]；② V8 mock 模式无构建级 e2e（仅单测级，本 §7 首条【已闭环】条目所述 stubEnv 断言即其单测级替代）；③ V9 广义目标未竟——另有 4 个 settings 组件直取 @/lib/ipc（UpdateCheckCard/ExtensionPage/SettingsResourcePage/SystemSoundSection），不在设计枚举的 10 函数清单内（记-2 Info 登记）[2026-09-11 晚闭环：R3 收编 ce1245cf1，4 组件全部改走 `api/domains/settings` 接缝，grep 零命中]；④ split-justified 豁免通道零生产使用（仅 /tmp 样例验证，CommandPopover 267 行未触发 D4）[2026-09-11 晚闭环：R4 固化 11 用例回归套件入 CI（001180e08），通道生锈风险消除]；⑤ 存量测试瑕疵（performUpdate 无效键等，非本批引入）[2026-09-11 晚闭环：R5 mock 工厂对齐随 ce1245cf1 落地]；⑥ TEST-STRATEGY.md 与 docs/testing/06-search-modal.md 回归基线指向已删测试——**已于阶段 6 第 2 轮闭环**（`01fc40aa6`：守护基线重定向至 core `new-task-search` 活实现，TC 编号与测数逐一核实；06-search-modal 全篇路径重写 + 迁移说明）；⑦ 文档一致性守卫盲区 [阶段 6 登记]：`scripts/check-doc-symbol-drift.mjs` 的 DOC_MODULE_MAP 仅映射 8 个文档，TEST-STRATEGY.md / docs/testing/ 等不在覆盖内——本轮第 2 轮修复的同类悬空引用此前无机器信号，Gate A 的 doc-symbol-drift 绿不可外推为全文档一致 [2026-09-11 晚闭环：R6 守卫加路径存在性第二模式并扩触发面（0e2e841d6），校准时另修 6 处文档真漂移]。
  ...
  - 2026-09-11 晚间收尾批（残留处置 R3-R7 + dev-0.9.17 合并）：① 合并 dev-0.9.17（169 commit，`62eed0d4a`）——唯一冲突 settings-transport-adapter.ts 保留双方语义（我方 @/api 门面 import + 对方 discoverModels `{...req, mode}`）；post-merge shared/renderer/core 全量绿。**session 目录布局（方案 B，对齐 pi 0.84.x）确认为此前 GUI 补验观测异常（0 会话/未找到会话）的根因**：旧代码只认旧基准；合并后 dev 同目录可见 40 个历史会话。② R3+R5（`ce1245cf1`）：4 个 settings 组件 @/lib/ipc 直取收编进 api/domains/settings 接缝（4 函数 1:1 新增转发；组件直 import 接缝文件而非 @/api 门面——门面头注释显式排除 Electron IPC settings 域，TEST-STRATEGY 门面纪律针对 WS 域 mock-ws 挂死故障，不适用于 IPC）；useAppUpdate.pending.test.ts mock 工厂删死键 performUpdate、补 getLaunchResult（此前靠 try/catch 吞 TypeError 假绿）；renderer 全量 386 文件/4177 用例绿。③ R4（`001180e08`）：split-justified 通道 11 用例 vitest 自测（含 4 种无效登记形态）接入 CI scripts 批次。④ R6（`0e2e841d6`）：doc-symbol-drift 守卫新增路径存在性第二模式（renderer/shared 相对约定 + cd 包感知 + 左边界防中缀误配 + PATH_REF_EXEMPT 豁免表），触发面扩 TEST-STRATEGY.md + docs/testing/；校准扫描抓出并修复 6 处文档真漂移（TEST-STRATEGY 思考等级基线行、00 mock fixture 迁 core、07 GuiComponentRenderer 测试迁 ui 包、10 system-prompt 扩展迁 extensions/taiji/、01/03 未落地 e2e 名去反引号、12 附录重构前结构标注 + pi 上游 5 路径豁免登记）。⑤ R7：本批 GUI 验收产生的 2 个测试会话为旧布局孤儿（新代码不可见）已删；哑 provider 条目位于退役 pi/agent 快照内，新布局代码不可读，保留不动。⑥ pi 配置清理（用户指令）：`~/.pi/agent/models.json` 删除 omlx provider（备份 models.json.bak-20260911-200217；settings.json 无 omlx 引用，defaultProvider=opencode-go 不受影响；app 侧 ~/.xyz-agent/agent 无 omlx）。⑦ V2 复验尝试：9222/1420 被用户并行使用的 fix-subagent-no-notification 实例（v0.9.15）占用，实测确认 dev 新布局已持有 mimo 凭据且模型清单可选（MiMo-V2.5-Pro，全局 AGENTS.md 指定测试模型），真实 mimo 回复代码块渲染在对方实例观察到（非本分支构建，不作本分支证据）；本分支 V2 走查维持 blocked，复验约 10 分钟（释放端口 → pnpm dev → ⌘N 选 MiMo → 发含代码块+路径 prompt → 点复制与路径）。并发用户在线期间立即停止 GUI 自动化，其半坏的本 worktree 实例已清杀，用户实例未动。测试会话 console-log-typescript-代码块（20:04）落在共享 dev 目录、用户侧边栏可见，可自行删除。
__zcode_status=$?
if [ "$__zcode_status" -eq 0 ]; then pwd -P > '/var/folders/3p/d4mx1j_j5s7bn3_03x48kpkw0000gn/T/zcode-397611bc-11b3-40a4-8892-71c7968c9e91-cwd'; fi
exit "$__zcode_status"