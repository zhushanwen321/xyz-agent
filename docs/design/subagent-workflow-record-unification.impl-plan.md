...
| W1 | 实施口径两处 | ① TUI `/subagents` 补全与 overlay 经 `queries.collectRecords` 缺省自动继承过滤，subagents.ts 零改动即达成 D1④（设计预期改该文件，实际无需）；② 子文件 identity 通路（scanFile/buildRecord）不携带 origin——origin 只活在主 session 自描述 entry，identity 是否携带来源留待 W2 executeWorkflowAgent 接线时决策（**终局：悬置被 Gate B 真机证伪**——身份面真身是 `.record-binding` sidecar，S3 FAIL 后 round-2 补齐双载体口径，见设计 D1 与 Gate B round-2 行） |
...
| Gate A | 全绿（1 项环境例外登记） | 1 | 2026-09-12 跑批：core 2891 ✓ / extension 919 ✓ / renderer 4147 ✓（junit 零 failure）/ runtime 5184/5185——1 失败 = thinking-level-effective-e2e G5「模型清单 47 个找不到满足条件模型」，环境依赖用例（依赖本机实时模型清单，换模型后清单漂移），该文件最后被 H1 时代 commit 触碰、与 H2 runtime 唯一改动（fb4e30ee2 投影层）无交集，失败模式为清单查找非代码路径，登记不豁免修复（测试应无匹配模型时 skip，随 runtime 测试债处理）；三侧 typecheck EXIT0 / extensions:typecheck+lint EXIT0 / S5 门 createRecord=0 + live 10 处全退役注释人工复核 / doc-symbol-drift 绿 |
| Gate B | round-1：4 PASS / 1 FAIL（S3 修复复验中） | 1 | 真机（HEAD 91db1ce70，mimo-v2.5-pro，隔离根 /tmp/h2-gateb-fj0o，N=6）：S1 PASS（record 为真相 + run 级完成通知恰 1 条/record 级回注 0）/ S2 PASS（SIGKILL 恢复，FileRunStore 重水合 + 治理面全对）/ S4 PASS（在途峰值=N=6 无饿死，重启后 6/6 completed）/ S6 PASS（pending 配对差集归零；引擎死亡 **1 秒内** 4 条 record engine_crashed 立即终态化=adopt 豁免直接证据；失败 swallow 回脚本；D6 静默 ✓）。**S3 FAIL**：归档/重启后默认 list/TUI 显示已归档 workflow record——根因 = 子文件身份面不透传 origin，W1 偏差「identity 是否携带来源留待 W2 决策」的悬置被真机证伪（设计被否谱系 #2 预言形态）。登记不入范围：worker-script-builder parallel 拒纯文本结果（:379，与 SKILL.md 契约矛盾，脚本语言本体=H2 out-of-scope，产品缺陷 backlog）+ A1（spawn 窗死亡孤儿无重建源，治理面正确仅 list 不可见）+ 错误文本被 parallel 包装吞掉。SKIP-GUI：用户在 feat-rename-session-mode worktree 另起 dev app 占 1420/9222，不干预用户进程，GUI 面以 CLI 降级证据 + staged bundle 重构建（含 H2 符号）覆盖 |
| Gate B | **终审 PASS（S1-S6 全绿）** | 3 | round-2 修复 committed 9c924862c（13 文件 +777/-21）：S3 修复复验真机 PASS（默认 list 空 / includeWorkflow 可见 / TUI overlay 空态）+ A3 三域喂入补齐（chat/tool 扩修：断链先证实后修——kickOffChatRound 两形态共用 observedEvent 顺带补齐 one-shot 缺失的守护刷新源①、runEngineTask journal 透传保持；红锚各点独立；真机取舍 = 与 workflow 域同构论证，单测走真实派发链）+ A2 冻结。全量 core 2903 / ext 922 绿 + 双 tsc EXIT0。Gate B 关闭，S1 PASS / S2 PASS / S3 PASS（复验）/ S4 PASS / S5 PASS（Gate A 机器门）/ S6 PASS；SKIP-GUI 子项以 CLI 降级证据覆盖，GUI 视觉终验归阶段 6 后用户侧确认 |

## 7 残留风险与变更历史

- 残留风险（2026-09-12 阶段6 终审重排：原编号跳跃 ④ 补齐，新增 ⑦）：① 设计行号基于 commit `22f77c157` 工作区（v2 声明），H1 落地后 subagent-service.ts 行号大幅漂移——实施一律以符号 grep 锚定（设计头部已声明）；② H1/H2 咬合点 subagent-service.ts（H1 刚重写编排核 + Continuation + origin 相关面）——W2 派发前 dev 必须先 read H1 终态的 settleOneShotOutcome / disposeAllRecords / adopt 分诊两处现状再动手，禁止按设计文档行号想象；③ 待验证检查点①-⑥（§5 末段）为实施期门：② projectLiveProgress 字段清单与 ③ views 数据源现状链是 W3 的 S1 等价表依据，W3 派发前须先产出对照表；④ 并发上限默认值入 S4 验收表（已兑现：N=6 双源一致入 Gate B 表）；⑤ D7 成功终态 reason=gc 的 GUI/通知投影兼容（Gate B S1/S6 真机已验：closed+gc 投影正常、成败判读看 outcome）；⑥ runtime `subagentRecordEquals` 尚缺 result/chatMode/resumable/engine/engineFallback/engineHandle 六字段比对（阶段3修复A相邻发现，存量非 H2 引入）——轮终迁移只变这些字段时 publish 可能滞后，修复涉及 publish 频率行为分析，独立 backlog 不入本设计范围；⑦ **Gate B 三项 out-of-scope backlog**（状态表行是快照、残留风险段是台账，故入此）：a. worker-script-builder.ts:379 parallel 对 fulfilled 但非对象结果一律转 failed——与 workflow-script-format/SKILL.md「agent() 返回 string」契约直接矛盾，且原始文本被丢弃错误文案吞掉（两项同点，脚本语言本体 = H2 out-of-scope，产品缺陷 backlog）；b. A1 spawn 窗死亡（register→sessionFile 回填窗）孤儿 record 三源全空无重建源（binding 写点全在回填之后）——治理面正确（终态化+注销补写）仅 list 不可见，属边缘形态观察项。
- 变更历史：
  ...
  - 2026-09-12 **H2 流水线终态**：复审收敛 must-fix=0 阶段 3 关闭（0303fa735）→ Gate A 全绿 1 环境例外登记（91db1ce70）→ Gate B 真机三轮：round-1 S3 FAIL（binding 身份面漏登记 origin，W1 悬置决策被证伪）→ round-2 修复 + 真机复验 PASS（9c924862c，13 文件：binding origin/终态快照 + 三域 live 喂入 + status elapsed）→ 终审 S1-S6 全绿（1eacf0255）→ 阶段 6 终审 2 MF + 4 SUG + 3 INFO 全 code-right 当轮修完（双载体口径入设计 D1/§3.5/被否谱系、D4 悬空符号清账、残留风险重排 + ⑦ backlog 台账、变更历史补齐）。**H2 交付。**
