...
| Gate B | round-1：4 PASS / 1 FAIL（S3 修复复验中） | 1 | 真机（HEAD 91db1ce70，mimo-v2.5-pro，隔离根 /tmp/h2-gateb-fj0o，N=6）：S1 PASS（record 为真相 + run 级完成通知恰 1 条/record 级回注 0）/ S2 PASS（SIGKILL 恢复，FileRunStore 重水合 + 治理面全对）/ S4 PASS（在途峰值=N=6 无饿死，重启后 6/6 completed）/ S6 PASS（pending 配对差集归零；引擎死亡 **1 秒内** 4 条 record engine_crashed 立即终态化=adopt 豁免直接证据；失败 swallow 回脚本；D6 静默 ✓）。**S3 FAIL**：归档/重启后默认 list/TUI 显示已归档 workflow record——根因 = 子文件身份面（session-reconstructor）不透传 origin，W1 偏差「identity 是否携带来源留待 W2 决策」的悬置被真机证伪（设计被否谱系 #2 预言形态），修复复验中；同批排查 A3（record totalTokens/turns 恒 0 usage 断链）+ 小修 A2（status elapsed 终态不冻结）。登记不入范围：worker-script-builder parallel 拒纯文本结果（:379，与 SKILL.md 契约矛盾，脚本语言本体=H2 out-of-scope，产品缺陷 backlog）+ A1（spawn 窗死亡孤儿无重建源，治理面正确仅 list 不可见）+ 错误文本被 parallel 包装吞掉。SKIP-GUI：用户在 feat-rename-session-mode worktree 另起 dev app 占 1420/9222，不干预用户进程，GUI 面以 CLI 降级证据 + staged bundle 重构建（含 H2 符号）覆盖 |
