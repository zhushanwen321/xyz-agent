...
**输入**: [架构评审问题记录](architecture-review-issues.md)（9 个盲点/细化点 D1–D9）
...
### 理由
- CONTEXT.md 原始设计本就有 `pi-adapter/` 子模块意图，原 6 层架构反而退化了。
...
### 决策：直接引用既有计划，不重复
- `docs/refactor/terminology-alignment-plan.md` 已有 R1–R5（sidecar→runtime、Pane→Panel、SystemChatMessage→SystemNotification、Drawer→SideInspector、Overview→PanelGrid）。
- 本设计新增的命名对齐：
...
| 子系统 | 职责 | 依赖 | 现状 |
|--------|------|------|------|
| M1 Process Supervisor | Runtime 子进程 spawn/kill/端口/健康 | RuntimeManager | ✅ 已存在 |
| M2 Window Manager | 原生窗口 + 跨窗口注册表（读副本） | WindowManager | ✅ 已存在 |
| M3 OS Gateway | 对话框/外链/快捷键/全屏 | ipc-handlers | ✅ 已存在 |
...
阶段 2 · Runtime 目录分层（机械重构，低风险）
  ...
  ⚠️ tsup.config.ts 的 entry/noExternal 同步更新（见 CLAUDE.md #12）
...
阶段 4 · 命名对齐（引用既有计划）
  └─ 执行 terminology-alignment-plan.md 的 R1–R5
...
2. **阶段 2 目录迁移必须同步 tsup.config.ts**——entry/noExternal/asarUnpack 联动，否则打包崩
...
| D7 | 命名债（引用 terminology-alignment-plan R1–R5） | 阶段 4 |
...
main/
├── main.ts            (应用入口 + 生命周期编排)
├── window-manager.ts  (窗口注册表)
├── runtime-manager.ts (Runtime 子进程管理，~300 行)
...
```
main.ts (编排)
   ├──► runtime-manager.ts   [M2 Process Supervisor]
   │       └─► 端口探测 / spawn·kill / health / portFile
   ├──► window-manager.ts    [M3 Window Manager]
   │       └─► 窗口注册表 (windowId → 最小投影，非完整 tree)
   ├──► ipc-handlers.ts      [M4 OS Gateway]
   │       ├─► 特权 handler (校验输入 → OS API)
   │       └─► 桥接 handler (读 M2/M3 状态)
   └──► shortcuts.ts         [M5 快捷键]
   规则：main.ts 纯编排无能力；子职责单一；M3 不存完整 tree
```
