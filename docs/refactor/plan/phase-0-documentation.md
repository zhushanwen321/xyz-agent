...
1. **`CLAUDE.md` 架构章节补充**（在现有「架构约定」附近新增小节）：
   ...
   - **启动时序契约**（8 步，见 design.md D1）：Main 启动 → RuntimeManager spawn → createWindow → 渲染进程 `useConnection.init` → 注册 `onRuntimePort` → `getRuntimePort()` → connect WS → 业务就绪。注明「Runtime 重启 → Main 推新端口 → 渲染进程重连」。
