...
**匹配不到 sessionFile 的统一保守规则**（合并原 §7.2 的无条件 error，R8 补时间维度）：有终态迹象（终态 entry / sidecar 线索）→ 保守 error；无任何终态信号 + start entry timestamp 距今 ≤ 7 天（start 早期 / 候选窗外）→ streaming；**无终态信号 + 距今 > 7 天 → error**（对齐级 6 GC 判别与 normalize default→error 的既有理由「未知更可能是终态」——否则存量产物（8/13 前 identity 尾部不可读）的历史孤儿永久 streaming，R5-MF-2 要消除的症状在该子集复活；正常 start 早期不受影响）。两个失败模式不再给出相反方向；时间源同级 6（now 注入 + start entry timestamp 锚）。
