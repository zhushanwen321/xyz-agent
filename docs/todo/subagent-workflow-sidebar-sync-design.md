...
| pi 进程 kill -9（subagent 运行中） | 主 session JSONL 无终态 entry（父扩展死，bg-notify 是唯一终态写入通路）；成功路径本就无 sidecar（A9 路径 I），失败路径的 `.finalized` 亦由父侧写、无人补 | 👉 extractor 六级投影（决策 5）：子进程活着 → streaming（真在跑）；子进程已死 → 读子进程 JSONL 末行 → done/error（孤儿）或轮次 entry → waiting（Path B）。session 重开后 focus 首拉即显示正确状态，无扩展依赖、无通知依赖。**限定（R8）**：冷路径 resume 的续聊轮窗口期内被 kill -9（resume 续写原文件、文件名时间戳在 60s 候选窗外 + 轮次 entry 未 flush + 修正源已死的三重交集）→ sessionFile 不可解析 → 统一保守规则（≤7 天 streaming，>7 天 error）——极低频已接受边界 |
...
**本章结论：7 个决策，核心是把 subagent 链路改造成 workflow 已验证的「信号 + 重拉」形态，并补齐对账与两处落盘。**
...
**错位事实**（v2 曾豁免，v3 修正）：扩展的 `record.status` 是会话生命周期状态——`running` = 「record 未 closed」，内部三种子状态（真在跑：活进程+非 timer-armed；chatMode 轮完成等续聊：isIdle；one-shot 完成可 resume：isResumable，进程已死），谓词是执行状态的真相但未暴露。重建矩阵分支 4（`record-store.ts:822-826`：无 sidecar / `.alive` 但 pid 死 / 软超时）落点 running——其中「无 sidecar」「pid 死」子形态**进程必然不存在，不可能在 streaming**，UI 显示 running 是误导（R6 修正：「软超时」进入路径的进程可能存在——超时只是防 pid 复用的保守措施，非死亡证明；本投影以级 4 的 pid 探活兜住该子形态）。扩展把「可续聊」编码进 running 只是实现耦合（`isResumable` 谓词读 `status==='running'`，subagent-service.ts:582），不是产品语义。
...
| 4 | `.alive` + pid 活（**R6 修正：去掉 1h 软超时条件**——软超时是扩展侧防 pid 复用的保守措施，在投影语境「pid 活即事实优先」：超时 + pid 活仍判 streaming，pid 复用误判的代价是显示 streaming 而非错误终态，方向安全） | streaming；waiting 依 idle 位（轻量事件 entry，A10a；无则 streaming） | Path A（chatMode 轮完成进程保活）与超 1h 的运行中活任务（R6：按 v7 字面条件该形态六级全 miss、按兜底解读则读中间态末行错误收敛——违反「不会错误收敛」） |
| 5 | 最后 bg-notify entry 为 **running 轮次通知**（status='running'，含 round 字段）且**轮次未超龄**（**R7 修正锚：轮次 entry 自身的 entry timestamp 距今 ≤ 7 天**——实测轮次 entry_ts 每轮刷新（round=1/round=2 各自时间戳）而 start timestamp 恒定，用 start 锚（会话年龄）会把 start > 7 天但天天续聊的活跃会话误杀为终态；轮次 entry_ts 距今 > 7 天 = 7 天无续聊活动 = 等续聊语义失效 → 轮次无效、落级 6 判终态，级联关闭的 chatMode 记录 waiting 因此有界） | waiting | chatMode 活跃会话（Path B：idle 超时进程已回收，`/`.alive 已删）——与孤儿同形，靠轮次 entry 区分。**冷路径窗口期缺口**：轮次 entry 与终态 entry 同样进 60s 窗口，窗口期内重拉（秒级信号触发）时轮次 entry 未落盘 → 落级 6 → 可能误报 done，flush 后对账修正回 waiting（「done → waiting」抖动 ≤60s+turn）——是否真实发生由 A10 探针实测，真实则级 6 加时间宽限（时间源同级 6：extractor 可选 now 参数 + start entry timestamp 锚） |
| 6 | 其余（孤儿与崩溃：无终态 entry、无 sidecar、无 `.alive`、无轮次 entry） | **时间判别（R5）**：start entry timestamp 距今 ≤ 窗口期 + 1 turn → 子 JSONL 不存在/不可读 → **streaming**（start 早期保守——spawn 窗口内子文件未建，误报 error 会与 §5.1 T+0s 直接矛盾）；距今 > GC TTL 余量（30 天 TTL，判别阈值取 **7 天**）且子 JSONL 不存在 → **error**（子 session 已被 GC 清理——`session-file-gc.ts` 每次 session_start 5% 概率扫 `<piAgentDir>/subagents`：mtime>30 天的 `.jsonl` 删前做 pid 探活豁免（活进程保留）；`.alive`/`.finalized`/`.cancelled`/`.patch` 作为独立条目按同 TTL 清理**且无豁免**——`.alive` 无 heartbeat（spawn 时写一次），30 天后活进程的 `.alive` 亦被删（已知边缘：idle 默认 5min 使该场景实际不可达）。「必然存在」断言已被证伪，无此判别则历史孤儿永久 streaming。**已接受边界（owner 裁决）**：`.alive` 被删且子 `.jsonl` 因 pid 豁免保留时，30 天+ 活进程（需显式超大 idleTimeoutMs 或 one-shot 跑满 30 天，idle 默认 5min 不可达）经级 6 读末行会错误收敛终态——接受该极端边界；若未来不接受，扩展侧 `.alive` 补 heartbeat 或 GC 加兄弟文件检查需单独立项）；文件存在 → 读子进程 JSONL 末行：正常收尾 → done；截断/异常 → error；**文件不存在且距 ∈（窗口期+1 turn, 7 天] → streaming**（保守，同 start 早期方向——spawn 失败或文件被外部删除的中间态，统一保守规则兜住） | 覆盖 one-shot 成功（roundToIdle 后窗口期内，无任何 sidecar/entry——靠子 JSONL 收尾判 done）、真孤儿、**级联关闭**（disposeAllRecords 不通知不写 sidecar：无轮次 entry 形态落本级 → done/error；**有轮次 entry 的 chatMode 记录落级 5 → waiting 永久**——/new /fork 前已跑过轮的会话，见映射表死行注）。时间源：extractor 增可选 `now` 参数（默认 `Date.now()`，测试注入固定值——保纯函数可测性），锚 = start toolCall entry timestamp |
...
2. **start entry timestamp 锚定匹配（窗口期内的主数据源，R5 新增；R6 补并发消歧）**：主 JSONL 每个 entry 自带 timestamp（pi `SessionEntryBase.timestamp`，已落盘）——`findSubagentSessionFile` 的匹配锚从「notify.startedAt（窗口期缺失）」扩为「start toolCall entry timestamp ≈ 子进程 spawn 时刻」，子文件名 ISO 前缀在既有 60s 匹配窗内可达（R6 实测：toolCall entry → 子文件名间隔 0.7~1.9s，余量 30 倍）。**该级让级 4/6 在 0~60s+turn 窗口期内可达**（秒级信号重拉的依赖）。**并发消歧（R7 实测定案：同 message 并行 toolCall 共享同一 entry timestamp，且「先 start 先建文件」的序配对被实测证伪——6 组可区分案例 4 组文件名时间戳与 toolCall 顺序颠倒 6-22ms，spawn 发起序保持（差 2ms）但子进程初始化抖动远超发起间隔，文件创建序随机；序颠倒时交叉错配且不触发任何守卫）**：消歧主键 = **subagentId ↔ 子文件 `subagent-identity` entry 的 `data.id` 精确匹配**——主 JSONL start toolResult 的 subagentId（extractor 已解析、record 主键）与候选子文件的 `subagent-identity` custom entry `data.id` 实测精确相等（子进程 session_start 写入；resume 多条 identity 实测全同 id）。**解析规格（R8 实测定案，R9 收敛单一口径）：扫描候选文件首部前 8 条 entry 寻找 customType=subagent-identity——不用「首条 custom」也不用字节窗（R9 实测证伪：identity entry 自身携带 task 全文可达 17-62KB，16KB 窗 3.8% miss，按条目口径结构性优）**（实测 identity 与 `unified-hooks:loaded` 的先后由扩展加载顺序决定，hooks-first 桌面环境 identity 在第 5 条 entry；当前内置扩展组合下 identity 前最多 session header + model_change + thinking_level_change + hooks:loaded（实测 399/399 在第 4/5 条），prompt 在其后不插队；**环境限定（R9）**：子进程扩展集合镜像主进程，用户加装更多第三方扩展时 session_start entry 可能挤占 8 条窗口——溢出走规则 ② 保守方向安全）。覆盖率按口径分列：全文存在率（旧口径）新版本产物 ~95%；**首部扫描可读率（实现口径）8/13 后新版本产物 100%（含 hooks-first 形态），8/12 前存量产物 0%**（旧实现 identity 在文件尾部/缺失——走保守规则；8/12 当天有 1 天过渡带：27 个文件已为新形态，边界日按产物实际形态判）。O(候选数) 首部读取、零时序假设（候选圈定的 60s 时间窗仅用于缩小候选集，圈外/超窗 → 保守，方向安全）。规则：① 候选文件 identity.id == subagentId 才认领；② identity 缺失/不可读（旧产物）或 identity 尚未写（极窄窗口）→ **不猜文件**，落统一保守规则（无终态迹象 → streaming；有 → error），由 bg-notify sessionFile（第 3 级）与 A10a payload 作窗口 flush 后的修正源；③ **级 6 读子 JSONL 末行判终态（kill -9 永久路径，无级 1 兜底）只允许使用 identity-id 确认归属的文件**——宁可保守 streaming 等修正源，不猜终态。诚实边界：timestamp 时间窗只用于圈定候选集（串行/并行通用），归属判定只信 identity-id。连带改动：extractor 的 toolCalls Map 值扩 entry timestamp 字段 + 新增候选子文件首部 identity 解析（首部扫描前 8 条 entry，见上方解析规格；worktree 模式的子文件创建延迟与 60s 候选窗交互由 P0 探针覆盖）；
...
**匹配不到 sessionFile 的统一保守规则**（合并原 §7.2 的无条件 error，R8 补时间维度）：有终态迹象（终态 entry / sidecar 线索）→ 保守 error；无任何终态信号 + start entry timestamp 距今 ≤ 7 天（start 早期 / 候选窗外）→ streaming；**无终态信号 + 距今 > 7 天 → error**（对齐级 6 GC 判别与 normalize default→error 的既有理由「未知更可能是终态」——否则存量产物（8/13 前 identity 尾部不可读）的历史孤儿永久 streaming，R5-MF-2 要消除的症状在该子集复活；正常 start 早期不受影响）。两个失败模式不再给出相反方向；时间谓词与级 6 判别同构（>7 天阈值 + now 注入 + start entry timestamp 锚——实施时抽公共谓词函数，防两处漂移）。
...
- **extractor 六级投影**（决策 5 表格的实现点）：`subagent-extractor.ts` 对每条记录按 entry → sidecar → `.alive`+pid → 轮次 entry → 子进程 JSONL 六级判定；sessionFile 经 §6.5 优先级链解析（start entry timestamp 锚为主数据源）；匹配不到的保守方向按 §6.5 统一规则三分支（有终态迹象 → error；无终态信号 + ≤7 天 → streaming；无终态信号 + >7 天 → error）。workflow 侧广播逻辑不变（已是信号）。
...
- v7：按 R5 对抗审查（2 must-fix / 6 suggestion / 3 info，报告 `-plan-review-r5.md`）修正：① **sessionFile 链第 1 级被实测证伪**（start toolResult details.sessionFile 恒 null——execute 在握手前 fire-and-forget 返回，真实 JSONL 双轨实测确认）——链重写：第 1 级降为「未来扩展修复后的自动升级点（投影不依赖）」，**新增 start entry timestamp 锚定匹配为窗口期主数据源**（pi SessionEntryBase.timestamp 已落盘，使级 4/6 在 0~60s+turn 窗口可达，秒级终态对最高频路径成立）；匹配不到的保守方向统一（有终态迹象→error / 无→streaming，消除 §7.2 与级 6 的相反规则）；② **级 6「孤儿文件必然存在」被 session-file-gc 证伪**——级 6 加时间判别（≤窗口期 → streaming 保守；> 7 天且无终态迹象 → error「子 session 已清理」），extractor 增可选 now 参数保纯函数可测；③ 级联关闭落点分类（无轮次 entry → 级 6；chatMode 带轮次 entry → 级 5 waiting，v8 补超龄衰减）；④ normalize 输入域补 running→streaming 与 falsy→streaming；⑤ 场景计数/发起点/死行注层级修正；⑥ §11 A10 补 GC 交互探针。
- v8：按 R6 对抗审查（3 must-fix / 3 suggestion / 3 info，报告 `-plan-review-r6.md`，真实 JSONL 实测驱动）修正：① **timestamp 锚补并发消歧**（实测：同 message 并行 toolCall 共享同一 entry timestamp，最近匹配使两 record 解析到同一文件——序配对 + 歧义判「匹配不确定」走保守规则 + 级 6 终态判定禁用歧义文件 + 诚实边界声明 + extractor toolCalls Map 扩 timestamp 连带）；② **级 5 补 7 天超龄衰减**（超龄轮次视为无效落级 6 判终态——使「7 天后接手」承诺在级序上真实可达，消 v7 的「waiting 永久 vs 7 天接手」自相矛盾）；③ **级 4 去掉 1h 软超时条件**（pid 活即 streaming——超 1h 运行中活任务在 v7 按字面六级全 miss、按兜底读中间态末行错误收敛；软超时降级为 pid 死场景的 .alive 可信度衰减语境；错位事实段「进程必然不存在」限定为 pid 死/无 sidecar 子形态）；④ GC 表述精确化（.jsonl 有 pid 探活豁免 / sidecar 独立 TTL 无豁免 / .alive 无 heartbeat——P0 探针补孤儿 sidecar 断言）；⑤ 级 6 补「中间态 + 文件不存在 → streaming」分支；⑥ 变更历史排序修正、计划版本号 v7 同步、P4 单测补 running/undefined 输入与并发消歧/超龄/软超时用例。
- v9：按 R7 对抗审查（2 must-fix / 1 suggestion / 1 info，报告 `-plan-review-r7.md`，真实 JSONL 实测驱动）修正：① **并发消歧主键换成 identity-id 精确匹配**（实测证伪 v8 序配对——6 组可区分并行案例 4 组文件时间戳与 toolCall 顺序颠倒 6-22ms，spawn 序保持但子进程初始化抖动使文件创建序随机，序颠倒交叉错配不触发守卫；新主键 = start toolResult subagentId ↔ 候选子文件 `subagent-identity` entry `data.id` 精确相等，实测确认、零时序假设；identity 缺失 → 保守规则；级 6 终态判定只认 identity 确认归属的文件；timestamp 窗降级为候选圈定）；② **级 5 超龄锚修正为轮次 entry 自身 timestamp**（实测轮次 entry_ts 每轮刷新而 start 恒定——start 锚误杀 start>7 天但天天续聊的活跃会话为终态；轮次 entry_ts 距今 >7 天 = 无续聊活动 7 天，等续聊语义失效落级 6；死行注「判终态 error」对齐为 done 或 error）；③ `.alive` 被 GC 删后 30 天+ 活进程级 6 错误收敛边界挑明为已接受（owner 裁决，需超大 idleTimeoutMs/30 天 one-shot 才可达，扩展侧 heartbeat 修复列为单独立项）；④ 变更历史实际重排为时间序（R6 曾声称修复未执行）。计划同步：P0 补「并发 2 subagent + kill -9 + 配对正确性」探针；P4 fixture 补序颠倒形态（identity 匹配仍正确）与轮次超龄两形态（防误杀回归）。
- v10：按 R8 对抗审查（1 must-fix / 1 suggestion / 3 info，报告 `-plan-review-r8.md`，4417 个子文件全量扫描实测）修正：① **identity 解析规格改「首部扫描前 8 条 entry」**（实测「首条 custom」不是不变式——identity 与 unified-hooks:loaded 先后由扩展加载顺序决定，hooks-first 桌面环境 identity 在第 5 条 entry；按旧口径桌面环境主键系统性失效、场景 7 必挂）；覆盖率口径分列（全文存在率 ~95% vs 首部可读率：8/13 后 100%、存量 0%——旧实现 identity 在尾部）；② 统一保守规则补时间维度（无终态信号 + >7 天 → error——否则存量产物历史孤儿永久 streaming，R5-MF-2 症状在 sessionFile-null 子集复活）；§5.2 kill -9 行补冷路径 resume 三重交集限定；③ INFO 采录：轮次 entry_ts = flush 时刻（对 7 天判定无影响）、resume 多 identity 全同 id（首部读取兼容）、worktree 候选窗适用性入 P0 探针。
- v11：按 R9 对抗审查（0 must-fix / 4 suggestion / 3 info，报告 `-plan-review-r9.md`——**审查循环收敛：连续九轮后首次零 must-fix**）收尾：16KB 字节窗备选口径删除（实测证伪：identity entry 携带 task 全文可达 17-62KB，按条目口径单一化）；§7.2 保守规则复述补齐三分支（R8 时间维度的同步残留）；「identity 前最多 4 条」补环境限定（第三方扩展可挤占 8 条窗口，溢出走保守方向安全）；§6 决策计数 6→7；8/12 版本边界补 1 天过渡带注；级 6 与保守规则时间谓词同构注（实施抽公共函数）；变更历史排序修正（v10 提交时 v9/v10 颠倒）。