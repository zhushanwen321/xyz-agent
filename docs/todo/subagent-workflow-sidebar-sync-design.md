...
2. **start entry timestamp 锚定匹配（窗口期内的主数据源，R5 新增；R6 补并发消歧）**：主 JSONL 每个 entry 自带 timestamp（pi `SessionEntryBase.timestamp`，已落盘）——`findSubagentSessionFile` 的匹配锚从「notify.startedAt（窗口期缺失）」扩为「start toolCall entry timestamp ≈ 子进程 spawn 时刻」，子文件名 ISO 前缀在既有 60s 匹配窗内可达（R6 实测：toolCall entry → 子文件名间隔 0.7~1.9s，余量 30 倍）。**该级让级 4/6 在 0~60s+turn 窗口期内可达**（秒级信号重拉的依赖）。**并发消歧（R6 实测：同 message 并行 toolCall 共享同一 entry timestamp，最近匹配会使两 record 解析到同一文件——「并行派 A、B」正是场景 1 核心形态）**：① 同 message 多个 subagent toolCall 时，按 toolCall 在 entry 内的出现顺序与窗口内候选文件的时间顺序**一一配对**（先 start 先 spawn 先建文件，序保持）；② 配对后仍有歧义（多候选彼此在匹配半径内且序不可分辨）→ 判「匹配不确定」，**不猜文件**，落统一保守规则（无终态迹象 → streaming；有 → error），由 bg-notify sessionFile（第 3 级）与 A10a payload 作窗口 flush 后的修正源；③ **级 6 读子 JSONL 末行判终态（kill -9 永久路径，无级 1 兜底）禁止使用歧义匹配的文件**——宁可保守 streaming 等修正源，不猜终态。诚实边界：timestamp 锚在串行启动下可靠（实测），同 message 并行下依赖序配对/保守回退。连带改动：extractor 的 toolCalls Map 值需扩 entry timestamp 字段（现状只存 agent/slug/task）；
