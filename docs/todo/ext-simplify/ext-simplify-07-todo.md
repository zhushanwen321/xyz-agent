...
> **一句话结论**：删除 updateTodos 的「Result 双字段 + dispatcher 翻译」错误协议——校验失败改为与同包 addTodos / handleSingleUpdate / handleDelete 一致的直接 throw，UpdateResult 收敛为 `{ updatedTodos, resultText }` 两必填字段并去 export，包内错误通道归一为单一 throw 协议；模型可见错误文案一类变化：4 条批量校验文案去除冗余 "Error: " 前缀（其中 id 不存在一条去前缀后自然与单条路径措辞一致），持久化与渲染链路零触及。low 群（双布尔返回值、completed 计数 4 处重复等 6 项）登记移交 code-simplify，不在本设计实施。

## 开篇（SCQA）

- **S（情境）**：`@zhushanwen/pi-todo`（v0.9.0）是 universal 组独立通用包——轻量三态任务清单（pending / in_progress / completed），`todo` tool 提供 list/add/update/delete 四 action，状态持久化复用 pi 框架自动记录的 toolResult entry details（不 appendEntry），配 agent_end → before_agent_start 延迟 steer、add 时 auto-GC（旧列表全部 completed 即先清空再新增，提交 8b7f85b8b）+ 软 10 项上限提醒（`RECOMMENDED_MAX_TODOS`，超限附加提醒不拒绝）、/todos 命令 TUI 视图、TUI 状态行 + widget 与 GUI widget 推送。8 个源文件约 1200 行 src。
...
**证据基线**：本文化引行号均为 2026-09-13 实读复核值（worktree feat-optimize-extension-overengineering-group-b，HEAD 0e714d2a1，包版本 0.9.0）。v1 起草基线（2026-09-12 实读）在同日 12:40 合入的 auto-GC feature（提交 8b7f85b8b，+183/-9，触及 model.ts / tool.ts / todo.test.ts / tool-prompt.test.ts / ARCHITECTURE.md）之前，其行号已被该提交重排——v2 已按 0.9.0 全量刷新（本设计全部核心触点恰在漂移范围内）。**审计修正（行号漂移，不影响决策）**：审计记 M4 为 `model.ts:172-213 / tool.ts:102-108`，当前实读 UpdateResult 接口 `model.ts:189-193`、updateTodos 函数 `model.ts:195-251`（4 处双字段 return 在 :208-236）、handleBatchUpdate `tool.ts:108-113`；审计记 handleAutoClear `handlers.ts:104-121`，实读 `handlers.ts:97-114`；completed 计数 4 处实读为 `model.ts:89 / render.ts:44 / render.ts:115 / component.ts:53`（审计记录记 model.ts:140-142 / render.ts:68-72）。pi 断言无关（本设计不涉 pi SDK 行为假设）；npm ls 核对 pi 实装 0.84.4。

---

## 1. 背景：被设计的系统是什么

**todo extension 解决的问题是「多步骤工作的进度对模型与用户都可见、可续」**。模型经 `todo` tool 自主建单/更新；每次工具调用的最终列表快照随 pi 框架自动序列化的 toolResult entry 落盘（`details: {action, todos, nextId}`，**无 error 字段**——model.ts:22-26、ARCHITECTURE.md:164），session 重开时 `reconstructState`（handlers.ts:59-93）回放最后一条 todo toolResult 重建状态。展示面四条通道：模型侧文本（formatTodoList）、TUI 状态行（renderStatusText）、TUI widget / GUI widget（buildGui / renderWidgetLines）、/todos 全屏命令视图（TodoListComponent）。
...
**Out-of-scope**：
- 审计 M5（isGui 四分支跨包同构：本包 `index.ts:48-65` makeRefreshDisplay 的 isGui 分支 :52-63，与 goal 包 `goal/src/projection/widget.ts:200` updateWidget 的 isGui 分支 :224-229 同构）——该发现按索引归入 13 号设计（extension-protocol 下沉侧，修复点在 protocol 组合 helper），本设计不展开不实施；
...
[模型] {"action":"update","updates":[{"id":1,"status":"completed"},{"id":1,"status":"pending"}]}
       ↓
[updateTodos] 重复 id 检测命中（model.ts:208-213）：
       ...
       ↓
[handleBatchUpdate]（tool.ts:110）if (r.error) throw new Error(r.resultText)
       ↓
[pi 框架] 工具错误展示 "Error: duplicate ids in updates" 给模型
[state] 不变（error 分支返回原列表，语义 = 无突变）
```

同一函数内，另一类校验走的是另一条通道（model.ts:199-204）：
...
|---|---|---|---|
| 批量项 text trim 后空串 | throw（model.ts:199-204） | — | `update item id N: text cannot be empty or whitespace-only` |
| updates[] 重复 id | Result（model.ts:208-213） | `duplicate ids in updates` | `Error: duplicate ids in updates` |
| id 不存在 | Result（model.ts:217-221） | `id N not found` | `Error: Todo #N not found` |
| 无 status 且无 text | Result（model.ts:224-228） | `update item for id N has neither status nor text` | 同 error 文案 |
| 非法 status 值 | Result（model.ts:231-236） | `invalid status: X` | `Error: invalid status 'X' for update item id N` |

成功路径：`Updated N todo(s)`（model.ts:247-250），handleBatchUpdate `return r.resultText!`（tool.ts:112）。

### 3.2 真实失败模式

- **F1（双字段漂移税）**：4 处错误 return 每处同步写两遍字段（model.ts:208-236），漏改一处即 error 与 resultText 文案漂移（「id 不存在」行已经分化：`id N not found` vs `Todo #N not found`）。error 字段全仓唯一生产读者是 tool.ts:110 的布尔判空——它作为「机器可读结构化错误」的设计意图零兑现。
...
- **F3（双轨被文档固化）**：tool.ts:67-70、index.ts:19-20、ARCHITECTURE.md:158-164 三处明文记载「model 层 Result（合法）+ dispatcher 翻译 throw」——新增校验的开发者面对两个可选通道无所适从，双轨将持续繁殖。

### 3.3 根因

**「纯函数不 throw」的函数式洁癖遇上了唯一调用方是 throw 协议的边界。** updateTodos 以「返回 Result 是合法的函数式模式」（ARCHITECTURE.md:162 自述）设计，但它的生产调用方只有 handleBatchUpdate 一处（grep 全仓证实：src 内 `updateTodos` 引用仅 tool.ts:22 import / :109 调用与测试），而该调用方所在协议层（pi tool execute）的错误表达就是 throw——Result 在边界处被立刻拆成「布尔 + 消息」翻译回 throw，翻译层即全部成本。同一函数内的 text 空串校验后来按 CT5 决策改为 throw（model.ts:199 注释），双轨就此同框，且三处文档为既有形态背书而非纠偏。

### 3.4 物理数据流（M4 触点图）

```
模型 todo 调用 → executeTodoAction（tool.ts:185）→ handleUpdate → handleBatchUpdate（tool.ts:108）
                    ├─ 校验失败：现状 = UpdateResult{error,resultText} → 翻译 throw（tool.ts:110）
...
[updateTodos] throw new Error("Todo #9 not found")                    ← 与单条路径 tool.ts:136 同款措辞
...
- **证据**：调用方唯一性（grep `updateTodos` 全仓仅 tool.ts:22/:109 + 测试）；error 字段零生产读者（grep `.error` 于生产代码（src 排除 __tests__）仅 tool.ts:110 布尔判空；测试的 9 处 `result.error` 断言已列入 §6 改写面）；包内 throw 先例（model.ts:144/:150、tool.ts:116-170）；测试现状已把 throw 视为一等通道（todo.test.ts:220 对 text 空串断言 `toThrow`）。
...
- **采用**：throw 文案 = 现 resultText 措辞去掉冗余 `"Error: "` 前缀（错误形态由 pi 工具错误通道表达，前缀是现状双字段时代的拼接残留）；「id 不存在」对齐包内既有单条措辞 `Todo #N not found`（handleSingleUpdate tool.ts:136、handleDelete tool.ts:170 同款）。这是本设计唯一有意的模型可见变化——4 条批量校验文案全部去前缀，其中 id 不存在一条去前缀后自然与单条路径措辞一致（是去前缀的副产品，非独立第二处调整）。
- **被否**：保留 `"Error: "` 前缀（最小 diff 但保留拼接残留，且与单条/delete 路径的无前缀文案不一致）；改用现 error 字段的内部措辞（`id N not found`——从未展示给模型，作为对外文案是措辞降级）。
- **证据**：单条路径无前缀文案已在生产使用（tool.ts:136/:170），模型对其重试行为正常；两条文案差异本身就是 F1 漂移的实例。
...
- **效果**：`resultText!` 非空断言（tool.ts:112）随之消失；目标 2 完整达成。发现 8 的 UpdateResult 部分就此关闭，其余部分（ValidStatus / AddResult / RefreshDisplayFn）留 D4 移交。

### 5.4 D4：low 群处置——移交 code-simplify（非 contested，直接登记移交）

**移交清单**（实现阶段批量执行；每项给出实读位置与指令，行号 = 2026-09-13 实读复核值，0.9.0 基线）：

| # | 位置 | 内容 | 移交指令 | 联动同步 |
|---|---|---|---|---|
| L1 | handlers.ts:97-114（函数）、:174-178（唯一生产调用点） | handleAutoClear 返回 `{handled, cleared}` 双布尔；实读证实 handled:false 与 handled:true+cleared:false 在调用点行为逐字节相同（都不刷新不 return 差异） | 收敛为单返回值（语义 =「是否已清空」），调用点改 `if (handleAutoClear(state)) refreshDisplay(ctx);` | steer.test.ts:71/:77/:86/:93/:235/:246 形状断言改写；ARCHITECTURE.md:88-94（`{handled, cleared}` 状态图）与 :113（handled 短路表述）同步 |
...
| L3 | render.ts:56-66（renderWidgetItem）vs render.ts:146-152（buildTodoListText 内联） | item 行 status→视觉映射双实现；唯一差异 = 非完成态文本 `fg("text")`（:64）vs `fg("muted")`（:151） | 参数化共用以 renderWidgetItem 为基，颜色差异作参数显式传入（零视觉变更）；是否统一颜色由实施时视觉比对另定，默认保留参数 | 无 |
| L4 | tool.ts:74 vs tool.ts:219 | `"No todos"` 空列表三元表达式重复 | executeTodoAction 改复用 handleList | 无 |
...
| L6 | handlers.ts:24（RefreshDisplayFn 自用）vs tool.ts:185-190/:243、index.ts:48（内联同签名）；model.ts:30（ValidStatus）、:121-127（AddResult） | 半吊子类型导出：单点别名存在却三处内联；两类型 export 无包外/测试消费 | RefreshDisplayFn 统一四处签名引用（或全部降内联）；ValidStatus / AddResult 去 export（UpdateResult 部分已由 D3 关闭） | 无 |
...
|---|---|---|---|---|
| P1 | throw 化后批量校验失败经 pi 工具错误通道展示、state 不变、会话不中断 | 验收 V1 实测（真实 pi CLI 触发 not found 错误） | ⛔ V1 | 现状 handleBatchUpdate 已在同一边界 throw（tool.ts:110），终态仅把 throw 点上移一层、pi 侧边界不变——by construction 成立；若实测发现 pi 对不同栈深的 throw 展示有差异（无机制依据），回退方案 B 并重审 D1 |
| P2 | 旧会话回放零影响 | 验收 V2 实测（重开 session 状态一致） | ⛔ V2 | 不适用：details 组装代码（tool.ts:225-229）与 TodoDetails 形状（model.ts:22-26，无 error 字段）零触及，纯类型层 + 错误通道改动 |

## 6. 实现（文件改动地图）

| 文件 | 动作 | 要点 |
|---|---|---|
| `src/model.ts` | 改 | updateTodos 4 处 Result-error return（:208-236）改 throw（文案按 §4.2 终态表 / D2）；UpdateResult（:189-193）收敛 `{updatedTodos, resultText}` 两必填 + 去 export（D3）；UpdateResult 与函数头注释同步 |
| `src/tool.ts` | 改 | handleBatchUpdate（:108-113）三行化，删 `if (r.error) throw` 翻译与 `resultText!` 断言；:67-70 错误处理约定注释改写（删除「Result 对象（合法）」双轨背书，改为包内单一 throw 协议表述） |
| `src/index.ts` | 改 | :19-20 头注释的错误处理一句同步（与 tool.ts 新表述一致） |
| `src/__tests__/todo.test.ts` | 改 | 4 个 error 分支用例（:223-249）改 `expect(...).toThrow(...)` + 新文案断言；5 处成功用例 `error` toBeUndefined 断言（:201/:214/:315/:322/:335）删除；其余用例（trim、completed 无拦截、auto-GC 等）不动 |
| `ARCHITECTURE.md` | 回写 | :158-164 错误处理段改写为单一 throw 协议 + UpdateResult 新形状（同 commit，文档同步纪律） |

包版本 patch bump（universal 组独立 npm 包；对外行为等价、仅错误文案一类变化（4 条去前缀）；导出面收缩 UpdateResult——全仓零导入、深路径消费者理论存在但无证据，按 patch 处理）。
