...
| `.changeset/config.json` ignore 数组末项 | `"@zhushanwen/pi-unified-hooks"`（u4/D6 落地，见 `docs/architecture/npm-publish-surface-guard.md` D6；本次核验实读确认）——第 1 轮影响面审查补登的第 5 处登记 | changesets 发布管线消费：`scripts/apply-version.sh:156,170` 与 `scripts/check-version-changes.sh:113,133` 读 ignore 构建排除集合（仅集合成员判断，对条目增删不报错）；changesets version/status 自身解析 config |
...
- `pi uninstall` 的确切子命令形态以本机 pi 0.84.4 实装为准（README 写 `pi uninstall npm:@zhushanwen/pi-unified-hooks`；子命令存在性有本仓两处实读佐证：dev-link 脚本 `dev-link-lib.sh` 头注释声明依赖 `pi install` / `pi uninstall` 完成全部卸载操作、`docs/architecture/file-lock-unification-and-reaper-sink.md:154` 否决的是 `pi extension uninstall` 子命令形态而非顶层 `pi uninstall`——第 2 轮修订补，README 形态大概率直接可用）；若该子命令不存在（概率低），等价手工操作 = 编辑 settings.json 删 :24 行 + 清 `~/.pi/agent/npm/node_modules/@zhushanwen/pi-unified-hooks`。
