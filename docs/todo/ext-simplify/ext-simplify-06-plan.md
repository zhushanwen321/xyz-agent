...
> **一句话结论**：`@zhushanwen/pi-plan`（0.4.4）的 6 组审计发现全部收敛——complete 动作砍掉结构性不可实装且静默丢弃用户选择的 tree isolation 档（3×3 交互矩阵降为 2×3）、goal 桥失败从静默 return false 改为「降级 steer + 显式告知」、三源模板机制收缩为 builtin 单源（砍 create-template action）、5 个内置模板步骤节标题统一并以守卫测试钉死解析正则、3 处防幻影动态 import 改静态（index.ts 局部 logger 使用随之删除；**包级 extension-logger 依赖因 compact.ts 守卫降级日志的真实消费而保留**，v2 修正）、PlanPhase 删除（isActive 单一编码）、pi-goal peer 依赖 optional 化。除 C8 是行为修复外全部为纯减法；实施前置 1 个行为断言探针（⛔ 见 §5.7）。
...
**证据基线**：pi SDK 断言全部核对自本 worktree 实装 `@earendil-works/pi-coding-agent@0.84.4` dist 编译产物（`npm ls` 确认版本），SDK 文件引用 `dist/core/extensions/types.d.ts`（下文简写 SDK types）。plan 包行号说明：v1 起草自称「2026-09-11 实读」，但其 compact.ts 行号经 2026-09-13 审查考古（review MF1）证实取自 crash-resilience 合入（8ca11d330，09-10）**之前**的过期快照——v2 已按审查实读刷新 compact.ts 已知行号（系统性 +4~+20），未逐点复核处以符号检索定位；tool.ts/state.ts/command.ts 行号与当前源码基本一致（±2 行）。与 goal 的接口以 `docs/todo/ext-simplify/ext-simplify-03-goal.md` 的结论为准：goal **保留** `pi.__goalInit` 跨扩展 API（GoalInitFn 为 API-1 单一权威源，goal §1；goal 侧行号已被 03 实施改变，语义不变；**2026-09-14 桥修复注记**：pi 0.84.4 下该挂载形态运行时恒不可达，通道已迁 `globalThis[Symbol.for]` slot 直挂，见 `goal-bridge-cross-extension.md`），Like* 归一与 VALID_TRANSITIONS 保留决策均不触及 plan 消费面。
...
④ isolation 分发:
   "compact" → ctx.compact(...) → onComplete: steer(goal 文案) + tryGoalInit
   "tree"    → ctx.ui.notify("Use /tree to manually navigate back. ...")        ← :253-256
               ①里用户选的执行方式（含 "Modify the plan first" 之外的三项选择）全部丢弃，
               无 steer、无 goalInit——plan mode 已退出，什么都没发生
   "direct"  → steer + tryGoalInit                                              ← :260-261
```
...
- **F5（M3，防幻影防御）**：3 处动态 import（index.ts:20-25、tool.ts:255、tool.ts:326）注释称「avoids cross-group static import」，但 tool.ts/index.ts → compact.ts 的包内链路无运行时外部依赖（compact.ts 的 `import type { GoalInitFn }` 类型擦除）——防的幻影不存在。代价：promise 链 + `.catch` + 一条 `logger.warn`（index.ts:2/:9/:23）。**v1 误判已修正（审查 MF1）**：这三行不是 extension-logger 依赖的唯一使用点——compact.ts:7/:12/:236/:246 的 guardStaleCtx onStale 降级日志（`logger.warn` ×2，crash-resilience u1 于 09-10 引入）是真实消费方，v1 起草时误读了过期快照。附带代价：`buildExecOptions` 被迫 async 化。
...
- **F7（发现 6，双重编码 + 死状态）**：`PlanPhase` 四态（state.ts:3）与 `isActive` 布尔在所有写入点冗余共存，无 invariant 保证（`executeSelectTemplate` tool.ts:205 不检查 isActive 即写 `phase="writing"`）。实读追加一条审计未点透的证据：**`phase="complete"` 是死状态**——`executeComplete` 写入它（tool.ts:319）与 `resetPlanState` 归零（tool.ts:330）之间的唯一 await 是 :326 的动态 import（模块缓存后近似同步），且它位于 `ctx.compact()`（fire-and-forget void，SDK types :246/:1270）启动**之前**——compact 事件不可能在该间隙触发，事件处理器读到该状态的窗口不存在（u1 删除该动态 import 后才严格无 await；P1 探针保留为门，防「纯同步推理已闭合」的误判）；等 compact 事件真正触发时缓存已被删、重建出的状态 isActive=false。因此 compact.ts:29 的 `state.phase !== "complete"` else 分支（「Awaiting user decision」文案，:29-31）不可达，brainstorming/writing 两态则零行为差异（唯一行为分支键于一个永不可观测的值）。
...
### 5.5 D5：防幻影动态 import 改静态 + index.ts 局部 logger 剥离（选定：改静态，包级依赖保留）

- **采用**：index.ts:20-25 改 `registerPlanEventHandlers` 静态 import 直调；tool.ts:255/:326 改静态 import（`buildExecOptions` 变同步）；index.ts 删局部 logger 使用（:2/:9/:23 随 `.catch` 消失）。**包级 `@zhushanwen/pi-extension-logger` 依赖保留**——compact.ts:7/:12/:236/:246 的 guardStaleCtx onStale 降级日志是真实消费方（crash-resilience D1 的降级语义登记，不可顺手删除）。
- **被否 a（v1 子项，审查 MF1 击穿）**：删包级 extension-logger 依赖——v1 声称「index.ts 三行是唯一使用点」基于 crash-resilience 合入前的过期快照；当前 compact.ts 有运行时 `logger.warn` ×2，删依赖直接 typecheck 红，且为删依赖顺手删守卫日志会破坏 crash-resilience D1 降级语义。
- **被否 b**：保留动态 import——tool.ts/index.ts → compact.ts 的包内链路无运行时外部依赖（GoalInitFn 类型擦除），防的幻影不存在；同包内静态 import 无循环（compact.ts 依赖 state.js、pi-ext-guards、extension-logger 与 node 内置，均不反向依赖 tool/index）。
- **证据**：compact.ts:5/:7（两个运行时 import）+ :236/:246（onStale warn）+ package.json:44-47（dependencies 两项）；`rg -n "logger" src/*.ts` 命中 index.ts 三行 + compact.ts 四行；index.ts:19 注释自述的「cross-group static import」前提对 index/tool → compact 链路无对应物。
- **效果**：G4 部分成立；-1 层 promise 链、`buildExecOptions` 去 async（为 D1/D2 的同步化铺路）。
...
**本章结论：改动收敛在 7 个源文件 + 5 个模板 + 7 个测试文件（含 compact.test.ts 不变）+ package.json，净删约 230 行（src 1096 行的 ~21%；D5 收缩后较 v1 估算 -20）。**
...
| V2 | goal 失败降级（负面行为反向验证） | G2 | ①不装 goal 包：对话框无 goal 选项；②装 goal 但 plan.md 全文无编号列表：选 goal 档；③先 `/goal <临时任务>` 建一个 active goal 再 complete 选 goal 档（构造成本极低，v2 补——reason 文案可信度是 D2 核心验收对象）；④（可选）删 plan.md 后 complete 选 goal 档 | ①无法选到 goal；②收到含 `no-steps` 与恢复动作的降级 steer + warning notify；③收到含 `init-refused` 与「先 /goal clear 或沿用现有 goal」指引的降级 steer；④收到 `plan-unreadable` 恢复指引；全部场景**没有**「Execute via /goal」承诺、没有第二个 goal widget（不该发生的不发生） |
...
| u1 静态 import + index.ts 局部 logger 使用剥离（D5） | index.ts/tool.ts + package.json（仅 version bump；dependencies 不动——extension-logger 保留见 D5）+ buildExecOptions 同步化 | 最小且零行为变更，独立可验收（typecheck + 编译产物无动态 chunk）；为 u2 解开 async 链，先行避免交叉 |
...
| u3 模板单源 + 标题统一（D3+D4） | templates.ts/tool.ts/templates/*.md + templates 测试 + 新守卫测试 + plan CHANGELOG.md 破坏性条目（isolation=tree 移除与 create-template action 移除两项 AI 接口面 breaking，含 D3 已接受代价的迁移指引） | 模板生成端与解析端改动必须同批（守卫测试同时钉死两者）；与 u2 无文件交集，可并行；CHANGELOG 落点归此单元（D3 恢复路径的执行落点） |
...
**引用文档**：`docs/todo/ext-simplify/ext-simplify-03-goal.md`——goal 保留 `pi.__goalInit`（GoalInitFn API-1 单一权威源；**2026-09-14 桥修复注记**：通道已迁 goalInit slot，见 `goal-bridge-cross-extension.md`）与 VALID_TRANSITIONS 状态表；plan 侧消费方式与接口引用不变，peer optional 化不影响 goal 侧声明。
