...
> **一句话结论**：把设计文档（`subagent-workflow-sidebar-sync-design.md` v8）的 M0-M3 展开为 7 个阶段（P0-P6），每阶段「开发内容（文件级）+ 单测 + E2E real 验证（启动真实 pi）+ DoD」；P0 探针先行消解全部未验证前提，P1/P2 可并行，P5 与 P3/P4 可并行。
...
| `packages/runtime/src/services/session/subagent-extractor.ts` | **六级投影**（设计决策 5 v6 表格）：① 终态 bg-notify entry（映射表；级联关闭实际落级 6）→ ② `.cancelled` → ③ `.finalized`+子进程 JSONL 末行 → ④ `.alive`+pid 活（**R6：去掉 1h 软超时**——pid 活即 streaming，超时活任务不再全 miss/错误收敛）→ ⑤ 轮次 running entry（含 round）**且未超龄**（start entry timestamp 距今 ≤ 7 天，超龄落级 6——级联关闭 chatMode 记录的 waiting 有界）→ waiting → ⑥ 孤儿兜底（**时间判别**：start entry timestamp 距今 ≤ 窗口期+1 turn 且子 JSONL 不存在 → streaming 保守；> 7 天且不存在 → error「子 session 已 GC 清理」；存在 → 末行正常收尾→done/截断→error。extractor 增可选 `now` 参数保纯函数可测）。**sessionFile 解析优先级链（设计 v7——R5 实测修正：details.sessionFile start 时恒 null，投影不依赖）**：**start entry timestamp 锚定匹配为主数据源**（锚 = start toolCall entry timestamp；**R6 并发消歧**：同 message 并行 toolCall 序配对（先 start 先 spawn 先建文件），歧义判「匹配不确定」走保守规则、级 6 终态判定禁用歧义文件——实测同 message toolCall 共享同一 entry timestamp，最近匹配会拿错）；extractor toolCalls Map 值扩 entry timestamp 字段（连带）> bg-notify entry sessionFile（`BgNotifyRecord` 补字段）> listItem > startedAt 锚兜底（无锚不匹配）。匹配不到的保守方向按设计统一规则（有终态迹象→error / 无→streaming）。waiting 的 idle 位从轻量事件 entry 读取（A10a）；A10 非 a 时级 ④ 回落 streaming、级 ⑤ 窗口 flush 后仍可用。A10 探针若证实冷路径抖动 → 级 6 加时间宽限（start entry timestamp 锚 + now 注入，P0 已列） |
