...
- **聚合归一化白名单截断**：`normalizeAggregatorResult`（utils.cjs:638-663）对 `must_fix_ids` 条目只保留 `{id, severity}`、对顶层只挑已知字段——任何新增条目字段（files/evidence/guidance/scores）会在此被**静默丢弃**。
- **证据裁决已存在**：aggregator prompt 模板（utils.cjs:317-405）已有 adjudication 段——逐条 must-fix 裁决证据（evidence / unverified / downgraded+reason）+ fixes_caution。**缺的是裁决结果的落盘与复活通道**（downgraded 条目去哪了无人记录）。
...
- **真问题被裁决误降级**：dormant 下轮可复活；复活率持续偏高 → 👉 `evidenceGate=false`。
...
reviewer 报告逐 issue 表增加「修复建议」必填列；aggregator 提取进 `guidance`；fixer 按指引定位（冲突以代码为准）；guidance 写 `state.issues[id].guidance`。
...
returnMeta resolve 与 `_callCache` 重放重建各扩 `{usage, durationMs}`（两个对称点）；引擎 run 启动生成 runId 并注入 `_runId`（worker rebuild 沿用同一值）。⛔ P-replay：重放 run 的 calls[] 同样含 usage——验收用「注入错误触发 rebuild」而非「同参数重跑」（v4 的 S3 写法错误：新引擎 run 无缓存可重放）。
...
| S4 引擎重放 + runId 稳定 | 目标 7 | 真实 run 中注入一次 agent 错误触发 worker rebuild（错误注入是流程内既定恢复路径，非 mock 依赖） | rebuild 后 _runId 不变（state 目录唯一）；重放调用在 calls[] 含 usage（⛔ P-replay） |
...
| T2 引擎 _runId 稳定注入 | run 启动路径 | 修 run 碎裂；与 T1 同为引擎但独立关注点 |
