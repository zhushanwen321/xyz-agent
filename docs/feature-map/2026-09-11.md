...
| 块 | 载体 |
|------|------|
| E1 崩溃台账 | `packages/shared/src/crash-journal-schema.ts`（event 20 值枚举 SSOT，v7 删 oom）+ runtime writer `packages/runtime/src/infra/crash-journal.ts`（10MB×3 段级联）+ main 双胞胎 `apps/electron/main/logs/crash-journal.ts`；写入点矩阵接线（session 生命周期 / 收割回收 / 条件信号 / supervisor 判别式四挂点 / marker 补记）；约束 C-data-20 |
