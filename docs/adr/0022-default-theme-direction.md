...
**Date**: 2026-06-20
> **裁决**：暗色冷蓝为真默认，真身代码跟随 ADR-0019
> **关联**：ADR-0019（视觉方向）、`design-tokens.md`、`design-system.md §10`、`settings/handoff-system.md §2/§11a`
...
| 主张方 | 默认 | 依据 |
|---|---|---|
| **设计层** | 暗色冷蓝优先（`--bg #1a1b1f`，accent `#4f8ef7`；原 `#0d0d0f`，2026-07-09 提亮校准） | `design-tokens.md`（标题「冷蓝暗色 SSOT」）、`design-system.md §10`（「暗色为主，亮色为辅」）、`PRODUCT.md`、ADR-0019 |
...
- **代价**：推翻 ADR-0019 的视觉方向主张；冷蓝暗色探索成果降级。
...
- **适合**：产品定位就是开发者工具暗色体验（与 ADR-0019 一致）。
...
产品真实默认 = **暗色冷蓝**（`--bg #1a1b1f` / accent `#4f8ef7`；原 `#0d0d0f`，2026-07-09 提亮校准）。真身代码跟随设计主张（ADR-0019）。
...
裁决 B 后，draft 演示态（cold-blue）与真身默认**将一致**（真身完成迁移后）。迁移期间 handoff `§2`/`§12` 仍如实记录真身当前为 light/neutral，并登记「待跟随 ADR-0022-B」待办。

## 落地状态

- **设计层**：✅ `design-tokens.md` / `design-system.md §10` / `PRODUCT.md` / ADR-0019 本就主张暗色冷蓝，无需改。
