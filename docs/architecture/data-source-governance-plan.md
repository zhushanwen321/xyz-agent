...
1. 新增 `check_pi_direct_write.py`，模式（对齐父文档 R1 定义）：扫描 `packages/runtime/src/` 与仓库根 `scripts/` 中指向 sessions 目录 **pi JSONL 本体**的写操作——`openSync('a')` / `openSync('w')` / `appendFile` / `appendFileSync` / `writeFile` / `writeFileSync`，**以及 `atomicWrite`**（已知 util 形态——`patchSessionCwd` 经 `utils/fs-utils.ts` 的 atomicWrite 整文件重写 JSONL，r3 补漏：不含它则该写点穿透）。**扫描范围排除测试文件（r5 补）**：`__tests__/` 目录、`*.test.ts`、`test/` 目录不进扫描——测试构造数据的写点非生产写路径，排除是政策性正确，其正确性不依赖条件 B② 的实现链深（r6 核正）。`src/` 内存在测试文件（`services/session/session-lifecycle-gate.test.ts:165` 真实 `writeFileSync` 且同文件 :162 含 `getSessionsDir` 调用；该测试两写点（:134/:165）目标均为 tmpdir 派生文件——`const dir = mkdtempSync(join(tmpdir(), …))` → `join(dir, …)` → `writeFileSync` 两跳链）：B② 取「同函数内直接赋值链」单跳实现时两跳派生不可见豁免、测试排除成为 W3 验收 4 与 W11 验收 1 exit 0 的必要条件；B② 若实现多跳传播分析则该两写点本可经目标豁免、排除仅余降噪意义——两种实现取向下 exit 0 均成立（双保险）；步骤 2 的 allowlist 枚举 grep 排除测试是同一纪律的核对侧。**匹配粒度（r4 落文件级定义，r5 补全为「条件 A 文件级邻近 + 条件 B 写目标豁免」两必要条件，对齐父文档 §3.6 R1）**：**条件 A（圈定候选）**——写调用所在文件内含 sessions 路径推导痕迹，痕迹限定**代码语境**：`getSessionsDir` 的 import/调用，或 `sessions` 出现在路径构造语境（`join(…, 'sessions', …)`、`'sessions/'` 路径段拼接）；注释、普通标识符（如 `sessions` Map 字段名——session-service.ts:110 即此形态）、字符串文案中的普通提及**不计入**。实现上：匹配前剥离注释行，滤注释形态与 W11 验收 1 的注释感知 grep（`grep -vE ':[[:space:]]*(//|\*)'`）同语义；注意参照实现 `check_path_whitelist.py` 是全文 `re.search` **不滤注释**（已 read 实测），R1 只对齐其结构体例，匹配逻辑**有意增加**注释剥离与语境限定——注释剥离在此是纵深防御而非当前命中的必要条件（r6 核正：`session-fork.ts:63` JSDoc 注释「（pi sessions 目录）」按条件 A 的路径构造语境模式即便不剥离也不命中——该文本既非 `'sessions'` 的 join 参数、也不含 `'sessions/'` 子串，已 read 实测）。剥离防两类形态：① 实现退化为裸 token 匹配（`sessions` 字符串全文搜——恰是条件 A 明言**不计入**的「字符串文案普通提及」形态；照抄参照实现的全文 `re.search` 即落入此形态，届时 :63 注释命中、fork 不命中声明失效，父文档 D3b）；② 未来注释文本出现 `join(x, 'sessions')` / `'sessions/'` 类真实路径构造模式（当前代码零实例，防御性预留）。该粒度覆盖目标路径为形参、函数体内无路径字面量的间接形态（session-file-utils 三条 legacy 写点即此形态：目标 filePath 形参 + 文件级 `getSessionsDir` import（:12）/ 调用（:735），文件级命中；若取函数级粒度则这些写点全部漏拦，allowlist 与 sidecar 豁免形同虚设，W11 验收 1 的「归零」空转）；调用参数或所在函数体直接含路径推导当然也命中。**条件 B（内置豁免，非 allowlist，均在写目标路径层级判定）**：① **sidecar 家族四后缀**（`.meta.json` / `.preset.json` / `.project.json` / `.handoff.json`——四后缀全集对齐父文档 D3b；`.handoff.json` 的写点 W11 迁移才出现，此处先预留，保证豁免清单与登记表 sidecar 家族条目始终一一对应，W11 步骤 7 迁入后核对；xyz 自有文件）——写目标表达式含后缀字面量（`filePath + '.meta.json'` 内联形态，session-file-utils.ts:146）**或**目标经含后缀拼接的 sidecar 路径 helper（`projectSidecarPath(filePath)` / `presetSidecarPath(filePath)` 间接形态，:223/:281——后缀在 helper 定义处、调用点无后缀字面量，两种形态必须都覆盖，否则 `.project.json`/`.preset.json` 两写点穿透）则不报错；文件内任意位置的后缀提及不豁免无关写点；② **非 sessions 目标目录**（调用点可见性规则）——写目标表达式（含同函数内直接赋值链，如 `const tmpFile = join(tmpdir(), …)` 随后两行的 `writeFileSync(tmpFile, …)`）可见地经 `tmpdir()` 或 xyz 自有目录推导函数（`getAttachmentsDir` / `getConfigDir` 等，脚本内维护枚举清单）构造，目标不指向 sessions 目录即不报错（session-lifecycle restore/fork 的两处 tmpdir 拷贝写 :435/:594 即此通道豁免——所在文件含 `getSessionsDir` import（:28）/ 调用（:536），无此通道则 W3 验收 4 与 W11 验收 1 的 exit 0 不可达）。**检出边界（docstring 写明）**：目标路径经形参间接且**整个文件**无任何 sessions 路径推导痕迹（代码语境，注释不计入）的写点不命中（跨文件数据流静态不可判定）——`session-fork.ts:175` 即此形态（调用点传 `getSessionsDir()`，fork 文件内唯一 `sessions` token 在 :63 注释、代码语境零痕迹），R1 不命中——fork 的守卫 = 登记表「创建型唯一写入口」声明 + S1 语义层（父文档 D3b 诚实声明，粒度与覆盖差以该声明为准）。实现风格对齐 `check_path_whitelist.py` 的 main 结构与 docstring 体例（读其开头复刻）；匹配逻辑按上述条件 A/B 与注释剥离实现，与该参照全文 `re.search` 的差异为有意设计（见条件 A），docstring 写明。
...
4. allowlist 验证：当前 HEAD 下 `python3 .githooks/check_pi_direct_write.py` exit 0（可达性按步骤 1 两必要条件机制归位：条件 A 命中集 = session-file-utils 三 legacy 写点，由 allowlist 覆盖；同文件三 sidecar 写点经四后缀豁免、session-lifecycle 两 tmpdir 写（:435/:594）经非 sessions 目标豁免、session-service 的附件（:1436）/配置（:1518）写所在文件零代码语境痕迹（`sessions` Map 字段名不构成痕迹）不进候选、测试文件不进扫描——无「必命中且无豁免」残留），且 allowlist 条目与 `grep -rn "persistSessionName\|persistHandedOff\|patchSessionCwd" packages/runtime/src --include="*.ts"` 排除注释后的代码命中一一对应（W1 扩围后预期 = persistSessionName 实现本体 + 非活跃调用点 + persistHandedOff 本体 + patchSessionCwd 本体，共 4 条链路写点；多登 = 掩盖未登记写方，漏登 = R1 误报，两者都算验收失败）。
...
- `packages/core/src/domain/chat/store.ts`（修改：`pendingBuffer`（L123）/ `drainPending`（L335）/ `abortPending`（:355，docstring 止于 :354）——`drainPending` 的 findIndex 文本匹配删除，改按条数 FIFO 取）
...
12. **`handoff-service.ts` 真实路径**（r5 补澄清）：`packages/runtime/src/services/handoff-service.ts`——在 `services/` 根下，**不在** `services/session/` 内（父文档 §2.5 写方 4 与本文档 W1 步骤 4、W11 步骤 4、附录 A #11 引用的均为该文件；:279/:286 锚已实测在位）。
