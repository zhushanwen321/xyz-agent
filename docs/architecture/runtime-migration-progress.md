...
**状态**: R0–R9 已完成（三层迁移 + 例外清理 + 骨架密度优化完结）
...
| R8 | `3fca1422` | ✅ | 收尾文档 + utils 共享层定位 |
| R9 | `32bb2b94` | ✅ | ports.ts 按域拆分（6 域文件）|
...
## R9 · 骨架密度优化：ports.ts 按域拆分 ✅

**触发**：用 recursive-skeleton 技能从骨架视角重新审视 runtime。P1 评估发现 ports.ts 是低密度枢纽——323 行/22 接口/6 个无关域（config/session/pi-engine/model/installer/tree）塞一个文件。每加一个 port、每改一个签名都在这同一个文件里改，是"按符号数量聚合"而非"按变化轴拆分"。

**修复**：拆成 `services/ports/{config,session,pi-engine,model,installer,tree}.ts`，每文件一个域。删 ports.ts，**无 barrel re-export**——每个引用点直指具体域文件（单一入口单一关注点）。跨域消费者（session-service）拿 2-3 个 import，诚实反映多域依赖。纯机械拆分，git 历史保留，20 引用点更新。

### SessionService 半构造评估（判定为非问题，有意不改）

P1 同时标了"SessionService 9 参数构造器 + 子模块传 this 的半构造自引用"为候选。**深挖后判定为误判**，记录原因避免后人重复评估：

- 3 子模块（SessionLifecycle/MessageDispatcher/SessionScanner）接收 `svc: ISessionServiceInternal`——这是**公共接口**，不是 private 成员。
- 与 R4a 根本不同：R4a 是 `as unknown as` 强转（private 跨类不可见）；这里是**无强转**的类型安全依赖倒置。
- "传 this"是 Facade 模式 + sessions Map 单写者的有意设计：子模块经接口访问 Map，打断模块环。注释坦承构造时序，但子模块只在 Facade 完全构造后才调 svc 方法。
- 9 参数是 Facade 固有扇入（7 依赖 + 1 闭包 + 1 配置），非低密度。options-bag 只是换皮，7+ 字段的 bag 同样臃肿。

**对照 user-memory「区分洁癖问题和真 bug」**：这是洁癖误判（看着丑但类型安全+有意设计），不是真问题。强行改（显式 context 对象）要么复制接口、要么加间接层，无架构收益，违反 Ponytail。

**教训**：P1 评估"候选优化"前应再读一层代码——P1 把"9 参数 + 传 this"凭表面判为低密度，但读下来是有注释守护、类型守护的有意设计。印证 user-memory「改之前先读、先分析：先不要改动，仅分析」。

---

## 迁移完结

三层架构（transport → services ← infra）落地完成，**services 零 infra 直接依赖**（含原 2 个例外均已消除）。R0–R9 共 17 个 commit，每阶段审计→修复→执行循环，审查产出（R2 反向依赖、R3b PiMessage 误判、R5 判据修正、R6 scanner 非整体跨层、R9 SessionService 误判）均被采纳并修正了原计划。
