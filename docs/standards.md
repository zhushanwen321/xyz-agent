...
修改以下任一目录时，**必须同步修改对应的 Electron 目录**：

- sidecar 核心逻辑（session-pool, event-adapter, server, rpc-client, config-store, session-scanner）
- 前端组件和 composables
- shared 类型定义

**检查方式**: `git diff --stat` 后确认 Electron 对应文件也被修改。pre-commit hook 会自动检查（见第 7 节）。
