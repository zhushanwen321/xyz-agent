...
src-electron/
  ...
  runtime/src/    Node.js Runtime 服务（原 sidecar，2026-06 重命名）
...
值变更时同步 `design-tokens.md`（CSS 变量层）与 `tailwind.config.ts`（Tailwind 映射层）。CSS 变量定义见 `src-electron/renderer/src/style.css`。
