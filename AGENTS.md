...
**Pi Extension 源码（本项目维护）**：`extensions/` 下 15 个 `@zhushanwen/pi-*` 包 + `extensions/shared/` 共享库（quota-providers / llm-shared / extension-logger），统一在本仓开发发布（旧仓 xyz-pi-extensions-workspace 已废弃，以本仓为准）。全集：ask-user / cache-probe（前缀指纹采集 + analyze.py 归因） / cw-tool / goal / model-switch / pending-notifications / permission / plan / rename-session / session-reader / scheduler / structured-output / subagent-workflow / todo / unified-hooks（新增/删包时更新此列举）。校验：`pnpm extensions:typecheck` / `extensions:lint` / `extensions:test`。
