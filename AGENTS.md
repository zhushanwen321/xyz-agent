...
- `extensions/` 目录下 15 个 `@zhushanwen/pi-*` extension 包 + `extensions/shared/quota-providers`，由本项目继续发布到 npm（main 线走 `npm-*` tag 人工版本判定，dev-npm 预发布走 changeset version）。完整清单见下方「Pi Extension 全集」
...
**Pi Extension 全集**（`extensions/` 下 15 个 `@zhushanwen/pi-*` 包；新增/删包时同步更新此表）：
